# deployment_log.md — v1.3.63

## IRASAI (naujausi virsuje)

---

## 2026-07-14 — M8 FRONTEND: ANKETA [S196]

"Mano augintinis" anketa (2 zingsniai) live. petshop-core v0.10.0. Vizualiai patikrinta. S196.

### S196-A — Recon frontend

Tema Flatsome Child (parent Flatsome). MyAccount page id 14, /my-account/. Permalink /%postname%/. Backend gyvas v0.9.0. Shortcode support. REST /wp-json/petshop/v1, nonce wp_rest.

### S196-B — Nauja klase Petshop_Pet_UI (3741 baitai)

1. Shortcode [petshop_pet_form] — anketa (veikia ir be prisijungimo)
2. MyAccount tab "Mano augintinis" (endpoint 'augintinis', /my-account/augintinis/), idedamas pries 'orders'
3. enqueue_assets su config (restUrl, nonce, isLoggedIn, homeUrl, petPageUrl) + Inter Google Font
4. should_load() gate — assets tik shortcode puslapyje arba MyAccount tab
5. flush_rewrite() aktyvuojant (kad endpoint veiktu)

### S196-C — pet-form.css (306 eil.)

Brand #2D5F3F, Inter, pill mygtukai (aktyvus=zalias), autocomplete dropdown, progreso juosta, rezultato kortele, mobile-first. Prefix pspet-.

### S196-D — pet-form.js (525 eil.)

2 zingsniai. Dinamiski laukai pagal rusi:
- dog: amzius+dydis+maitinimas+maistas(autocomplete)
- cat: amzius+sterilizuota+maitinimas+maistas
- bird: rusis(tekstas)+amzius
- rodent/reptile/other: rusis(tekstas)
- fish: akvariumo tipas(pills)
Progresinis rusiu rodymas (Suo/Kate/Kitas -> isskleidzia bird/rodent/fish/reptile/other).
Poreikiai (tik dog/cat). Maisto autocomplete per /brands + specialus opts.
localStorage juodrastis (DRAFT_KEY, 30d TTL, schema_version 1).
Griztancio anonim. prompt (Testi/Pradeti is naujo).
Submit: prisijunges -> POST /pet-profile -> rezultatas; anonim. -> rezultatas su email CTA -> POST /magic-login/request.

### S196-E — Deploy + vizualus testas (browser=1)

core v0.10.0. Assets 200. Test puslapis /anketa-testas/. Screenshot'ai:
- 1 zingsnis: title OK, 8 pills, poreikiai atsiranda po dog click (5)
- 2 zingsnis: "Dar keli klausimai apie Reksa", 10 pills, autocomplete
- 0 JS klaidu

### S196-F — LT vardo linksniavimo fix

"apie Reksas" (blogas linksnis) -> "apie „Reksas"" (kabutese, isvengiam linksnio). Deploy live.

### S196 — LIVE

petshop-core v0.10.0. Anketa gyva (shortcode + MyAccount tab). Vizualiai patvirtinta.

### S196 — SEKANTIS (M8 frontend likusi dalis)

- render_account_page kol kas rodo tik anketa. Reikia: jei augintinis jau yra -> profilio ekranas (pet-dashboard).
- pet-photo upload/serve endpoint
- pet-dashboard agregatorius (viskas vienam ekranui: refill zied., priminimai, lentynele, mitybos ritmas, aktualu siandien)
- refill-feedback endpoint (POST)
- profilio ekrano HTML/CSS (pagal galutinius dizainerio vizualus)
- iliustracijos (tikslus prasymas dizaineriui: 7 rusys x 3 dydziai)
- MyAccount tab stilius Flatsome
