...
Versija: 1.2 · Data: 2026-08-16 · Statusas: ĮGYVENDINTA DEV APLINKOJE (E1–E6)

v1.2 pakeitimai — VISI iš realaus serverio, ne iš teorijos (recon + testai 2026-08-16):
- §3.1: pridėtas `rodyta` tipas (be jo „įdėjimo dalis" neturi vardiklio); `po1` pažymėtas nenaudojamu; įrašyta, kad `preke_id` visada produkto ID, o vitrinos `data-cid` yra MnM `child_item_id` — reikalingas vertimas;
- §3.2: `dydis` ateina iš dėžės PRODUKTO meta `_ps_laukas_dydis` („400 g" → „400"), pildo serveris;
- §3.3: skirtukas NĖRA filtras dėžės viduje — kiekvienas skirtukas yra atskiras dėžės produktas-brolis; pildo serveris;
- §5.2: pridėta atskira `piltuvelis` sritis, nes unikalios sesijos nesudedamos per prekių eilutes;
- §4: be sutikimo `sessionStorage` nenaudojamas.

Įgyvendinimo pastaba: E2 padarytas ATSKIRU moduliu `petshop-statistika-vitrina.php`, kuris kabinasi ant vitrinos DOM per delegated listener'ius. `petshop-laukai.php` (183 KB) NEKEISTAS — jei vitrina persidarys, suges tik matavimas, ne pardavimas.

v1.1 pakeitimai: + `_ps_irenginys` užsakymo meta ir įrenginių pjūvis pardavimams; + `parduota_sd`, `be_sav_suma`, ribos-variklio skaitliukai; + ekrano sekcijos „Dovana — ar riba veikia" ir „Įrenginiai — per kur perka"; piltuvėlio pjūviuose + įrenginys.
...
**sritis='pardavimai' (iš užsakymų):** `parduota` (deze_id = konteinerio/pako ID, preke_id = komponento ID arba 0 konteinerio lygmens eilutei; kiekis, suma_ct, sav_ct), `parduota_sd` (konteinerio lygmens eilutė, kai dėžėje YRA dovanos eilutė: kiekis=1, suma_ct = viso konteinerio suma — čekių su/be dovanos palyginimui; „be dovanos" = parduota(preke_id=0) − parduota_sd), `dovana` (dovanų eilutės atskirai: kiekis + sav_ct, preke_id = dovanos prekės ID — „kurią dovaną renkasi" sąrašui), `be_savikainos` (eilučių sk., kur savikaina nežinoma) ir `be_sav_suma` (tų eilučių suma_ct — kad marža skaičiuotųsi tik iš eilučių su savikaina), `grazinta` (NEIGIAMI kiekis/suma_ct/sav_ct, rašoma GRĄŽINIMO dienos data — savininko sprendimas: istorija atgaline data nesikeičia), `atskirai` (to paties komponento pardavimai NE rinkinyje — kanibalizacijai; preke_id užpildytas, deze_id=0), `dp_pakopa` (DP pirkimai pagal pakopą: deze_id = pako ID, pakopa rašoma į `dydis` lauką kaip 'x1'/'x2'/'x3').

Pardavimų eilutėse `irenginys` dimensija pildoma iš užsakymo `_ps_irenginys` — įrenginių pjūvis pinigams veikia iš VISŲ užsakymų.
...
| `_ps_irenginys` | UŽSAKYMO lygmuo (ne eilutės) | 'mobile' / 'desktop' / '' — checkout puslapio JS įrašo į paslėptą lauką (`woocommerce_checkout_fields` hidden field arba `woocommerce_checkout_create_order` iš POST), serveris user-agent NESKAITO. Jokio identifikatoriaus — tai užsakymo atributas, sutikimo nereikia, veikia 100 % užsakymų. JS neveikus — '' | **NAUJAS** |
...
| `rodyta` | prekės kortelė pateko į ekraną arba atidaryta jos greita peržiūra | komponento ID | '' |
| ~~`po1`~~ | **NENAUDOJAMA.** Vitrinoje paspaudimas ant dėžės langelio nuima vieną vienetą — tai paprastas `iseme`, ne atskiras tipas (patikrinta kode 2026-08-16) | — | — |
| `krepselis` | dėžė sėkmingai įdėta į krepšelį | 0 | dėžės suma eurais, pvz. '48.12' |

„Pirmą kartą sesijoje" dedupikuoja kliento JS, ne serveris. **Be statistikos sutikimo `sessionStorage` NENAUDOJAMAS** — vėliavėlė laikoma tik JS kintamajame, kad įrenginyje neliktų jokio pėdsako.

**`rodyta` be vardiklio nėra „įdėjimo dalies".** Šis tipas spec v1.1 buvo praleistas, nors abu ekranai jo reikalauja: be jo prekių lentelė gali atsakyti tik „kiek parduota", bet ne „ar prekė traukia".

**`preke_id` VISADA yra WooCommerce produkto ID.** Vitrinos DOM `data-cid` yra MnM vaikinės eilutės ID (`wc_mnm_child_items.child_item_id`) — pvz., `mnm_quantity[19570]` sėdi elemente `pslk-in-907`. Vertimą cid → product_id daro PHP paduotas žemėlapis; be jo elgsena ir pardavimai gyventų skirtingais raktais ir niekada nesusijungtų (reali klaida, sugauta testuojant 2026-08-16).

### 3.2. `dydis` — PATIKSLINTA PAGAL REALYBĘ
'400' / '800' / '100'; '' dėžėms be dydžio lygmens.

**Šaltinis — dėžės PRODUKTO meta `_ps_laukas_dydis`, ne krepšelio pasirinkimas.** Reikšmės ten saugomos su vienetu („400 g", „800 g", „100 g"), todėl normalizuojamos į skaitmenis (`Petshop_Statistika::dydis_normalus()`). Lauką pildo SERVERIS įrašymo metu — naršyklė jo siųsti neprivalo.

### 3.3. `skirtukas` — LEISTINOS REIKŠMĖS (uždaras sąrašas)
`be_vistienos`, `monoproteinas`, `visi`, `isrankioms`, '' (dėžė be skirtukų). **PATIKSLINTA: skirtukas nėra filtras dėžės viduje.** Kiekvienas skirtukas yra ATSKIRAS dėžės produktas-brolis (34942 „800 Be vištienos", 34945 „400 Monoproteinas" ir t. t.), o perjungimas — navigacija tarp produktų. Todėl skirtuką, kaip ir dydį, pildo SERVERIS iš dėžės produkto (`Petshop_Statistika::dezes_skirtukas()`), o ne naršyklė. Tai patikimiau ir nepriklauso nuo JS. Jei ateityje atsiras naujų skirtukų — reikšmė = skirtuko slug'as, ekranai rodo dinamiškai.
...
**KRITINĖ TAISYKLĖ: unikalios sesijos NESUDEDAMOS.** `sritis='laukai'` eilutėse `sesiju` skaičiuojamas kartu su `preke_id`, todėl viena sesija, įdėjusi keturias prekes, duoda keturias eilutes po 1 sesiją. Sudėjus jas piltuvėlyje gaunasi „atidarė 2 → prisidėjo 8" — neįmanomas rezultatas (reali klaida, sugauta 2026-08-16).

Todėl piltuvėliui rašoma ATSKIRA sritis `piltuvelis`: tie patys tipai (`atidare`, `idejo`, `min_pasiekta`, `krepselis`, `nupirko`), bet GROUP BY BE `preke_id`, `kiekis`=0, `sesiju`=COUNT(DISTINCT sesija). Ekranai sesijų skaičius ima TIK iš šios srities.
...
- **riba kaip variklis**: sesijos su `dovana_atrakinta` → +1 į tipas='riba_pasieke'; iš jų sesijos, kurios tą pačią dieną turėjo `krepselis` → +1 į tipas='riba_pasieke_krepselis'; sesijos SU `krepselis` bet BE `dovana_atrakinta` → +1 į tipas='riba_nepasieke_krepselis' (o visų sesijų be ribos sk. = atidare − riba_pasieke). Iš šių keturių skaitliukų ekranas skaičiuoja „pasiekė ribą → nupirko" vs „nepasiekė → nupirko" konversijas (nupirko dalis — per `nupirko` sesijų santykį; aproksimacija per krepšelį leidžiama, pažymėti tooltip'e).
...
- Dovanos (`_ps_dovana`) → papildomai tipas='dovana' (preke_id = dovanos prekės ID). Konteineriai, kurių dėžėje yra dovanos eilutė → papildomai tipas='parduota_sd' (kiekis=1, suma_ct = konteinerio suma).
- Eilutės be savikainos → sav_ct=0 IR tipas='be_savikainos' +1 IR tipas='be_sav_suma' su tų eilučių suma_ct (kad marža ekrane skaičiuotųsi tik iš eilučių su savikaina).
- VISOS pardavimų eilutės rašomos su `irenginys` iš užsakymo `_ps_irenginys` ('' jei nėra).
...
9. **Piltuvėlis** — žingsniai: Atidarė → Prisidėjo bent vieną (idejo sesiju) → Pasiekė minimumą → Įsidėjo į krepšelį → Nupirko; perėjimų %; „Piltuvėlis pjūviais" lentelė pagal skirtuką (perjungiama į dydį/rinkinį/**įrenginį** GET parametru); žyma „iš sutikusių su statistika".
...
11. **Dovana — ar riba veikia** (kaip makete): 4 KPI — Nupirkta su dovana (parduota_sd ÷ parduota konteinerių, % ir delta p.p.), Vid. čekis su dovana (parduota_sd suma ÷ kiekis) vs be dovanos ((parduota(preke_id=0) − parduota_sd) santykis), Čekio prieaugis (skirtumas €; šalia — dovanos kaštas/dėžei = dovana sav_ct ÷ parduota_sd kiekis), Grąža (prieaugis ÷ kaštas, tooltip su „orientacinis" pastaba iš maketo). Du blokai: „Kurią dovaną renkasi" (tipas='dovana' pagal preke_id, top 3 su %) ir „Riba kaip variklis" (iš 5.3 skaitliukų, žyma „iš sutikusių"). „Skaitosi taip" tekstas — iš maketo.
12. **Įrenginiai — per kur perka** (kaip makete): lentelė mobile/desktop — Atidarė (visų lankytojų, su %), Įsidėjo į krepšelį, Konversija (iš sutikusių, silpnesnė raudona), Nupirkta dėžių, Pajamos, Vid. čekis (pirkimo stulpeliai — iš `_ps_irenginys`, visų užsakymų). „Skaitosi taip" tekstas — iš maketo.
...
### 11.1. FAKTINĖ BŪKLĖ 2026-08-16 (dev.avesa.lt)

| Etapas | Būklė | Kuo patikrinta |
|---|---|---|
| E1 schema ir meta | **PADARYTA** | `DESCRIBE` per tiltą: `ps_laukai_ivykiai` +4 stulpeliai, `ps_ataskaitu_dienos` sukurta, `ps_stat_schema`=2 |
| E2 vitrinos įvykiai | **PADARYTA** | Playwright suvaidino kelią: 4 įdėjimai + 1 išėmimas → 14 įrašų su teisingais `kiek_dezeje` (1,2,3,4 įdedant; 4 prieš išimant), `dydis`=800, `skirtukas`=be_vistienos, sesija |
| E3 agregavimas ir cron | **PADARYTA** | 2026-08-15 → 17 eilučių; konteinerio suma 4812 ct sutampa su rankine vaikų suma; valymas netrina neagreguotų dienų |
| E4 UI karkasas | **PADARYTA** | Ekrano nuotrauka; 0 JS klaidų |
| E5 „Surenkami" v2.0 | **PADARYTA** | Ekrano nuotrauka; pelnas 4,49 € sutampa su rankiniu skaičiavimu (3731 − 3216 − 66) |
| E6 „Rinkiniai" | **PADARYTA (be duomenų)** | Ekranas veikia, bet dev'e nėra nė vieno paruošto rinkinio ar DP pako — kanibalizacijos verdiktai NEPATIKRINTI su realiais duomenimis |

**Moduliai `mu-plugins`:** `petshop-statistika.php` v2.0 · `petshop-statistika-vitrina.php` v1.0 · `petshop-ataskaitu-agregavimas.php` v1.0 · `petshop-ataskaitos-ui.php` v1.0 · `petshop-rinkiniu-ataskaita.php` v2.0 · `petshop-paruostu-ataskaita.php` v1.0

**LIKO (savininko sprendimu):**
1. E6 DoD neužbaigtas — reikia dviejų testinių užsakymų (vienas su rinkiniu, vienas su komponentu atskirai), kad kanibalizacijos verdiktai būtų patikrinti, o ne tik parašyti.
2. `min_pasiekta`, `krepselis`, `dovana_atrakinta`, `dydis_perjunge` kode yra, bet testas jų nepasiekė (dėžės minimumas 7 vnt., testas surinko 4) — reikia pilno kelio testo iki krepšelio.
3. Produkcijai: `ps_stat_pradzia` nustatyti launch datą; patikrinti, ar Complianz statistikos kategorija realiai duoda `cmplz_has_consent('statistics')`.
