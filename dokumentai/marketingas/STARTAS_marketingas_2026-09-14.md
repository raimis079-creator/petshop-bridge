...
## S1685 (09-15 vakaras) — „Pakartoti tą patį" laiškas GYVAI
Naujas mu-plugin **`petshop-pakartoti.php` v1.0.0** (md5 9916c1ad…, repo `deploy/pakartoti/petshop-pakartoti-v1.0.0.php`) + šablonas **`mu-plugins/ps-sablonai/refill-pakartoti.php`** (md5 e2a0f238…, repo `deploy/pakartoti/refill-pakartoti.php`); bak `ps-backups/petshop-pakartoti.php.bak_s1685`, `refill-pakartoti.php.bak_s1685`; deploy `irankiai/s1685_ml/mn/mo`, render testas `s1685_mm` (nesiunčiant). Core `refill.php` neliestas — pakeistas per `petshop_email_template_path` prio 20. Laiškas: tema „Laikas pakartoti: {prekė}"; „ši prekė gali baigtis apie rugsėjo 21 d."; „Tas pats užsakymas vienu paspaudimu — N vnt., X € — pristatysime taip pat greitai, kaip praeitą kartą"; mygtukas „Pakartoti tą patį" → `/kasa/?add-to-cart=ID&quantity=N` (kiekis iš paskutinio užsakymo, kaina dabartinė su PVM); antrinės: Peržiūrėti prekę · Patikslinti priminimą · Nenoriu tokių priminimų (`Petshop_Sutikimai::optout_url`). Eligibility prio 30: prekė ne publish / neparduodama / nėra sandėlyje → `preke_nera_sandelyje` (deferred, ne terminal). Be augintinio vardo (linksniai), be automatinio papildymo pažadų.
RADINYS: refill variklis seka kiekvieną maisto prekę atskirai — konservų pirkėjui su 1 vnt. 400 g laiškas siūlo „1 vnt., 2,20 €" (žemiau mažo krepšelio ribos). Siūlymas (Raimio sprendimas): refill_due grupuoti pagal paskutinį užsakymą — vienas laiškas „pakartoti visą užsakymą" (visos maisto prekės, kiekiai, suma), ne po prekę. Kol nepadaryta — pirmi laiškai eis tik naujiems pirkėjams nuo 09-15 (Raimio sprendimas: esamiems be žymos nesiųsti), t. y. realiai nuo spalio pradžios.
