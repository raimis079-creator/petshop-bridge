# deployment_log.md — v1.3.62

## IRASAI (naujausi virsuje)

---

## 2026-07-14 — M8 BACKEND PAPILDYMAI [S195]

Augintinio profilio backend praplestas M8 UI poreikiams. petshop-core v0.9.0. Svarus — 0 fatal. S195.

### S195-A — Recon + auditas

ps_pets: 12 laukų. 0 augintiniu DB (svarus laukas — jokios migracijos nereikia, tik ALTER ADD). product_brand taksonomija egzistuoja (4Dogs, 8IN1, Josera...) — autocomplete saltinis. ps_pet_products dar nebuvo.

### S195-B — ps_pets schema papildyta (7 nauji laukai)

species_detail, is_sterilised, current_food_free_text, primary_product_id, photo_file_id, status, deleted_at. Per ensure_columns() ALTER TABLE ADD COLUMN IF NOT EXISTS (idempotentiska migracija esamai lentelei).

Nauji enum'ai:
- species: dog, cat, bird, rodent, fish, reptile, other (pasalinta both/unknown)
- primary_need: daily, digestion, skin_allergy, sterilised, picky_eater (hypo->skin_allergy)
- is_sterilised: yes, no, unknown

### S195-C — Nauji endpointai

- PATCH /pet-profile/{id} — dalinis update
- DELETE /pet-profile/{id} — SOFT delete (status=deleted, deleted_at). Deaktyvuoja reminders. Jei buvo primary — kitas tampa primary.
- GET /brands?q=&species= — brendu autocomplete is product_brand taksonomijos
- LIST dabar filtruoja status='active' (deleted nerodomi)

### S195-D — ps_pet_products lentele (nauja klase Petshop_Pet_Products)

Sieja augintini su produktais (lentynelei, refill, rekomendacijoms). Laukai: pet_id, user_id, product_id, sku, relationship_type(primary_food|treat|supplement|care|other), source(user|order_inference|admin), confidence, last_order_id, last_purchased_at. UNIQUE(pet_id, product_id).

REST:
- POST /pet-product-link — priskirti (IDOR apsauga)
- GET /pet-product-link?pet_id=N — lentynele (su produktu nuotraukom/kainom/stock)

infer_from_order(): auto-priskyrimas jei user turi TIK 1 augintini (confidence 0.7). Jei keli — nepriskiria (UI klaus).

### S195-E — Event Registry +2 M8 event'ai

pet_product_assigned + refill_feedback_submitted -> CANONICAL_EVENTS (abu true, source yra). Schemos sukurtos.

### S195-F — Empirinis testas 8/8

7 nauji laukai PRESENT. Create bird+species_detail. skin_allergy enum OK. PATCH OK. Brands(Jose->Josera). pet-product-link+shelf. Soft delete (status=deleted, list neitraukia). Visi ✅.

### S195 — LIVE

petshop-core v0.9.0. M8 backend paruostas. ps_pets praplesta, ps_pet_products nauja. 4 nauji endpointai.

### S195 — SEKANTIS

M8 frontend (kai dizaineris atiduos galutinius vizualus). Dar reikes: pet-photo upload/serve endpoint, pet-dashboard agregatorius, refill-feedback endpoint, MyAccount tab, anketos forma. Iliustracijos — tikslus prasymas dizaineriui kai prireiks.
