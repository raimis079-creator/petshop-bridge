...
| OPS-02 | **Cron užduotys** serveriai.lt (žr. §8e) | 🟡 **2026-08-21:** WP mato 26 gyvus cron'us; po failų perkėlimo veikia per stub'us. Lieka URL patikra po T-0 |
| OPS-03 | Hardkodinti `dev.avesa.lt` nustatymuose | 🟢 **2026-08-21 permatuota:** `woocommerce_email_header_image` ir `cmplz_preloaded_privacy_info` **švarūs**. Liko `wcdn_settings` logotipas (PDF sąskaitos) + `flatsome_registration`; kitos 4 opcijos — transientai |
...
| ~~OPS-11~~ | TEMP snippetų trynimas | ✅ **2026-08-21:** 498 ištrinta per SQL, kopija `ps-backups/temp-snippets-20260821-192445.json`; 1868→1370, aktyvūs 73→73 (§8hh) |
...
| OPS-15 | **Aktyvūs pluginai** vs TŽ §11.4 riba ≤25 | 🟡 **27 → 26** 2026-08-21: `postit` (pašto kodai, API raktas tuščias, 0 kabliukų, 1 138 deprecated/parą) išjungtas; svetainė sveika (§8hh) |
| OPS-18 | `force_ssl_checkout` → `yes` po perjungimo (§8bb) | 🔴 T-0 |
| ~~OPS-19~~ | Paysera testinė prekė | ✅ **2026-08-21:** `ps-testas-1-eur` nebėra (išmatuota) |
...
## §8hh. ★ RINKINIAI: DU MODELIAI, DU LANGAI — 2026-08-21 (vakaras) [S1200–S1214]

> **🔴 PIRMA TAISYKLĖ ŠIAM BLOKUI.** Yra **DU** rinkinių modeliai. Techniškai abu
> yra MnM konteineriai — būtent todėl per šią sesiją buvo supainioti **tris kartus**.
> Prieš bet kokį siūlymą — pirmas klausimas sau: **kuris modelis?**

| | Surenkami | Paruošti |
|---|---|---|
| Modulis | `petshop-laukai.php` | `petshop-rinkiniai.php` |
| Langas | `page=ps-laukai` | `page=ps-rinkiniai` |
| Kas dėlioja | klientas pats | savininkas |
| Kaina | pakopinė nuolaida nuo sumos | fiksuota |
| Skiriamasis meta | `_ps_laukas = yes` | jo NĖRA |
| Kiek dabar | 14 | 13 (7 MnM + 6 DP) |

### Įdiegta

| Failas | Versija | Kas |
|---|---|---|
| `petshop-laukai.php` | **1.43** | grupės nuotrauka veikia parduotuvėje ir admin kortelėje |
| `petshop-rinkiniai.php` | **1.31** | negrįžtamas trynimas · šiukšlinė kaip 4-a būsena · modelių atskyrimas |

### 🔒 UŽRAKINTI SPRENDIMAI

- **Du langai — dvi sienos.** `_ps_laukas` filtras stovi TRIJOSE vietose:
  `rinkiniai_mnm()`, `rinkiniai_dp()`, `siuksliadezeje()`. Naujas laukas pro
  visus tris praeis savaime. **Nenuimti.**
- **Į WooCommerce prekių langą nelendam.** Visas ciklas — sukurti, redaguoti,
  ištrinti, apsigalvoti — telpa į du savininko langus. Jei sprendimas reikalauja
  eiti į WC, sprendimas neteisingas.
- **Prekių trynimas NIEKADA.** Savininkas trina tik rinkinį (konteinerį).
  Komponentai — atskiros katalogo prekės, apie rinkinį nieko nežino.
- **Šiukšlinė = būsena, ne blokas.** Atskiras sąrašas dubliuotų tai, ką lentelė
  jau moka (rikiavimas, paieška, nuotraukos), ir prie 50 rinkinių užimtų
  geriausią ekrano vietą.
- **Valymas tik po GALUTINIO trynimo.** Kol daiktas šiukšlinėje — gali grįžti;
  per anksti nuimtos eilutės grąžintų tuščią rinkinį.
- **Vienas įėjimas grupei.** Kataloge matomas tik jis; poreikius ir dydžius
  klientas renkasi vitrinoje.

### Grupių nuotraukos — `ps_laukai_grupiu_foto`

```
sunys 35004 · kates 35005 · kramtalai 35006 · kons_sunims 35030 · kons_kates 35031
```
Nuotraukos buvo įdėtos teisingai visą laiką. `foto_id()` gyveno TIK admin lange —
kabliuko į WooCommerce nebuvo, todėl kataloge kabėjo WC vietos užpildas.
Visi 14 laukų turi `_thumbnail_id = 0`.

### Įėjimai (visi penki, sudėti 2026-08-21)

```
sunys       → 34937 Skanėstų dėžė šuniui
kates       → 34936 Skanėstų dėžutė katei
kramtalai   → 34933 Kramtalų dėžė šuniui — hipoalerginė   ⚠ Claude nuožiūra
kons_sunims → 34944 Konservu deze 400 Be vistienos
kons_kates  → 34948 TEST Konservu deze katei Be vistienos
```

### Meniu „Susidėk savo rinkinį" — visi penki 200

Buvo rankiniai custom-link punktai su įkaltais slug'ais į seniai ištrintas
testines prekes. **Tėvinis punktas `#` — teisinga**, jis tik atidaro pomeniu.

### ★★★ ĮRODYMAS, KAD TRYNIMAS PALIEKA PĖDSAKUS ★★★

`wc_mnm_child_items` rasta **12 konteinerių ID be jokio įrašo**:
```
34198-34206, 34479, 34481, 34966
```
Trynimas per rinkinių/laukų langus savo eilutes valo. Trynimas per WooCommerce
prekių sąrašą — ne. Iš ten šie ir atsirado.

### ★ Ištaisyta sena klaida

Iki v1.29 trynimas į šiukšlinę PIRMA ištrindavo krepšio eilutes, o tik paskui
trynė. **Grąžintas rinkinys grįždavo tuščias.**

### 🔴 Neišspręsta — naršyklė per tiltą

Playwright **tris kartus** nukrito į „Prisijungti" (be sesijos žetono → su žetonu →
abu domenų variantai). Įtariamas `petshop-login-sargas.php`, **neišmatuota**.
Pasekmė: admin langų JS pusės vizualiai nepatikrinta — tikrino savininkas.

---

## 8ii. ★ UŽSAKYMŲ BLOKAS: SIUNTŲ REGISTRAS, SARGAI, ZB KELIAS — 2026-08-22 [H200–H210]

> Sesijos tikslas (Raimis): užsakymai turi būti ne gerai, o idealiai — su jais dirbs
> darbuotojas. Pradėta nuo 7 sandėlių scenarijaus patikros, baigta keturiais
> įdiegtais moduliais. Visi matavimai — gyvi, be užsakymų kūrimo (AVPN ir likučiai
> nepaliesti; po kiekvieno testo `_stock` pjūvis — NEPAKITO).

### 8ii.1 SEPTYNIŲ SANDELIŲ SCENARIJUS — IŠMATUOTA (H200)

Po vieną realią prekę iš visų 7 šaltinių per `AV_Source::resolve()`:
```
av → av/any · vf,zb,quattro,prins,belcor_tofu,ambrosia → venipak
grupių 7 · siuntų 7 · tipas MIXED — mechanizmas teisingas 7/7
```
**Raimio sprendimas (rizika A, priimta):** 3+ siuntų užsakymo nuostolingumas —
jo rizika, sistemoje nieko nekeičiam. NEBEKELTI be jo iniciatyvos.

### 8ii.2 KASOS RIBOJIMAS MIŠRIAM KREPŠELIUI — VEIKIA (H201, gyvas krepšelio testas)
```
AV+VF krepšelis → tik VENIPAK kurjeris + paštomatas (LP DINGO)
tik AV          → Venipak + LP Express + LP kurjeris
```
Snippetas 194 „Venipak Shipping Lock v3" — IŠJUNGTAS negyvas dublikatas
(ribojimą daro kitas sluoksnis); kandidatas trynimui.
Pastaba: mišriam krepšeliui lieka Venipak paštomatas — 7 siuntos į tą patį
paštomatą važiuotų kaip 7 dėžės (žinoma, priimta).

### 8ii.3 🔴 VENIPAK NIUANSAS: VIENAS META RAKTAS (H202)

`venipak_shipping_order_data` — VIENAS raktas užsakymui
(`{manifest, pack_numbers[], status}`). `pack_numbers` masyvas = kelios dėžės
VIENOJE registracijoje, ne kelios siuntos. **Mišriam užsakymui antra grupės
registracija PERRAŠO pirmos numerius.** Iš čia — savas kaupiamasis sluoksnis:

### 8ii.4 ✅ SIUNTŲ REGISTRAS + SEKIMO LAIŠKAS (H204)
```
mu-plugins/petshop-siuntu-laiskai.php  v1.1 · md5 07724b10d8aee5fb
mu-plugins/petshop-desk.php            v3.16 · md5 4af7d2334427b742
```
- `_ps_siuntos` — kaupiamasis, raktas pagal sandėlį (pakartotinė registracija
  atnaujina, ne dubliuoja). Desk `vp_reg` po kiekvienos sėkmės kviečia
  `Petshop_Siuntos::prideti_is_plugino()`. Plugino raktas NELIEČIAMAS.
- **Sekimo laiškas klientui** — kaip senoje sistemoje (Raimis: „1 laiškas,
  visi sekimo numeriai"), tik be rankinio rašymo: `page=ps-siuntos-laiskas&id=X`,
  peržiūra + mygtukas „Siųsti klientui", įspėjimas „registruota X iš Y".
  Desk veiksmuose — „Sekimo laiškas", kai užsakymas turi numerių.
- **MIXED apmokėjimo laiške** — pastaba „bus pristatytas N atskiromis siuntomis…
  numerius atsiųsime atskiru laišku" (tik processing/on-hold, tik klientui).
- Testai 6/6 (34886 + fake antras sandėlis; plugino meta atstatyta baitas į baitą).

### 8ii.5 ✅ §18.3 UŽBAIGIMO SARGAS (H206)

Užsakymas su `_ps_shipments` > 1 NEGALI tapti `completed`, kol registruotos ne
visos siuntos. `woocommerce_before_order_object_save` grąžina buvusį statusą +
pastaba istorijoje. **Sargas gali tik SUSTABDYTI užbaigimą** (kaip nepamokėtų
modulis). Apėjimas ypatingam atvejui: meta `_ps_uzbaigti_be_siuntu=1`.
Testai 5/5, tarp jų realus `update_status('completed')` → DB liko processing;
laiškai nešauna (from==to perėjimo kabliukai nevyksta); `_stock` NEPAKITO.

### 8ii.6 ✅ ZB PERDAVIMO KELIAS UŽBAIGTAS (§19.12 uodega — H207–H209)
```
mu-plugins/petshop-av-dropship.php  v1.2 · md5 76f28a41170b86da
```
ZB kortelėje perdavimo lange:
- **ZB kodas** paryškintai iš `ps_sources` (stulpelis `source`, ne `supplier` —
  spėjimas krito matavime, pataisyta; Monge 17394→01M210202, Biovet 16953→01BV0101);
- **„Kopijuoti"** prie užsakymo — `kodas TAB kiekis` į iškarpinę (ZB formai);
- **„Lipdukas"** — vieno užsakymo Venipak PDF atsisiuntimas (Raimis prikabina
  ZB sistemoje); neregistruotai siuntai — užrašas vietoj mygtuko;
- **„Pažymėti ZB perduotais"** — `_ps_dropship_sent` + pastaba (laiško kelio
  nėra, todėl žymė rankinė; be jos ZB užsakymai sąraše kabotų amžinai).
⚠️ Lipduko gyvva patikra NEPADARYTA — testinė siunta V07267E1000003 Raimio
ištrinta iš Venipak 08-06; funkcija sena (v1.1 laiškuose veikė). Patikra su
pirma realia siunta.

### 8ii.7 ✅ DROPSHIP SLA SARGAS (H210)
```
mu-plugins/petshop-dropship-sargas.php  v1.0 · md5 561285de45f48121
```
Valandinis cron `ps_dropship_sargas`: perduota tiekėjui (>24 val., filtras
`ps_dropship_sla_valandos`) + vis dar processing/on-hold → `_ps_sla_velavimas`
+ pastaba → desk „Klausimuose" kortelė **„Tiekėjas vėluoja — perduota prieš
24+ val."** Sargas gali TIK uždėti žymę. Savaitgalis — žmogaus „Laukti".
Testai 3/3 (žymėjimas · idempotencija · šviežias nežymimas), viskas išvalyta.

### 8ii.8 BŪKLĖ PO SESIJOS — B REKOMENDACIJŲ PJŪVIS
```
✅ šiandien   kliento komunikacija skaidant siuntą (§18.4) · §18.3 lite
              (registras + užbaigimo sargas) · ZB kelias · dropship SLA aging
⏸ po launch  pilnas siuntos būsenų ciklas (perduota→išsiųsta→pristatyta) —
              nestatyti aklai, laukti realių siuntų (tas pats principas kaip §19.11)
🔴 liko       ES „Atsisakyti sutarties" + KR-AVPN kreditinė (didesnis gabalas)
              SLA/MIXED ataskaitos dienos suvestinėje (po launch, kai bus duomenų)
🟡 stebėti    oversell langas dropship prekėms (ZB/VF sync valandinis) —
              be veiksmo, matuoti po launch
```
Backup'ai: `ps-backup/*.bak_h204|h206|h208|h210` · repo `deploy/` visi keturi
failai · snippetų lentelė išvalyta iki 0 QQ (valytojas ištrynė ir save, H205).

---

## 8jj. ★ DARBALAUKIS: FILTRAI, UI, VENIPAK MANIFESTAI — 2026-08-22 (vakaras) [H211–H232]

**Kontekstas:** tęsinys po §8ii. Raimis testavo darbalaukį gyvai (savo užsakymais,
realiomis Venipak registracijomis) — ir šis testavimas rado defektą, kurio jokie
ankstesni matavimai nebuvo palietę (8jj.6). Sesijos forma: Raimis nurodo → Claude
diegia per tiltą → Raimis tikrina naršyklėje → kita iteracija.

**Versijų kelias per vakarą:** desk **v3.17 → v3.29** (13 versijų),
dropship **v1.2 → v1.6** (5 versijos). Kiekviena versija: token_get_all sargas,
backup į `ps-backup/*.bak_hNNN`, md5, archyvas repo `deploy/`.

### 8jj.1 ✅ ES 14 DIENŲ ATSISAKYMAS (H211–H212) — REGISTRO UODEGA UŽDARYTA

`petshop-atsisakymas.php` **v1.0** (naujas mu-plugin):
- Mygtukas kliento užsakymo puslapyje (details/summary forma, order key + nonce)
- Langas 30 d. nuo sukūrimo — **teisinį 14 d. terminą vertina žmogus**, ne kodas
- `pazymeti()` → `_ps_withdrawal` + `_ps_withdrawal_reason`, gavimo patvirtinimas
  klientui, pranešimas parduotuvei
- Desk: atsisakymas → **Klausimai** su kortele (data, priežastis, tvarkos priminimas)

**Recon H211:** wcdn aktyvus su creditNote; `invoiceNumberFormat=AVPN{order_number}`;
`petshop_avpn_counter=238`, `iapv=145`.
🟡 **KR-AVPN kreditinės numeracija NEPATIKRINTA** — tikrinti vizualiai prie pirmos
realios kreditinės (T-0 sąraše).

### 8jj.2 ✅ FILTRŲ SLUOKSNIS (v3.18, v3.20, v3.21, v3.22)

Raimio prašymas: „uzsakymo nr., data, data nuo iki, klientas, pasto adresas,
pagal telefona, uzsakymo busenas".

| Filtras | Kur | Pastaba |
|---|---|---|
| Bendra paieška | viršus | +adresas, miestas, pašto kodas, įmonė (buvo tik nr./vardas/el.p./tel./prekė/siunta) |
| **Užsak. Nr.** | 2 eilutė | atskiri **matomi** laukai, AND semantika, LIMIT 150 |
| **Klientas / el. paštas / įmonė** | 2 eilutė | |
| **Telefonas** | 2 eilutė | |
| **Adresas / miestas / pašto kodas** | 2 eilutė | |
| Data (+intervalas nuo–iki) | 1 eilutė | buvo |
| **Būsena** | 1 eilutė | **visose eilėse** (buvo tik „Visi“) |
| **Mokėjimas** | 1 eilutė | apmokėta / neapmokėta / Paysera / pavedimas |
| **Laukiantys** | 1 eilutė | 2+ / **3+** / 5+ d. („Amžius“ atmestas kaip klaidinantis — skamba kaip kliento amžius; 3+ pridėta Raimio prašymu: „aktualu 2, 3 d.“) |

Kartu: **sąskaita ir kreditinė be WC lango** — veiksmuose „Sąskaita“ (tik apmokėtiems,
`wcdn_print_invoice`), atsisakymo kortelėje „Kreditinė“ (`wcdn_print_creditnote`),
masinis „Sąskaitos“ apačios juostoje. WC sąrašas nebereikalingas nė vienam
kasdieniam veiksmui.

**Pataisyta:** juodas neįskaitomas dropdown — aktyvaus filtro tamsus fonas
persiduodavo `option` elementams (`.pd-sel option{background:#fff;color:#1C201D}`).

### 8jj.3 ✅ UI PERDARYMAS DARBUI (v3.19)

- Eilutės bazė 50 → **38 px**, mažesni šriftai/tarpai
- **Prekės pilnais pavadinimais**, po vieną eilutę, nekarpomos + **26 px miniatiūros**
  (pakuotojui „…3 kg“ ir „…15 kg“ turi skirtis iš pirmo žvilgsnio)
- Šaltinio ženkliukas **prie kiekvienos eilutės** (ne bendrai užsakymui) — mišriame
  iškart matyti, kuri prekė iš kur
- **Amžiaus indikatorius** po būsena: „nuo vakar“ · „laukia N d.“ (2+ oranžinė, 5+ raudona)
- Ribos tekstas **tik šiandienos** užsakymams (senam „keliaus rytoj“ 17-tą dieną — melas)
- Viršuje po ribų: „ribos šiandien praėjo · nauji keliaus rytoj“
- **Išsiųsti** eilė (kelyje + įvykdyti) apatinėje rail dalyje su skaitikliu (v3.23)
- **Auto-refresh 60 s** (v3.28) su keturiais saugikliais: skirtukas matomas · nieko
  nepažymėta · neatidarytas skydelis/dialogas · žymeklis ne įvesties lauke.
  Rytinėje eigoje išjungtas.

### 8jj.4 ✅ LP + DROPSHIP KONFLIKTAS → KLAUSIMAI (v3.19)

LP Express galimas tik iš AV. Dropship eilutė su LP pristatymu — fiziškai neįmanomas
derinys, kuris kitaip išlįstų tik prie lipduko. Dabar `klausimas()` grąžina
„LP negalimas — siuntoje ne-AV prekių“. Gyvai: #34883 (ZB + LP) perkeltas į Klausimus.

### 8jj.5 ✅ TEISINGAS PRANEŠIMAS PO APMOKĖJIMO (v3.24)

**Raimio radinys:** „sistema parašė, kad užsakymas perkeltas į Nauji — as jo nematau“.
Matavimas: #34952 apmokėtas → `klausimas()` = „Trūksta sandėlyje“ → nukrito į Klausimus,
o pranešimas jau buvo pasakęs „Nauji“ (rašomas prieš klausimo patikrą).

Dabar: `apmoketa_klausimas` → „Užsakymas #N apmokėtas, bet perkeltas į KLAUSIMUS:
{priežastis}. Spręsk kortelėje. [Atidaryti Klausimus]“. Dialogo tekstas nebežada
„Naujų“ absoliučiai.

### 8jj.6 🔴 VENIPAK JSON DEFEKTAS — RASTAS RAIMIO TESTU (v3.26)

**Simptomas (Raimis):** „Rytinė eiga — nesuprantu kas čia, niekas neveikia?“ —
visur „— nėra“, „registruota 0 iš 3“, pakartotinis mygtukas → „Venipak negrąžino
atsakymo“.

**Tikroji priežastis:** `siuntos_kodas()` naudojo `maybe_unserialize()`, o pluginas
`venipak_shipping_order_data` saugo **JSON** eilute. Todėl desk **nuo v2.x
NEMATĖ nė vieno Venipak siuntos kodo**:
- „Paruošta siųsti“ eilė visada 0 (užsakymai su siuntomis likdavo „Naujuose“)
- rytinėje eigoje „registruota 0 iš N“ net po sėkmingos registracijos
- registracija kartojosi → **dublikatai Venipake**

**Fix:** `json_decode()` pirmiau, `maybe_unserialize` kaip atsarga.
**Po fix (gyvai):** 35037 → V07267E1000004 · 35036 → V07267E1000005 · 34886 →
V07267E1000003, visi teisingose eilėse.

**Pamoka:** defektas gyveno kelias versijas, nes dev'e iki šiol nebuvo nė vienos
gyvos registracijos per rytinę eigą — visi testai matavo kodo logiką, ne realų
plugino įrašą. Savininko rankinis testas sugavo tai per 10 minučių.

### 8jj.7 ✅ REGISTRACIJOS SAUGIKLIS (v3.25)

`vp_reg` dabar registruoja **tik** tuos, kurie (a) neturi siuntos kodo ir
(b) turi paštomatą, jei vežėjas paštomatinis. Praleisti — įvardijami pranešime
(„#34881 be paštomato“). Mygtukas rodo **likusį** skaičių arba „✓ visos registruotos“.
Priežastis: pakartotinis paspaudimas siųsdavo dublikatus ir lauždavo visą grupinį XML.

### 8jj.8 ✅ PAKUOČIŲ SKAIČIUS IR PAŠTOMATUI (v3.27) + LAIŠKO PERŽIŪRA (dropship v1.3)

**Raimio klausimas:** „manes niekur nepaklause del pakuociu skaiciaus… i pastomata
taip pat dvieju pakuociu“.

Dvi spragos, abi užtaisytos:
1. `reikia_pakuociu()` klausė tik kurjerio/LP — dabar ir **venipak_pastomatas**
   (į paštomatą kiekviena dėžė = atskira siunta)
2. Grupinė registracija `packs[]` **ignoruodavo** (pluginas taiko tik kai grupėje
   1 užsakymas) — dabar kelių dėžių užsakymai automatiškai atskiriami ir
   registruojami po vieną su savo `packs[]`, likusieji bendrai
3. **Kur nurodyti:** rytinės eigos Venipak lentelėje prie neregistruotų —
   laukelis „dėžių [N]“, išsaugo iškart

**Dropship v1.3:** `laisko_html($src,$uzsakymai)` išskirtas kaip **viena tiesos vieta**
siuntimui ir peržiūrai; mygtukas „Peržiūrėti laišką“ rodo tikslų laišką su tema
(„užsakymas Y-m-d“) ir gavėju.

### 8jj.9 🔴 MANIFESTAI PAGAL SANDĖLĮ MASINIAME KELYJE (v3.29) — RAIMIO ESMINIS RADINYS

**Raimio klausimas (su Venipak savitarnos ekranu):** „Venipak reikia kiekvienam
sandėliui savo manifesto… o dabar viskas vienoje krūvoje“.

**Matavimas:** 35036/35037 (AV, per rytinę eigą) — manifestas 001 ✅ teisingai.
35038 (mišrus) ir 35041 (ZB) — **taip pat 001** ❌, nes registruoti per apatinės
juostos mygtuką, kuris šaudavo į plugino bulk (`shopup_venipak_shipping_dispatch`),
o pluginas turi vieną globalų manifestą.

**Fix:** naujas `vp_bulk` veiksmas. Pažymėtus **grupuoja pagal sandėlį**, kiekvienai
grupei uždeda **jos manifestą** (AV=001, VF=002, ZB=003, Quattro=004, Prins=005,
Ambrosia=006, Belacor=007), taiko tuos pačius saugiklius (jau registruoti / be
paštomato), **mišrius praleidžia** su užrašu „mišrus — per rytinę eigą“, pildo
`_ps_siuntos` registrą. Testas: grupė VF=[35035], praleisti 35041/35038/35037 „jau“.

**Nepataisoma atgal:** 35038 ir 35041 jau guli 001 manifeste Venipak sistemoje —
tvarkoma savitarnoje kartu su testinių siuntų trynimu (Raimio darbas).

⏸ **ATVIRAS KLAUSIMAS RAIMIUI (Q-VP-ADRESAS):** siuntėjo adresas visose siuntose —
UAB Avesa, Liucionys (§16 sprendimas). Manifestai jau atskiri, bet paėmimo adresas
juose lieka Avesos. Ar VF/ZB atveju kurjeris kviečiamas tiekėjo adresu per jų pačių
sutartis, ar reikės ir **paėmimo adresą** keisti pagal sandėlį?

### 8jj.10 ✅ MIŠRŪS UŽSAKYMAI — TAISYKLĖ IŠAIŠKINTA (be kodo)

Raimio klausimai serijoje. Užfiksuota, kad ateity nebūtų aiškinamasi iš naujo:

- **Mišrus ≠ klausimas.** Mišrumas yra normalus darbas → guli „Naujuose“ su būsena
  „Vykdomas“. Klausimai — tik ten, kur sistema pati negali tęsti.
- **Lemia sandėlių skaičius, ne AV buvimas:** `count($sal) > 1`. ZB+VF (du dropship,
  be AV) elgiasi lygiai kaip AV+ZB.
- **Automatika mišriam:** šaltinių priskyrimas, surinkimo lapas AV daliai, laiškai
  **abiem** tiekėjams (užsakymas pats išsiskaido), AV nurašymas, klientui §18.4
  pranešimas apie dvi siuntas.
- **Rankomis lieka VIENAS žingsnis:** Venipak registracija (pluginas vienam užsakymui
  temoka vieną siuntą).
- ⏸ **Q-VP-MISRUS:** ar automatizuoti mišrių registraciją per `_ps_siuntos`
  (registruoti du kartus, kiekvieną su savo manifestu)? Kliūtis: pluginas siunčia
  **viso užsakymo** svorį — abiejose siuntose būtų pilnas svoris → permoka Venipakui,
  nebent skaičiuoti dalies svorį ir perduoti `packs[]`. Sprendimas Raimio.

### 8jj.11 ✅ LAIŠKO TIEKĖJUI REDAGAVIMAS (dropship v1.4–v1.6)

- **v1.4:** kiekiai rašomi „N **vnt.**“ (Raimis: „kartais ir pakuotemis siunciame“ —
  vienetai turi būti įvardyti aiškiai)
- **v1.5:** laukelis **„Prierašas laiške (nebūtina)“** — laisvas tekstas patenka tarp
  lentelės ir „Linkėjimai“; kiekvienam tiekėjui atskiras; vienkartinis (po išsiuntimo
  laukas tuščias)
- **v1.6:** peržiūroje gyvas tekstas rodomas **tikroje vietoje** (prieš linkėjimus),
  ne apačioje — buvo mano klaida piešiant peržiūrą; **pačiame laiške vieta visada
  buvo teisinga**

### 8jj.12 TESTINIŲ DUOMENŲ CIKLAS (H229, H232)

Raimio prašymu: visi užsakymai ištrinti (18, prieš tai atšaukti — likučiai grįžo),
sukurta 12 testinių per visus sandėlius (AV/VF/ZB/Quattro/Belacor/Ambrosia, kurjeris/
paštomatas/LP, apmokėti/ne, du mišrūs, vienas be paštomato). Po Raimio pastabos
(„nera pastomatu, gaunasi ne pilni uzsakymai“) — visi 12 ištrinti. **Lentelė tuščia;
Raimis kurs savo per kasą su tikrais paštomatais.**

Trynimo protokolas (užfiksuoti): `update_status('cancelled')` **prieš** `delete(true)`,
kad suveiktų likučių grąžinimo hookai; `pre_wp_mail → __return_false` visai operacijai;
saugiklis pagal kliento vardą, kad nenušluotų ne savo duomenų.

### 8jj.13 BŪKLĖ PO SESIJOS

```
desk v3.29 · md5 a7ca1391    dropship v1.6 · md5 b3855941
atsisakymas v1.0 · siuntu-laiskai v1.1 · sargas v1.0 (iš §8ii)
backup'ai: ps-backup/*.bak_h213|h214|h215|h216|h220|h222|h228|h230
repo deploy/ : visi failai · snippetai 3690–3711 išjungti
užsakymų lentelė: 0 (sąmoningai — Raimio testams)
```

**Atviri darbai (užsakymų blokas):**

| ID | Kas | Būsena |
|---|---|---|
| Q-VP-ADRESAS | paėmimo adresas pagal sandėlį? | ⏸ Raimio sprendimas |
| Q-VP-MISRUS | mišrių registracijos automatizavimas su svorio dalyba | ⏸ Raimio sprendimas |
| KR-AVPN | kreditinės numeracija vizualiai | 🟡 prie pirmos kreditinės |
| ES POST kelias | atsisakymo mygtukas naršyklėje | 🟡 nepatikrinta vizualiai |
| Paštomato pasirinkimas desk'e | dabar tik per WC užsakymo langą | ⏸ idėja |
| Venipak testinės siuntos | V07267E1000003–007 trynimas savitarnoje | ⏸ Raimio darbas |
| Siuntos būsenų ciklas | perduota→išsiųsta→pristatyta | ⏸ po launch (§19.11) |
| SLA/MIXED ataskaitos | dienos suvestinėje | ⏸ po launch |
| Oversell langas | dropship prekėms | 🟡 stebėti po launch |

**Pamokos (šios sesijos):**
1. **Gyvas savininko testas mato tai, ko nemato jokie kodo testai** (8jj.6 JSON
   defektas gyveno kelias versijas).
2. **Testas toje pačioje užklausoje po `file_put_contents` naudoja SENĄ klasę
   atmintyje** — tikrinti antra užklausa (H217 pamoka; pirmas rezultatas
   `opcija_yra:false` buvo melagingas).
3. **Pranešimas vartotojui turi būti rašomas PO to, kai būklė galutinai nusistovi**,
   ne prieš (8jj.5).
4. **Peržiūra ir siuntimas — viena kodo vieta** (`laisko_html`), kitaip peržiūra
   pradeda meluoti (8jj.11).
5. Terminologija svarbi: „Amžius“ → „Laukiantys“; „kabo“ → „laukia“. Raimis atmetė
   du pavadinimus iš eilės — žodis darbo lange turi būti darbo, ne techninis.

---

## 8kk. ★ MIŠRŪS: DEFEKTAI, EILĖ, DARBO LANGAS — 2026-08-23 [H233–H243]

Dvi sesijos vieną dieną. I sesija (pokalbis „Uzsakymu tvarkymas2") — defektai ir
mišrių architektūra. II sesija (šis registras) — darbo lango srautas, matomumas.

### 8kk.1 I SESIJOS DARBAI (H233–H241) — pagal desk.php antraštę ir sesijos įrašus

| H | Kas | Failas |
|---|---|---|
| H233 | 🔴 **Dvigubo užsakymo rizika:** konsoliduotos eilutės (`_ps_konsolidacija`) vis tiek patekdavo į tiekėjo laišką. Sutvarkyta. + trūkęs trynimo mygtukas atšauktiems | dropship v1.7 · desk v3.30 |
| H234 | 🔴 **`_ps_dropship_sent` buvo viso užsakymo:** perdavus VF, Prins laiškas dingdavo tyliai. Dabar `_ps_dropship_sent_src` JSON pagal sandėlį; senas raktas skaitomas atgaliniam suderinamumui | dropship v1.8 |
| H235 | **Viena mišrumo taisyklė:** mišrus = `sandėlių > 1`, nesvarbu ar tarp jų AV. Sąrašas, filtras ir rytinė eiga skaičiuoja vienodai | desk v3.31/3.32 |
| H236 | **Eilė „Mišrūs" + sprendimo kortelė:** eilutė kiekvienam sandėliui, radio „tiesiai klientui / į AV", gyva suvestinė (siuntų sk., svoris, kliento sumokėtas pristatymas). Nespręsti į rytinę partiją nepatenka | desk v3.33 |
| H237 | Kortelė perdaryta pagal Raimio pastabas; svoris pagal sandėlį rytinėje eigoje. Vežėjo kaštai sąmoningai nerodomi, kol nėra tarifų lentelės | desk v3.34 |
| H239 | 🔴 **Sprendimas atskirtas nuo vykdymo** (Raimis: „sistema vėl sprendžia už mane"): „Patvirtinti planą" tik įrašo `_ps_misrus_sprendimas`. Vykdymą pradeda žmogus: „Į tiekimo partiją" / „Perduoti tiekėjui" / „Keisti planą" | desk v3.35 |
| H240 | **Perdavimas pagal sandėlį:** kiekvienas neperduotas tiekėjas — savo mygtukas („Perduoti VF"), kad išsiuntimo laiką valdytų žmogus ir siuntos klientą pasiektų kartu | desk v3.36 |
| H241 | Sekimo laiško UI (`ps-siuntos-laiskas`): prierašas klientui, peržiūra, mygtukas pilkas kol ne visos siuntos turi kodus | siuntu-laiskai v1.2 |

### 8kk.2 II SESIJA (H242) — MIŠRŪS = SPRENDIMAS + PALEIDIMAS VIENOJE VIETOJE

**Problema (Raimio ekranas):** patvirtinus planą užsakymas dingdavo iš Mišrių į
„Nauji", o žalia juosta liepdavo „paleisk mygtukais" — kurių tame lange nebėra.
Darbuotojas eidavo ieškoti užsakymo kitoje eilėje. Srauto lūžis.

**Sprendimas (desk v3.37):**
- `eile()`: mišrus lieka `misrus`, kol planas turi nepaleistų „į AV" dalių
  (`kons_laukia()` netuščias). Iš Mišrių išeina tik paleidus.
- Mišrių langas — **dvi sekcijos:** „Reikia sprendimo (n)" (radio kortelės) ir
  „Planas įrašytas — nepaleista (n)" (būklės pagal sandėlį + mygtukai
  „Į tiekimo partiją (n)" · „Perduoti X" · „Keisti planą").
- Pranešimas po patvirtinimo dabar sako, KUR užsakymas nuėjo (`misrus_cia` /
  `misrus_ok`). Jei viskas „tiesiai" — grįžta į „Nauji", ten įprastas „Perduoti".
- Suvestinės brūkšnys („per AV —") pašalintas — svoris rodomas tik kai > 0.

**Patikra:** Playwright, #35068 (VF→AV, AMB→AV) ir #35067 (ZB→AV, VF→tiesiai)
matomi antroje sekcijoje su teisingais mygtukais; rail skaitliukas Mišrūs = 4;
JS klaidų 0. Ekranai `screenshots/h242_misrus.png`.

### 8kk.3 II SESIJA (H243) — PIPELINE JUOSTA + BŪSENOS CHIP'AI

**Klausimas (Raimis):** „kaip greitai pasižiūrėti, kur sudėti užsakymai tiekėjams,
kur užsakymai paruošti išsiųsti" — duomenys buvo, matomumo nebuvo.

**Sprendimas (desk v3.38):**
- **Pipeline juosta** kiekvieno lango viršuje, kiekvienas skaičius — nuoroda:
  `Laukia sprendimo · Nepaleisti planai · Neperduota tiekėjams · Laukia prekių ·
  Paruošta siųsti · Išsiųsta šiandien`. Nuliai pritemdyti. Skaičiuojama toje
  pačioje `skaiciai()` praeigoje — papildomų užklausų nėra.
- „Neperduota tiekėjams" atidaro `eile=nauji&zvilgsnis=neperduota` — filtruotą
  sąrašą tik su neperduotom grupėm (patikrinta: skaitiklis 6 = eilučių 6).
- **Chip'ai** Vykdymo stulpelyje (eilėse nauji/misrus/laukia): žalias
  `VF ✓ 11:04` (perduota, laikas iš `_ps_dropship_sent_src`), gintarinis
  `PRI ⏳` (neperduota), pilkas `ZB→AV` (planuota per AV). Plano ženkliuko
  „perduota X" uodega pašalinta — chip'ai tą rodo geriau.
- „Išsiųsta šiandien" — statusas kelyje/įvykdytas su `date_updated_gmt` nuo
  vietinės vidurnakties. 🟡 Formulę pasitikrinti su realiais užsakymais.
- CSS: paleidimo kortelės būklė nebeužlenda ant svorio (tuščias ribos stulpelis).

**Patikra:** Playwright trys langai (`h243_nauji.png`, `h243_misrus.png`,
žvilgsnio filtras) — pipeline, chip'ai, sekcijos matomi; JS klaidų 0.

### 8kk.4 II SESIJA (H244–H245) — SISTEMOS GEOGRAFIJA

**Problema (Raimis):** „praktiškai niekas neaišku koks Tiekimas, kur nuorodos,
kaip pažiūrėti — visiškas chaosas". Tiekimo partijos ir perdavimo ekranas gyveno
atskiruose puslapiuose be jokios matomos navigacijos.

**H244 (desk v3.39):**
- Kairėje juostoje skiltis **ĮRANKIAI**: „Tiekimas" (su kaupiamų/užsakytų partijų
  skaitliukais) ir „Perdavimas tiekėjams".
- Pipeline juostoje nauja grandis **„Partijose"** → ps-tiekimas.
- `kons_ok` pranešime nuoroda „Atidaryti Tiekimą".
- Tuščias langas rodo užsakymo kelio žemėlapį vietoj mįslės „arba viskas padaryta".

**H245 (desk v3.40) — pataisos po Raimio testo:**
- „Partijose" skaitliukas skaičiavo ir TUŠČIAS partijas → JOIN su eilutėmis.
  Ištrintos 2 tuščios testų partijos (Prins #3, VF #4).
- Rail „Perdavimas tiekėjams" vedė į `ps-dropship`, kuris be transient'o atsidaro
  tuščias → dabar veda į `eile=nauji&zvilgsnis=neperduota`.
- Po „Patvirtinti planą" grįžtama su `#pd-m{ID}` inkaru tiesiai prie nuleistos
  kortelės (oranžinis rėmelis per `:target`).
- 🔴 **Tarpinė v3.40 ~10 min buvo sulaužiusi Mišrių langą** (printf trūko `$id`
  argumento paleidimo kortelėje, ArgumentCountError). Pagauta savo vizualia
  patikra, hotfix v3.40.1 tą pačią sesiją.

### 8kk.5 II SESIJA (H246–H248) — TIEKIMO SRAUTAS UŽBAIGTAS

**H246 (tiekimas v1.5) — rankinis ŽB kanalas.** Raimis: „ZB iš viso aš rankiniu
būdu turiu suvesti prekes į jų sistemą". `RANKINIAI = array('zb')`; ŽB partijos
mygtukas „Uždaryti partiją — suvesiu Žalioji Banga sistemoje", laiškas
NESIUNČIAMAS. Prie užsakytų partijų — kopijuojamas sąrašas `SKU⇥kiekis⇥prekė`
su „Kopijuoti sąrašą" (ŽB atveju atidarytas iš karto).

**H247 (tiekimas v1.6) — 🔴 VENIPAK REGISTRACIJA + LIPDUKAS.** Raimis: „kaip
galima mesti viduryje darbo, tai juk pagrindinis darbas". Laiške rašėm „paims
Venipak kurjeris", bet kurjeris nebuvo užsakytas ir tiekėjas neturėjo lipduko.
Dabar paspaudus „Užsakyti iš tiekėjo":
- siunta registruojama `go.venipak.lt/import/send.php` (DOMDocument XML:
  description/manifest/shipment/sender/consignee/attribute/pack);
- `pack_no` rezervuojamas per patį plugino skaitiklį (`venipak_pack_number`
  UPDATE +1, formatas `V{userid}E{7 skaitm.}`) — nesikerta su užsakymų siuntomis;
- lipdukas (`ws/print_label`) prisegamas prie laiško;
- `venipak_pack` + `venipak_manifest` stulpeliai `gaj6_ps_tiekimas` (ALTER TABLE,
  nes dbDelta veikia tik aktyvacijoje); kortelėje — siuntos nr. ir „Lipdukas PDF"
  (`admin_post_ps_tiekimas_lipdukas` su nonce);
- **registracijai nepavykus partija NEUŽDAROMA**, laiškas nesiunčiamas, ekrane —
  ką atsakė Venipak (transient `ps_tiek_vp_klaida_{pid}`).
- Neregistruojama: „tiekėjas atveža pats" ir rankiniai tiekėjai.

**H247.1–H247.2 — Raimio pataisos:**
- Laiške buvo perteklinis tekstas („užregistruota mūsų sutartimi, kurjerio kviesti
  nereikia") → lauk. Liko: *„Siuntos lipdukas — laiško priede. Siuntos nr. X."*
- Buvo sukurtas „Tiekėjų sandėlių adresai" puslapis → **PAŠALINTAS**. Raimis
  kurjerį iš tiekėjo sandėlio kviečia pats Venipak sistemoje, todėl siuntėjas —
  standartinis iš plugino nustatymų (`shopup_venipak_shipping_field_sender*`:
  UAB Avesa, 302568442, Liucionių g. 46, LT-15166, Raimundas Bulakas).

**H248 (tiekimas v1.7) — laiško adresatų varnelės.** Raimis: „laiškus aš pats
išsiunčiu tiekėjams; pradžioje bus niuansų, todėl reikia kad kopija eitų į
terra@petshop.lt". Nustatymas `ps_tiek_laiskai`:
`[ ] Siųsti laišką tiekėjui` (numatyta OFF) · `[x] Siųsti man` (numatyta ON).
Siunčiant tik sau — tema `[PERSIŲSTI {tiekėjas}] …`. Nuėmus abi — partija tik
uždaroma (`uzsakyta_be_laisko`).

### 8kk.6 II SESIJA (H249–H251) — MATOMUMAS: KUR UŽSAKYMAS, KUR LAIŠKAI

**H249 (desk v3.41/3.42) — eilės ženklas.** Raimis: „kur, kokioje eilėje visi šie
užsakymai yra?". „Visi užsakymai" ir paieškos rezultatuose po numeriu — spalvotas
eilės ženklas-nuoroda (`.pd-ez`, spalva iš `EILES[$k][2]`). Atskirose eilėse
nerodomas. ⚠️ Pirmas bandymas nerodė nieko — `lentele()` negaudavo `$eile`/`$f`;
reikėjo išplėsti parašą `lentele($eilutes,$statusai,$eile='',$f=array())`.

**H250 (dropship v1.11 + tiekimas v1.7.1 + desk v3.42) — laiškų archyvas.**
Raimis: „kur visi šitie laiškai yra? sakei padarei, kad ateina į terra@petshop.lt
— kur?". Spraga: varnelės buvo TIK tiekimo lange, o `ps-dropship` siuntė tiesiai
tiekėjui be kopijos. Dabar:
- dropship lange tos pačios dvi varnelės, bendras `ps_tiek_laiskai`;
- `Petshop_AV_Dropship::archyvuoti()` → `ps_laisku_archyvas` (200 paskutinių:
  laikas, kam, tema, kontekstas, priedų vardai, pilnas HTML);
- rašo ir dropship, ir tiekimo partijų laiškai;
- užsakymo pastaba dabar nurodo tikruosius gavėjus.
- ⚠️ `ps-laiskai` iš pradžių davė 403 — submenu registruotas anksčiau už tėvinį
  meniu; pataisyta `add_action('admin_menu', …, 20)`.

**H251 (dropship v1.12) — „Laukia išsiuntimo".** Raimis: „reikia lauko, kur dar
neišsiųsti laiškai, jei noriu ko nors pasižiūrėti". Įrankiai → **Laiškai**, du
skirtukai:
- **Laukia išsiuntimo (n)** — `laukiantys_perdavimo()` paima VISUS processing
  užsakymus su `neperduotos()` (ne tik pažymėtus transient'e), grupuoja pagal
  tiekėją: užsakymų sąrašas, tiekėjo el. paštas, „Peržiūrėti laišką"
  (`laisko_html` inline) ir „Eiti perduoti X →". Apačioje — blokas „Prekės į AV
  sandėlį" su kaupiamomis partijomis ir nuoroda į Tiekimą.
- **Išsiųsti (n)** — archyvas su pilnu tekstu.

### 8kk.7 BŪKLĖ PO 2026-08-23/24 SESIJOS

```
desk v3.42          md5 4034d65f7033c464a2c796075e41f122
dropship v1.12      md5 dd66608b0e9c57ced81c681863d9c897
tiekimas v1.7.1     md5 03aa64ebbf1293a00db0600c738ba426
backup: ps-backups/*.bak_h242 … .bak_h250
repo deploy/ : sinchronizuota (b64 + plain, visi trys)
TEMP snippetai 3772–3795 — deaktyvuoti (trinti per WP admin)
DB: gaj6_ps_tiekimas + venipak_pack, venipak_manifest
Nauji options: ps_tiek_laiskai, ps_laisku_archyvas
testiniai užsakymai 35055–35068; partijos #2 ŽB, #5 VF, #6 AMB (kaupiamos)
```
...
| ID | Kas | Būsena |
|---|---|---|
| VP-TESTAS | Pirmas realus „Užsakyti iš tiekėjo" — patikrinti, ar Venipak priima XML, ar lipdukas teisingas | 🔴 pirmas rytojaus darbas |
| D-kosmetika | suvestinės žodynas, terminų triukšmas eilutėse, filtrų slėpimas Mišriuose | ⏸ |
| Išsiųsta-šiandien formulė | `date_updated` vs siuntos registracijos data | 🟡 stebėti |
| Etapas 4 laiškas | vienas sekimo laiškas visoms siuntoms — E2E su realia siunta | 🟡 |
| ŽB partija #2 | eilutė rodo į ištrintą užsakymą #34882 — sena šiukšlė, išvalyti | 🟡 |

**Pamokos (II sesijos):**
1. **Sprendimo ir vykdymo langas — tas pats langas.** Jei pranešimas liepia
   „paleisk mygtukais", mygtukai privalo būti matomi tame pačiame ekrane.
2. **Matomumas > nauji mechanizmai.** Pipeline, chip'ai, eilės ženklai, laiškų
   langas nepridėjo nė vieno naujo duomens — tik parodė esamus.
3. **Nebaigto srauto negalima palikti „vėlesniam etapui".** Laiškas, žadantis
   kurjerį, kurio niekas neužsakė, yra defektas, ne neįgyvendinta funkcija.
4. **Ta pati taisyklė turi galioti visuose languose.** Varnelė tik viename iš
   dviejų laiškų siuntimo ekranų = spraga, kurią Raimis rado per 10 minučių.
5. Nieko neišradinėti: kai Raimis sako „procedūra jau suderinta", reikia daryti
   tiksliai taip, o ne pridėti paaiškinimų tiekėjui ar naujų adresų puslapių.
6. Bash heredoc be kabučių žaloja `$` runnerio kode → runnerį generuoti tik per
   python; dispatch be `browser=1` neįdiegia Playwright.
7. Vizuali patikra pagauna tai, ko JSON nepagauna (ArgumentCountError, 403).

---

## 8ll. ★ ATASKAITŲ SISTEMA — E0 RECON UŽDARYTAS — 2026-08-26 [S1260–S1269]

**Statusas:** ✅ E0 baigtas, spragų nėra. Kodo **nerašyta**. E1a nepradėtas —
laukia 6 Raimio sprendimų.
**Dokumentas:** `dokumentai/ataskaitu_recon_v1.md` v1.1, commit `a247d81`, 37 936 B.
**Metodas:** 5 tilto runai `browser=0` (#4945–#4949), rezultatai
`deploy/r010–r014_recon.json`. Skenuota **137 failai** + 53 lentelės, 0 spėjimų —
kiekvienas teiginys su failo eilute arba DDL.

### Ką rado — trys sprendimą keičiantys radiniai

| # | Radinys | Pasekmė planui |
|---|---|---|
| **1** | Abu Desk Venipak keliai (desk:693–695 grupinis, desk:782–784 vieno sandėlio) kviečia tą pačią `Petshop_Siuntos::prideti_is_plugino()` (siuntu-laiskai:58) | **Q7 nereikalingas.** `do_action('petshop_siunta_sukurta',…)` dedamas į `petshop-siuntu-laiskai.php:77`. `petshop-desk.php` lieka **nepaliestas** |
| **2** | `class-shipments.php:14–16` — „SĄMONINGAI NESAUGOM: sandėlio, konkrečių prekių ID… inferred reikšmės vėliau taptų melagingais duomenimis" | `ps_shipments` **negali** būti `ps_fakt_siuntos`. §4.3 = atskira lentelė + nuoroda į `shipment_id` |
| **3** | `ps_shipments` 23 našlaičiai (order_id 35059–35066, `wc_orders`=0); `agreguoti_diena()` persuka tik 3 d. (agregavimas:28) | **T-0 rizika.** Q8 nepakanka — reikia atskirai valyti `ps_shipments`, `ps_carts`, `ps_ataskaitu_dienos` sritis `pardavimai`/`parduotuve`. Dev'e „pajamos" = 781,51 € iš 21 neegzistuojančio užsakymo |

### Spragos patvirtintos empiriškai (grep, 137 failai)

```
utm_source                                   0 rezultatų
utm_                                         0
woocommerce_order_refunded                   0
woocommerce_product_object_updated_props     0
```
→ plano §1 spragos #4 (kanalas), §4.4 (grąžinimai), §4.6 (kainų istorija) — tikros.

**`payment_complete` prioritetai UŽIMTI iki 30:** av-order p5 → av-reduce p15 →
core/event-emitters p20 → partijos p25 → refill-engine p30.
`petshop-faktai.php` privalo eiti **≥35**.

### Trys moduliai, kurių master plane nėra

| Modulis | Ką daro | Pasekmė |
|---|---|---|
| `petshop-pardavimai.php` v1.0 | ABC, 30/90/365 d., marža, dienos atsargai iš `wc_order_product_lookup`+`wc_order_stats` į prekės meta `_ps_sales_*`/`_ps_abc`; cron 04:50 | §5.4 ir §5.5 jį **dubliuotų** |
| `petshop-innodb.php` v1.0 (S912) | `add_filter('query')` prideda `ENGINE=InnoDB` kiekvienam CREATE TABLE | §3 taisyklė 5 **jau įgyvendinta**; visos 53 `ps_*` lentelės InnoDB |
| `petshop-statistika-vitrina.php` v1.0 | vitrinos statistika | įtraukti į §1 inventorių |

### Kas JAU yra ir neturi būti kuriama iš naujo

- **Savikaina eilutėje užšaldoma:** `_ps_savikaina_vnt` + `_ps_savikaina_saltinis`
  (statistika:60–61, :276–307). §4.2 **kopijuoja**, neperskaičiuoja.
- **MNM/DP/dovanų išpjaustymas:** `agregavimas::pardavimai()` :282–405 per
  `_mnm_cart_key`/`_mnm_container`. §4.2 perima.
- **Krepšelio piltuvėlis:** `ps_carts` (`converted_order_id`, `status`); dev'e
  `converted` 4 / `abandoned` 31 / `expired` 46 / `active` 1. §4A.3 šaltinis yra.
- **Promotion uplift:** `ps_akcijos` + `ps_akciju_prekes` (`reg_kaina`/`akc_kaina`/
  `nuo`/`iki`). §5.6 šaltinis yra.
- **Sell-through per tiekėją:** `ps_tiekimas.gauta` + `ps_tiekimas_eil.qty_gauta`.
  §5.7 šaltinis yra.
- **Ataskaitų karkasas:** `Petshop_Ataskaitu_UI` turi visus §3 taisyklės 6 metodus
  (`juosta`, `kpi`, `diagrama`, `lentele`, `piltuvelis`, `maza_imtis`).

### Ko NĖRA (naujas kodas privalomas)

- Užsakymo/eilutės faktų lentelių (`ps_fakt_*`) — nė vienos.
- Kanalo/UTM fiksavimo — jokio.
- Grąžinimo fakto kabliuko — jokio.
- Kainų istorijos kabliuko — jokio.
- Email paspaudimo laiko `ps_email_jobs` lentelėje — **nėra**, tik `sent_at` →
  §5.9 „paskutinis paspaudimas 5 d." kaip aprašyta neįgyvendinama.

### Versijos — plano §1 lentelė pasenusi

| Modulis | Plane | Realiai |
|---|---|---|
| `petshop-statistika.php` | 2.1 | **2.2** |
| `petshop-desk.php` | — | **v3.47 (H265)** |
| `petshop-akcijos.php` | — | **1.8** |
| UI / agregavimas / eksportas / fulfillment-source | 1.0 | 1.0 ✅ |

### Options ir cron — T-0 reikšmės (išmatuota)

| Option | Reikšmė | Statusas |
|---|---|---|
| `ps_paleidimo_data` | `2026-10-01` | ✅ įrašyta |
| `ps_stat_pradzia` | `false` | 🔴 T-0 |
| `ps_stat_pvm` | `false` | 🔴 T-0 |
| `woocommerce_prices_include_tax` | **`yes`** | ⚠️ kainos DB su PVM — §3 taisyklė 3 reikalauja apibrėžti, kur PVM nuimamas (S842 pamoka) |
| `ps_ataskaitu_agregavimas` cron | 00:15 UTC = **03:15 vietos** | ✅ atitinka planą |

Papildomai: **17 užsakymo statusų**, iš jų **9 LP Express** (`wc-lp-*`) — §5.x filtrai
turi imti Desk `STATUSAI` žemėlapį (desk:275–281), ne WC septynetuką.

### ⏸ LAUKIA RAIMIO — 6 klausimai prieš E1a
...
|---|---|---|---|
| **Q-E0-1** | Ar `do_action('petshop_siunta_sukurta',…)` eina į `petshop-siuntu-laiskai.php:77`, o `petshop-desk.php` lieka nepaliestas? (§8ll) | E1b siuntų faktai | prieš E1a |
| **Q-E0-2** | Patvirtinti: `ps_fakt_siuntos` = **atskira** lentelė, nes `ps_shipments` sandėlio nesaugo principingai (class-shipments.php:14–16, §8ll) | E1b | prieš E1a |
| **Q-E0-3** | `petshop-pardavimai.php` v1.0 (ABC, 30/90/365 d., marža) — palikti ir §5.4/§5.5 statyti ant jo meta, ar perkelti į faktus? (§8ll) | E5 apimtis | prieš E1a |
| **Q-E0-4** | Kur nuimam PVM: rašymo momentu ar ekrane? `woocommerce_prices_include_tax=yes` (§8ll) | visi pinigų KPI | prieš E1a |
| **Q-E0-5** | Email atributacija be paspaudimo laiko: `sent_at` bazė ar naujas laukas `ps_email_jobs` kontrakte? (§8ll) | §5.9 | prieš E6 |
| **Q-E0-7** | Ar T-0 sąrašą papildom: išvalyti `ps_shipments` + `ps_carts` + `ps_ataskaitu_dienos` testinius įrašus? Q8 nepakanka (§8ll) | T-0 · DoD #9 | prieš T-0 |
| **Q-VP-ADRESAS** | Venipak paėmimo adresas: ar VF/ZB siuntoms reikia tiekėjo adreso vietoj Avesos? Manifestai jau atskiri (§8jj.9) | Venipak tikslumas | prieš launch |
| **Q-VP-MISRUS** | Ar automatizuoti mišrių užsakymų Venipak registraciją (svorio dalyba + packs[])? Dabar rankomis (§8jj.10) | mišrių sparta | — |
...
| ~~Q-MENIU~~ | ✅ **UŽDARYTA 2026-08-21:** tėvinis punktas `#` yra teisingas (tik atidaro pomeniu); visi PENKI vaikiniai vedė į 404 — pataisyti, atsako 200 (§8hh) | — | — |
| **Q-SLUG-RINK** | 🟡 Du „Susidėk“ slug'ai apsikeitę vietomis: **skanėstų** → `/kramtalu-deze-suniui/`, **kramtalų** → `/skanestu-deze-suniui-hipoalergine/`. Keisti pigu dabar, po launch — 301 skola (§8hh) | UX · SEO | prieš T-0 |
| **Q-TEST-PAV** | 🟡 Septyni „TEST Konservu deze…“ pavadinimai be nosinių; #34944 dabar oficialus konservų šunims **įėjimas iš meniu** (§8hh) | vitrina | prieš T-0 |
| **Q-KRAMT-IEJ** | 🟡 Kramtalų įėjimu paskirta #34933 „hipoalerginė“ — **Claude nuožiūra**, tarp dviejų poreikio dėžių natūralių durų nebuvo. Perkeliama viena žyme (§8hh) | vitrina | — |
| **Q-SARGAS-SVARA** | 🟡 **Sutarta, neparašyta.** Naktinė duomenų ataskaita prie esamo `petshop-sargas.php`: pirmos naktys BE taisymų, valymas tik po GALUTINIO trynimo (§8hh) | higiena | — |
...
| ~~Q-ZB-SAKNIS~~ | ✅ **UŽDARYTA 2026-08-21:** po nakties ZB importo pavadinimuose **0** (publish 0, dvigubai koduotų 0). Pataisa laikosi (§8hh) | — | — |
