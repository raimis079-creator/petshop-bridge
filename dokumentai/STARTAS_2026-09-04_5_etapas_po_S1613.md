...
- `mu-plugins/petshop-darbalaukis.php` **v3.14.1** (235 926 B, md5 `dc990f3c7577508b47e1f7465ab3066b`) = repo `deploy/petshop-darbalaukis.php` (+ `.php.b64`). Kopijos `ps-backups/petshop-darbalaukis-v3111…v314-BACKUP-2026-09-04.php`.
...
- Testinis klientas 5787 (`s1609.klientas@avesa.lt`). Testiniai užsakymai **#35414–#35444, #35450, #35771–#35772, #35774–#35780** — trinti 6 etape. Po S1613: #35416 completed (fiktyvus LP CC000000001LT, `lp-delivered`), #35414 su `_ps_velavimo_laiskas`, #35418 completed (ZB, sistema), #35421 processing (Prins išsiuntė, laukia AV). Opcijos `ps_e3_oid`, `ps_e3_oid2`, `ps_audit_mail`, `ps_venipak_sekimas_paskutinis`, `ps_velavimo_laiskai_paskutinis` — trinti su testiniais.

## Ką darbalaukis daro pats (v3.14.1, trumpai)
...
## PIRMA — Raimio atsakymai (S1612/S1613 prielaidos, spec §12.3 sąrašas 1–8)
1) sekimas 30 min / 60 d. / ≤200; 2) „grįžta“ pagal tekstą; 3) kelių dėžių „paėmė“ nuo pirmos; 4) dalinis grįžimas — 5 etapas; 5) „Siųsti iš naujo“ `_ps_av_reduced_qty=q`; **6) vėlavimo laiško tema „Užsakymas Nr. N — surinkimas užtruko“ + antraštė „Surinkimas užtruko“; 7) „Likusių … prekių … Išsiųsime jį“ (ar „jas“); 8) užsakymas Klausimuose „[T] vėluoja“ vėlavimo laiško NEGAUNA** — palikti ar keisti (viena sąlyga). Radiniai sprendimui: V14 (variklio `klausimas()` LP klaidos nemato — taisyti variklyje?), LP plugino kabliukas (realus API keičiant statusą).

## 5 ETAPAS (eilės nekeisti be Raimio) — spec §7/§12
Redaguoti (adresas/paštomatas, kiekiai, dalinis atšaukimas/grąžinimas/persiuntimas), Sąskaita (AVPN/IAPV), „Kaip mato klientas“ (svečiui), „Naujas užsakymas“ (telefoninis), „Atsiėmimas AV“ (`local_pickup`), paskyros UI lokalizacija. Kiekvienam — spec/registras pirma, recon, Raimio sprendimai, tik tada kodas.
