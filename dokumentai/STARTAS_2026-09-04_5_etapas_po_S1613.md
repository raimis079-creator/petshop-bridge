...
1) sekimas 30 min / 60 d. / ≤200; 2) „grįžta“ pagal tekstą; 3) kelių dėžių „paėmė“ nuo pirmos; 4) dalinis grįžimas — 5 etapas; 5) „Siųsti iš naujo“ `_ps_av_reduced_qty=q`; 6–7) atsiimta (tekstas ir tema „Jūsų užsakymą Nr. N dar komplektuojame“ — Raimio, v3.14.2); **8) užsakymas Klausimuose „[T] vėluoja“ vėlavimo laiško NEGAUNA** — palikti ar keisti (viena sąlyga). Radiniai sprendimui: V14 (variklio `klausimas()` LP klaidos nemato — taisyti variklyje?), LP plugino kabliukas (realus API keičiant statusą).
