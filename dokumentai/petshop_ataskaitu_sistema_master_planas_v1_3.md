# PETSHOP ATASKAITŲ SISTEMA — MASTER PLANAS v1.3
2026-08-26 (v1.3 — E0 Recon įtrauktas; Q-E0-1…7 užrakinti; §15 keitimai 1–21 priimti; naujas radinys #22 — PVM vienetų nesutapimas) · ankstesnė: v1.2 2026-08-25 · Autorius: Claude (planas) · Vykdytojas: Opus (kodas) · Kontrolė: Claude · Sprendimai: Raimis
Horizontas: 5 metai. Statusas: PLANAS v1.3 — **PATVIRTINTAS**. E0 ✅ uždarytas. Opus pradeda **E1a**.
...
10. **v1.3 / Q-E0-4:** santykiai ir dalys skaičiuojamos **tik tarp to paties vieneto dydžių** (be PVM su be PVM). S842 pamoka (225 %) + §0A radinys #22.
...
- `kaina_vnt_ct` ← `$item->get_total() / kiekis` (**be PVM**); PVM eilutėje nesaugomas atskirai — jis §4.1 lygyje (Q-E0-4).
...
| **E1a Faktų minimumas** | 4.1 + 4.2 + kanalas iš WC sesijos (4.7 minimumas) + Paysera/pakuotės nustatymai + kontribucija; `petshop-faktai.php` v1.0. **v1.3:** kabliukas p≥35; savikaina kopijuojama iš `_ps_savikaina_vnt`; `kaina_ct`/`pvm_ct` iš `line_total`/`line_tax`; išpjaustymas perimtas iš `agregavimas::pardavimai()` | 1 realus dev užsakymas (mišrus, su dovana, su kuponu, su utm) → 4.1/4.2 sutampa su Woo iki cento; kontribucija ranka kaip S817; `klientas_naujas` teisingas antram užsakymui; **`Σ kaina_ct + Σ pvm_ct = $order->get_total() − pristatymas`** | sesija 2 |
| **E1b Siuntos + grąžinimai** | 4.3 (`do_action` į **`petshop-siuntu-laiskai.php:77`**, Desk NELIEČIAMAS — Q-E0-1), naktinis `ps_shipments` sutikrinimo cron (`sandelis=NULL` aplenkusioms), 4.4, `ps_tarifai`, backfill dev (testiniai=1). **v1.3 / Q-E0-7:** pridėti `Petshop_Ataskaitu_Agregavimas::perskaiciuoti($nuo, $iki)` — agregatų persukimas bet kuriam intervalui, ne tik `PERSUKTI=3` | siunta Desk'e → 4.3 eilutė su tos dienos tarifu; siunta **aplenkiant** Desk → cron sukuria faktą su `sandelis=NULL`; grąžinimas → 4.4 ir 5.2 tos dienos minusas; `perskaiciuoti('2026-08-01','2026-08-05')` perrašo 5 dienas idempotentiškai | sesija 3 |
...
3. **Neliečiama:** `petshop-laukai.php`, Fulfillment, AV_Limit, Stock_Service, `petshop-xml.php`, importų profiliai, **`petshop-desk.php` (VISAS — Q-E0-1, Q7 atšauktas kaip nereikalingas)**, `petshop-pardavimai.php` (Q-E0-3), `core/class-shipments.php`. Vienintelis leistas svetimo failo pakeitimas: **`petshop-siuntu-laiskai.php:77`** — viena `do_action` eilutė.
...
---
**Kitas žingsnis: Opus pradeda E1a** (§7) — `petshop-faktai.php` v1.0, lentelės §4.1 + §4.2, kanalo minimumas §4.7, nustatymai §4.9.
E0 ✅ uždarytas (`dokumentai/ataskaitu_recon_v1.md` v1.1). Claude tikrina DoD kiekvieno etapo pabaigoje; Raimis sako „toliau“ arba „taisyti“.
