# deployment_log.md — v1.3.60

## IRASAI (naujausi virsuje)

---

## 2026-07-14 — REFILL ENGINE (M11) [S193]

Self-calibrating refill tracking. petshop-core v0.7.0. Svarus — 0 fatal. Aukstiausias decision Nr.: S193.

### S193-A — Dizaino sprendimas

Recon patvirtino: WC weight tuscias, pa_pakuotes_dydis formatas nevienodas. Teorinis "kiek gramu per diena" skaiciavimas = daug prielaidu = daug klaidu. SPRENDIMAS: self-calibrating is pirkimo istorijos (mokosi is kliento realaus elgesio, ne is teoriniu normu). Per klienta+produkta (pet_id nullable — galima susieti veliau).

### S193-B — Petshop_Refill_Engine (364 eil.)

DB gaj6_ps_refill_tracking: user_id, product_id, pet_id(NULL), last_order_id, last_purchase_date, purchase_count, avg_interval_days, predicted_empty_date, confidence, status. UNIQUE(user_id, product_id).

Srautas: woocommerce_payment_complete + processing/completed (priority 30) -> tikrinam ar uzsakyme yra maisto produktu (FOOD_CATEGORY_SLUGS: sausas-maistas-sunims/katems, maistas-sunims/katems, hipoalerginis) -> track_purchase().

### S193-C — Kalibravimo logika

1-as pirkimas: estimate_interval() pagal pa_pakuotes_dydis (<500g->14d, 500-3000g->30d, >3kg->60d; fallback WC weight; default 30d). confidence=0.4.

2+ pirkimas: realus intervalas tarp pirkimu. Svertinis vidurkis (0.3*senas + 0.7*naujas — labiau svertam i nauja). Ribos 7-180d. confidence=0.7 (2-as) arba 0.9 (3+).

parse_weight_grams(): parsina "1,5 kg", "720 g" i gramus (kablelis kaip desimtainis, kg/g).

### S193-D — Cron + refill_due event

wp_schedule_event('ps_refill_daily_check', daily, 08:00). check_due(): randa active irasys kur predicted_empty_date <= today+5d -> fire refill_due event per Registry (schema: pet_id, product_id, predicted_empty_date, confidence). Po fire -> status='notified'.

Sender mirror: PS_REFILL_CANDIDATE=true/false, PS_NEXT_REFILL_DATE=artimiausia data.

### S193-E — Apsaugos

Idempotencija: order meta _ps_refill_tracked='yes'. Begalinio ciklo apsauga: remove/add_action apie save(). Tik maisto produktai (is_food_product tikrina product_cat slug'us). Tik registruoti klientai (ne sveciai). Tik apmoketi (date_paid tikrinama).

### S193-F — Empirinis testas

1-as pirkimas: interval=30d(default), confidence=0.4, predicted=Jul4. 2-as pirkimas (po 40d): interval=37d(kalibruotas), confidence=0.7, predicted=Aug20. Idempotencija OK. Tik food tracked (1 irasas). Cron uzregistruotas Jul15 08:00. Visi ✅.

### S193 — LIVE

petshop-core v0.7.0. Refill Engine M11. gaj6_ps_refill_tracking lentele. Cron aktyvus.
