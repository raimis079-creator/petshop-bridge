...
## Darbalaukis v3.43 (GYVAI, ping 200, VERSIJA 3.43) — atšaukto užsakymo atkūrimas, kai klientas sumokėjo pavedimu
Raimis: klientė pasirinko Paysera, neapmokėjo, WC po 90 min (hold stock) atšaukė, po to sumokėjo bankiniu pavedimu.
- md5 8b2f896b2020fe09179e357b6729e264 (buvo v3.42 fc993421…); bak `ps-backups/petshop-darbalaukis.php.bak_s1680k`; repo `deploy/petshop-darbalaukis-v3.43.php`, DATA `deploy/dl_v343.txt`, įrankis `irankiai/s1680_k.php`.
- Atšaukto NEAPMOKĖTO užsakymo kortelėje (be grąžinimų, be AVPN PDF) — mygtukas „Gavome pavedimą — atkurti" (dialogas su suma; GET `atkurti`, `atkuriamas()`/`atkurti_vykdyti()`): mokėjimo būdas → `bacs`, žymės `_ps_av_reduced`/`_ps_av_restored` išvalomos (jei užsakymas anksčiau buvo processing → cancelled, kad AV mažinimas suveiktų iš naujo), pastaba, `payment_complete()` (WC leidžia iš `cancelled`) → processing, AV/variklio kabliai (fiksuoti, mazinti, partijos) suveikia kaip įprastam apmokėjimui, laiškai įprasti. Pending→cancelled atveju AV žymių nėra — kelias švarus.
- Netestuota gyvai; pirmas testas — Raimio atvejis.
