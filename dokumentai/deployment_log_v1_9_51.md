# DEPLOYMENT LOG v1.9.51

### S1644–S1660 (2026-09-09, naktis 00:30–03:00) — **T-0 MIGRACIJA ĮVYKDYTA. petshop.lt GYVAS ant naujo serverio, realus €1,00 Paysera pirkimas praėjo (AVPN011000, likutis nurašytas).** Startas kitam langui: `STARTAS_2026-09-09_po_T0.md`.

> Pridėti po šia antrašte (virš S1643).

- **Testinis užsakymas Paysera (`s1644.php` GO/CL):** #35867 (Miamor 2,05 €, stock 0) sukurtas ir ištrintas; galutinis **#35868** — JosiDog JOS0888 ×1, €1,00, stock 1133, gw paysera, pending, key `wc_order_jhEMy1YBoScj1`.
- **Semantika (`s1645.php`):** counter = KITAS numeris (variklio komentarai: KR „KITAS, kaip AVPN"; PPK „sava eilė nuo 101"). Raimis: **PVM sąskaitos nuo 11000** → AVPN 11000, KRAVPN/IAPV/PPK 101, wcdn 388 NELIESTA. Metadatos: `ps_laisku_archyvas` 0, dev-pastas log lentelės nėra, e2e/e17 ids nėra; **38 `ps_*_bak` opcijų PALIKTA** (valymas po T-0); `dev.avesa.lt` options 11.
- **Veidrodžio recon (`s1646.php`):** dev-router.php ir dev/ katalogo NEBĖRA (išvalyta anksčiau). Rasta: root `perkelti-r186.php`, MU `petshop-dev-veidrodis.php` + `petshop-dev-pastas.php`, wp-config R194 blokas. Paysera settings tik `enabled` (test_mode kitur, žr. žemiau).
- **BLOKAS A (`s1647.php`):** skaitikliai 366→**11000** · 107→101 · 172→101 · 104→101; blog_public 0→**1**; force_ssl no→**yes**; wp-config **R194 pašalinta** (403 B, BAK `ps-backups/wp-config-BAK-T0-*.php`); **ištrinta su BAK**: perkelti-r186.php, petshop-dev-veidrodis.php, **petshop-dev-pastas.php (laiškai nuo šiol išeina realiai)**; `ps_dev_pastas_leisti` delete; super-cache konfige dev 0; flush.
- **Paysera test_mode (`s1648.php`):** rasta `paysera_payment_main_settings` (projektas 29276) ir `paysera_delivery_settings` — abiejose `test_mode` yes→**no**.
- **BLOKAS B (`s1649.php`):** `siteurl`+`home` → **https://petshop.lt**; 9 opcijos su dev.avesa.lt perrašytos PHP lygiu (flatsome_registration, ps_feeds_paskutinis, wp_subscription_paypal, ps_cc_v2_preview, ps_import_tempas_host/paskutinis, cmplz_transients, 2 transient) → **liko 0**; flush.
- **DNS (Raimis, klientams.iv.lt zona id=447163, TTL 15 min):** A @ .16→**79.98.29.24**, A @ .15 pašalinta (reikšmės ištuštinimas); www CNAME/MX/SPF/DKIM/DMARC/dev1 nekeista. Propagacija: 8.8.8.8/1.1.1.1 naują IP rodė ~3 min.
- **SSL incidentas:** serveris atidavė SENĄ 2022-12 pasibaigusį sert. (klaidingai nukreipiau į iv.lt mokamus — teisinga vieta **DirectAdmin isopas.serveriai.lt:8443/CMD_SSL?domain=petshop.lt**, serveris gyvunai2). Perjungta „Naudoti jau įdiegtą" → **Let's Encrypt** — veikia (telefonas/inkognito). Raimio PC ESET SSL Filter kešavo seną — lokali, ne serverio.
- **ABU-3 (`s1650.php`, Playwright, bridge WP URL perjungtas į petshop.lt):** host petshop.lt, is_ssl 1, siteurl ✓; feed failai kaina24 6,0 MB · kainos 5,6 MB · google 5,0 MB; kadrai `t0_titulinis/t0_preke/t0_paymt` — titulinis NAUJAS, CSS/img ✓, JS/HTTP klaidų 0; order-pay svečiui prašo el. pašto patvirtinimo (WC guest verification — norma). Node fetch TLS validavo → sertifikatas galioja.
- **RAIMIS-4 (`s1651.php`): REALUS €1,00 Paysera 02:30:57 — PRAĖJO.** Callback automatinis, likutis 22763 1133→**1132**, **AVPN011000** (nauja numeracija ✓), faktai įrašyti (pajamos 0,83 be PVM · PVM 0,17 · marža 0,14 · kontribucija −0,31), laiškai „Naujas užsakymas" + „Įvykdytas" išėjo realiai. `transaction_id` tuščias (plugino elgsena, ne klaida).
- **RADINYS + PATAISA — statusas po apmokėjimo:** užsakymas gavo **completed**, ne processing (aplenktų darbalaukį!). Priežastis (`s1653–s1657`): pluginas `woo-payment-gateway-paysera` skaito `paysera_payment_status_settings` (`PayseraPaymentSettings::STATUS_SETTINGS_NAME`), opcijos NEBUVO → default `->setPaidOrderStatus('wc-completed')` (`PayseraPaymentSettingsProvider`). Filtrų `payment_complete_order_status` mūsų kode 0, prekė ne virtual, needs_processing 1. **Įrašyta** `paysera_payment_status_settings['paid_order_status']=['wc-processing']`; verifikacija (`s1658.php`) per patį providerį: paid=**wc-processing**, pending=wc-pending ✓. #35868 liko completed (istorinis; trinti/palikti — Raimio sprendimas atviras).
- **Galutinė (`s1659.php`, `s1660.php`):** Warning 0, sargas tuščias, cron 84 gyvi, feed amžius ~22 h (atsinaujins 04:30), skaitikliai avpn **11001**/101/101/101, užsakymų 1, Σ `_stock` **293 355** (−1 JosiDog). Aktyvių snippetų 71 = LIVE produkciniai (Build-a-box, Feeding Calc, 301, Stock Service, Sources…) — NELIESTA; TEMP 1 neaktyvus — norma.
- **Bridge pastabos:** `mjs_template.mjs` WP → `https://petshop.lt` (lokaliai). run.sh fazė NEgali nešti papildomų GET parametrų (encodeURIComponent) — parametrus hardkodinti payload'e. Konteineryje petshop.lt per proxy neprieinamas (host_not_allowed) — visos patikros per bridge.
- **Cron perkelti (Raimis, 03:16, DirectAdmin CMD_CRON_JOBS):** visos 12 eilučių → https://petshop.lt (Claude sutikrino raidė į raidę: grafikai, import_key=v, id 3/2/7/5, backup bkY7…, watch wTn5…); gyvybė patvirtinta serverio pusėje (`s1663.php`): `ps_import_tempas_paskutinis` 03:08:47 id=3 processing hostas petshop.lt, 200 „Import #3 complete". PC ESET — perkrovus sutvarkyta. Flatsome licencija — atidėta rytui (Raimis).
- **LIKO (rytas):** Raimis — Sender.net webhook, GSC + Ads Brand ON, Kaina24/Kainos.lt feed URL resubmit (OPS-06: `https://petshop.lt/feed/...`), Flatsome licencija + soc. nuorodos, pirma REALI Venipak/LP siunta (J1), TTL 300→3600 po paros, eShoprent neišjungti iki patvirtinto kelio; #35868 trinti/palikti; local_pickup #16 ir welcome modal PALIKTA OFF (nespręsta). Claude — 3 rankinės patikros (T0 §PO), istorijos `s1643_h2` aukštesnei versijai, `ps_*_bak` valymas, titulinio 9 img.
...
- **Raimio lentelė (8/8 įrašyta):** #19145 11,80; #15928 4,00; #15920 4,89; #15938 2,11; #15935 3,04; #17916 3,54; #15914 1,87; #15993 5,40. Visos `_ps_sandelis=av`. 4 kortelės (15938/15935/17916/15914) DB SKU kiti (640560/6403961/641003/640049) — sargas praėjo pagal pavadinimo žodžius; 3 turėjo faktūrinę be +15 % (10,26/3,48/4,26/4,6953) — pakeltos.
- **Georplast Excel (135 eil., kaina×1,15):** įrašyta 20 (visos pagal SKU, visos av), palikta 24 (esama savikaina DIDESNĖ už promo — **Raimio sprendimas: neliesti, papildomų nerašyti**), nerasta 91 (nei SKU=ref, nei EAN), dubliai 0. Paliktų sąrašas `analize/s1642_g.json` → `paliktos`.
- **Radinys (probe #15938):** Georplast kortelės DB EAN meta apskritai NETURI (`_ean` visoje DB 1 785, `_global_unique_id` 2 285 iš ~3,5 tūkst.), o SKU — kiti nei tiekėjo ref → 91 nerastos yra matching'o riba, ne klaida. **Raimio sprendimas: kandidatų paieškos nedaryti, naujų kortelių nekurti — nerastos paliekamos.**
- **Katalogas v8.7.6→v8.7.8 GYVAI** (md5 `17392b381b190e04133195a6d99d0407`; kopija `ps-backups/petshop-katalogas-v877-BACKUP-2026-09-08.php`, grandinė v875/v876 backup'ai ten pat; repo `deploy/` atnaujintas; payload'ai `irankiai/s1642_ka.php`/`s1642_kd.php` — 2 dalių gzip+b64, gyvo md5 sargas, token_get_all, ping→rollback): **rinkiniams nerodomas ženkliukas „be šaltinio"** — Raimio pastaba, kad RINK-* (AV komponentų rinkiniai) žymimi klaidingai. Šaknis: `nesalt`=`_ps_be_saltinio=1` (naktinio registro sync'as; rinkiniai registro įrašų neturi pagal apibrėžimą — S1258 norma), DP paveldėjimas buvo, rinkiniams — ne. Pakeitimai: (a) sąraše `$tipas='rinkinys'` dabar apima ir **Mix and Match** produktus (`terminai()` grąžina termų VARDUS, ne slug — v8.7.7 klaida, taisyta v8.7.8: tikrinami 'Mix and Match' IR 'mix-and-match'); `nesalt` sąraše = `_ps_be_saltinio && $tipas!=='rinkinys'`; (b) kortelėje `nesalt` papildomai `!has_term('mix-and-match','product_type')` ir tuščių šaltinių tekstas rinkiniui — „Rinkinys — šaltinius turi komponentai". Filtras „Be šaltinio registre" seka `nesalt` — rinkiniai iš jo dingsta. Patikra Playwright (`analize/s1642_kq3.json`, shot `s1642_kat876_rink`): q=RINK- → RINK eilučių 10, su ženkliuku 0, ženkliukų puslapyje 0. Faktas: RINK-* yra WooCommerce **Mix and Match** (`_mnm_*`, pvz. #35400 RINK-TUALETO-START), `_ps_rinkinys` metos NETURI.
- **Katalogas v8.7.9 GYVAI** (md5 `236f5659f4016a51e1b53cea3d9e0e1d`; kopija `ps-backups/petshop-katalogas-v878-BACKUP-2026-09-08.php`; repo `deploy/` atnaujintas): **DP pakuočių savikaina** — Raimio sprendimas „pradžioje tegul daugiau-pigiau būna savikaina, ten su ja viskas aišku". Pridėta atsarga sąraše IR kortelėje: jei cost dar null ir yra `_dp_base_product_id` — savikaina imama iš BAZINĖS meta kaskados (`_cost_price`→`_vf_cost`→`_zb_cost`) × N (registro paveldėjimas v5.9/v6.0 lieka pirmas). Patikra `irankiai/s1642_v.php` (`analize/s1642_v.json`): DP pakuočių 21, visų bazinės turi aktyvias ps_sources eilutes (reg_paveldi 21, meta_paveldi 0, be_savikainos 0) — dev'e paveldėjimas iš registro veikė ir anksčiau, v8.7.9 pridengia registro spragos atvejį. Jei Raimis gyvai matys DP su savikainos brūkšniu — imti konkretų ID ir tirti atskirai. **Rinkinių (MnM) savikaina — ATVIRA**: mechanizmo nėra, savikaina priklauso nuo kliento pasirinktos kombinacijos; pasiūlyti variantai (blogiausia kombinacija / vidurkis / diapazonas, plg. strategijos §14.1–14.2) — laukiama Raimio sprendimo.
- Metodas kitiems savikainų failams: payload su sargais SKU==ref ARBA EAN, tuščia/mažesnė → rašom `number_format(x,4)`, didesnė → paliekam ir raportuojam; TEMP valymas run'o pradžioje.

### S1641 (2026-09-08, diena) — KATALOGAS v8.7.5: VARIACINIŲ PREKIŲ KAINA SĄRAŠE

> Pridėti po šia antrašte (virš S1640). Tema: **sąrašo KAINA/ANTKAINIS variacinėms prekėms rodė „—" — pataisyta per `_price` fallback; kortelė ir kainos redagavimas NEKEISTI (Raimio sprendimas — tik rodymas).** Bridge: `s1641_get` (failo b64), `s1641_d1/d2` (deploy 2 dalimis, TEMP), `s1641_ver` (patikra). Rezultatai `analize/s1641_*`.

- **Diagnozė:** brūkšnius rodė TIK `variable` tipo prekės. WooCommerce variacinės prekės TĖVUI `_regular_price` išvalo — kaina gyvena variacijose, o tėvo `_price` WC pats sinchronizuoja kaip mažiausią variacijos kainą (pvz. 19262/SKU 4158=1,49; 19265/11837=3,73; 19268/11184=3,73). Katalogo `surinkti()` skaitė tik `_regular_price` → null.
- **Pataisa (2 vietos, abi būtinos):** (1) `surinkti()` po `$kaina = $mv('_regular_price', true);` — jei null IR tipas `variable` (iš `$tipai`), imamas `$mv('_price', true)`; (2) meta preload raktų sąrašas (`meta_key IN`) papildytas `'_price'` — **be šio rakto fallback tyliai negrąžino nieko** (pirmas deploy 9aa1e650… buvo nepilnas, patikra parodė null → rasta, kad preload riboja raktus). ANTKAINIS pasitaisė automatiškai (skaičiuojamas nuo $kaina; marža null ten, kur nėra savikainos — norma, prekės ir buvo „be savikainos" eilėje).
- **Deploy:** backup `petshop-katalogas.php-v872-BACKUP-20260908` (md5 99cfa781e5d89b8760f7212df01bee0e); gyvas md5 **dd973f3e664dcaf7f24d4dde6610d9b5** (494 306 B); tarpinis nepilnas 9aa1e650… perrašytas. Repo `deploy/petshop-katalogas-v8.7.5.php`. Patikra gyvai per `surinkti()`: 19259 (simple) 4,18/39,9 %; 19262=1,49; 19265=3,73; 19268=3,73.
- **Scope sargai:** kortelėje kainos laukas variacinei NEliestas (redaguojama variacijų skirtuke); greitas kainos redagavimas sąraše variacinei rašytų į tėvo `_regular_price` — žinoma sena elgsena, nekeista. Versijos 8.7.3/8.7.4 praleistos sąmoningai (sticky fiasco istorija) — nauja = 8.7.5.

### S1640 (2026-09-08, vidudienis) — PRIMINIMŲ LAIŠKAI: peržiūra terra@, inventorius, ATVIRAS BLOKAS „ryt/poryt"

> Pridėti po šia antrašte (virš S1639). Tema: **bacs „gautas"+dunning-1+cart-abandoned-1/2 peržiūrai; nustatyta, kad TEKSTAI IR TERMINAI SUDERINTI (TZ v1.46 §4.4.2/4.4.5, liepa) ir gyvi kode; užfiksuota, kas iš doktrinos NEĮGYVENDINTA.** Bridge: `s1640_d…m` (TEMP). Testinis #35866 ištrintas, PDF/IAPV000171 ištrinti, 18272=5, `ps_dev_pastas_leisti` ištrinta. Skaitikliai: IAPV 172 (T-0 → 101).

- **Suderinta ir GYVA:** 20 kodo šablonų `petshop-core/templates/emails/` (4.4.5 „byte-tiksliai") + 15 DB juodraščių redaktoriuje (draft → sistema ima kodo šabloną). Dispatch flows 21. Terminai 4.4.2: krepšelis **+2 val./+24 val.**; **dunning — 3 prisilietimai per 5–7 d.**; bacs — priminimas 48 val. (S1147) + atšaukimas po 3 parų (S1146 — WC kelias neveikia, sargas yra, automatinio atšaukėjo nėra).
- **Peržiūros pamoka:** `Petshop_Laiskai_Turinys::perziura($flow)` — teisingas peržiūrų kelias (sintetinis payload, kodo-šablono fallback); `Email_Layout::wrap()` ima ASSOC masyvą su 'body' (ne sąrašą!); dispatch raktas — flow (`payment_failed`), ne šablono vardas; `payload['total']` — GRYNAS tekstas (emitteris strip_tags'ina; wc_price HTML → matomi span'ai).
- **ATVIRAS BLOKAS (Raimis: „ryt ar poryt prieisime"), paleidimo NEBLOKUOJA:** (1) dunning-2 ir dunning-3 šablonai + serijos trigeris (5–7 d. langas, stop apmokėjus); (2) bacs 48 val. priminimo trigeris (on-hold bacs, vienkartinis; tekstas — spręsti: dunning-1 ar atskiras); (3) 3 parų automatinis atšaukėjas (ar lieka rankinis su žyme — Raimio sprendimas); (4) Raimio klausimas dėl TERMINŲ TEKSTUOSE (kiek laikom prekes / iki kada apmokėti) — tai 4.4.5 pataisa, jo sprendimas; (5) cart_abandoned trigerių gyvo šaudymo patikra (jobs 17 istorinių — E2E netikrinta); (6) 4.4.5 šablonų .md dokumentai bridge repo NĖRA (Raimio archyve) — įsikelti prieš redaguojant.

### S1639 (2026-09-08, rytas/diena) — LAIŠKŲ ŠABLONAI: pilna peržiūra gyvai, Raimis PATVIRTINO VISUS

> Pridėti po šia antrašte (virš S1638). Tema: **visi kliento laiškai išsiųsti į terra@ su tikrais duomenimis; darbalaukis v3.39.2→v3.39.3 (vienas „išsiųstas" laiškas su PVM PDF, be prekių sąrašo pilname); vertimai v1.1→v1.2 (refund LT).** Bridge: `s1639_a…z`, `s1640_a…c` (TEMP). Rezultatai `analize/s1639_*`, `s1640_*`.

- **Dev-pastas leidimų sąrašas**: `ps_dev_pastas_leisti='terra@petshop.lt'` (v1.2 mokėjo, kodo nekeista) — testų metu išeidavo tik laiškai, kur VISI gavėjai terra@; po sesijos opcija IŠTRINTA (vėl viskas gaudoma).
- **Patikrinti ir Raimio PATVIRTINTI šablonai** (testiniai #35862/35863/35865, tikri nr.: Venipak 107744316, LP HC034722339LT): 1) apmokėjimas gautas (+pristatymo eilutė, MIXED pastaba); 2) išsiųstas pilnas — Venipak IR LP keliai per `issiusta()` automatą; 3) sekimo numerių (mygtukas, Petshop_Siuntos); 4) dalinės „Išsiųsta 1 iš 2" (Šioje siuntoje/Dar keliaus) ir „2 iš 2 — visas" (+PDF); 5) vėlavimo „dar komplektuojame" (`velavimo_laiskai()` sim. diena — SVARBU: simuliuojama data turi būti darbo diena); 6) atšauktas (tik iš processing — WC iš completed atšaukimo laiško nesiunčia); 7) dalinis grąžinimas + kreditinė KR-AVPN000106 su PDF (per `kreditine_numeris/pdf`; per `wc_create_refund` kreditinė pati NEgimsta — tik per darbalaukio KR eilę).
- **Darbalaukis v3.39.3** (Raimio sprendimas „2 ir 3 laiškas dubliuoja — turi būti 1"): `siuntos_laiskas()` pilname laiške (viena siunta ARBA paskutinė „visas išsiųstas") prekių sąrašo NĖRA ir segamas PVM PDF (`_petshop_completed_pdf`/`petshop_generate_invoice_pdf`); dalinių laiškai NEKEISTI. Deploy 2 dalimis, sargas gyvo md5 `f94d08e8…`, naujas `4f37977e6d094ecbed32db3a9594e234`, kopija `ps-backups/petshop-darbalaukis-v3392-BACKUP-2026-09-08.php`, ping 200. Repo `deploy/petshop-darbalaukis.php(+.b64)`.
- **Vertimai: gyvai buvo v1.1** (ne v1.2, kaip skelbė STARTAS — repo „v1.2 klasė" gyvai niekada negyveno!). Gyva linija → **v1.2**: +4 refund body sakiniai (`Your order from/on %s has been (partially) refunded…` → „Jums grąžinta (dalis) pinigų…"). md5 `5806edbf…`, kopija `-v11-BACKUP-`, repo perrašytas gyva linija.
- **Vežėjo nustatymo pamoka**: laiško vežėją lemia `vezejas()` iš pristatymo METODO PAVADINIMO („LP …" → lp) — testinis užsakymas be shipping eilutės nukrenta į Venipak. Testuose visada dėti shipping item.
- **ATVIRI T1 (variklio, neliesti be Raimio):** (a) `petshop-core/includes/class-shipments.php:131` „Array to string" warning'ai skaitant `_ps_siuntos`; (b) tas pats „meta grįžta masyvu" `json_decode` TypeError refund kablyje (laiškas išeina, bet FATAL po jo) ir darbalaukio :998; (c) LP sargas `issiusta()` reikalauja registro įrašo — simuliacijoje reikėjo `_ps_siuntos` rankinio, gyvai užpildo LP pluginas: T-1 pravaryti vieną tikrą LP registraciją.
- **Valymas**: #35862/63/64/65 ištrinti (force), PVM AVPN000362/363 + Kreditinė KR-AVPN000106 PDF ištrinti, likučiai atstatyti iki vieneto (18236=5, 18269=10 — refund restock buvo negrąžinęs, atstatyta ranka; 18272=5, 16745=20), `ps_dev_pastas_leisti` ištrinta. Skaitikliai: AVPN 365, KR 107 (T-0 → visi 101, kaip plane).
...
#### S1637 — VALYMAS (T-0 pakartojimas, Raimis: „viską išvalyk prieš keliant")
- `s1637_v.php` = s1622 A + S1632 revert (bak 1131: stock 1046/del 85, own 28/del 864; 551 partijų; lookup SQL sync) + opcijos (e2e/e17/e2f/fe_id, laiškų archyvas, dev-pašto žurnalas 59, smtp_bak, s1632_bak/seen) + saugikliai #35834 (shop_coupon) ir #35840–56 (products) — patikrinta D faze, ne užsakymai.
- Ištrinta: 17 užs. (#35816–#35832 be #35825, +#35835) + refund #35833; visos ps_ lentelės 0; PDF perkelti. Kopija `ps-backups/valymas-2026-09-07/` (20 failų). Q: ping 200, Warning 0.

#### S1637 — LIKUČIAI: pirmas bandymas → 5 Raimio klaidos → NUO NULIO
- Švieži eksportai 09-07 (Raimis ZB dropship IŠĖMĖ — todėl Σ mažesnės už 09-06). Payload formatai kaip S1631/S1632; options bazinių `[qty] ` prefiksas nukerpamas, attr vardas iš Options lapo.
- Pirmas apply: 546 partijų + 170 stock + 217 var, ZB/VF skip (kietai), Σ patikra 9 775=9 775 / 696=696 — BET Raimis rado 5 klaidas (pvz. Apollo AP2020\* 26/22/10). **ŠAKNIS DVIGUBA:** (1) eksporto eilutės su TUŠČIU kiekiu (589) buvo praleidžiamos — jų `_stock` liko su S1632 revert'o (pre-09-06 dev) reikšmėmis; (2) revert atstatė senienas ir prekėms visai be eksporto. **PAMOKA (į taisykles): tuščias kiekis šaltinyje = 0, ne „neliesti".**
- **NUO NULIO (Raimio sprendimas, jo patvirtinta procedūra):** `s1637_n0.php` — kopija (`ps-backups/likuciai-pries-nulinima-2026-09-07_1905/`, 4 229 meta + 630 partijų) → DELETE visos partijos → visi `_stock`='0' (3 532) → `_own` DELETE (85) → lookup 0 → kontrolė Σ=0/0/0/0. Tada apply iš naujo (tie patys skriptai) + Excel + `n2` S: 4 baziniai iš Options be Products kiekio (62=8, 2359=4, 11190=6, 711002=10).
- **KRYŽMINĖ (n2 K, kiekviena eilutė):** šaltinis→DB 1 010/1 010, nesutampa 0; DB→šaltinis AV našlaičių 0; Products↔Options 45 dubliuojantys baziniai — konfliktų 0; Σ `_stock` **10 499=10 499**, `_own` **581=581**; partijos 619, kiekis_liko 9 488. **RADINYS:** 300 tiekėjų prekių `_stock` (Σ ~27,9 tūkst.) = VF/ZB feed VEIDRODŽIAI, cron perrašinėja — į AV kontrolę NEĮEINA (variklis skaito `_own`); K skriptas juos atskiria per tėvo `_vf/_zb_qty`.

#### S1637 — TIEKĖJŲ EXCEL (73/73)
- Builder fix: Raimio faile antraštės 2-oje eilutėje → `min_row=3`. R: 168 eil., fizinis>0 — 73, **14 be savikainos** (JOS0799, Bioveterinary 2702/03/04/B29194/b43849, Monge M14229/m126xx/m144xx). **Raimio sprendimas:** tuščia Excel savikaina → kortelės kaskada `_cost_price`→`_vf_cost`→`_zb_cost` (log v1.9.0: desk kaskada ta pati). Apply 73 ok / 0 klaidų, own Σ 581, idempotencija `ps_s1636x_done`=73 patikrinta pakartotiniu R.

#### S1637 — BELACOR TOFU (Raimio planas, 3 iteracijos kol susikalbėta)
- Eksporto TOFU\* nerasti nes SKU ilgesni už WP (TOFUARB5027998 vs TOFUARB5027). WP: BeloCat 17 (#17659–17707, sand `belcor_tofu`, sav 2,30/kokoso 2,50) + Purrfect 3 (#17710/14/19) + DP rinkiniai #35840–56 (neliesti).
- **KRITINIS RADINYS (recon prieš rašant — po Raimio „tu nzn kur kas gyvena"):** katalogo v8.7.0 užrakinta taisyklė — ne-XML sandėliams (`belcor_tofu`/ambrosia/prins/quattro) `_stock`=TIEKĖJO, AV lentyna=`_own_stock_qty`; o **partijų variklio `av_laukas()` (S590) tikrina tik `_vf/_zb_qty`** → `priimti()` belcor prekei keltų `_stock` (ne tą lauką). Variklis NELIESTAS — `s1637_b.php` po `priimti` laukus sustato CRUD'u. **ATVIRAS T1: suvienodinti dviejų variklių sandėlio-lauko taisykles (darbalaukio „Gauta" belcor'ui turi tą pačią duobę).**
- Atlikta: 17 BeloCat fiziniai (iš kopijos pries-nulinima, Σ 139; pušis 4, persikas 27 — irgi į AV, Raimio „aš juos turiu savo sandėlyje") → `_own` + 13 partijų (sav 2,30, tiekėjas Belacor, „Fizinis likutis (Tofu i AV, S1637)"); Belacor `_stock`: 15 kvapų = 80, **pušis #17671 ir persikas #17677 = 0**. Purrfect 3 → `_ps_sandelis`=av, kiekiai nelietti (Raimis pats vedė 9/0/2 kataloge). Kryžminė 0 nesutapimų, ping 200.

#### S1637 — KATALOGO STICKY (v8.7.3→v8.7.4→ATSTATYMAS; NEKARTOTI be Raimio)
- Raimio kadras: antraštė kabo vidury sąrašo. Playwright diagnostika (`s1637_j.php`, eval per shots): **@media ≤1400px `overflow-x:auto` ant `.pskat-lent-lauk` daro jį sticky konteineriu → thead nebelimpa (1366px: top=-180)** — net kai turinys telpa. Tai S903 „sąmoningo kompromiso" tikroji kaina.
- v8.7.3 (media išimtas, puslapio h-scroll) — Raimis: „netelpa į ekraną". v8.7.4 (vidinis scroll + fiksuotas thead klonas #ps-thead-fix, Playwright 1366/1907 OK) — Raimis: „langas nesutvarkytas, kaip buvo". **ATSTATYTA v8.7.2** iš `-v872-BACKUP` (md5 99cfa781… gyvai ir repo), gyvo kadras patvirtino rytinį vaizdą; jei Raimiui rodė kitaip — naršyklės kešas/zoom (laukiam inkognito patikros). **PAMOKA: UI/layout keitimai prieš paleidimą — tik su Raimio užsakytu maketu; simptomas paliktas kaip žinoma v8.7.2 savybė.**
- Deploy mechanika veikė švariai: 2 dalių gz+b64 (`ka/kd`, part1 md5 sargas, naujo md5 sargas, token_get_all, gyvo md5 sargas, kopija, ping→rollback). Kopijos: `-v872-`, `-v873-BACKUP-2026-09-07.php`.

#### S1637 — LIKO / T-0 DELTOS
- **T-0 naktį D2/D3 NEBEKARTOTI** — valymas ir pilnas kėlimas ATLIKTI šiandien; naktį tik: Raimio parduotų **DELTA** (SKU+kiekiai) → mažas skriptas su ta pačia kryžmine; + jei bus naujų testinių — jų valymas.
- **63 nerastos su kiekiais** (JOS0731 q94, HYPS06-2 q44, HM-\*, 82700\*, 81602201/02, 22215, 024088, 40612, 10139/10153; 6×-dp ir 4×sku1665\* — nurašytina) — kiekiai 0, laukia Raimio: sukurti / nurašyti / SKU poros. TOFU dalis išspręsta atskirai (žr. aukščiau).
- Valymo saugiklių sąrašo priminimas kitam valymui: #35834 kuponas, #35840–56 rinkiniai, #35790/#35796 paslaugos.
- Options „nerasta 2" — iš 63 grupės.
...
### S1636 (2026-09-07, vakaras) — Welcome modal v1.2: „Sveiki sugrįžę“ (B) migruotiems klientams

> Tema: Raimio idėja — iššokantis pasisveikinimas pirmą kartą prisijungusiam senam klientui. Recon → tekstas (3 iteracijos, galutinis Raimio) → v1.2 → deploy → testai → kadras → valymas. Bridge `irankiai/s1636_r/f/d.php`, kadras `screenshots/s1636_welcome_B.png`.

#### S1636 — RECON
- Klientai JAU WP vartotojai: 5 704 users, **5 666 importuoti 2026-08-31** (`_ps_importas`, `_ps_banga`, `_ps_imone`, istorija `_ps_ist_n/_ist_suma/_ist_pirmas/_ist_paskutinis`). „Rodyta 1ד šablonas jau naudojamas (`ps_content_seen_*`).
...
#### S1636 — KATALOGO ŠOKINĖJIMAS (Raimio radinys) → v8.7.2
- Simptomas: ps-katalogas admin'e scrollinant lentelės antraštė „plaukioja", pro plyšį matosi eilutės. Recon (Playwright, testuotojas, `s1636_j.php` matavimai): `--ps-virsus` matuotas tik load'e (171 px su breadcrumb); paslinkus breadcrumb nuslenka, `.pskat-bar` apačia pakyla iki 100 px, o sticky thead likdavo ties 171 → ~70 px plyšys. Tik admin, tik šis langas, frontend nepaliestas.
...
#### S1636 — T-0 / ATVIRA
- **T-0 nakties žingsnis (Raimio sprendimas): `petshop_welcome_modal_enabled` → 1** — be jo nei A, nei B nerodomi. Įjungus, VISI 5 666 importuoti pamatys B po pirmo prisijungimo.
- Raimis dar neatsakė (iš S1635): baigė testuotis? testinių sąrašas; šablonų terra@→uzsakymai@; švieži eksportai.

---
### S1635 (2026-09-07, diena) — T-1 smulkmenos + DP kraikų rinkiniai + pp2d laiškas

> Tema: Raimio dienos užduotys prieš startą. Bridge įrankiai `irankiai/s1635_*` (git), kadrai `screenshots/s1635_*`. LIKUČIŲ KĖLIMAS DAR NEĮVYKĘS — Raimis testuojasi, po to: valymas → švieži eksportai → kėlimas (ŠIANDIEN, jo sprendimu).

- **Kuponas `Petshop.10`** #35834: −10 % prekėms, be pristatymo, individual_use, be limitų; išskirta 10 maisto kat. (šunų/kačių + brand'inės). Funkcinis: maistui NE, žaislui TAIP; frontend krepšelyje mišriam pritaikė tik ne-maistui (−0,92 €). T1: „Nuolaidų kodai“ langas (T0 plane).
- **Likučiai klientui**: `woocommerce_stock_format` low_amount→**no_amount** („Turime“, be skaičių). Patikrinta gyvai.
- **FB**: `follow_facebook` + footer widget (buvo plikas facebook.com) → facebook.com/Petshop.lt. Instagram — vėliau.
- **Frontend checkout E2E** (#35835, svečias): krepšelis 2 prekės → kuponas → Venipak+bacs → laiškai (gautas/apmokėjimas) → Neapmokėti→(apmokėta)→Gauti+Neišrūšiuoti; likučiai be dvigubo nurašymo (AV per partijas po apmokėjimo, VF rezervas checkout'e — S1620 logika, suderinta). Į T-0 valymą.
- **petshop-vertimai.php v1.1 (mu, NAUJAS)**: WC failed/cancelled laiškų kūnai LT (WC11 naujos eilutės, kurių nėra .mo) + Flatsome „Pages found“ ir kt. Laiškų antraštės/temos nustatymuose LT. Funkciškai patikrinta (failed HTML „Deja, užsakymo…“).
- **Paieška**: Flatsome `search_result`→0 — „Posts/Pages found“ sekcijos prekių paieškoje IŠJUNGTOS (Raimiui jos nereikalingos). Laikinas CSS nuimtas.
- **DP KRAIKŲ RINKINIAI (galutinė forma!)**: 17 vnt. #35840–#35856 — po vieną TOFU BeloCat kvapą, PAGAL ETALONĄ #35096 (knyslės): **simple + `_dp_base_product_id`(vienetinė) + `_dp_pack_qty`=8**, kaina fiksuota =8×vnt−10 % (32,04–35,28 €), be sale meta (variklis pats piešia ×8 VNT., EKONOMIŠKA juostą, „Sutaupote 10 %“); kat. DAUGIAU=PIGIAU(91)+Kraikai(107). KELIO PAMOKA: pirmi 2 bandymai (bendras MnM konstruktorius; MnM su per-item nuolaida + AKCIJA) — NE standartas; **prieš kuriant turinį PIRMA prašyti etaloninės prekės**. Variklio DP mechanizmą žino `petshop-rinkiniai.php` (v1.0). 5 kvapų vienetinių nėra — rinkiniai nekurti. Kadre pastaba: išparduotų kortelėse „Neturime“ užlipa ant „×8 VNT.“ — praeis sutvarkius likučius; norint slėpti — variklio keitimas su leidimu.
- **pp2d laiškas („Ar su užsakymu viskas gerai?“)**: šaltinis `petshop-core/templates/emails/post-purchase-2d.php` (+2 d. nuo IŠSIUNTIMO, ne pristatymo — todėl negali teigti „gavote“). Po diskusijos Raimis patvirtino (S1635): **tema „Jei ko prireiktų – parašykite mums“, H1 „Esame šalia“** (tema≠H1), sakinys „…ar dėl užsakymo kiltų klausimų, tiesiog atsakykite…“, naujas preheader. Deploy b64+md5 2a09c32b…, atsarga `deploy/post-purchase-2d-ATSARGA-s327.php`; renderio patikra: sena frazeologija 0. Laiškas lieka ĮJUNGTAS (spam dev'e — konteksto artefaktas; TR ryto mail-tester parodys).
- **GA4**: svetainėje GTM-MF3GZGT (tas pats kaip gyvame petshop.lt) — lankytojų skaičiavimas po perjungimo tęsis toje pačioje paskyroje automatiškai. T0+1 PRIEŠ Ads: WC purchase/add_to_cart e-commerce sekimas (~1 val.) — T0 plane.
- Higiena: kaina0/be kat./be apr. = 0; mix-and-match kortelės tvarkingos (mano klaidingas aliarmas — MnM foto nereikalinga); test- slug'us Raimis tvarko pats. Paieškos URL `?s=&post_type=product` — norma, paaiškinta.
- **VALYMUI T-0 (papildyta)**: + #35834 (kuponas LIEKA — netrinti!), #35835 (trinti), #35840–#35856 (LIEKA — prekės!), `ps_e2f_fe_id` opcija (trinti), Raimio testavimo užsakymai (sąrašą duos).

---

### S1634 (2026-09-06, naktis → 09-07)

> Pridėti po šio antraštės (virš S1633). Tema: **B punktai (T0 planas): nav paslaptis, EAN recon, priešpaleidiminis strigčių recon + SMTP fix, PILNAS E2E (spec §10.6) — loginių klaidų NERASTA.** Bridge: `s1634_r/r3/r4` (recon), `s1634_g.php` (PATAISYTAS sargas — commit'intas), `s1634_p/p2` (strigčių recon), `s1634_s5` (SMTP fix), `s1634_e.php` (E2E fazės 1–9,P,X,K,Z), `s1634_q` (ids fix). Kadrai `screenshots/s1634_*`. Dokumentas **T0_PLANAS v1.1**.

#### S1634 — NAV PASLAPTIS IŠSPRĘSTA (sargo klaida, ne markup)
- S1630/S1633 tuščias nav: sargas be `SECURE_AUTH_COOKIE` gaudavo login puslapį + regex ieškojo `dl-nav`, o realus konteineris — `div.dl-eiles` su `a.dl-e` (ne `<nav>`; vienintelis `<nav>` puslapy — juosta). Fix: `irankiai/s1634_g.php` (abu cookies; nav=juosta; eilių sk fallback visada). Patikrinta gyvai.

#### S1634 — EAN LAPE (Raimis: T1)
- Lapą generuoja variklis `petshop-av-sheets.php` v1.0 (10 335 B, md5 13c793bd…): SKU + pastaba + „dalis siuntos (N)“ + suminis YRA; EAN barkodų ir dovanėlės kortelės NĖRA. `_ean` meta 1 774 prekėms. **Raimio sprendimas: EAN → T1** (kartu su skenavimu). Dovanėlės kortelė — §11.4 atviras.

#### S1634 — STRIGČIŲ RECON (radiniai → T0 v1.1)
- **Paysera `paysera_payment_main_settings.test_mode=yes`** (projektas 29276, slaptažodis yra) → T0 D11: prieš €2,21 perjungti `no`.
- **siteurl/home = dev.avesa.lt IR DB, IR wp-config konstantose** → T0 D7: kartu su bloko trynimu DB → petshop.lt (vien konstantų nuėmimo neužtenka).
- **SMTP from buvo terra@petshop.lt su force** → SUTVARKYTA: from `uzsakymai@petshop.lt` (bak `ps_s1634_smtp_bak`); WC admin gavėjai jau buvo uzsakymai@. Host isopas.serveriai.lt:465 SSL.
- **Sender = petshop-esp v0.4.0** (mk/tk raktai, webhook secret, 1 REST route; webhook log 0 — dev norma; laiškų nesiunčia, tik įvykiai per api.sender.net) → T0 E rytas: webhook URL Sender panelėje → petshop.lt (`?rest_route=` jei Q-WPJSON 403).
- **wp-super-cache aktyvus** → T0 D13: purge + konfigo dev-URL patikra. Serijų tikri vardai: `petshop_avpn_counter` 354 / `petshop_iapv_counter` 169 / `petshop_kravpn_counter` 105 / `petshop_ppk_counter` 103 (prieš →101 pasitikrinti „kitas ar paskutinis“ semantiką). admin_email terra@gyvunai.lt (neblokuoja). WP 6.9.4 / WC 11.0.1 / PHP 8.3.20, DB 400 MB, diske 349 GB, debug.log švarus.

#### S1634 — PILNAS E2E (spec §10.6) — VERDIKTAS: LOGINIŲ KLAIDŲ NERASTA, PALEIDIMAS NEBLOKUOJAMAS
Nauji testiniai **#35827–#35832** (+refund #35833), lipdukai V07267E2F001–006 tiesiogiai (Venipak/LP/Paysera nekviesti), partija #22, laiškai per dev-pastas (47 žurnale).
- **F1 grynai AV** #35827: sukurtas→Surinkti AV auto→Surinkti→Lipdukas→Kurjeris paėmė→completed; AVPN000354; `_stock` 42→40 ✓.
- **F2 VF dropship** #35828: kortelė→Užsakyti iš VF (laiškas terra@ IŠLEISTA, be lipdukų — su_lipdukais=0)→VF išsiuntė→completed; „antras laiškas tą pačią dieną“ įspėjimas veikia.
- **F3 mišrus į AV** #35829: Neišrūšiuoti→Auto pasiūlymas teisingas→VF veža į AV→partija #22→Gauta (AV +1)→Surinkti→completed; AV 25411 0→+1→0 ✓, tiekimo laiškas „UAB Avesa · prekių užsakymas“ ✓.
- **F4 atsiėmimas** #35830: inst #16 ON→forma→Surinkti→Paruošta atsiimti (laiškas ✓)→Klientas atsiėmė (laiškas su AVPN000357 PDF [pr:1] ✓)→completed; inst #16 OFF ✓.
- **F5 telefoninis pavedimu** #35831: on-hold, IAPV išankstinė [pr:1] ✓, **likutis NEnurašytas on-hold** (39), po „Pažymėti apmokėtu“ −3 → 36 ✓.
- **F6 pakartotinis** #35827: `_ps_siunta_grizta` (fikt.)→Klausimas ✓; „Siųsti iš naujo“ **teisingai BLOKUOJAMAS** („pirma Pakartotinis arba Be mokesčio“); po „Be mokesčio“→„AV dalis: Surinkti AV→naujas lipdukas“, grizta meta nuimta, `_ps_siuntos_senos`=[E2F001], nauja siunta E2F006, likutis nejudėjo (net zero) ✓.
- **F7 grąžinimas** #35831: dalinis (1/3, atsisakymas, tinkama)→KR-AVPN000105 2,45 €, AV +1 (37), skaitiklis 105→106, refund `_ps_kreditine`, `_ps_grazinta_q`=1; **pristatymas daliniam atsisakymui teisingai NEgrąžintas**; Klausimas „grąžink pinigus“→„Grąžinta“→nuimtas ✓.
- **F8 PPK** #35832: telefonu grynais→processing→„Suformuoti kvitą“→PPK000103 6,59 €, skaitiklis 103→104, PDF ✓.
...
### S1633 (2026-09-06, naktis)

> Pridėti po šio antraštės (virš S1632). Tema: **Raimis: „noriu naktį an→tr paleisti — ar realu?“ → verdiktas TAIP su sąlygom; sargų patikra; detalus T-0 planas.** Bridge: `irankiai/s1633_g.php` (sargai, READ-ONLY). Naujas dokumentas **`dokumentai/T0_PLANAS_2026-09-08_v1_0.md`** (bazė — DOD-18_perjungimas.md + S1631/S1632 deltos, grafikas A–E).

#### S1633 — VERDIKTAS (Raimis sutiko)
- Realu, jei pirmadienio E2E (spec §10.6) praeina; rimta loginė klaida → STOP, data keliama. Kritinis kelias: pir — EAN recon + E2E su dienoraščiu; an diena — taisymai + Excel įkėlimas + T-1 pasiruošimas (DOD-18 §3); naktis — DOD-18 §4 + likučių kėlimas. T1 neblokuoja: legacy→av, „Geriausia iki“, „Pridėti prekę“, paskyros UI. 341 savikainų NEblokuoja (likučiai be partijų, sudės per Gavimą).
- **DU ALIARMAI:** (1) TTL 3600→300 — T-2, Raimis DirectAdmin VĖLIAUSIAI pir rytą; (2) Paysera pilnas ciklas netestuotas (DOD-18 §8 „didžiausia rizika“) — naktį €2,21 bus pirmas; nepavykus — bacs/COD veikia, Paysera rytą. Priežiūros režimo nėra; eShoprent išjungimas nesuderintas.

#### S1633 — SARGŲ BŪKLĖ (`s1633_g.php`) — ŽALIA
- Cron'ai gyvi: sekimas kas 30 min (pask. 21:08 — 9 nr., 4 klaidos = Raimio ištrinti V072…, laukta), dropship-sargas 19:13, atšauktų valymas 00:20, vėlavimo laiškai 11:00; viso 84. SLA vėlavimų 0, „siunta grįžta“ 0. dev-pastas leisti=terra@ (žurnalas 30), laiškų archyvas 32. pickup #16 išjungtas. legacy-301 map yra, GA4 yra. Warning 0, ping 200. Import tempas gyvas (id=3, 21:08).
- Kabliukas: eilių nav regex (`dl-eiles`/`dl-nav`) tuščias antrą kartą (S1630, S1633) — markup pasikeitęs? Pirmadienį pažiūrėti tikrą HTML; funkciškai netrukdo.
- **T-0 planui pridėta (mano pastebėjimas):** naktį žingsnis 7 — kartu IŠJUNGTI dev-pastas v1.2 (kitaip klientų laiškai iš petshop.lt neišeis).

---

### S1632 (2026-09-06, naktis)

> Pridėti po šio antraštės (virš S1631). Tema: **Raimis: „gali dabar su skriptu perkelti — mini testas“ → likučių APPLY dev'e, 3 radiniai (dublikatai, „Variacijos kodas“ nepatikimas, ZB/VF semantika) → Raimio sprendimas: tiekėjų likučių NEKELIAM → rollback + Excel pildymui.** Bridge: `irankiai/s1632_a.php` (apply C,P0–P6,V), `s1632_v.php` (patikra + kadrai), `s1632_r5.php` (10546/10586 recon), `s1632_s.php` (kadras su secure_auth), `s1632_rb.php` (rollback). Kadrai `screenshots/s1632_*`.

#### S1632 — APPLY EIGA IR KLAIDOS (mini testo vertė)
- **Klaida 1 (mano):** šablonas fazę perduoda GET rakto REIKŠME (`?ps_key=P1`), ne `&f=` — pirmas paleidimas 6× vykdė P0 → 570 partijų dublikatų. Išvalyta C faze, raktas pataisytas. **PAMOKA į deploy taisykles.**
- **Klaida 2 (eShoprent):** eksporte dublikatai tuo pačiu EAN (JOS0915 93 + JOS0915574-1 9 → ta pati WP prekė; antra eilutė perrašydavo). FIX: grupavimas pagal WP prekę, kiekiai SUMUOJAMI (`ps_s1632_seen` tarp chunk'ų; prieš rašymą trinamos tos prekės S1632 partijos — idempotencija).
- **Klaida 3 (eShoprent):** „Variacijos kodas“ NEPATIKIMAS — 10546: Pilka→`10546-R`, Raudona→`10546-Mė` (sumaišyti). FIX: atributo reikšmė PIRMIAU, kodas tik atsarga. Po fix: variacijos 239/239, Σ696=696, neatitikimų 0 (recon r5: 10546 G2/Jv2/R3/P2, 10586 vs 10586Jumbo — tiksliai).
- `-dp` prekės (7, „Daugiau=pigiau“ skelbimai) — praleidžiamos (rinkiniai, Raimio S1631 sprendimas).

#### S1632 — RADINYS: ZB/VF SEMANTIKA (kadras `s1632_s_preke_partija.png`)
- Didžiausia „partija“ — Family Dog 15 kg (ZB dropship): eShoprent „Turimas kiekis 729“ = **ZB feed'o likutis**, ne fizinis AV sandėlis. Eksportas dropship kiekių neatskiria. Skriptas 168 tiekėjų prekėms (85 VF + 83 ZB) buvo sukūręs fiktyvų `_own_stock_qty` + partijas.
- **RAIMIO SPRENDIMAS (GALIOJA): ZB ir VF prekių likučių iš eksporto NEKELIAM.** Vietoj to — Excel pagal sandėlius, Raimis surašo TIK fizines atsargas, tada įkeliam.
- **Rollback (`s1632_rb.php`):** 168 atstatyta iš `ps_s1632_bak` (`_own_stock_qty` sena reikšmė / delete), jų S1632 partijos ištrintos, `_cost_price` perskaičiuotas; liko 551 partija; ping 200.

#### S1632 — GALUTINĖ DEV BŪKLĖ (mini testas OK)
- Paprastos AV prekės: ~722 su `_stock` iš eksporto, iš jų 551 su pradinio likučio partija (savikaina iš eksporto→kortelės, tiekėjas 'eShoprent', importuota=1, pastaba 'Pradinis likutis (testas S1632)'); be savikainos — tik `_stock`. Variacijos: 239 su `_stock` po spalvą/dydį. Patikra: paprastų Σ sutampa (delta 13 = EAN dublikatų suma — patikros pusėje be EAN atsargos, DB teisinga); frontas gyvas (kadras), ping 200, klaidų 0.
- Atsarginės: `ps_s1632_bak` (1131 įrašų — senos reikšmės), `ps_s1632_seen`. **T-0/6 etapo valymui:** S1632 partijos + abi opcijos + testiniai `_stock` — trinti prieš tikrą T-0 kėlimą.
- **Excel Raimiui:** `tiekeju_prekiu_fiziniai_likuciai_S1632.xlsx` — lapai VF (85) ir ZB (83): SKU, pavadinimas, eShoprent kiekis (info), MANO FIZINIS KIEKIS (pildo), savikaina (iš eksporto/kortelės, koreguojama), geriausia iki (nebūtina). Grąžinus — įkėlimo skriptas (Gavimo logika: `_own_stock_qty` per `Petshop_Partijos::priimti`).

#### S1632 — T-0 PROCEDŪRA (atnaujinta, keičia S1631 p.3)
1. Švieži eksportai + Raimio užpildytas tiekėjų Excel. 2. Valymas (S1632 testinės partijos/opcijos/likučiai). 3. `s1631_d.php` dry-run → patvirtinimas. 4. `s1632_a.php` logika (C,P0–P6): AV paprastos + variacijos IŠ eksportų, **ZB/VF praleidžiamos**; tiekėjų fizinės — iš Raimio Excel atskiru žingsniu. 5. `s1632_v.php` patikra + kadrai.

---

### S1631 (2026-09-06, naktis)

> Pridėti po šio antraštės (virš S1630). Tema: **Raimis atsiuntė eShoprent eksportus („dabartiniai petshop.lt likučiai, prieš pat migruojant reiks įsikelti“) → likučių migracijos recon + DRY-RUN (nieko nerašyta).** Failai: `products_export_specials_eshoprent_2026-09-06_17_09_30.xlsx` (1 691 prekių; 3 lapai Products/Categories/Specials) ir `options_export_eshoprent_2026-09-06_17_22_12.xlsx` (99 prekių, 332 variacijų eilučių SU kiekiais po spalvą/dydį, Σ1 098; „Variacijos kodas“ 295). Bridge: `irankiai/s1631_r.php` (atitikimas), `s1631_r2.php` (nerastų analizė), `s1631_r3.php` (WP variacijų padengimas), `s1631_r4.php` (options→variacijos), `s1631_d.php` (DRY-RUN). Visi READ-ONLY.

#### S1631 — RADINIAI
- Eksporte 1 084 prekės su likučiu (Σ19 217 + 3 kuponai „999“). Atitikimas: 935 SKU + 80 EAN = 1 015 (94 %). Nerastų 63 analizė: 11 — SKU su tarpais/tab (trim išsprendžia), 11 — Trixie `PET` priešdėlis (eksporte `PET0TR…`, WP `0TR…` — taisyklė „nubraukti PET“), 41 tikrai nėra (4 kuponai + 37 prekės ~200 vnt.).
- **Variacijos:** WP 70 variantinių / 241 variacija (83 be SKU; SKU tipo `640405-ZA`) — su Products eksportu nesutampa NĖ VIENA (eShoprent bazinė eilutė = bendras kiekis visoms spalvoms). **Options eksportas išsprendžia:** 239/332 eilučių → 234/241 WP variacijų (116 pagal „Variacijos kodą“, 123 pagal bazinį SKU + atributo reikšmę — veikia ir be-SKU variacijoms); nesutapo 10, visos q0. Patikrinta: bazinė eilutė = Σ variacijų (640405: 3=1+1+1+0) — dubliavimo nėra.
- `Petshop_Partijos::priimti()` (perskaityta `deploy/petshop-partijos.php.b64`): reikalauja savikainos >0, tik `product` (ne variacijoms), PATS pakelia AV likutį (+kiekis, per WC CRUD) ir perskaičiuoja `_cost_price` → apply seka prekei su partija: `_stock`→0, tada `priimti(kiekis)`.

#### S1631 — RAIMIO SPRENDIMAI (GALIOJA)
- Draft/kita su likučiu ir WP prekės ne eksporte — **„ten visokie rinkiniai, jų nereikia“**: skriptas rašo TIK į publish paprastas prekes ir variacijas; draft neliečiami, nulinimo klausimo nebėra.
- „Geriausia iki“ čia nepildoma (§19.9 „tuščias = norma“ — atskiras T-0 klausimas lieka).

#### S1631 — DRY-RUN (`s1631_d.php`, `analize/s1631_d2.json`)
- Paprastos: **rašoma 894** (Σ17 004); praleista 69 variantinių tėvai, 82 draft/kita, 4 kuponai; **nerasta 35** (sąrašas ataskaitoje; JOS0731 q94, HYPS06-2 q44, `024xxx` 11 prekių…) — Raimiui: sukurti ar nurašyti.
- Variacijos: **rašoma 239** (Σ696); 82 eilutės = WP paprastos (dengia Products), 10 nerasta (q0). Pvz. 640049 Žalsva 7→6.
- Savikaina partijoms: WP kortelėje 553/894, **be — 341** (jiems tik `_stock`, be partijos). Apply fazė papildomai ims savikainą iš eksporto „Savikaina“ stulpelio (462/1084 turi) — spraga mažės; likusias Raimis sudeda iki T-0 (spec §12.7 p.5).

#### S1631 — T-0 PROCEDŪRA (planas)
1. T-0 naktį — ŠVIEŽI abu eksportai iš eShoprent (šie — repeticija). 2. `s1631_d.php` dry-run su šviežiais → Raimio patvirtinimas. 3. Apply (dar neparašytas; bus `s16XX_a.php`): paprastoms su savikaina — `_stock`→0 + `Petshop_Partijos::priimti` (kiekis, savikaina iš eksporto→kortelės, geriausia_iki tuščia, pastaba „Pradinis likutis T-0“, importuota=1); be savikainos — tik `_stock`; variacijoms — tik `_stock` (partijų variklis nepalaiko). Eilių/likučių sumos prieš/po, vizuali patikra. SKU taisyklės: trim + `PET` nubraukimas + EAN atsarga.

---

### S1630 (2026-09-06, naktis)

> Pridėti po šio antraštės (virš S1629). Tema: **Raimio klausimai — (1) „Paruošta siųsti: ar man ką daryti, ar laiškai išeis patys, kai Venipak nuskaitys?“; (2) „Geriausia iki“ — dabar ar T1?** Kodo nekeista. Bridge: `irankiai/s1626_r.php` (sveikata — pavadintas s1626, vykdytas S1630). Dokumentai: spec v1.18 (§12.6), STARTAS 23.

#### S1630 — SVEIKATA (recon `s1626_r.php`)
- Dev sveikas: ping 200, Warning 0, TEMP ištrinta 1. Gyva (md5): darbalaukis **v3.39.2** `f94d08e832f3852653ecde5cfdc27ef8` (544 460 B), juosta **v1.10** `1015dea6290f7c3486a50f2adc193cc4` (23 264 B), tiekimas v1.10.1 `cf7677a8c355c58cc79a489dae4114f1`, dev-pastas v1.2 `f761afc71f5e42f5205366076d79d16f` — sutampa su STARTAS 22 (po S1629). Pastaba: `dl-eiles` regex grąžino tuščią nav (kodas 200) — netikrinta, kitame run'e pažiūrėti, ar markup'as pasikeitė.

#### S1630 — ATSAKYMAS (1): „Paruošta siųsti“ — automatika (kodas `venipak_sekimas()`, `sekimo_kandidatai()`, antraštė v3.11/v3.12; spec §12.2, p.116)
- Darbuotojui daryti nieko nereikia: cron `ps_venipak_sekimas` kas 30 min tikrina visus `_ps_siuntos` numerius (AV ir tiekėjų dalis, ≤60 d., ≤200/run). Venipak kodai 1/2/3/6/9 → dalis išsiųsta + laiškas klientui su sekimo numeriu pats (`kas=Venipak`); 9 → „Pristatyta“ paskyroje (be laiško); „return/sender“ → Klausimas „Siunta grįžta“. „Kurjeris paėmė“ / „[T] išsiuntė“ — rankinis tik jei sekimas vėluoja.
- Dev'e #35817/#35822–#35826 Venipak numeriai Raimio ištrinti → įvykių nebus, eilė liks iki testinių valymo (6 etapas).

#### S1630 — ATSAKYMAS (2): „Geriausia iki“ → **T1 (Raimio sprendimas, GALIOJA)**
- Recon: REGISTRAS §19.9, log S757–S765 (08-11), TŽ 38.5/38.6, `petshop-av-tiekimas.php` v1.10.1 `priimti()` (`geriausia_iki` iš „Gauta“ lauko), darbalaukis v3.38 („galiojimas prie Gauta — neprivalomas“).
- Gyva: partijos su data + FEFO; katalogo eilė „Artėja geriausia iki“ (v5.8); akcijų taikinys „Geriausia iki“ (v1.8) — nuolaida visai prekei. Nepadaryta: TŽ 38.5 atskiras skelbimas + 38.6 `/pasiulymai/geriausia-iki/` (liko „ATVIRA“ nuo 08-11, niekur nesuplanuota).
- Raimis: „automatikos negali būti tik pusiau, bent kol kas su darbuotojo priežiūra; reikia lango prie Pasiūlymai — Geriausia iki“. Įvertinimas ~2 sesijos. **Sprendimas: T1** (po paleidimo; prieš E2E neįterpiam). Detalės — spec §12.6.
- Neatsakyta (liko): T-0 pradinio likučio partijoms — ar sudedamos ir „geriausia iki“ datos (~300 prekių, kur svarbu) ar tuščia (§19.9 „tuščias = norma“). Klausti T-0 skripto metu.

---

### S1629 (2026-09-06, naktis)

> Pridėti po šio antraštės (virš S1628). Tema: **Raimis: „kodėl Prekės rodo 453? geriau kiek parduodama, ne kiek jų nėra“.** `petshop-juosta.php` v1.9 → **v1.10** (23 264 B, md5 `1015dea6290f7c3486a50f2adc193cc4`; kopija `ps-backups/petshop-juosta-v19-BACKUP-2026-09-06.php`; repo `deploy/petshop-juosta.php` + atsarga `-v19-ATSARGA.php`): „Prekės“ skaitliukas = **parduodama** (katalogo „prekyboje“ `visos_kruvoje − uzsakyti`), žalias (`psj-sk-p`); title „parduodama N · išparduota M (katalogo duomenys HH:MM)“. Cron `ps_juosta_reikia_cron` opcijoje `ps_juosta_reikia` saugo `[išparduota, laikas, parduodama]` (senas formatas 2 laukai — skaitliukas neberodomas, kol perskaičiuos). Deploy `s1629_e23d.php` (md5 sargai, kopija, ping 200) + perskaičiavimas toje pačioje sesijoje (0,9 s): **parduodama 2 219 · išparduota 453**; juostoje „Prekės 2219“ ✓.

---

### S1628 (2026-09-06, naktis)

> Pridėti po šio antraštės (virš S1627). Tema: **Raimis (kliento kadras): užsakymo puslapyje klientas mato „Spausdinti sąskaitą · Spausdinti kvitą · Spausdinti važtaraštį ×2“ — „jam reikia tik sąskaitos“.** Recon (`s1628_r.php`, `s1628_r2.php`, `s1628_r3.php`): WCDN 7.3 `Frontend::create_print_button_order_page()` (`woocommerce_view_order`, `woocommerce_thankyou`) ir `create_print_button_account_page()` (`woocommerce_my_account_my_orders_actions`) rodo po mygtuką kiekvienam įjungtam šablonui — invoice, receipt, delivery-note, packing-slip (abu „važtaraštis“); tipus duoda `Utils::get_template_types()` su filtru **`wcdn_template_types_from_order`**. „Atsisakyti sutarties (ES 14 d. teisė)“ — mūsų `petshop-atsisakymas.php` (H212), neliesta.
- **Naujas mu-plugin `petshop-klientui.php` v1.0** (1 426 B, md5 `96ee85d5b669dc78a38642dcc6e8379f`; repo `deploy/petshop-klientui.php`): filtras `wcdn_template_types_from_order` — ne admin'e (front-end / be `edit_shop_orders`) paliekamas tik `invoice` (galioja ir `print-order` endpoint'ui — klientas kvito/važtaraščio URL neatidarys). Admin'e ir darbuotojams (WC užsakymo langas, `wp_ajax_print_order`) — kaip buvo; WCDN šablonai nustatymuose neišjungti. Deploy `s1628_e21d.php` (b64 md5 sargas, `token_get_all`, naujas failas — rollback = pašalinimas, ping 200).
- **v1.0.1 (Raimio kadras: prisijungęs kaip administratorius „ačiū“ puslapyje matė 4 mygtukus — v1.0 darbuotojams paliko viską):** front-end'e — **visiems** tik `invoice` (`is_admin()` → viskas: WC užsakymo langas, `wp_ajax_print_order`). md5 `ccf9032d78416e805d90993b7fe1def1`, kopija `ps-backups/petshop-klientui-v10-BACKUP-2026-09-06.php`; patikra `s1628_e22d.php`: „ačiū“ #35826 admin (uid 1) / Inga / klientas — po 1 „Spausdinti sąskaitą“, kliento užsakymo puslapis — 1.
- **Patikra v1.0:** klientas #5787, užsakymas #35826: prieš — 4 mygtukai (`wcdn_print_invoice/receipt/packingslip/deliverynote`), po — **1 „Spausdinti sąskaitą“**; paskyros sąrašo veiksmai — tik `wcdn_print_invoice`; „Atsisakyti sutarties“ liko. Playwright (`s1628_k.php`, `screenshots/s1628_klientas_uzsakymas_35826.png`) — peržiūrėta: vienas mygtukas.

---

### S1627 (2026-09-06, naktis)

> Pridėti po šio antraštės (virš S1626). Tema: **Raimis: „Inga daugiau nieko nemato — nei ataskaitų, nei langų; man į ataskaitas — per Flatsome“ → juosta v1.9 „Ataskaitos ▾“.** Recon (`s1627_r.php`, `s1627_r2.php`): rolė `ps_darbuotojas` (Inga #5788) turi `manage_woocommerce` + `edit_shop_orders` (22 teisės) → visi katalogo langai ir ataskaitos (`petshop-reports` → `ps-pardavimai`, `ps-prekes`, `ps-atsargos`, `ps-klientai`, `ps-menuo` …, cap `manage_woocommerce`) jai **prieinamos**, juosta ataskaitų puslapiuose (`ps-*`) rodoma — trūko tik **nuorodos**: WP kairysis meniu juostos puslapiuose paslėptas (v1.1), todėl į ataskaitas eita per admin bar (Flatsome) arba URL.
- **`petshop-juosta.php` v1.8 → v1.9** (22 383 B, md5 `02a0778d47633bff5638accb63e3a8fa`; kopija `ps-backups/petshop-juosta-v18-BACKUP-2026-09-06.php`; repo `deploy/petshop-juosta.php` + atsarga `-v18-ATSARGA.php`): dešinėje, prieš „Sąskaitos“ — **„Ataskaitos ▾“** (paspaudimu / užvedus): Pardavimai ir pelnas · Prekių analizė · Atsargos ir pirkimas · Klientų analizė · Mėnesio uždarymas · Klientai (`manage_woocommerce`) · Petshop langai (`manage_options` — tik Raimis); aktyvus punktas žalias. Deploy `s1627_e20d.php` (md5 sargai, kopija, ping 200).
- **Patikra (Playwright Ingos akimis, `s1627_k.php`):** darbalaukyje išskleistas sąrašas su 6 punktais (be „Petshop langai“ — teisė), `ps-prekes` „Prekių analizė“ atsidaro su juosta ir aktyviu „Ataskaitos“; JS klaidų 0. Kadrai `screenshots/s1627_juosta_ataskaitos_inga.png`, `s1627_ataskaita_prekes_inga.png`.
- **Raimio sprendimas (09-06 naktis, GALIOJA):** Inga — kaip administratorius; langų nerodom **dėl patogumo dirbti, ne dėl slėpimo**. Ataskaitos (ir piniginės) darbuotojui matomos — rolė `ps_darbuotojas` su `manage_woocommerce` lieka kaip yra, atskiros teisės `ps_ataskaitos` NEdarom.

---

### S1626 (2026-09-06, naktis)

> Pridėti po šio antraštės (virš S1625). Tema: **Raimis (kadras): Dropshipping tuščia — „Čia tuščia — nieko daryti nereikia“, jokių laukų užsakymui į AV.** Priežastis: `laisku_korteles()` (tiekėjų mygtukai, rinkiklis, archyvas) buvo kviečiama tik kai eilėje yra užsakymų (`$rows`); be jų — bendras tuščios eilės tekstas. **v3.39.2** (md5 `f94d08e832f3852653ecde5cfdc27ef8`; kopija `ps-backups/petshop-darbalaukis-v3391-BACKUP-2026-09-06.php`; repo `deploy/` + atsarga `-v3391-ATSARGA.php`): `laiskai` eilė be paieškos → `laisku_korteles()` visada → mygtukai „Užsakyti į atsargas iš: VF · ZB · Prins · Belacor · Quattro · Ambrosia“, kaupiamos partijos, „Išsiųsti laiškai tiekėjams (n)“ matomi ir tuščioje eilėje; tekstas „Laiškų tiekėjams nėra — viskas išsiųsta. Prekių į atsargas — pasirink tiekėją viršuje.“ Bridge: `s1626_e19a/e19d.php` (deploy, Q: `tiek_mygtukai` 1, `tuscia` 0, Warning 0), `s1626_k.php` (kadrai `screenshots/s1626_tuscia_dropshipping.png`, `s1626_tiek_zb.png` — peržiūrėta).

---

### S1625 (2026-09-06, vakaras)

> Pridėti po šio antraštės (virš S1624). Tema: **Raimis: „laiško negavau, Venipak tik 1 lipduką sukūrė, viską patikrink; lipduke ištrink mano vardą; darbuotojas prašo Pastabų lauko užsakymo kortelėje“ → dev-pastas v1.2 (leidžiamų sąrašas), Venipak siuntėjo asmuo, darbalaukis v3.39 (vidinės pastabos) → v3.39.1 (radinys: grynai tiekėjo užsakymas rūšiuojamas „veža į AV“), TIKRI Venipak testai.** Bridge: `irankiai/s1625_r.php`, `s1625_r2.php` (recon), `s1625_e15a/e15d.php` (v3.39), `s1625_e16d.php` (dev-pastas + siuntėjas + opcija), `s1625_e17t.php` (1/2/3/D/K), `s1625_e18a/e18d.php` (v3.39.1). Kadrai `screenshots/s1625_e17_skydelis_vidine.png`, `s1625_e17_dropshipping.png`; lipdukas `analize/` (PDF b64 `s1625_e17t2.json`).

#### S1625 — RADINIAI IR ATSAKYMAI RAIMIUI
- **„Laiško negavau“** — dev-pastas: dev'e VISI `wp_mail` sugaunami (Raimio 17:21:43 `[PERSIŲSTI Vetfarmas] užsakymas` → žurnale, neišėjo). **dev-pastas v1.2** (6 135 B, md5 `f761afc71f5e42f5205366076d79d16f`; kopija `ps-backups/petshop-dev-pastas-v11-BACKUP-2026-09-06.php`; repo `deploy/petshop-dev-pastas.php`): `ps_dev_pastas_leisti` — 1 (viskas) arba el. paštų sąrašas kableliais → laiškas išleidžiamas tik kai VISI gavėjai sąraše, žurnale žymė „IŠLEISTA ·“; nustatyta **`terra@petshop.lt`**. Klientų / tiekėjų adresai — toliau sugaunami.
- **„Venipak tik 1 lipduką“** — tai Raimio partija **#21** (VF, „Venipak kurjeris į AV“, `venipak_pack` V07267E1000058, manifestas …0906002): dėžių laukas liko 1 (numatyta) → 1 pack. Lipdukų sk. = kortelės laukas „dėžių“. Ne klaida; patikrinta tikru Venipak (žr. testus).
- **„Ištrink mano vardą“** — `shopup_venipak_shipping_settings[shopup_venipak_shipping_field_sendercontactperson]` „Raimundas Bulakas“ → **„UAB Avesa“** (buvusi reikšmė `ps_s1625_vp_sender_buvo`); tą patį raktą naudoja ir variklis (`Petshop_AV_Tiekimas::venipak_registruoti` `$snd['person']`). Lipduko PDF (#35823): „Sender: UAB Avesa, Liucionių g. 46, LIUCIONYS, LT-15166; UAB Avesa +37068187787“ — vardo nėra ✓ (PNG peržiūrėtas).
- **Darbuotojo Pastabos** — v3.39 (žr. žemiau).

#### S1625 — `petshop-darbalaukis.php` v3.38.1 → **v3.39** → **v3.39.1** (543 4xx B, md5 `566db6434d5291a01c568793351aabc8`; kopijos `ps-backups/petshop-darbalaukis-v3381-…`, `-v339-BACKUP-2026-09-06.php`; repo `deploy/` + atsargos `-v3381-`, `-v339-ATSARGA.php`)
- **v3.39 — Vidinės pastabos:** skydelyje po „Dabar:“ laukas **„Pastabos (vidinės — mato tik darbuotojai)“** (geltonas textarea, „Išsaugoti“, „išsaugota · laikas · vardas“); meta `_ps_vidine_pastaba`, `_ps_vidine_pastaba_kas`; AJAX `ps_dl_vidine` (nonce `ps_dl_vidine_{id}`, ≤2000 simb., tuščia — ištrina); įvykis `vidine_pastaba` (prieš/po); sąraše prie numerio **✎** (title — tekstas), atnaujinamas be perkrovimo. Klientui nerodoma niekur (ne WC pastaba, ne laiške).
- **v3.39.1 — radinys teste:** v3.2 taisyklė „siūlyti „[T] veža į AV“, kai to tiekėjo užsakymas į AV atviras“ (`eilutes_kelias()` `atvira_partija`) veikė ir **grynai tiekėjo** užsakymams → vieno sandėlio → sistema surūšiavo pati kaip `i_av` → #35823/#35824 dingo iš Dropshipping į „veža į AV“ be darbuotojo sprendimo. Dabar pasiūlymas tik mišriam (yra AV eilutė — `eilutes_saltinis` per kitas eilutes); #35826 (grynai VF, VF partija atvira) → „VF siunčia klientui“ ✓.
- Deploy 2 dalimis ×2 (`e15a/e15d`, `e18a/e18d`; sargai, kopijos, ping 200), maži failai `e16d` (dev-pastas b64 md5, kopija, ping; opcija + siuntėjas toje pačioje užklausoje).

#### S1625 — TIKRI VENIPAK TESTAI (Raimis: „aš paskui Venipak ištrinsiu“; `testuotojas`; LP nenaudotas)
1. **#35822** AV 19475×2 → Surinkti → lapai → skydelio „Lipdukas“ su `n=2` (dėžių) → darbalaukis (`ps_dl_veiksmas` `lipdukas`) → 302 į variklio `ps_desk_veiksmas` `vp_reg` (testas seka Location) → **`vp_ok` „1 · AV“**, `_ps_siuntos.av` **2 numeriai V07267E1000059, V07267E1000060**, manifestas 07267260906001, `venipak_shipping_order_data` weight 0.8 / products 2 / sent; pastaba „Venipak registracija darbalaukyje (AV, manifestas 001): sėkminga“; skydelis → Paruošta „Kurjeris paėmė“ (neklikinta).
2. **#35823, #35824** (VF ×1) — po v3.39.1 kelias grąžintas „VF siunčia klientui“ (skydelio nuoroda); **#35826** naujas grynai VF → iškart „siunčia klientui“; kortelė „Lipdukai (3)“ (`vp_reg&ids=35823,35824,35826&sandelis=vf`) → **`vp_ok` „3 · VF“**, po 1 numerį: **V07267E1000061 / 062 / 063**, manifestas 07267260906002; kortelė „✓ lipdukas“ ×3, „Užsakyti iš VF (3 užs. + 0 prek. į AV)“ → POST su lipdukais, kurjerio sąrašu, prierašu „TESTAS iš dev — prašome nevykdyti“ → `dl_laiskas` „VF|3 užs. → terra@petshop.lt“; dev-pastas: **„IŠLEISTA · [PERSIŲSTI Vetfarmas] užsakymas 2026-09-06 → terra@petshop.lt [4 priedai: 35823/35824/35826 lipdukai + manifestas-07267260906002.pdf]“** — Raimis turi gauti; archyve prierašas ✓; #35823 → Paruošta „VF išsiuntė“. Lipduko PDF #35823 102 KB — siuntėjas be vardo (PNG).
3. **Vidinė pastaba #35821** — AJAX ok, meta, `vidine_kas` „2026-09-06 17:53:26|Testuotojas“, skydelis rodo, sąraše ✎, įvykis `vidine_pastaba`. Playwright: skydelis su geltonu lauku ir „Išsaugoti · išsaugota · laikas · Testuotojas“, sąraše ✎ prie #35821 ✓; JS/HTTP klaidų 0.
- **Venipak ištrinti (Raimis savitarnoje): V07267E1000058 (partija #21), 059–060 (#35822), 061–063 (#35823/824/826)**, manifestai 07267260906001/002.
- **Testiniai duomenys:** #35822–#35826 (35825 — ? nesukurtas/praleistas ID), partijos #19–#21, opcija `ps_e17_ids`, `ps_s1625_vp_sender_buvo` (palikti — atstatymui), dev-pastas žurnalas, archyvas.
- **Eilės po:** Gauti 6 · Surinkti AV 1 · Dropshipping 0 · Paruošta 5 · Visi 10; Warning 0.

---

### S1624 (2026-09-06, vakaras)

> Pridėti po šio antraštės (virš S1623). Tema: **Raimis pažiūrėjo v3.38 gyvai — „jau tikrai geriau, bet neliko laiko, kur galėčiau kažką parašyti laiške“ → prierašas tiekėjui.** Bridge: `irankiai/s1624_r.php` (recon), `s1624_e12a.php`+`s1624_e12d.php` (darbalaukis v3.38.1), `s1624_e13d.php` (tiekimas v1.10.1), `s1624_e14t.php` (T · K). Kadras `screenshots/s1624_e14_prierasas.png`.

#### S1624 — `petshop-darbalaukis.php` v3.38 → **v3.38.1** (538 908 B, md5 `b07ffb5a500c27e7dad1373c8ab77130`; kopija `ps-backups/petshop-darbalaukis-v338-BACKUP-2026-09-06.php`; repo `deploy/` + atsarga `-v338-ATSARGA.php`) · **variklis `petshop-av-tiekimas.php` v1.10 → v1.10.1** (77 645 B, md5 `cf7677a8c355c58cc79a489dae4114f1`; kopija `ps-backups/petshop-av-tiekimas-v110-BACKUP-2026-09-06.php`; repo `deploy/`)
- **Priežastis:** prierašo laukelis buvo (`pastaba`, „Prierašas laiške“, vienos eilutės input tarp varnelių) — nematomas; peržiūra jo nerodė; o be dropship (savas Tiekimo laiškas `uzsakyti()`) prierašas **iš viso neįeidavo** — variklio laiške pastabos nebuvo.
- **Darbalaukis:** laukas **„Prierašas tiekėjui (įeis į laišką po prekių lentele)“** — textarea per visą plotį, virš varnelių; peržiūroje gyvai (`.dl-perz-pastaba` po klientų lentele / prieš AV lentelę; be dropship — prieš parašą); `uzsakyti_vykdyti` rašo `pastaba` į `ps_tiekimas.pastaba` (istorija + savas laiškas).
- **Variklis:** `uzsakyti()` laiške po `laisko_dalis()` — `ps_tiekimas.pastaba` pastraipa (nl2br), jei netuščia. Dropship laiškas (`laisko_html`) pastabą turėjo — nekeista.
- **Patikra (`e14t`):** textarea yra, senas input nėra; #20 (VF kaupiama — **Raimio sudėtos prekės** Chicken&Rice ×1, JosiCat 15+3 ×2, Mini Duck 10 kg ×1 + testo Festival 900 g ×1) → „Užsakyti iš VF“ be dropship su prierašu → `ps_tiekimas.pastaba` įrašyta, archyvo laiške `[PERSIŲSTI Vetfarmas] UAB Avesa · prekių užsakymas` prierašas **po AV lentele, prieš „Ačiū“** ✓ (tik terra@, dev). Playwright: kortelė su prierašu, peržiūra rodo prierašą po klientų lentele ✓, JS klaidų 0. Deploy: ping 200, rollback nereikėjo.
- **Pastaba Raimiui:** #20 buvo tavo kaupiama VF partija — testas ją „užsakė“ (uzsakyta, laiškas tik man); Laukiam ją rodo su „Gauta“ — testinė, į 6 etapo sąrašą.

---

### S1623 (2026-09-06, vakaras)

> Pridėti po šio antraštės (virš S1622). Tema: **Raimio klausimas „kam Tiekimas ir Laiškai juostoje?“ → abu išimti (juosta v1.8); „prekių sau užsakyti nepatogu, ID — pievos“ → PREKIŲ RINKIKLIS pagal ženklą (darbalaukis v3.38); „Gauta“ su savikaina → gavimo partija (variklis `petshop-av-tiekimas.php` v1.10, Raimio leidimas „daryk“).** Bridge: `irankiai/s1623_r.php`…`s1623_r5.php` (recon), `s1623_e9a.php`+`s1623_e9d.php` (darbalaukis v3.38), `s1623_e10d.php` (juosta + tiekimas), `s1623_e11t.php` (T testas · K kadrai). Rezultatai `analize/s1623_*.json`, kadrai `screenshots/s1623_e11_rinkiklis_josera.png`, `s1623_e11_laukiam_savikaina.png`, `s1623_e11_archyvas.png`.

#### S1623 — RAIMIO SPRENDIMAI (09-06, GALIOJA)
- **Tiekimas ir Laiškai juostoje nereikalingi** („Ne noriu, o reikia“). Pagrindas iš kodo: Laiškų „Laukia“ = tie patys variklio blokai (`Petshop_AV_Dropship::kortele`, `laukiantys_perdavimo()`) kaip Dropshipping eilė — dvi vietos tam pačiam „Užsakyti“; unikalu tik archyvas → darbalaukis. Tiekimas: po v3.37 viskas kortelėje + Laukiam; unikalu — eilučių taisymas (perkelta į kortelę), „Likučių atnaujinimas“ (Gavimas daro geriau), istorija (užsakymo istorija). Langai `ps-tiekimas`/`ps-laiskai` lieka pasiekiami tiesiogine nuoroda.
- **Prekės sau (be kliento užsakymo):** iki šiol tik Tiekimo lange pagal SKU/ID („kažkokios pievos“). Sprendimas: Dropshipping eilėje tiekėjų mygtukai + rinkiklis pagal ženklą; „ieškoti per 40 pavadinimų ir atsiminti“ — ne.
- **(4) „Gauta“ su savikaina → partija** — leidimas keisti variklį („daryk“).

#### S1623 — `mu-plugins/petshop-darbalaukis.php` v3.37.1 → **v3.38** (537 273 B, md5 `eb5bc9b38f80b8e80ebfce2ca24931e6`; repo `deploy/petshop-darbalaukis.php` + `.b64`; atsarga `deploy/petshop-darbalaukis-v3371-ATSARGA.php`; kopija `ps-backups/petshop-darbalaukis-v3371-BACKUP-2026-09-06.php`)
- **„Užsakyti į atsargas iš: VF · ZB · Prins · Belacor · Quattro · Ambrosia“** Dropshipping eilės viršuje (`&tiek=src`) — atsidaro to tiekėjo kortelė ir be užsakymų (tuščia „Veža į AV“ su rinkikliu).
- **Rinkiklis** (`.dl-rink`, vietoj `datalist`/SKU laukelio): „Pridėti prekių iš VF ▾“ → **Ženklas** (`product_brand`, tik to tiekėjo — SQL per `_ps_sandelis`; VF 41 ženklai, ZB 23) · **„išparduota (reikia užsakyti)“** (katalogo `uzsakyti` taisyklė = parduodama ≤ 0 → `wc_product_meta_lookup.stock_quantity ≤ 0 / outofstock`) · **ieškoti sąraše** (≥2 simb.) → lentelė: prekė (pavadinimas su pakuote) · SKU · **AV** · **Tiek.** (`_vf_qty`, kitiems „—“) · pask. gavimas (`ps_partijos` data · vnt.) · savik. (paskutinė partija / `_vf_cost` / `_zb_cost` / `_cost_price`) · kaina · varnelė + kiekis (įjungiamas pažymėjus) → **„Pridėti pažymėtas (n)“**. AJAX `ps_dl_atsargos` (nonce `ps_dl_zurnalas`; ≤300 eil., išparduotos eilutės raudonu AV). `tiekimas_vykdyti` `ka=prideti` su `prekes[id]=qty` — kelios iš karto, sujungiama su esama tos prekės eilute (kaip variklio 'pridėti', tik `$wpdb` tiesiogiai į `ps_tiekimas_eil`); SKU/ID kelias paliktas.
- **Atsargų eilutės kortelėje:** kiekis (`onchange` → išsaugo) ir „×“ (išima) — `ka=eilute` (tik `order_id IS NULL`, tik kaupiama, tik to tiekėjo).
- **Laukiam „Gauta“:** prie eilutės **savikaina € be PVM** (numatyta iš paskutinės partijos / `_vf_cost` / `_zb_cost` / `_cost_price`; tuščia — tik likutis).
- **`view=laiskai`** — išsiųstų laiškų tiekėjams archyvas darbalaukyje (`ps_laisku_archyvas`; data · tema · kontekstas · kam · priedai · „Rodyti“ pilnas tekstas); nuoroda „Išsiųsti laiškai tiekėjams (n)“ Dropshipping eilės apačioje.
- CSS `.dl-ats-tiek`, `.dl-rink*`, `.dl-eil-*`, `.dl-arch-t`; JS `dlRinkKrauti/dlRinkSk` (ženklas/išparduota → `change`, paieška 300 ms), rinkiklio ir archyvo atidarymas.

#### S1623 — `mu-plugins/petshop-juosta.php` v1.7 → **v1.8** (19 806 B, md5 `150dbae59f4623f223c349355b52c415`; kopija `ps-backups/petshop-juosta-v17-BACKUP-2026-09-06.php`; repo `deploy/petshop-juosta.php`) — punktai „Tiekimas“ ir „Laiškai“ išimti (skaičiavimas paliktas). Juosta dabar: ⌂ · Užsakymai · Rytinė eiga · Prekės · Gavimas · Rinkiniai · Akcijos · riba · paieška · Sąskaitos · Žurnalas · vartotojas.

#### S1623 — VARIKLIS `mu-plugins/petshop-av-tiekimas.php` v1.9.3 → **v1.10** (77 228 B, md5 `c22055fdc7307ef4c3190a138cdb0bf9`; kopija `ps-backups/petshop-av-tiekimas-v193-BACKUP-2026-09-06.php`; repo `deploy/petshop-av-tiekimas.php`; Raimio leidimas 09-06)
- `priimti()`: kai `savikaina[eid] > 0` → `Petshop_Partijos::priimti(product_id, {kiekis, savikaina, EUR, geriausia_iki, tiekejas, tiekimas_id=partija, pastaba})` — gavimo partija su savikaina, AV likutis kyla ten (S590 taisyklė), `_cost_price` perskaičiuojamas (tiekėjo prekėms — ne); `Petshop_AV_Stock::increase` tada NEkviečiamas (be dublio). Be savikainos — kaip buvo. Galiojimas iš formos `YYYY-MM` → partijai paskutinė mėnesio diena. Trūkumas → nauja partija, K2 — nekeista. **Priežastis:** du priėmimo keliai (Tiekimo „Gauta“ — tik likutis; Gavimas — partija su savikaina) → per „Gauta“ gautos prekės neturėjo partijos → „Partijose trūksta n vnt.“ kiekviename užsakyme, savikaina nesiskaičiuoja; po T-0 pradinių partijų problema būtų grįžusi su pirmu gavimu.

#### S1623 — DEPLOY IR PATIKRA
- Deploy: darbalaukis 2 dalimis (`e9a`/`e9d`, gyvo md5 sargas v3.37.1, kopija, ping 200), juosta + tiekimas vienoje payload'e (`e10d`: b64 md5 sargai, `token_get_all`, gyvo md5 sargai, kopijos, ping po kiekvieno, rollback abiejų — nereikėjo). `php -l` lokaliai.
- **T (`e11t`, `testuotojas`):** `tiek=zb` → kortelė „Užsakyti iš ZB · 0 užs.“ + VF; mygtukai „Užsakyti į atsargas iš: VF ZB Prins Belacor Quattro Ambrosia“; AJAX: Josera (term 314) **213** prekių (pvz. A/S Chicken&Rice 12,5 kg · AV 0 · VF 100 · sav 16,90), VF išparduota **179**, `q=festival` → 3 (Festival 12,5 kg AV 3 / VF 696 / sav 31,15; 3 kg; 900 g), ZB išparduota 57; „Pridėti pažymėtas“ 18154×2 + 25415×1 → **#19** (2 eil.) → kortelėje 2 eilučių formos su kiekiu ir „×“ → kiekis 18154 → 3 ✓ → 25415 išimta ✓ → „Užsakyti iš VF“ be dropship → savas laiškas `[PERSIŲSTI Vetfarmas] UAB Avesa · prekių užsakymas 2026-09-06` ✓ → Laukiam forma: `gauta[31]`, **`savikaina[31]=31.15`** (iš `_vf_cost`) → „Gauta“ su 31,50 ir galioja 2027-06 → **`ps_partijos` #22** (18154, 3 vnt., 31,50 €, iki 2027-06-30, tiekejas vf, tiekimas_id 19, „Tiekimas: VF partija #19“), `ps_partijos` 10→11, AV 18154 3→**6** (be dublio), `nurasyti(18154,1,dry)` → padengta 1, nepadengta 0 (įspėjimo nebus) ✓; `view=laiskai` 200, 29 laiškai, „Rodyti“ ✓; Warning 0. **Playwright:** rinkiklis su Josera + „festival“ (3 eil., varnelė, „Pridėti pažymėtas (1)“), juosta be „Tiekimas“/„Laiškai“, archyvas išskleistas; JS klaidų 0 (vienas `ERR_HTTP2_PROTOCOL_ERROR` resursas — ne mūsų).
- **Testiniai duomenys:** `ps_tiekimas` #19 (gauta), `ps_partijos` #22, AV 18154 = 6, dev-pastas +1, archyvas +1.
- **Žinoma / atidėta:** rinkiklio lentelėje skaičių stulpeliai „pill“ stiliumi (`.dl-tbl` paveldas) — kosmetika; `_cost_price` tiekėjo prekėms neperrašomas (variklio taisyklė); „Tiek.“ tik VF (kitiems tiekėjams likučio meta nėra).

---

### S1622 (2026-09-06, popietė)

> Pridėti po šio antraštės (virš S1621). Tema: **6 ETAPO VALYMAS ĮVYKDYTAS (Raimis: „išvalyk senus, padaryk naujus testus, atsargiai su LP — ji išsikviečia automatiškai“) + NAUJI E2E TESTAI po valymo (5 keliai, LP nenaudotas, Venipak/Paysera nekviesti).** Kodo pakeitimų NĖRA (darbalaukis v3.37.1). Bridge: `irankiai/s1622_valymas.php` (D dry-run · A apply · Q po), `s1622_r.php`, `s1622_r2.php` (recon), `s1622_e2e.php` (fazės 1–5, Z). Rezultatai `analize/s1622_*.json`.

#### S1622 — VALYMAS (planas v1.2 §1–2; A–H be atsakymų NELIESTA)
- **D (dry-run):** 66 užsakymai (#35087…#35815) + 8 refund'ai; LP metos turėjo #35416 (`_woo_lithuaniapost_barcode` CC000000001LT, `lp-delivered`, testinis), #35802/#35804 (`lp-parcel-failed`); Venipak `_ps_siuntos` 17 užs. (V07267E1000030…057, …099 — Raimis trina Venipak savitarnoje, jei dar yra). **LP kabliai:** tik `woocommerce_order_status_changed` (`Woo_Lithuaniapost_Admin_Order_Service::on_status_changed`) ir `woocommerce_thankyou` — trynimas (`delete(true)`) statuso nekeičia, LP nekviečiama; A fazėje visi LP/Venipak/mūsų variklių kabliai ant trynimo/statuso (`Petshop_Siuntu_Valymas`, `Petshop_Faktai::statusas`, `Petshop_Uzsakymu_Ivykiai::busena`, `Petshop_AV_Reduce::grazinti`, `Petshop_Juosta`, LP) nuimti tik toje užklausoje, `pre_wp_mail=false`.
- **A (apply):** kopija **`uploads/ps-backups/valymas-2026-09-06/`** (22 failai: `wc_orders.json.gz` — 74 užsakymų/refund'ų `get_data()` + metos + eilutės, 60 KB; kiekviena trinama lentelė `*.json.gz`; `opcijos.json.gz`; `wcdn/{invoice,creditnote,receipt}/` — 93+4+2 PDF **perkelti**, ne ištrinti). Ištrinta: refund'ai 8 → užsakymai 66 (po vieną, `delete(true)`, klaidų 0) → `wc_orders` 0; lentelės (sargai `testinis=1` / `aplinka='dev'` / visos): `ps_shipments` 25, `ps_fakt_siuntos` 25, `ps_fakt_uzsakymai` 69, `ps_fakt_eilutes` 113, `ps_fakt_grazinimai` 11, `ps_fakt_atsargos_d` 31 765, `ps_uzsakymu_ivykiai` 647, `ps_tiekimas` 8 + `_eil` 11, `ps_plan_events` 7, `ps_carts` 131, `ps_refill_tracking` 32, `ps_email_jobs` 378, `ps_event_log` 868, `ps_action_tokens` 415, `ps_ataskaitu_dienos` 838, `ps_kontrole_dienos` 9, `ps_web_ivykiai` 1 106, `ps_web_dienos` (`aplinka='dev'`) 423, `ps_sargas_klaidos` 3 198; WC našlaičiai: `wc_order_stats` 12, `order_items` 27, `itemmeta` 235, `product_lookup` 13, `tax_lookup` 11; `wc_customer_lookup` 26 (be užsakymų, testiniai el. paštai) → liko 1 (#24 gutulis@gmail.com — B klausimas); opcijos `ps_s1617_test`, `ps_audit_ids`, `ps_e3_oid`, `ps_e3_oid2`, `ps_s1621_oid`, `ps_s1621b_oid`, `ps_audit_mail`, `ps_dev_pastas_zurnalas` — ištrintos. **Q:** viskas 0, PDF 0/0/0, darbalaukis Visi 0, ping 200, Warning 0.
- **NELIESTA (sprendimų laukia):** B vartotojai (visi), C `ps_partijos` 10, D augintiniai/`ps_pets*`/`ps_consent_log`, E — padaryta kartu (dev faktai), **F likučiai** (19475 `_stock` 40→32 po S1621, dabar 26 po E2E; 19708 16→15; 35357 6→4; AV 18154 3, 18599 38, 25411 0, 19756/16889/16727 = 1 — atstatyti pagal F su Raimio reikšmėmis), `ps_laisku_archyvas` (26 + nauji), skaitikliai (AVPN 350 „kitas“, IAPV 169 — → 101 T-0), `ps_ist_*`, `ps_dim_*`, feeding, `testuotojas` #75.

#### S1622 — NAUJI E2E TESTAI (darbalaukis v3.37.1, `testuotojas`, tikri endpoint'ai; LP NENAUDOTAS; siuntos numeriai fiktyvūs per `venipak_shipping_order_data` + `Petshop_Siuntos::prideti_is_plugino` — Venipak nekviestas; Paysera nekviesta; laiškai — dev-pastas)
1. **#35816 grynai AV** (19475×2, Venipak kurjeris, bacs `payment_complete`): surūšiavo pati → Surinkti AV → „Surinkti“ (lapai) → lipdukas (fikt. V07267E1E2E001) → **„Kurjeris paėmė“** → completed AVPN000344. Eilės 0→Surinkti 1→0, Visi 1.
2. **#35817 + #35818 grynai VF** (35357×1): Dropshipping (VF) → lipdukas (fikt.) → kortelė „Užsakyti iš VF (2 užs. + 0 prek. į AV)“ → POST tik #35818 → laiškas `[PERSIŲSTI Vetfarmas] užsakymas 2026-09-06` → Paruošta „VF išsiuntė“ → completed AVPN000346. **#35817 paliktas Dropshipping** (su lipduku — Raimiui pamatyti kortelę).
3. **#35819 mišrus** (19475×1 + 25411×1): Neišrūšiuoti → „VF veža į AV“ → Dropshipping (skydelis „Užsakyti iš VF į AV“) → kortelėje „Pridėti“ 18154×1 → užs. tiekėjui **#18** → „Užsakyti iš VF“ be dropship (uzsakymai tuščias) → **savas Tiekimo laiškas** `[PERSIŲSTI Vetfarmas] UAB Avesa · prekių užsakymas 2026-09-06` (`dl_uzsak_av` „laiškas išėjo tik man“) → Laukiam → „Gauta“ (18154 AV 2→3, 25411 rezervuota) → Surinkti → lapai → lipdukas (fikt.) → „Kurjeris paėmė“ → completed AVPN000347.
4. **#35820 Atsiėmimas AV** (inst #16 ON → `ps_dl_naujas` grynais 19708×1 → OFF): Surinkti → „Paruošta atsiimti“ (laiškas) → „Klientas atsiėmė“ → completed AVPN000348, laiškas su PDF. inst16=0 ✓.
5. **#35821 telefoninis pavedimu** (19475×3, `venipak_kurjeris` 3,99): on-hold IAPV000168, likutis nekeistas (29) → Neapmokėti → skydelis „Pažymėti apmokėtu“ → processing AVPN000349, likutis 29→26, Surinkti AV (paliktas).
- **Po visų:** Gauti 2 · Neišrūšiuoti 0 · Laukiam 0 · Surinkti AV 1 (#35821) · Dropshipping 1 (#35817) · Paruošta 0 · Klausimai 0 · Neapmokėti 0 · **Visi 6**; dev-pastas 17 (visi testiniai adresai, 0 išėjo); `ps_tiekimas` #18 gauta; Warning 0. Pastebėta: skydelio „Kurjeris paėmė“/„VF išsiuntė“ per GET be dialogo → „klientui nepranešta (varnelė nuimta)“ — sekimo laiškas priklauso nuo dialogo varnelės (UI numatyta įjungta), ne klaida. Partijų įspėjimai (#19475, #19708) — kaip S1621 (b) iki T-0.
- **Testiniai duomenys (6 etapui, T-0 pakartoti A):** #35816–#35821, `ps_tiekimas` #18 (+ eil.), opcija `ps_e2e_ids`, `ps_laisku_archyvas` +2, dev-pastas 17, faktai `testinis=1`, likučiai (F).

---

### S1621 (2026-09-06, diena)

> Pridėti po šio antraštės (virš S1620). Tema: **Raimio bandymas „praeik visą grandinę kaip darbuotojas“ (mišrus užsakymas AV+VF, prekės į AV, atsargos) → sisteminių klaidų sąrašas → Raimio sprendimai → darbalaukis v3.36 → v3.37 → v3.37.1: VIENA KORTELĖ PER TIEKĖJĄ (Laukiam / Dropshipping / Tiekimo langų dubliavimas panaikintas).** Variklis, tema, Pragma — neliesti. Bridge: `irankiai/s1621_e1r.php` (R recon prekės/tiekimas), `s1621_e2p.php` (P variklio recon), `s1621_e3t.php` (T/L/S grandinė v3.36 — #35814), `s1621_e4p.php` (P `siusti()`/`paruosti()`), `s1621_e5a.php`+`s1621_e5d.php` (v3.37 deploy), `s1621_e6t.php` (T/C/S/K grandinė v3.37 — #35815 + kadrai), `s1621_e7r.php` (R laiškų archyvas), `s1621_e8a.php`+`s1621_e8d.php` (v3.37.1 deploy). Rezultatai `analize/s1621_*.json`, kadrai `screenshots/s1621_e6_dropshipping_vf.png`, `s1621_e6_laukiam.png`.

#### S1621 — RAIMIO BANDYMAS (v3.36, #35814) IR KĄ MAČIAU
- **Užduotis:** mišrus užsakymas Grancarno 400 g ×4 (AV) + Josera Mini Lamb ×1 (VF; kataloge **900 g** #25411, ne 800 g) — VF prekę parsivežti į AV; kartu iš VF į atsargas Josera Festival 12,5 kg ×2 (#18154) ir Exclusion kiauliena 400 g ×6 (#18599, AV 32 + VF); visa grandinė iki laiško VF (tiekėjui — 1 laiškas), Venipak nekviesti.
- **Eiga (`testuotojas`, tikri endpoint'ai, laiškai `pre_wp_mail` prior. 4):** #35814 (5787, Venipak kurjeris inst 2, bacs, `payment_complete`, 18,68 €) → Neišrūšiuoti → skydelyje „VF veža į AV“ → **pats iškrito į Laukiam** (kelio keitimas = mišraus planas = surūšiuota; „Surūšiuota“ mygtuko nebeliko) → Laukiam VF kortelė **„Kartu su Dropshipping iš VF (2 prek.)“** (kartu senas #35090 FURminator, varnelė įjungta) → POST tik #35814 → užsakymas tiekėjui **#16** kaupiama → atsargos pridėtos **Tiekimo lange** (`veiksmas()` `pridėti` `nauja_sku`), darbalaukyje negalima → Dropshipping kortelė VF: „Užsakyti iš VF (8 užs.)“ **DISABLED** („pirma lipdukai“, 8 VF dropship, dalis be lipdukų) → apeita: `ps_dropship_send` su 1 užs. (#35443, turėjo V07267E1000055) + `su_partija=16`, lipdukai/sąrašas OFF → **vienas laiškas** `[PERSIŲSTI Vetfarmas] užsakymas 2026-09-06` → terra@ (#35443 Churu ×1 · UAB Avesa: Josera ML ×1, Festival ×2, Exclusion ×6 · „atsiimsime patys — lipdukų nereikia“); #16 `uzsakyta`; #35814 „Laukiam iš VF“; #35443 → Paruošta. Eilės Visi 64→65, Laukiam 1→2, Paruošta 0→1 — paaiškinta. Išorėn 0 laiškų.
- **Klaidos (Claude, Raimis sutiko):** (A) kelio keitimas skydelyje pats rūšiuoja; (B) atsargų į AV — tik Tiekimo lange; (C) „vienas laiškas“ = prekės į AV įkaitas VISŲ to tiekėjo dropship lipdukų (užraktas serveryje visiems, varnelės neatrakina); (D) „Peržiūrėti laišką“ Dropshipping kortelėje be „+ į AV“ dalies; (E) Laukiam kortelėje seni užsakymai pagal nutylėjimą įtraukti; (F) pastaba „Partijos nurašytos −0 · Partijose trūksta 4 vnt.“ kiekvienam AV prekės užsakymui be `ps_partijos`; (G) VF veidrodis `_stock` mažėja nuo rezervacijos (žinoma, sync atstato).
- **Raimio sprendimai (09-06, GALIOJA):** „Dropshipping“ terminas lieka. Jokių „tu/aš“ — žmogus dirba su jam suteiktais įgaliojimais (atsargų pridėjimas — kortelėje visiems su teise). **Tiekėjui — 1 laiškas dienai per sistemą; antrą tą pačią dieną gali suformuoti ir išsiųsti tik darbuotojas** (išimtys, bet būna) — mygtukas tas pats, su įspėjimu. (A) — palikti kaip dabar (vieno sandėlio — pati, mišrus — darbuotojas). (F) — kelias **(b): T-0 pradinio likučio partija kiekvienai AV prekei** (kiekis = likutis, savikaina iš kortelės; savikainas Raimis sudeda iki paleidimo, techninis darbas) — įspėjimai iki tol lieka.

#### S1621 — `mu-plugins/petshop-darbalaukis.php` v3.36 → **v3.37** → **v3.37.1** (519 930 B, md5 `c146f8452274b514046452be455fe6e3`; repo `deploy/petshop-darbalaukis.php` + `.b64`; atsarga `deploy/petshop-darbalaukis-v336-ATSARGA.php`; kopijos `ps-backups/petshop-darbalaukis-v336-BACKUP-2026-09-06.php`, `…-v337-BACKUP-2026-09-06.php`)
- **Viena kortelė per tiekėją (`laisku_korteles()`, eilė Dropshipping):** tiekėjai = dropship grupės ∪ „veža į AV“ neužsakyta/kaupiama ∪ kaupiamos partijos su eilutėmis. Dalys: **„Siunčia klientui“** (kaip buvo; varnelė `data-lip` 0/1) · **„Veža į AV“** (užsakymų `i_av` eilutės, varnelės `.dl-av-cb` → hidden `ids_av`; kaupiamos partijos „į atsargas“ eilutės) · **„į atsargas“** — laukelis „prekė arba SKU“ (datalist per `ps_dl_prekes`, `AV n`), kiekis, „Pridėti“ → `ps_dl_tiekimas` `ka=prideti` → variklio `veiksmas()` 'pridėti' (`atvira_partija` sukuria) · „Kaip atkeliaus į AV“ / svoris (numatytas iš katalogo) / dėžės (tik kai yra prekių į AV) · **vienas mygtukas „Užsakyti iš [T] (n užs. + m prek. į AV)“** (ZB: „Užsakyti iš ZB į AV (m prek.)“, be laiško; ZB dropship — „Pažymėti ZB perduotais“ kaip buvo) · „Peržiūrėti laišką“ — **pilnas** (`laisko_html` + `laisko_dalis` AV lentelė) · prierašas, lipdukai, kurjerio sąrašas, siųsti tiekėjui, kopija man. **Įspėjimas** „[T] šiandien jau užsakyta HH:MM — tai bus antras laiškas“ (iš `ps_laisku_archyvas` „Perdavimas tiekėjui: X“ šiandien + `ps_tiekimas.uzsakyta` šiandien). Lipdukų užraktas — **JS tik pažymėtiems** (`be>0` → disabled + title); pastaba po kortele pakeista.
- **Savas `admin_post_ps_dl_uzsakyti` (`uzsakyti_vykdyti`, nonce `ps_dl_uzs_{src}`):** pažymėtos `ids_av` eilutės → `ideti_eilute` + `_ps_konsolidacija` + įvykis `kons`; partijos `pristatymas/svoris/dezes` → `ps_tiekimas` (kaip variklio 'issaugoti'); nėra pristatymo ne-ZB → klaida „pasirink Kaip atkeliaus į AV“; **yra pažymėtų dropship → `do_action('admin_post_ps_dropship_send')` su `su_partija`** (variklio `siusti()`: laiškas, lipdukai, `paruosti()`, `uzdaryti_po_laisko`); **nėra → `do_action('admin_post_ps_tiekimas')` `uzsakyti`** (savas laiškas / ZB be laiško); nieko → klaida. Grįžimas per `grizti_cia()` (naujos žinutės `prideta`, `nerasta`; `issaugota` trumpinta; `ps_dl_kartu` išimtas).
- **`laukiam_korteles()` — tik A blokas** (užsakyta → „Gauta“; eilučių grupavimas tik `b.busena='uzsakyta'`); B blokas, „Kartu su Dropshipping“, `dlt_` formos — IŠIMTI. `tiekimas_vykdyti`: `ka` ∈ uzsakyti / priimti / **prideti** (kartu išimtas). `faktai()`: `i_av` neužsakyta/kaupiama → `eiles['laiskai']`, užsakyta → `laukiam`; takelio `kons` → eilė laiskai; `$leid['laiskai']=['kons']`; skydelio `tiek_url` kaupiama → laiskai. Rytinė eiga: 2-as žingsnis „Dropshipping — lipdukai ir užsakymai tiekėjams (siunčia klientui · veža į AV · į atsargas)“, 3-ias („Užsakyti iš tiekėjų į AV“) išimtas. Eilių aprašai atnaujinti. CSS `.dl-tk-h3`, `.dl-antras`, `.dl-ats-f`. JS: `dlPerskaiciuoti(fid)` (n/m/be, `%n`/`%m`), inicializacija visoms `dlf_` formoms, „į atsargas“ paieška (250 ms, ≥2 simb., ne skaičiui).
- **v3.37.1:** Laukiam „Gauta“ formai grąžintas `$cia` (v3.37 Playwright kadre `Warning: Undefined variable $cia` 3680 — pagauta vizualiai); ZB kortelė be prekių į AV — be mygtuko „(0 prek.)“.
- **Deploy:** gzip+b64 2 dalimis (`e5a`/`e5d`, `e8a`/`e8d`: part1 md5 sargas, bendras md5, `token_get_all`, gyvo md5 sargas v3.36 `dd0202f4…` / v3.37 `91535bd9…`, kopija, ping 200, rollback nereikėjo). `php -l` lokaliai prieš kiekvieną.
- **Patikra (`e6t` C/S, `testuotojas`, gaudyklė prior. 4 — šįkart tuščia, laiškas patikrintas iš `ps_laisku_archyvas`):** **#35815** (tas pats mišrus) — sistema pati pasiūlė „VF veža į AV“ (atviras #16, taisyklė) → „Surūšiuota“ → **Dropshipping** (12→13; Laukiam nekito), skydelis „Dropshipping · toliau: Užsakyti iš VF į AV“ → kortelė „Užsakyti iš VF (7 užs. + 2 prek. į AV)“ (#35090 + #35815) → „Pridėti“ `JOS0913`×2 (SKU) ir `18599`×6 (ID) → **#17** kaupiama, „4 prek. į AV“, peržiūra su AV lentele (FURminator, Josera ML, Festival ×2, Exclusion ×6), įspėjimas „VF šiandien jau užsakyta 14:15“ → POST `ps_dl_uzsakyti` (uzsakymai=35442 su lipduku, ids_av=35815, tiekėjas atveža) → 302 `dl_laiskas` „VF|1 užs. → terra@petshop.lt“; archyve **vienas laiškas** (#35442 Exclusion Mediterraneo ×1 · UAB Avesa: Festival ×2, Exclusion ×6, Josera ML ×1 · lipdukų nereikia; priedai [] — lipdukai/sąrašas off testui); #17 `uzsakyta` su 3 eil. (35815 eilutė 1645 + 2 atsargos); #35815 „Laukiam iš VF“ (Dropshipping 13→11, Laukiam 1→2, Paruošta 1→2 — #35442 perduota) → Laukiam kortelė #17 (3 „Gauta“ laukai) → „Gauta“ → #17 `gauta`, **#35815 → Surinkti AV** (Laukiam 2→1, Surinkti 7→8), abi eilutės `av`. Likučiai: 19475 36→32 (rezervas), 18154 AV 0→**2**, 18599 AV 32→**38**, 25411 gauta +1 → rezervuota (AV 0). Dev-pastas 45→48 (2 užsakymo + 1 VF), 0 išėjo. **Playwright:** Dropshipping (4 kortelės; VF: Siunčia klientui 6 · Veža į AV #35090 · į atsargas laukelis · Lipdukai (5) · „Užsakyti iš VF (6 užs. + 1 prek. į AV)“ užrakintas, kol pažymėti be lipduko · įspėjimas „14:15, 15:01“), Laukiam (VF #16 su „Gauta“); JS/HTTP klaidų 0. Skydelio kadras — Playwright timeout (ne klaida), nepakartotas.
- **Testiniai duomenys (6 etapui):** užsakymai **#35814, #35815** (processing; 35814 Laukiam #16, 35815 Surinkti AV), užsakymai tiekėjams **#16** (uzsakyta), **#17** (gauta), #35442/#35443 `_ps_dropship_sent_src.vf`, opcijos `ps_s1621_oid`, `ps_s1621b_oid` (trinti), `ps_laisku_archyvas` +2, likučiai 18154/18599/25411 AV (žr. aukščiau). TEMP snippet'ai trinami kiekvieno run'o pradžioje (liko 1 neaktyvus).
- **Žinoma / atidėta:** `.dl-laisko-nust` „kopija man“ persikelia į naują eilutę (kosmetika); peržiūra be pasirinkto pristatymo rodo „1 dėžė — lipdukai bus priede“ (variklio `laisko_dalis` numatytoji); Laukiam kortelėje seni neužsakyti (#35090) — varnelės pagal nutylėjimą įjungtos (nekeista); (G) VF veidrodis — variklis.

#### S1621 — SESIJOS BŪKLĖ
- Dev sveikas: darbalaukis **v3.37.1** gyva, ping 200; eilės po darbo: Gauti 9 · Neišrūšiuoti 3 · Laukiam 1 · Surinkti AV 8 · Dropshipping 11 · Paruošta 2 · Klausimai 9 · Neapmokėti 3 · **Visi 66**. Kiti failai nekeisti (base.php v2.12, dev-pastas v1.1, juosta v1.7, vertimai v1.2, Pragma OFF). Skaitikliai AVPN 342 („kitas“), IAPV 168.
- **T-0 sąrašas papildytas:** pradinio likučio partijos visoms AV prekėms (b).
- Toliau — STARTAS 14: Raimio kiti pastebėjimai · 6 etapo A–H · EAN lape · E2E.

---

### S1620 (2026-09-06, rytas)

> Pridėti po šio antraštės (virš S1619). Tema: **Raimio patvirtinimai (C a–c + PPK kvitas) · nelaukto priedo patikra per `pre_wp_mail` (#35813) · dev-pastas v1.0 → v1.1 · Pragma recon (tik skaitymas).** Kodo darbalaukyje / variklyje / temoje — NĖRA (v3.35.1 / base v2.12 nekeisti). Bridge: `irankiai/s1620_e1.php` (R recon · T testas su gaudykle · P Pragma recon), `s1620_e2d.php` (D dev-pastas v1.1 deploy · Q patikra + Pragma eilutės), `s1620_e3r.php` (paskyros tekstų recon), `s1620_e4d.php` / `e4d2.php` (vertimai v1.1 / v1.2 deploy), `s1620_e5s.php` (Playwright), `s1620_e6r.php` / `s1620_e7r.php` (6 etapo recon); `analize/s1620_*.json`; `screenshots/s1620_e5_svecio_uzsakymas_35813.png`; `deploy/petshop-dev-pastas.php` v1.1, `deploy/petshop-vertimai.php` v1.2.

#### S1620 — RAIMIO PATVIRTINIMAI (09-06, GALIOJA)
- **C (a):** neapmokėtas atsiėmimo užsakymas į darbą neina; klientas moka atvykęs → „Apmokėta grynais“ → surinkimas prie kliento. **C (b):** po „Klientas atsiėmė“ — savas laiškas su PVM sąskaitos PDF (tekstas Claude siūlymas, taisomas Sąskaitų lange); WC „išsiųstas!“ laiškas išjungtas. **C (c):** jungiklis — WC zonos instancija #16 (išjungta), ne opcija. **PPK kvito išvaizda** — patvirtinta. Spec v1.15 §12.5 atnaujinta.

#### S1620 — NELAUKTO PRIEDO PATIKRA (e1 T, tikros nuorodos, tas pats hostas)
- **Metodas:** TEMP snippet'e `pre_wp_mail` gaudyklė **prior. 4** (prieš dev-pastas prior. 5), įjungiama opcija `ps_s1620_gaudykle` tik testo metu, rašo `gettype`/`var_export`/failų vardus į `ps_s1620_laiskai`, grąžina `$pre` (dev-pastas toliau gaudo). Instancija #16 ON → **#35813** (naujas, grynais, AV 19708, 2,60 €, processing, AVPN000341) → Surinkti → lapai (`_ps_surinkta`) → tikra nuoroda `ats_paruosta` → tikra `atsieme` → completed → instancija OFF, opcijos išvalytos (0 `ps_s1620_%`).
- **Rezultatas:** „Jūsų užsakymas Nr. 35813 paruoštas atsiimti“ — `attachments` **`''` (string)**, priedų NĖRA; dev-pastas v1.0 `count((array)'')` = **1 — skaičiavimo artefaktas**, ne priedas (tas pats visiems mūsų `WC()->mailer()->send()` be 5-o argumento — `WC_Emails::send()` numatytoji `$attachments=''`). „Užsakymas Nr. 35813 atsiimtas — PVM sąskaita faktūra Nr. AVPN000341“ — **array(1)**: `uploads/wcdn/invoice/PVM-saskaita-AVPN000341.pdf` (33 854 B) ✓. R fazė: dev-pastas žurnale WC kliento laiškai „gautas“ / „išsiųstas!“ rodė 1 (tikras WC priedas per `woocommerce_email_attachments`), admin „Naujas užsakymas“ 0, mūsų — 1 (artefaktas). Temoje/pluginuose nieko nepriseginėjama — klausimas uždarytas.
- **dev-pastas v1.1** (4 945 B, md5 `ed7e80a87be1994244a565385fe46af0`; kopija `ps-backups/petshop-dev-pastas-v10-BACKUP-2026-09-06.php` md5 `744c69c9…`; repo `deploy/petshop-dev-pastas.php`): `priedai` = `count(array_filter((array)…))` (tik tikri failai) + naujas laukas `failai` (basename'ai, rodomi lange `<small>`). Deploy e2d D: md5 sargas gyvo ✓, b64 md5 ✓, `token_get_all` ✓, `php -l` lokaliai ✓, ping 200 / fatal 0, `md5_po` ✓; Q: `Petshop_Dev_Pastas::VERSIJA` 1.1 ✓. Senų žurnalo įrašų `priedai` (iki 42) — seni skaičiai (1 = galimai `''`), nekeista.
- **Eilės:** prieš Gauti 9 · Neišrūšiuoti 3 · Laukiam 1 · Surinkti 7 · Dropshipping 12 · Paruošta 0 · Klausimai 9 · Neapmokėti 3 · Visi 63; po Gauti 9 · … · **Visi 64** (+#35813 completed). **Gauti 11 → 9 (S1619 pabaiga → S1620 pradžia) — paaiškinta:** „Gauti“ = darbuotojas neatidarė skydelio (`_ps_matyta`, v3.9); S1619 e6s Playwright kadrai (`eile=visi&atidaryti=ID`) atidarė #35811 (+ vieną iš 35809/35810) PO S1619 eilių užrašymo → 2 užsakymai pažymėti matytais. Ne regresija. Dev-pastas 38 → 42 (4 #35813 laiškai, 0 išėjo). Skaitikliai: **AVPN 341**, PPK 103, KR-AVPN 105, IAPV 167. 19708 `_stock` 17 → 16. TEMP 1 neaktyvus.

#### S1620 — PRAGMA RECON (e1 P + e2d Q; tik skaitymas, kodas NELIESTAS)
- `plugins/petshop-pragma/petshop-pragma.php` v1.0.0 (22 662 B, md5 `36c374cded28a12199c4e68809c1f6d7`, 532 eil., aktyvus; repo kopijos nėra). Opcijos: `petshop_pragma_production_mode` (OFF), `petshop_pragma_known_emails` (1: raimundas@gyvunai.lt), `petshop_pragma_log` (12; nuo 06-05 kas mėn. „Automatinis eksportas PRALEISTAS — production mode isjungtas“; 05-26 rankinis „eksportas sekmingai. Uzsakymu: 1“). Cron: `wp_schedule_single_event('petshop_pragma_monthly_export')` 5 d. `mktime(8,…)` serverio laiku = **11:00 LT** (žurnalas 11:00–11:03), kitas 2026-10-05. `uploads/*pragma*` nėra (txt kuriami laikinai).
- **Logika:** `petshop_pragma_run_export($y,$m)` → `wc_get_orders(status=completed, date_created mėnesio ribose, limit -1)` → filtras `_petshop_invoice_document_type==='invoice'` && `_petshop_avpn_number` → `generate_imones` (kliento ID/vardas/adresas/tel/el. paštas/įmonė), `generate_pardavimai` (TSV: užs. nr · **'2' = PVM sąskaita** · '1' · data `date_paid` (kitaip sukūrimo) `Y.m.d` ×2 · AVPN · kliento ID · 6 tušti · suma su PVM · EUR · PVM · EUR · be PVM (`get_subtotal`) · EUR · mokestis „smulkaus/krepselio“ · 5 tušti), `generate_pardavimai_sum`; `send_email` → prod `info@audac.lt` / test `terra@petshop.lt` (`[TEST]`), priedai txt + naujų klientų PDF, kopija terra@ su `[PRAGMA] … OK/KLAIDA`. Admin langas „Pragma eksportas“: prod jungiklis, rankinis eksportas (metai/mėnuo), AVPN backfill visiems completed be numerio (vienkartinis), žurnalas.
- **RAIMIO SPRENDIMAS (09-06, GALIOJA): Pragma NELIEČIAMA — duomenų perdavimo forma suderinta su buhalterija, viskas lieka 1:1 kaip dabar; kreditinės (KR-AVPN) ir PPK kvitai — buhalterė rankomis (PDF iš Sąskaitų lango); atrankos logika nekeičiama. Tema BAIGTA, klausimų nebekelti.** Recon pastabos (tik žiniai, ne darbui): KR-AVPN kreditinių (WC refund / `_ps_kreditine` / `_wcdn_creditnote_pdf` niekur); PPK kvitų (grynų priėmimas — ne sąskaita; nereikalingi `pardavimai`, gal kasos žurnalui); atrankos data — `date_created`: užsakymas sukurtas VIII, įvykdytas IX → nepatenka nei į VIII (09-05 dar ne completed), nei į IX eksportą (Claude prielaida: filtruoti pagal `date_completed` / AVPN datą). Atsiėmimo užsakymai (completed be siuntos) — patenka kaip visi. Kiekio mažinimo (#4) / grąžinimo (A) `_ps_kiekis` refundai `get_total()` nekeičia → eksporto suma = AVPN suma (teisinga; kreditinė — atskira eilutė, jei bus).

#### S1620 — RAIMIO SPRENDIMAI 09-06 (II): 5 etapo likučiai → T1; paskyros angliški tekstai — dabar
- **„Pridėti prekę“ (§6c 4 p., §11.3) — T1 (po paleidimo).** Užrašyta spec §12.6: variantas „naujas mažas susietas užsakymas + apmokėjimo nuoroda“ (kaip „Pakartotinis“), sprendimai (reikia ar užtenka B; nuoroda ar „sumokės kurjeriui“; siųsti kartu ar atskirai) — T1. **Paskyros UI lokalizacija — T1**, išskyrus esamus angliškus tekstus — jie verčiami DABAR.
- **Recon e3r (tik skaitymas, klientas 5787, curl su prisijungimo slapukais):** `get_locale` lt_LT, WC 11.0.1 lt_LT įkeltas; meniu Skydelis · Mano augintinis · Užsakymai · Adresai · Paskyros duomenys; statusai visi LT (+ LP `wc-lp-*` LT). Puslapiai skydelis / užsakymai / adresai / paskyros duomenys / augintinis / view-order (#35776 completed su „Siuntos“ 1 iš 2 · 2 iš 2 Išsiųsta, #35421 processing, #35780 cancelled) / prisijungimas — **angliškų tekstų NĖRA**. **Rasta:** svečio užsakymo puslapis (`order-received`, po WC 10 min. malonės laiko — `woocommerce_order_email_verification_required`) rodo WC `templates/checkout/form-verify-email.php` sakinį **angliškai** („To view this page, you must either login or verify the email address associated with the order.“; lt_LT vertimo nėra), antras to šablono sakinys („We were unable to verify…“) — irgi be vertimo.
- **`mu-plugins/petshop-vertimai.php` v1.0 → v1.2** (2 650 B, md5 `8654dcc7c5445b477b19c7b4c23d1637`; kopijos `ps-backups/petshop-vertimai-v10-, v11-BACKUP-2026-09-06.php`; repo `deploy/petshop-vertimai.php`): žodyne naujas domenas `woocommerce` su 2 sakiniais — „Norėdami matyti šį puslapį, %1$sprisijunkite%2$s arba patvirtinkite el. pašto adresą, nurodytą užsakyme.“ / „Nepavyko patvirtinti nurodyto el. pašto adreso. Bandykite dar kartą.“ (filtras keičia tik neišverstus — jei WC vertimas atsiras, jo neužklos). Deploy e4d/e4d2 (D su md5 sargais + msgid sargu WC šablone + kopija + `token_get_all` + ping 200/0; Q `apimtis` 11 → 13, `__()` abu LT). **Vizuali patikra** — Playwright `screenshots/s1620_e5_svecio_uzsakymas_35813.png`: „Dėkojame. Jūsų užsakymas priimtas. Norėdami matyti šį puslapį, prisijunkite arba patvirtinkite el. pašto adresą, nurodytą užsakyme. · El. paštas * · PATVIRTINTI“ ✓.
- **Pastebėta (T1):** darbalaukio „Kaip mato klientas“ (`get_checkout_order_received_url`) svečio užsakymui po 10 min. rodo darbuotojui tą pačią patvirtinimo formą, ne užsakymą. Claude siūlymas T1: darbalaukyje filtras `woocommerce_order_email_verification_required` → `false` prisijungusiam darbuotojui (`ps_darbuotojas` / `manage_woocommerce`). Registruoto kliento užsakymui — paskyros `view-order`, veikia.

#### S1620 — 6 ETAPO RECON (e6r U/L/K + e7r R; tik skaitymas, NIEKAS NETRINTA) → `dokumentai/6_ETAPAS_VALYMO_PLANAS_v1.md`
- **Užsakymai:** `wc_orders` 72 = **64 užsakymai + 8 refund'ai**, VISI testiniai (08-26 #35087/88/90/100 terra@gyvunai.lt AVPN272–275; 09-02…09-06 #35414–#35813); HPOS on. WC našlaičiai iš senų #3300–#3304: `wc_order_stats` 4, `order_items` 10, `product_lookup` 6, `tax_lookup` 4. `wc_customer_lookup` 33 (testiniai).
- **Mūsų lentelės (visos testinės):** `ps_shipments` 25, `ps_fakt_siuntos` 25, `ps_fakt_uzsakymai` 67 (testinis=1 visi, 7 našl.), `ps_fakt_eilutes` 109 (legacy 51 · vf 22 · prins 10 · ambrosia 9 · zb 5 · quattro 4 · belcor 1), `ps_fakt_grazinimai` 11, `ps_fakt_atsargos_d` 31 765 (testinis=1), `ps_uzsakymu_ivykiai` 630 (324 našlaičiai), `ps_tiekimas` 6 + `_eil` 5, `ps_plan_events` 7, `ps_carts` 131, `ps_refill_tracking` 30, **`ps_email_jobs` 375 (18 pending post_purchase — po paleidimo išeitų testiniams adresams!)**, `ps_event_log` 866, `ps_action_tokens` 415, `ps_ataskaitu_dienos` 838 (visos `aplinka=dev`), `ps_kontrole_dienos` 9, `ps_sargas_klaidos` 3 193; PDF `wcdn/invoice` 91 (4,5 MB) · `creditnote` 4 · `receipt` 2; opcijos `ps_s1617_test`, dev-pastas 42. **Palikti:** `ps_ist_*`/`ps_v_analize_*` (istorija 9000xxxxx), `ps_dim_klientai`/`ps_kl_suvestine`/`ps_nl_snapshot`, `ps_sources`, `ps_feeding_*`, `ps_fakt_kainos` (tikrinti testinis), `ps_av_zurnalas` 557 (auditas), `ps-backups` 368 (59,5 MB).
- **Neaišku (klausimai A–I plane):** vartotojai #4949 `test@test.lt`, #4950 `testijusaite@gmail.com`, #24 `gutulis@gmail.com` (testiniai ar legacy?); `ps_partijos` 10 (08-10/12, savikainos) — testinės ar tikros; `ps_pets` 69 (38 Raimio #1); AV likučiai 16727/16889/19756 = 1 (`_own_stock` tuščias — saugoma ne postmeta; pradinės iš `ps_ivykiai`); 19708 `_stock` 16 → 20. Skaitikliai („kitas“): AVPN 342, IAPV 168, KR-AVPN 105, PPK 103.
- **Antras patikrinimas (e6r3, tikslinimai prie plano v1 §1–§3):** refund'ų tėvai: 35788←35414 (kiekis), 35789←35425 (kiekis), 35795←35434, 35798←35436, 35800←35777 (+35091/35092←35090, 35093←35088). **#35781–#35784 — PREKĖS** (09-04 „Konservų dėžė 400/800 g Įvairovė“ publish/draft, 2 TRIXIE draskyklės draft) — kito projekto, **NELIESTI**; #35785–#35787 revizijos; #35773 nėra. `ps_fakt_uzsakymai` našlaičiai: 35240, 35262–35266, 35288; `ps_fakt_grazinimai` ne darbalaukio: 35088, 35090 ×2. `ps_uzsakymu_ivykiai` 630 = 303 (testiniai #35414+) + 35090 ×4 + `0` ×3 + po 1 keistiems ID 35457…35766 (žingsnis 32 — našlaičiai). `ps_tiekimas_eil`: 16727←35421, 16889←35434/35439, 35357+33902←35450. `ps_refill_tracking` testiniai #42–#45 (user 1 ← 35442), #46–#48 (5787 ← 35780/35779/35440). `wc_customer_lookup` #85–#96 — testiniai svečiai (terra@petshop.lt, telefonu/atsiemimas *@dev.avesa.lt, #86 = 5787). `wc_order_stats` 76 = 65 testinių + 3300–3304 + 35087–35100 + 35091–35093. Vartotojai #5757–#5785 (08-31 15:35, tikri el. paštai) — legacy importas, NELIESTI. Testinės opcijos papildomai: `ps_audit_ids` (35414–35434), `ps_e3_oid`=35450, `ps_e3_oid2`=35771; `ps_test_product_id`=34889 — partijų projekto, palikti. `wcdn_invoice_number_counter` 364 — WCDN savas, palikti.
- **Likučiai — prie F:** 19708 Animonda `ps_av_zurnalas` paskutinė `_stock` operacija **43→48 (2026-08-07, gavimas)**, po to žurnale tik `_cost_price`/`_ean`; testų nurašymai žurnale nerašomi → „20“ (log S1619) yra reikšmė PRIEŠ S1619, ne prieš visus testus. Prieš apply — grandinė iš `ps_ivykiai` (`sena/nauja`, `_stock`) + užsakymų pastabų, Raimis patvirtina galutinę. 16727/16889/19756 — AV žurnale įrašų nėra; 35357 (VF) — `_stock` jau 6 (buvo 4 po S1619) — VF sync atstatė ✓.
- **Cron su užsakymais po valymo:** `ps_venipak_sekimas`, `ps_velavimo_laiskai`, `ps_fakt_siuntu_sutikrinimas`, `ps_dropship_sargas` — dirbs su tuščiu (norma).
- **Siūloma eiga (Claude):** įrankis su dry-run → Raimio peržiūra → apply (kopija, žingsnis per lentelę su sargu, užsakymai per `delete(true)` po vieną) → Q (visi 0, Playwright) → E2E → T-0 pakartoti + skaitikliai → 101.

#### S1620 — BŪKLĖ SESIJOS PABAIGOJE
- **Gyva dev'e:** darbalaukis **v3.36** (`dd0202f4…`), tema base.php **v2.12**, juosta v1.7, kliento-siuntos v1.3, **dev-pastas v1.1** (`ed7e80a8…`), **vertimai v1.2** (`8654dcc7…`); variklis ir Pragma nekeisti. Instancija #16 išjungta. Skaitikliai AVPN 341 · IAPV 167 · KR-AVPN 105 · PPK 103 (T-0 → 101). Eilės Visi 64. Testiniai +#35813 (atsiėmimas, completed, AVPN000341) — trinti 6 etape.
- **Dokumentai:** log v1.9.27 (šis), spec v1.15 (§12.5 patvirtinimai + Pragma; §12.6 T1 sąrašas), **`dokumentai/6_ETAPAS_VALYMO_PLANAS_v1.md`** (recon + klausimai A–I), STARTAS `dokumentai/STARTAS_2026-09-06_5_etapas_13_po_S1620.md`.
- **Darbuotojo paskyra — recon (e8r):** rolė `ps_darbuotojas` (yra; `Petshop_Teises`): read, upload_files, manage_woocommerce, view_woocommerce_reports, prekės (edit/publish/delete/terms), užsakymai (edit/read_private), read_private_shop_coupons, `ps_akcijas_taikyti`; NĖRA: plugins.php / users.php / wc-settings (403). Po prisijungimo → `admin.php?page=ps-katalogas` (`Petshop_Teises::po_prisijungimo`); juosta: Rytinė eiga · Gavimas · Rinkiniai · Akcijos · Sąskaitos (manage_woocommerce — darbuotojas mato) · Žurnalas · + Naujas užsakymas; `wc-orders` / `edit.php?post_type=product` pasiekiami URL (nuorodų nėra). Raimio klausimai — STARTAS.
- **Darbuotojos paskyra SUKURTA (e9c, Raimis 09-06: „prisijungimą pilną, užrakinti snippet ir pluginus; Inga, terra@petshop.lt“):** #5788 `inga`, terra@petshop.lt (el. paštas laisvas — tikrinta), rolė `ps_darbuotojas`, display „Inga“, locale lt_LT. Teisės ✓: manage_woocommerce, edit_shop_orders, edit_products, upload_files; ✗: activate_plugins, manage_options, edit/manage_snippets, list_users, edit_pages/posts, edit_theme_options; URL: ps-desk / naujas / saskaitos / katalogas 200, plugins.php / users.php / snippets / wc-settings / options-general 403; po prisijungimo → Katalogas. Slaptažodžio nustatymo laiškas (`retrieve_password`, `ps_dev_pastas_leisti=1` tik tai užklausai, po to opcija ištrinta) — `wp_mail` true, dev-pastas negaudė → išėjo į terra@petshop.lt (nuoroda 24 h, dev.avesa.lt hostas). `testuotojas` #75 — palikti iki E2E, tada trinti (planas B).
- **Raimio pastebėtos klaidos admin'e (09-06, ekrano kopija `eile=nauji`) → darbalaukis v3.35.1 → v3.35.2 (e10a/e10d):** (1) „+ Naujas užsakymas“ — matėsi tik žalias langelis be teksto: `.dl a{color:var(--melyna)}` (0,1,1) viršijo `.dl-e-naujas{color:#fff}` (0,1,0) → selektorius `.dl a.dl-e-naujas` (+ `:hover` be pabraukimo); (2) sąraše prekės pavadinimas trumpintas iki 42 ž. + „…“ — nesimatė svorio/pakuotės (Raimis: „ypač konservai, maistas“) → `$pav = $e0['n']` (pilnas; skydelis ir taip rodė pilną). Deploy: 497 517 B, md5 `cc27c448df44786340da7066ee01b7bc`, gzip+b64 2 dalimis (part1 md5 `5742882e…`), gyvo sargas `4e8c1366…` ✓, `token_get_all` ✓, `php -l` ✓, kopija `ps-backups/petshop-darbalaukis-v3351-BACKUP-2026-09-06.php`, ping 200/0, Q VERSIJA 3.35.2 ✓. **Playwright** `screenshots/s1620_e11_sarasas_neisrusiuoti_v3352.png`: mygtukas baltu tekstu ✓, „Animonda Carny … 400 g“, „QUATTRO 1,5 kg … su lašiša ir kriliu“ pilni ✓. Repo `deploy/petshop-darbalaukis.php` (+`.b64`) v3.35.2, `deploy/petshop-darbalaukis-v3351-ATSARGA.php`.
- **Raimio pastebėjimai (II, ekrano kopija `eile=visi`; sprendimai 09-06 „2 – ok, 3 – a ir b“) → darbalaukis v3.35.2 → v3.36 (`s1620_edit_v336.py`, e12a/e12d deploy, e13t testai, e14s kadrai):** **Filtrai „Visi“ lange:** Data „Nuo–iki“ (`<input type=date>` nuo/iki → `data=intervalas`, variklio `datos_riba('intervalas')` jau buvo), **Apmokėjimas** (pavedimu=bacs / Paysera / grynais=cod / neapmokėtas), **Būsena** (vykdomas / sustabdytas / įvykdytas / atšauktas / grąžintas / neapmokėtas-nepavykęs), **Suma € nuo–iki**, Rikiuoti **„seniausi pirmi“**, mygtukas „Rodyti“, „išvalyti“ valo viską; `url()` išsaugo naujus raktus. **Bet kuris filtras → variklio `gauti('visi')` (naujausi 200, pastaba „Rodomi 200 naujausių — siaurink datą“), ne puslapis po 50** (anksčiau vykdymas/vezejas filtravo tik puslapį) — chips `b` (kelyje/įvykdyti/atšaukti/išsiųsta šiandien) dabar taikomi ir variklio sąraše (`filtruoti()`). **Atšauktų trynimas:** sargas `istrinamas()` — TIK cancelled/lp-cancelled, ne `is_paid`/`date_paid`, be refund'ų, be `_petshop_completed_pdf`, be `_ps_siuntos`/`_ps_dalys_issiusta` (apmokėti atšaukti su AVPN+KR-AVPN — niekada); (a) skydelio footer „Ištrinti“ (GET `istrinti`, dialogas; po veiksmo peradresuojama be `atidaryti`) + filtro chip „Atšaukti“ nuoroda „Ištrinti neapmokėtus atšauktus (N)“ (POST `ps_dl_istrinti_atsauktus`, nonce, dialogas; N=0 — pilka); (b) cron `ps_dl_atsauktu_valymas` kasdien 03:20 Vilnius — atšauktas neapmokėtas, `date_modified` > `ATSAUKTU_DIENOS`=30 → šiukšlinė; šiukšlinėje neapmokėtas > 30 d. → `delete(true)`; ataskaita opcijoje `ps_dl_atsauktu_valymas_paskutinis`. Trynimas = WC šiukšlinė (`$o->delete(false)`, HPOS būsena `trash`), pastaba + `ps_uzsakymu_ivykiai` `istrinti`, `laiskai_off`. **Deploy:** 510 451 B, md5 `dd0202f4239125f422634554c4086370`, gyvo sargas `cc27c448…` ✓, `token_get_all` ✓, `php -l` ✓, JS `new Function` ✓, kopija `ps-backups/petshop-darbalaukis-v3352-BACKUP-2026-09-06.php`, ping 200/0, Q VERSIJA 3.36. **Testai (e13t, tikros nuorodos, `testuotojas`):** T1 nuo–iki 09-05 → 7 (35801–35807; DB 13 = +5 pakartotinio paslaugų užsakymai 35792/93/94/97/99, darbalaukyje nerodomi pagal v3.21 — teisinga, +35791 trash), forma `nuo` value ✓ / `intervalas` selected ✓ / išvalyti ✓; `mok=grynais` 9 = DB cod 9 ✓; `st=atsauktas` 13 ✓; suma 20–40 → 14 ✓; `r=seniausi` 54 rikiuota ✓ (35414 → 35813); chip įvykdyti + pavedimu 5 ✓; chip atšaukti 13 + bulk (1) ✓; be filtro 44 eil./puslapis, selectai 6, inputai nuo/iki/suma ✓. T2 sargas 14 atšauktų: skydelio nuoroda = `istrinamas()` visiems ✓ (tik #35791 trinamas — kiti „apmokėti“ Paysera simuliacija); #35791 → 302 „perkeltas į šiukšlinę“, status `trash`, pastaba, įvykis `istrinti` ✓, sąraše nebėra ✓; #35434 (apmokėtas, KR) → „trinti negalima“ ✓ (liko cancelled). T3 bulk (0) → „perkelta 0“ ✓ (N>0 kelias — ta pati `i_siuksline` kilpa). T4 cron kitas 2026-09-07 03:20 ✓, `atsauktu_valymas()` → 0/0/0 (visi jaunesni nei 30 d.). Eilės prieš/po: Visi 64 (35791 — pakartotinio, į skaičius nėjo). Dev-pastas 42. **Playwright** `screenshots/s1620_e14_visi_filtrai_v336.png` (nuo–iki + grynais), `s1620_e14_visi_atsaukti_v336.png` (chip Atšaukti + „Ištrinti neapmokėtus atšauktus (0)“) ✓. Repo `deploy/petshop-darbalaukis.php` (+`.b64`) v3.36, `deploy/petshop-darbalaukis-v3352-ATSARGA.php`. Pastaba 6 etapui: #35791 šiukšlinėje (status `trash`).
- **Sesijos pabaiga (Raimis: „dokumentuoti ir pereiti į naują langą“):** darbalaukis v3.36, tema base v2.12, juosta v1.7, kliento-siuntos v1.3, dev-pastas v1.1, vertimai v1.2, Inga #5788; eilės Visi 64 (+#35791 trash); TEMP 1 neaktyvus. Plano būklė — STARTAS „PRADĖTI NUO ČIA (S1621)“: 0–5 padaryta, liko 6 etapas (E2E + valymas A–H) ir Raimio loginės klaidos (naujame lange).
- **Kitas langas:** Raimio loginės / sisteminės klaidos → 6 ETAPAS — Raimio atsakymai A–H (valymo planas §3) → valymo įrankis dry-run → apply → Q → E2E → T-0 pakartojimas + skaitikliai → 101.

---

### S1619 (2026-09-06, naktis)

> Pridėti po šio antraštės (virš S1618). Tema: **5 etapas — PPK „PINIGŲ PRIĖMIMO KVITAS“ + „Apmokėta grynais“ + telefoninio pavedimo likutis (v3.33.2 → v3.34, tema base.php v2.11 → v2.12) ir C „ATSIĖMIMAS AV“ — VISAS, bet IŠJUNGTAS (v3.34 → v3.35 → v3.35.1).** Raimio sprendimai — log S1618 „RAIMIO ATSAKYMAI (09-06, PPK + B + C)“ + STARTAS 09-06 „gerai“. Variklis neliestas (visi keliai `eilutes_kelias` / `rusiuoti` / `auto_rusiuoti` — darbalaukyje). Bridge: `irankiai/s1619_r1.php` (recon PPK+C), `s1619_r2.php` (variklis: vezejas/klausimas/kabliai), `s1619_r3.php` (§18.3 sargas, WC laiškai), `s1619_edit_v334.py` + `s1619_v334_funcs.php`, `s1619_base_receipt_block.php`, `s1619_e1a/e1d.php` (v3.34 + base v2.12 deploy, T1–T3), `s1619_e2d.php` (tikros nuorodos — kvitas, apmoketa), `s1619_edit_v335.py` + `s1619_v335_funcs.php`, `s1619_e3a/e3d.php` (v3.35 deploy + local_pickup instancija), `s1619_e4d.php` (C testai), `s1619_edit_v3351.py`, `s1619_e5a/e5d.php` (v3.35.1 + T3), `s1619_e6s.php` (Playwright). `analize/s1619_*.json`, `screenshots/s1619_e6_skydelis_35811_atsiemimas.png, s1619_e6_saskaitos_sablonai.png, s1619_e6_naujas_apmokejimas.png` (35809 kadras — Playwright timeout, nepadarytas).

#### S1619 — RECON (r1–r3, tik skaitymas)
- **WC 11.0.1 `wc_maybe_reduce_stock_levels`:** filtras `woocommerce_payment_complete_reduce_order_stock` → `false` baigia PRIEŠ `set_stock_reduced(true)`; `woocommerce_can_reduce_order_stock` veikia tik `wc_reduce_stock_levels` viduje, o žymė vis tiek įrašoma → likutis nebūtų nurašytas ir vėliau. Todėl B (6) — abu filtrai (`telefonu_likutis`, `telefonu_likutis_wc`). Varikliai (av-reduce, partijos, faktai) kabinasi tik `payment_complete` / `processing` / `completed` — on-hold neliečia.
- **Variklis:** `Petshop_Desk::vezejas()` — VENIPAK / LP pagal pavadinimą, kitaip `'kita'` (atsiėmimui). `klausimas()` — LP taisyklė tik LP; `Trūksta sandėlyje` galioja. Auto rūšiavimas (`auto_rusiuoti`, kablys `woocommerce_payment_complete` 100), keliai (`_ps_kelias`), `rusiuoti` — darbalaukyje. `Petshop_Siuntos::uzbaigimo_sargas` (§18.3) — tik kai `_ps_shipments > 1`, apėjimas `_ps_uzbaigti_be_siuntu=1`. WC laiškai: completed/processing/new_order ON, on-hold OFF; WC completed laiško tema (tema) — „Jūsų užsakymas Nr.N išsiųstas!“.
- **Zona „Lietuva“ (id 1):** Venipak kurjeris 2/6/7/8, paštomatas 3, LP 12/15, free_shipping 1 (off); `local_pickup` NEBUVO. Svetainė: **darbo laikas I–V 09:00–18:00, VI 10:00–15:00** (Kontaktai #34521, Apie mus #34515); Venipak siuntėjas — UAB Avesa, Liucionių g. 46, Liucionys, 15166. `uploads/wcdn/receipt/` katalogas jau buvo (WCDN). Testuotojas: display_name „Testuotojas“.

#### S1619 — v3.34 PPK + „Apmokėta grynais“ + B (6) — GYVA; tema base.php v2.12
- **v3.34** (475 224 B, md5 `94d92bcc20882428622e5b1792782566`; kopija `ps-backups/petshop-darbalaukis-v3332-BACKUP-2026-09-06.php`; repo `deploy/petshop-darbalaukis-v3332-ATSARGA.php`, `-v334-ATSARGA.php`). **Tema `base.php` v2.12** (31 916 B, md5 `4b865eef60ee9405cb58de4f51864d88`; kopija `ps-backups/theme-wcdn-base-v211-BACKUP-2026-09-06.php`; repo `deploy/wcdn-base.php`) — vienas naujas blokas prieš `<div class="wcdn-document">`: `$template==='receipt'` → kvito HTML ir `return` (sąskaitų / išankstinių / kreditinės šakos NELIESTOS; `s1619_base_receipt_block.php`).
- **„Apmokėta vietoje“ → „Apmokėta grynais“** (`mok=grynais`, `cod` + pavadinimas „Apmokėta grynais“; formos tekstas, dialogas, pastabos, peradresavimas — „kvitą (PPK) suformuok skydelio „Sąskaitos“ bloke, kai klientas sumoka“).
- **PPK:** `grynais($o)` = `cod` + `is_paid`. `dokumentai()` grynais apmokėtam — įrašas `ppk` (nr · data · suma · PDF · „priėmė X“) arba mygtukas **„Suformuoti kvitą“** (dialogas: suma, „pinigus priėmė“ — darbuotojas; JS `skDok` — `x.btn/x.bt/x.dlg`). GET `kvitas` → `kvitas_vykdyti`: `kvitas_numeris` (`petshop_ppk_counter`, KITAS, nuo 101 → `PPK000101`; meta `_petshop_ppk_number/_date/_suma/_kas`), `kvitas_pdf` (kaip `kreditine_pdf`, `$template='receipt'`, `$order['receipt']` = nr/data/suma/žodžiais/paskirtis/priėmė/mokėtojas/kontaktas → `uploads/wcdn/receipt/Kvitas-PPK000101.pdf`, meta `_petshop_ppk_pdf`), pastaba, įvykis `kvitas`; antras spaudimas — „jau suformuotas“. **`suma_zodziais()`** LT (euras/eurai/eurų, tūkstantis/-čiai/-čių, ct skaičiais; lokaliai patikrinta 13 reikšmių: 24,47 → „Dvidešimt keturi eurai 47 ct“, 1200 → „Vienas tūkstantis du šimtai eurų 00 ct“, 111 → „Šimtas vienuolika eurų 00 ct“). `dok_faila` + `ppk`. **Sąskaitų lange** tipas „Kvitas“ (`t=ppk`; į sumą NESKAIČIUOJAMAS; data — v3.34 iš `date_paid`, v3.35 iš `_petshop_ppk_date`).
- **B (6):** `telefonu_likutis` — `_ps_telefonu` + on-hold + neapmokėtas → WC likutis nenurašomas; nurašoma „Pažymėti apmokėtu“ (processing). Kasos bacs — nekeista.
- **Testai (e1d/e2d; e1d GET nuorodos 403 — `wp_nonce_url` grąžina `&amp;`, e2d `html_entity_decode` — pamoka):** #35807 (cod „Apmokėta vietoje“, processing) → skydelio dok `[AVPN000335, ppk „Suformuoti kvitą“]` → tikra nuoroda → 302 „kvitas **PPK000101** (7,78 €) suformuotas“, meta ✓, PDF 32 406 B, dok po `[…, PPK000101 · PDF · priėmė Testuotojas]`; pakartotinai → „jau suformuotas“; `saskaitos&t=ppk` 1 dok. suma 0,00; `dok_pdf&t=ppk` inline ✓. **#35809** (naujas grynais → cod „Apmokėta grynais“, processing, likutis 6→5) → **PPK000102** ✓. **#35808** (naujas pavedimu → on-hold, `stock_reduced=false`, `_reduced_stock` tuščia, likutis 6 nepakito ✓ → engine „Pažymėti apmokėtu“ (`be_laisko=1`) → processing, „Likutis sumažintas 5→4“, „Payment complete.“, varikliai ✓). Forma: `value="grynais"` ✓, `vietoje` nėra ✓. PDF PPK000102 peržiūrėtas (PNG): logotipas, „Pinigų priėmimo kvitas – PPK000102 · Data“, „Pinigus gavo: UAB Avesa…“ / „Mokėtojas: UAB Testas, TELEFONU Testas S1619 · tel.“, lentelė Paskirtis („Už prekes pagal užsakymą Nr. 35809, PVM sąskaita faktūra Nr. AVPN000337“) · Suma 15,08 € · Suma žodžiais („Penkiolika eurų 08 ct“) · Grynaisiais, parašų linijos „Pinigus priėmė: Testuotojas“ / „Pinigus įmokėjo“, pastaba „PVM sąskaita faktūra išrašoma atskirai“, poraštė. Eilės po deploy = prieš (Gauti 8 · Neišrūšiuoti 2 · Laukiam 1 · Surinkti 7 · Dropshipping 10 · Paruošta 0 · Klausimai 9 · Neapmokėti 3 · Visi 58); po testų Gauti 10 · Dropshipping 12 · Neapmokėti 3 · Visi 60 (+#35808, #35809). Dev-pastas 26 → 28, 0 išėjo. Skaitiklis PPK → 103 (T-0 → 101).

#### S1619 — v3.35 → v3.35.1 C „ATSIĖMIMAS AV“ (visas, IŠJUNGTAS) — GYVA
- **v3.35** (494 361 B, md5 `51172cea296c748ca96ff4f18a150917`; kopija `-v334-BACKUP-2026-09-06.php`) → **v3.35.1** (497 247 B, md5 `4e8c13665454cb6d7413ba8892bf4b6a`; kopija `-v335-BACKUP-2026-09-06.php`; repo `deploy/petshop-darbalaukis.php` + `.b64`).
- **Jungiklis (Claude prielaida c):** WC zonos „Lietuva“ **`local_pickup` instancija #16** „Atsiėmimas AV — Liucionių g. 46, Liucionys“ (cost 0, tax none) — sukurta e3d **IŠJUNGTA** (`is_enabled=0`). Raimis įjungia WC → Pristatymas → atsiranda kasoje IR „+ Naujas užsakymas“ formoje (`naujas_pristatymas` ima iš zonos, tipas `av`). Opcija `ps_dl_atsiemimas_av` nebenaudojama (vienas jungiklis).
- **Atsiėmimo užsakymas** (`atsiemimas()` — pristatymo eilutė `local_pickup` arba „Atsiėmimas AV“): `eilutes_kelias` tiesiai → **i_av** (tiekėjo prekės tik per AV), `galimi.tiesiai=false`; `faktai()['ats']`, žymės `_ps_atsiemimas` {paruosta, laiskas, atsieme}; eilutės žingsniai Surinkti → Paruošta atsiimti → Klientas atsiėmė (i_av: + Užsakyti… į AV → Gauta į AV); takelis `ats_par` / `atsieme` vietoj lipduko / issiusta; eilės **Surinkti** (rus, vietoje, ne lapas) → **Paruošta** (lapas, ne baigta). Skydelis: vezejas „Atsiėmimas AV“, pastaba „ATSIĖMIMAS AV (lipduko nėra): surinkti → …“, „Redaguoti“ nėra; filtras „Pristatymas“ + „Atsiėmimas AV / kita“.
- **Veiksmai:** GET `ats_paruosta` (+`be_laisko`) → `atsiemimas_paruosta` (apmokėtas + surinktas; laiškas klientui per `WC()->mailer()` pagal šabloną; be el. pašto — „paskambink“); GET `atsieme` → `atsiemimas_atsieme` (`_ps_dalys_issiusta.av` kanalas `atsiemimas`, `_ps_uzbaigti_be_siuntu=1`, **completed su `laiskai_off`** (v3.35.1 — WC completed laiško tema „išsiųstas!“ netinka), tema AVPN PDF, po to **savas laiškas su PVM sąskaitos PDF** `ats_ivykdyta_laiskas` — tekstas Claude siūlymas); GET `grynais` → `apmoketa_grynais` (skydelio footer'yje neapmokėtam atsiėmimo užsakymui: cod „Apmokėta grynais“ + `date_paid` + processing su `laiskai_off`; kvitas PPK — atskirai).
- **Laiško šablonas** `ps_dl_ats_laiskas` (Sąskaitų lange po kreditinės šablono, `ats_sablono_forma`, POST `ps_dl_ats_sablonas`): tema / tekstas (Raimio patvirtintas 09-06, 7 d.) / `{neapmoketa}` pastraipa / **adresas** „Liucionių g. 46, Liucionys (Nemenčinės sen., Vilniaus r.)“ / **darbo laikas** „I–V 09:00–18:00, VI 10:00–15:00“ / `iv_tema` „Užsakymas Nr. {nr} atsiimtas — PVM sąskaita faktūra Nr. {avpn}“ / `iv_tekstas` „Sveiki, {vardas}. Ačiū, kad atsiėmėte užsakymą Nr. {nr}. Pridedame PVM sąskaitą faktūrą Nr. {avpn} ({suma} €). Gražios dienos, Petshop.lt komanda …“.
- **Testai (e3d D/Q, e4d, e5d; instancija testams įjungta ir vėl išjungta — C fazė: formoje „Atsiėmimas AV“ nėra ✓):** forma su instancija ON — 5 būdai (+ „Atsiėmimas AV“) ✓; **#35810** (grynais, AV 19708 + VF 35357) → pristatymo eilutė `local_pickup` #16, processing, AVPN000338, skydelis vezejas „Atsiėmimas AV“, VF eilutė **i_av** („VF veža į AV“), Neišrūšiuoti (2 šaltiniai — taisyklė) ✓; **#35811** (pavedimu, 19708) → on-hold, IAPV000167, skydelyje `grynais` nuoroda ✓ → „Apmokėta grynais“ → processing cod, auto „Surinkti“ ✓ → lapai (engine, tikra nuoroda) → `_ps_surinkta` ✓ → btn „Paruošta atsiimti“ (dialogas su adresu ir valandomis) → 302 „klientui pranešta“, dev-pastas „Jūsų užsakymas Nr. 35811 paruoštas atsiimti — Petshop.lt“ ✓ → btn „Klientas atsiėmė“ → 302, **completed, AVPN000339**, `_ps_dalys_issiusta.av` ✓, `_ps_uzbaigti_be_siuntu` ✓; **bet WC laiškas „Jūsų užsakymas Nr.35811 išsiųstas!“** išėjo (dev-pastas) → v3.35.1. **#35812** (v3.35.1: grynais → lapai → paruošta `be_laisko` → atsiėmė) → completed AVPN000340, WC „išsiųstas“ NĖRA, savas „Užsakymas Nr. 35812 atsiimtas — PVM sąskaita faktūra Nr. AVPN000340“ su 1 priedu ✓. Eilės po kiekvieno deploy = prieš; po testų Gauti 11 · Neišrūšiuoti 3 · Laukiam 1 · Surinkti 7 · Dropshipping 12 · Paruošta 0 · Klausimai 9 · Neapmokėti 3 · **Visi 63** (+3 atsiėmimo testiniai). Dev-pastas 37, 0 išėjo. **Playwright (e6s):** `s1619_e6_skydelis_35811_atsiemimas.png` — sąraše „Atsiėmimas AV“, #35810 „VF veža į AV“, skydelis ✓ Surinkti ✓ Paruošta atsiimti ✓ Klientas atsiėmė, Sąskaitos su „Suformuoti kvitą“, istorija; `s1619_e6_saskaitos_sablonai.png` — abu šablonai; `s1619_e6_naujas_apmokejimas.png` — „Apmokėta grynais“.
- **Claude prielaidos (Raimiui tvirtinti):** (a) **neapmokėtas atsiėmimo užsakymas į darbą neina** (kaip visi — variklio `_ps_source` neapmokėtam nėra; `rusiuoti`/`keisti_kelia` reikalauja apmokėto): klientas atvyksta ir moka → „Apmokėta grynais“ → rūšiavimas/surinkimas prie kliento → „Klientas atsiėmė“; laiško `{neapmoketa}` pastraipa dėl to praktiškai nenaudojama (paliktas šablone). Jei Raimis nori ruošti neapmokėtus — atskiras sprendimas. (b) Po „Klientas atsiėmė“ — savas laiškas su PVM sąskaita (WC „įvykdytas“ off). (c) Jungiklis — WC instancija (vietoj opcijos).
- **Pastebėta / nepatikrinta:** #35811 laiškas „paruošta atsiimti“ dev-pastas'e turėjo **1 priedą** (nelauktą; temoje `wp_mail` filtro nėra, tik `woocommerce_email_attachments` — completed/processing); dev-pastas laiko tik skaičių — patikrinti kitame lange (kas prisegta). `Petshop_AV_Source::resolve()` paieška e3d AV prekės nerado (parašas / struktūra kita) — AV testams naudota 19708 (legacy, `_stock`). Pastabose „Likutis sumažintas … (20→20)“ + „AV nurašymas 20→19“ — variklio elgsena, kaip kitiems.

#### S1619 — BŪKLĖ SESIJOS PABAIGOJE
- **Gyva dev'e:** `petshop-darbalaukis.php` **v3.35.1** (md5 `4e8c13665454cb6d7413ba8892bf4b6a`), tema `base.php` **v2.12** (`4b865eef…`), juosta v1.7, kliento-siuntos v1.3, dev-pastas v1.0; variklis nekeistas. WC zona „Lietuva“ `local_pickup` #16 — **išjungta**. Skaitikliai: AVPN 340, IAPV 167, KR-AVPN 105, **PPK 103** (T-0 → visi 101). Eilės: Gauti 11 · Neišrūšiuoti 3 · Laukiam 1 · Surinkti 7 · Dropshipping 12 · Paruošta 0 · Klausimai 9 · Neapmokėti 3 · Visi 63. Dev-pastas 37. TEMP 1 neaktyvus.
- **Testiniai papildyti (6 etapui):** **#35808** (telefonu pavedimu → apmokėtas, VF), **#35809** (grynais, PPK000102), **#35807** PPK000101, **#35810** (atsiėmimas, Neišrūšiuoti), **#35811**, **#35812** (atsiėmimas, completed, AVPN000339/340); `uploads/wcdn/receipt/Kvitas-PPK000101–102.pdf`; likutis 35357 `_stock` 4 (buvo 6), 19708 `_stock` 17 (buvo 20).
- **Repo:** `deploy/petshop-darbalaukis.php` (+`.b64`) v3.35.1, `deploy/petshop-darbalaukis-v3332-ATSARGA.php`, `-v334-ATSARGA.php`, `deploy/wcdn-base.php` v2.12; `irankiai/s1619_*`; `analize/s1619_*.json`; `screenshots/s1619_e6_*.png`.
- **Dokumentai:** log v1.9.26 (šis), spec v1.14, STARTAS `dokumentai/STARTAS_2026-09-06_5_etapas_12_po_S1619.md`.
- **Kitas langas:** Raimio patvirtinimai (a–c + laiško „atsiimtas“ tekstas + PPK kvito išvaizda) → nelaukto priedo patikra → Pragma (recon + klausti) → 6 etapas (valymas, faktų `legacy` → `av` su leidimu, skaitikliai → 101, tikra darbuotojo paskyra).

---

### S1618 (2026-09-05, vėlus vakaras)

> Pridėti po šio antraštės (virš S1617). Tema: **5 etapas A „GRĄŽINIMAS“ (v3.31 → v3.32) ir B „+ NAUJAS UŽSAKYMAS“ telefonu (v3.32 → v3.33 → v3.33.1 → v3.33.2) — Raimio sprendimai log S1617 „RAIMIO ATSAKYMAI“ A ir B.** Variklis neliestas; `kreditine_vykdyti` neliesta (nauja `grazinimas_vykdyti` naudoja tas pačias pagalbines `kreditine_numeris` / `kreditine_pdf` / `kreditine_kortele`). Bridge: `irankiai/s1618_edit_v332.py` (tikslūs `replace` su sargais ant pilno v3.31 + `php -l` STABDIKLIS `subprocess`+`sys.exit`) + `s1618_v332_funcs.php`; `s1618_e1a.php` (gz+b64 dalis 1 → `ps-backups/dl-v332.part1`), `s1618_e1d.php` (D deploy su sargais + ping + eilės prieš · Q eilės po · T1 · T2), `s1618_e2s.php` (S Playwright kadrai · C valymas). `analize/s1618_e1a/e1d/e2s/e2c.json`, `screenshots/s1618_e2_kortele_35450_forma.png, s1618_e2_kortele_35450_atsisakymas_dalinis.png, s1618_e2_skydelis_35777_forma.png, s1618_e2_klausimai_35777.png`.

#### S1618 — RAIMIO ATSAKYMAI (09-06 rytas) — į kitą langą (C + PPK)
- **A „Grąžinimas“ — visos prielaidos a–d GERAI** (brokas/klaida pristatymas pagal varnelę ir daliniam; „tinkama“ galima ir brokui; dalinai grąžinta eilutė pilname atšaukime visu kiekiu; tiekėjo prekė be nurašymo — AV likutis nekeičiamas, rankomis).
- **B (5) „Apmokėta vietoje“ — NAUJAS REIKALAVIMAS:** jei klientas apmoka vietoje ir išrašoma sąskaita, **automatiškai turi būti išrašomas PINIGŲ PRIĖMIMO KVITAS (PPK)**. Kodas dar ne. Claude klausimai prieš kodą (atsakyti kitame lange): (1) PPK tik grynaisiais, ar ir kortele (kortelei paprastai užtenka terminalo čekio)? (2) serija ir numeracija — pvz. „PPK“ + sava eilė nuo 101 (kaip AVPN/IAPV/KR), skaitiklis `petshop_ppk_counter`? (3) kada išrašomas — iš karto sukūrus „Apmokėta vietoje“ užsakymą (mokėjimo data = užsakymo), ar įvykdžius kartu su AVPN? (4) šablonas — temos `woocommerce-delivery-notes/base.php` (`$template='receipt'`, reikia leidimo liesti temą) ar atskiras darbalaukio PDF (Dompdf, kaip kreditinė per temos šabloną)? (5) privalomi rekvizitai: UAB Avesa duomenys, kvito nr./data, mokėtojas (vardas, įmonė), suma skaičiais **ir žodžiais**, paskirtis („už prekes pagal užsakymą #N / PVM sąskaitą AVPN…“), pinigus priėmė (darbuotojo vardas, parašas) — ar reikia dar ko (kasininko pareigos, antspaudas)? (6) kur matomas — skydelio „Dokumentai“ ir Sąskaitų lange kaip naujas tipas „Kvitas“, PDF čia; siųsti klientui el. paštu kartu su sąskaita ar tik spausdinti? (7) B (6)–(8) ir C (9)–(10) klausimai (log S1618 „Claude prielaidos“, STARTAS C) — dar be atsakymo.

#### S1618 — RAIMIO ATSAKYMAI (09-06, PPK + B + C) — GALIOJA, kodas kitame lange
- **PPK:** (1) tik **grynaisiais** („arba pavedimu“ — mokėjimo būdai vietoje: grynais arba pavedimu; **kortelių nėra — terminalo nėra**; Claude supratimas: formoje „Apmokėta vietoje“ → „Apmokėta grynais“ (PPK), kortelės variantas išimamas — patvirtinti); (2) serija PPK, sava eilė **nuo 101** (`petshop_ppk_counter`, T-0 → 101); (3) **NE automatiškai** — darbuotojas spaudžia „Suformuoti kvitą“ tuo metu, kai klientas moka (mygtukas skydelio „Dokumentai“ bloke grynais apmokėtam užsakymui); (4) šablonas **temoje `woocommerce-delivery-notes/base.php` — LEISTA** (`$template='receipt'`, kaip kreditinei); (5) rekvizitai gerai (UAB Avesa, nr./data, mokėtojas, suma skaičiais ir žodžiais, paskirtis su užsakymo nr. ir AVPN, „pinigus priėmė“ — darbuotojo vardas), **parašas ranka ant kvito, antspaudo nereikia**; (6) **tik spausdinti** — el. paštu nesiunčiamas; matomas skydelio „Dokumentai“ ir Sąskaitų lange (tipas „Kvitas“).
- **B (6):** pavedimu — likutis **nurašomas tik apmokėjus** (ne on-hold metu): telefoniniams (`_ps_telefonu`) on-hold užsakymams blokuoti WC `woocommerce_can_reduce_order_stock`, nurašoma pažymėjus apmokėtu (processing). Kasos bacs užsakymų elgsena nekeičiama (Claude prielaida — klausta tik apie telefoninius). **(7):** laiške **be Paysera / „apmokėti internetu“ nuorodos — tik bankinis pavedimas** (kaip dabar, nekeisti). **(8):** LP Express — el. paštas privalomas — TAIP.
- **C (9):** atsiėmimo adresas ir darbo laikas — **„Petshop darbo laikas yra parašytas“** (imti iš svetainės — kontaktų puslapis / poraštė; recon, adresas — tikėtina Liucionių g. 46, Liucionys, kaip Venipak siuntėjo nustatymuose — patvirtinti). **(10):** laiško „paruošta atsiimti“ tekstą siūlo Claude (pateiktas 09-06, žr. STARTAS), Raimis tvirtina.
- **Raimis 09-06 „gerai“:** patvirtinta — „Apmokėta vietoje“ → **„Apmokėta grynais“** (kortelės varianto nėra); atsiėmimo adresas **Liucionių g. 46, Liucionys**, darbo laikas iš svetainės; laiško „paruošta atsiimti“ tekstas (STARTAS) **PATVIRTINTAS** su 7 d. saugojimu — daryti kaip taisomą šabloną (kaip kreditinės laiško).
- **Eilė kitam langui:** PPK (B papildymas: „Apmokėta grynais“ + kvitas + likutis tik apmokėjus) → C „Atsiėmimas AV“ (visas, išjungtas).

#### S1618 — RECON B (r1–r4, tik skaitymas)
- **WC pristatymo zonos:** „Lietuva“ (tik šalis LT): Venipak kurjeris inst 2 (fee 3,30 be PVM, ≤50 kg), 6 (6,60; 50–70), 7 (9,90; 70–100), 8 (20,65; 100–200); Venipak paštomatas inst 3 (1,78; ≤24,9 kg; **nemokamas nuo 30 €**); LP Express paštomatas inst 12 (`woo_lithuaniapost_lpexpress_terminal`, plan TERMINAL, fixed 1,78, free ≥30); LP kurjeris inst 15 (HANDS, kaina pagal svorį — nustatymuose tuščia). „Neringos savivaldybė“ (LT + pašto kodai 931*): inst 9/10/13 su kitomis kainomis (kurjeris 24,79) — **v3.33 klaidingai ėmė šią zoną** (pirma su LT), pataisyta v3.33.1. Pristatymo PVM 21 % (`WC_Tax::find_shipping_rates`).
- **Venipak paštomatai:** plugino `wc-venipak-shipping/venipak-fetch-pickups.php` — `venipak_fetch_pickups('LT')` iš `uploads/venipak/pickups.json` (1,77 MB, 567 LT), `venipak_find_pickup_by_id`, **`venipak_store_order_pickup($order,$id)`** (rašo `venipak_pickup_point` + `venipak_pickup_point_data` snapshot). LP terminalai — `gaj6_woo_lithuaniapost_unisend_terminals` (531 LT; `lpexpress_terminals` tuščia); užsakymo meta `_woo_lithuaniapost_lpexpress_terminal_id` + `_woo_lithuaniapost_lpexpress_terminal` („name - address, city“) + `_woo_lithuaniapost_lpexpress_shipping_method_instance_id`. Variklio `Petshop_Desk::vezejas()` — pagal shipping eilutės pavadinimą (VENIPAK / PAŠTOMAT / LP).
- **Mokėjimai:** paysera on, bacs on („Bankinis pavedimas“), cod/cheque off (užsakymui `payment_method=cod` vis tiek įrašomas). **Tema:** `petshop_get_invoice_document_type` — bacs + pending/on-hold → proforma (IAPV), kitaip AVPN; **AVPN priskiriamas jau processing/on-hold metu** (per admin „Naujas užsakymas“ laiško priedą `woocommerce_email_attachments`) — ir kasos užsakymams (#35436 processing → AVPN000305, #35442 → AVPN000311 + IAPV000162), tai NE darbalaukio regresija; `woocommerce_order_status_completed` prior. 5 → `_petshop_completed_pdf`. WC `customer_on_hold_order` laiškas **išjungtas**; „Užsakymas gautas“ laišką (prekės, rekvizitai bacs, IAPV PDF) siunčia temos `petshop_send_order_received_email` (planuojamas `wp_schedule_single_event` po kasos `checkout_order_processed`).
- **Klientai:** HPOS `wc_order_addresses` (billing: first/last, company, address, city, postcode, email, phone; 51 eil.) + `users` 5 703 / `billing_phone` 5 595. **Prekės:** `wc_product_meta_lookup` (sku, min_price, stock), 2 672 prekės + 241 variacijos; `wc_get_price_including_tax`; `Petshop_AV_Stock::qty`.

#### S1618 — v3.33 → v3.33.2 „+ NAUJAS UŽSAKYMAS“ (Raimis B) — GYVA
- **v3.33.2** (461 149 B, md5 `29c9bede2199f95ad38b0591878a2f8a`; kopijos `ps-backups/petshop-darbalaukis-v332-, v333-, v3331-BACKUP-2026-09-05.php`; repo `deploy/petshop-darbalaukis.php` + `.b64`, `deploy/petshop-darbalaukis-v332-ATSARGA.php`). Deploy trimis žingsniais (e3d v3.33 · e4d v3.33.1 · e6d v3.33.2), visi su sargais + ping 200; **e4d pirmas bandymas STOP `b64/gz`** — payload'e `re.sub` įdėjo netikslų b64, sargas sustabdė, nieko neįrašyta (pamoka: dalis dėti `str` pjūviu ir tikrinti `c==p1/p2` prieš siuntimą; taip padaryta e6a/e6d).
- **Kur:** nuoroda **„+ Naujas užsakymas“** eilių juostoje dešinėje (`dl-e-naujas`, matoma visose eilėse) → langas `view=naujas` (`naujas_langas()` / `naujas()` / `naujas_skriptas()` — atskiras `<script id="dl-nu-js">` po `skriptas()`; dl-js yra IIFE, todėl `window.dlgForm` / `window.dlEsc` eksportuoti — v3.33.2, T e5s „esc is not defined“).
- **Forma (4 kortelės):** 1) Klientas — paieška (AJAX `ps_dl_klientai`: `wc_order_addresses` billing pagal el. paštą / telefoną (skaitmenimis) / vardą, dedupe email|phone, naujausi; + vartotojai `WC_Customer`; iki 8) → užpildo laukus (vardas·, pavardė, telefonas·, el. paštas (gali nebūti; **LP — privalomas**), įmonė, adresas/miestas/pašto kodas (kurjeriui privalomi)); `kl[uid]` — rasto vartotojo ID (`customer_id`), kitaip svečias. 2) Prekės — paieška (AJAX `ps_dl_prekes`: pavadinimas / SKU, publikuotos prekės ir variacijos, rikiuojant instock pirma; grąžina kainą su PVM su akcija, reguliarią, WC ir AV likutį, svorį) → lentelė: kiekis ir kaina taisomi (perbraukta reguliari, jei akcija), „išimti“; **Nuolaida €** + pastaba (privaloma, jei > 0). 3) Pristatymas — select iš `naujas_pristatymas()` (zona „Lietuva“; Venipak kurjeris — instancija pagal svorį; paštomatas; LP paštomatas; LP kurjeris; „Atsiėmimas AV“ tik su opcija `ps_dl_atsiemimas_av` — C jungiklis, dabar nėra) su **numatyta kaina** (fee × 1,21; nemokama nuo ribos; tekstas „pagal parduotuvę: 2,15 €, nemokamas nuo 30,00 €“) — darbuotojas gali pataisyti; paštomatų sąrašas per esamą `ps_dl_vietos` (`pastomatas` / `lp`) su paieška pagal miestą; pastaba užsakymui. 4) Apmokėjimas — „Pavedimu“ (numatyta) / „Apmokėta vietoje“; sumos (prekės · nuolaida · pristatymas · iš viso); „Sukurti užsakymą“ → JS patikra (prekės, vardas, tel., kurjeriui adresas, paštomatas, LP el. paštas, nuolaidos pastaba) → dialogas → POST.
- **POST `ps_dl_naujas`** (nonce `ps_dl_naujas`, `edit_shop_orders`) → `naujas_vykdyti`: patikros kaip JS + LP terminalas sąraše, Venipak paštomatas `venipak_find_pickup_by_id`; `wc_create_order(customer_id, created_via=darbalaukis, pending)`, billing/shipping adresas LT, `prices_include_tax`, pastaba; eilutės `WC_Order_Item_Product` — subtotal = įrašyta kaina × q (be PVM per `wc_get_price_excluding_tax`), total = minus proporcinga nuolaidos dalis (paskutinei — likutis; WC rodo „nuolaidą“); jei kaina ≠ parduotuvės — eilutės meta `_ps_kaina_pakeista` „buvo → yra“; `WC_Order_Item_Shipping` (method_id, instance_id, plugino pavadinimas — variklio `vezejas()` atpažįsta; total = kaina/(1+PVM)); Venipak → `venipak_store_order_pickup`; LP → 3 plugino meta; meta `_ps_telefonu` (laikas|kas), `_ps_nuolaida`, `_ps_nuolaida_pastaba`; `calculate_totals(true)`; pastaba; **vietoje** → `payment_method=cod` „Apmokėta vietoje“, `set_date_paid`, `update_status('processing', …, true)` (kaip Paysera — varikliai suveikė: `_ps_source`, faktai, partijos, `_ps_groups`, auto rūšiavimas); **pavedimu** → `bacs` „Bankinis pavedimas“, `update_status('on-hold')` (WC nurašo likutį kaip kasoje), su el. paštu — `do_action('petshop_send_order_received_email', $id)` → temos laiškas „Jūsų užsakymas Nr. N gautas“ su rekvizitais ir **IAPV PDF** (v3.33.1). Įvykis `naujas`; peradresavimas į Gauti (vietoje) / Neapmokėti (pavedimu) su `atidaryti=id` ir pranešimu (pavedimui — rekvizitai ir paskirtis, kad darbuotojas galėtų pasakyti telefonu).
- **Testai:** T1 langas 200, 4 sekcijos, JS, `data-pm` su 4 būdais, klientų paieška „Testas 34“ → 1 (AUDITAS Testas 34, pask. #35777), telefonu → 6, prekių „exclusion“ → 12 su kainomis/AV. **v3.33 T2/T3** (#35801 pavedimu + paštomatas 3648 + nuolaida 1,50 → on-hold, IAPV000163, `venipak_pickup_point_data` ✓, sumos 22,18+3,79−1,50 = 24,47 ✓; #35802 vietoje + LP → processing, `_ps_source=vf`, faktai ✓, **bet inst 13/9 (Neringa) ir LP be el. pašto → plugino API klaida `receiver.contacts.email`, `lp-parcel-failed` → Klausimas**; klientui laiško nebuvo). **v3.33.1 e4d:** #35806 pavedimu + paštomatas → **inst 3**, on-hold, IAPV000165, dev-pastas +2 (admin „Naujas užsakymas“ + klientui „Jūsų užsakymas Nr. 35806 gautas“ **su 1 priedu**) ✓; T3 LP be el. pašto → `dl_klaida` „LP Express siuntai reikia kliento el. pašto…“ ✓ (užsakymas nesukurtas); #35807 vietoje + Venipak kurjeris → **inst 2**, 3,30 + PVM = 3,99 ✓, processing, skydelis „Dropshipping (VF) · Užsakyti iš VF“ ✓. **e7s Playwright:** tuščias langas — 4 būdai, numatyta kurjeris 3,99; užpildytas per JS — 2 prekės (kiekis 3), nuolaida 2,00 su pastaba, Venipak paštomatas 2,15 (nemokamas nuo 30), paieška „Nemenčinė“ → 4 iš 567, sumos 18,96 − 2,00 + 2,15 = 19,11 ✓, kadrai `screenshots/s1618_e5_naujas_tuscias.png, s1618_e5_naujas_uzpildytas.png, s1618_e5_eiles_juosta.png`; be JS klaidų. Eilės po kiekvieno deploy = prieš; po testų: Gauti 8 · Neišrūšiuoti 2 · Laukiam 1 · Surinkti 7 · Dropshipping 10 · Paruošta 0 · Klausimai 9 · Neapmokėti 3 · Visi 58 (skirtumas = 7 nauji testiniai užsakymai, paaiškinta). Dev-pastas 26, 0 išėjo.
- **Testiniai papildyti (6 etapui):** #35801–#35807 (`_ps_telefonu`; 35803–35805 sukurti v3.33 ant seno kodo per nepavykusį e4d run'ą), skaitikliai dev'e AVPN 336, IAPV 166. **LP dėmesio:** #35802/#35804 turi `_woo_lithuaniapost_parcel_create_error` (LP pluginas processing metu pats kviečia LP API — plugino elgsena, kaip kasoje) — Klausimas „Siuntos sukurti nepavyko“ / „LP negalimas — ne-AV prekės“ (variklio taisyklė, teisinga).
- **Claude prielaidos (Raimiui):** (a) kainos — darbuotojo įrašyta kaina su PVM = eilutės subtotal, nuolaida € skirstoma proporcingai per eilučių total (WC „nuolaida“, sąskaitoje matosi); kupono nėra; (b) pristatymo kaina — iš WC instancijos nustatymų su nemokamo riba, darbuotojas gali perrašyti; LP kurjeris (inst 15) — 0 € numatyta, įrašyti rankomis; (c) „Apmokėta vietoje“ — `payment_method=cod` su pavadinimu „Apmokėta vietoje“ (cod gateway lieka išjungtas kasoje), `date_paid` dabar; (d) pavedimu — WC on-hold nurašo likutį iš karto (kaip kasoje); (e) LP reikalauja el. pašto (plugino API), Venipak — ne; (f) Paysera nuoroda pavedimo laiške nerodoma — temos laiškas kaip kasoje (rekvizitai + IAPV); jei Raimis nori — pridėti `get_checkout_payment_url()`.

#### S1618 — RECON A (tik skaitymas, iš repo v3.31 `?ref=4fd00d2b`)
- Kreditinės mechanizmas (v3.25): `kreditine_juodrastis` (eilutės iš `_ps_atsaukta`/refund), `kreditine_kortele` (įrašų sąrašas + formos, `kr` → PDF/siųsti), `kreditine_vykdyti` (POST; `wc_create_refund` be pinigų, `_restock_refunded_items`, WC riba, `$kr_kuriama`, `laiskai_off`), `kreditine_numeris`, `kreditine_pdf`. Klausimas „Grąžink klientui pinigus“ — `faktai()` 716: `grazinti()` įrašai ir `! $f['kl']`. Klausimas „Klientas atsisako“ — variklio `Petshop_Desk::klausimas()` pagal `_ps_withdrawal` (+`_ps_withdrawal_reason`); completed → `''` (todėl įvykdytam — tik skydelis). Skydelis — `skydelis()` masyvas → JS `rodyti(o,r)` (footer `skV`, `skPr` = pristatymas; `redaguoti()` ten pat deda formą). Likučio grąžinimas — kaip `kiekis_vykdyti` (`_ps_av_reduced_qty` → `likutis()` AV, `_reduced_stock` → `wc_update_product_stock` veidrodis). JS formų šablonas — `.dl-kr-f` (`krSum`, `.dl-kr-s` → `dlgForm` su `data-dlg`).

#### S1618 — v3.32 „GRĄŽINIMAS“ (Raimis A) — GYVA
- **v3.32** (424 436 B, md5 `3c126a8cabc21cb2faa393892e723803`; kopija `ps-backups/petshop-darbalaukis-v331-BACKUP-2026-09-05.php`; repo `deploy/petshop-darbalaukis.php` + `.b64`, `deploy/petshop-darbalaukis-v331-ATSARGA.php` = buvusi v3.31).
- **Kur:** mygtukas „Grąžinimas“ (1) Klausimo „Klientas atsisako“ kortelėje (`dl-gr-b`, forma `$papild`; pastaba perrašyta: prekės pas mus — „Atšaukti“, klientas grąžina — „Grąžinimas“; ankstesni `_ps_grazinti_rankomis` įrašai su KR rodomi čia pat + „Grąžinta“), (2) skydelio footer'yje (`skGraz`, kai `paid && (completed || kl „Klientas atsisako“)`) — forma atsiveria `skPr` bloke vietoj pristatymo („atgal“ grąžina pristatymą).
- **Forma** (`grazinimo_forma`, klasė `dl-kr-f dl-gr-f`, skydelyje `dl-gr-sk`): priežastis (`atsisakymas` „Klientas atsisako (14 d.)“ / `brokas` / `klaida` „Mūsų klaida“), eilutės iš `grazinimo_eilutes()` — kiekis > 0, be `_ps_atsaukta` / `_ps_kreditine`, likutis `q − _ps_grazinta_q` (numatyta — visas likutis), pristatymas (varnelė, suma = pristatymo eilutės − jau grąžinta refund'uose; rodoma tik kai > 0), „Prekės tinkamos prekybai — grįžta į AV likutį“ (numatyta ✓). JS `grSync`: brokas → „tinkama“ nusiima; atsisakymas + ne visi kiekiai → pristatymas užrakintas ir nužymėtas (tekstas „atsisakymas — tik grąžinant visą užsakymą“); `krSum` skaičiuoja „Kreditinė iš viso“. Dialogas prieš POST (`data-dlg`).
- **POST `ps_dl_grazinimas`** (nonce `ps_dl_graz_{id}`, `edit_shop_orders`, užraktas 30 s) → `grazinimas_vykdyti`: (1) kiekiai ≤ likutis; `visas` = visi likučiai pilnai ir nebuvo ankstesnio dalinio; pristatymas — atsisakymas tik `visas`, brokas/klaida — pagal varnelę; 3,99 € NIEKADA; WC riba `get_remaining_refund_amount` (klaida su paaiškinimu); (2) „tinkama“ → `_ps_av_reduced_qty` → `likutis()` AV, `_reduced_stock` → WC veidrodis (metos mažinamos/trinamos), tiekėjo prekė be nurašymo — nekeičiama (pastaboje „likutis nenurašytas (tiekėjo prekė) — AV nekeista“); `_restock_refunded_items` += q visada; (3) `wc_create_refund` (`refund_payment=false`, `restock_items=false`, `$kr_kuriama`, laiškai off) → refund meta `_ps_kreditine`=1, `_ps_grazinimas`=priežastis → `kreditine_numeris` (KR-AVPN) + `kreditine_pdf`; nepavykus — likučiai ir metos atstatomos; (4) eilutėse `_ps_grazinta_q` += q, pilnai grąžinta → `_ps_kreditine`=KR; įrašas `_ps_grazinti_rankomis` {…, `ka` „Grąžinimas (priežastis): n× prekė + pristatymas — netinkama prekybai…“, `graz{priezastis,tinkama,visas}`, `kr{…}`} → Klausimas „Grąžink klientui pinigus“ (PDF · siųsti klientui · Grąžinta — v3.25 mechanizmas); pastaba, įvykis `grazinimas`, juostos kešas. Laiško klientui NĖRA.
- **Deploy e1d D:** part1 md5 `ac5a6c2a…` ✓, gz+b64 → 424 436 B md5 ✓, `token_get_all` ok, gyvas = v3.31 ✓, backup ✓, ping po įrašymo 200 be Fatal, ping2 200. Klausimų kortelės prieš = po (35436, 35100, 35414, 35423, 35434, 35439). **Eilės po (e2c C):** Gauti 1 · Neišrūšiuoti 2 · Laukiam 1 · Surinkti 7 · Dropshipping 8 · Paruošta 0 · **Klausimai 7** (+#35777 — T1 grąžinimas, paaiškinta) · Neapmokėti 0 · Visi 51.
- **T1 (#35777 completed, AUDITAS Testas 34, 44,91 €):** skydelio `graz` forma yra (2 880 B); POST klaida, 1× HAP žaislas (iid 1556) + pristatymas, tinkama → 302 `dl_info` „grąžinimas įrašytas: kreditinė KR-AVPN000104 (15,31 €) · likutis: WC veidrodis +1 → 7“; refund **#35800** −15,31 (1 eilutė + 1 pristatymas), `_ps_grazinimas=klaida`, **KR-AVPN000104**, PDF `uploads/wcdn/creditnote/Kreditine-saskaita-KR-AVPN000104.pdf` 34 606 B (peržiūrėta PNG: prekė −9,17, pristatymas −3,49, PVM −2,65, grąžinama −15,31 ✓); `_stock` 35357: 6 → 7, `_reduced_stock` 1 → nuimtas, `_ps_grazinta_q` 1, eilutės `_ps_kreditine` KR-AVPN000104, `_restock_refunded_items` 1; skaitiklis 104 → 105; Klausimų kortelė #35777 „Grąžink klientui 15,31 €“ + įrašas su KR · PDF · siųsti klientui ✓ (kadras). Dev-pastas 18 (0 išėjo). Skydelis po: liko tik eilutė 1557 (1/1), pristatymo varnelės nebėra (jau grąžintas) ✓.
- **T2 (#35450 processing, `_ps_withdrawal` TEST):** kortelė „Klientas atsisako“ su mygtuku „Grąžinimas“, forma (select, 3 q įvestys, pristatymas, tinkama) ✓ (kadras `s1618_e2_kortele_35450_forma.png`: brokas → tinkama off, viso 38,99; `…_atsisakymas_dalinis.png`: 1 vnt. nuimtas → pristatymas užrakintas, viso 34,77). Meta nuimta (e2c C `meta_nuimta=1`), Klausimai vėl 7 kortelės.
- **Claude prielaidos (Raimiui):** (a) brokas / mūsų klaida — pristatymas pagal varnelę ir daliniam grąžinimui; (b) „tinkama prekybai“ galima pažymėti ir brokui (darbuotojas sprendžia, numatyta ✗); (c) dalinai grąžinta eilutė (`_ps_grazinta_q` < q) vėliau pilname atšaukime pateks į kreditinę visu kiekiu — kraštinis atvejis, WC riba sustabdys; (d) tiekėjo („tiesiai“) prekė grįžta fiziškai į AV, bet likutis AV nekeičiamas (nebuvo nurašyta) — pastaboje pasakyta, rankomis.
- **Testiniai papildyti (6 etapui):** refund #35800, KR-AVPN000104 PDF, likutis 35357 `_stock` 7 (buvo 6).

---

### S1617 (2026-09-05, naktis)

> Pridėti po šio antraštės (virš S1616). Tema: **5 etapas #6 „Pakartotinis užsakymas“ (spec §12.5 „Naujas užsakymas“) — darbalaukis v3.20 → v3.21 → v3.22, juosta v1.5 → v1.6.** Raimis 09-05 (STARTAS): (a) viena virtuali paslaugos prekė „Pakartotinis siuntimas“ (kaina = įkainis + 3,99, „už užsakymą #N“) → AVPN įprastai, faktuose įprasta prekė; (b) darbalaukyje naujas užsakymas NERODOMAS — būseną rodo pradinio kortelė ir skydelis; (c) apmokėjus — naujas užsakymas įvykdomas automatiškai su AVPN laišku, pradiniame atsiranda „Siųsti iš naujo“; į Surinkti / lapus / Venipak neina. Laiško tekstą siūlo Claude, tvirtina Raimis. Variklis (AV_Order / AV_Reduce / Partijos / Faktai / Fulfillment / tema `functions.php`) NELIESTAS. Bridge: `irankiai/s1617_r1/r2/r3.php` (recon), `s1617_e1a/e1d.php` (v3.21 + juosta v1.6 deploy + testai D/T1/T2/T3/P), `s1617_e2a/e2d.php` (v3.22 deploy + Z/Y1/Y2), `s1617_edit_v321.py`, `s1617_edit_v322.py`. `analize/s1617_*.json`, `screenshots/s1617_e1_klausimai_apmoketa.png`, `s1617_e1_skydelis_35436.png`, `s1617_e2_klausimai_forma.png`, `s1617_e2_surinkti_35436.png`, `s1617_e2_apmokejimas_svecias.png`, `s1617_e2_klausimai_apmoketa.png`, `s1617_e2_wc_uzsakymas_n3.png`.

#### S1617 — RECON (r1/r2/r3, tik skaitymas)
- **WC 11.0.1 „apmokėti užsakymą“** (`class-wc-shortcode-checkout.php` 86–125, endpoint `order-pay`, kasa page 13): registruoto kliento užsakymui reikia **prisijungti** (`pay_for_order` cap = `user_id === order user_id` ARBA užsakymas be user_id); svečio (customer_id 0) — tik nuoroda su `key`. „Ačiū“ puslapis svečiui: `woocommerce_order_received_verify_known_shoppers` → `Users::should_user_verify_order_email` (filtras `woocommerce_order_email_verification_required`). `customer_invoice` WC laiškas išjungtas, tekstas bendrinis; temos override tik `customer-processing-order.php`. Dev laiškus gaudo `petshop-dev-pastas.php` (`pre_wp_mail` prior. 5).
- **Paysera** (`woo-payment-gateway-paysera/src/Entity/class-paysera-payment-gateway.php`): `process_payment` → pending + redirect; callback `woocommerce_api_wc_gateway_paysera` → `confirmOrder` → `isPaymentValid` (valiuta + suma tiksliai) → **`update_status(paid status, manual=true)` — NE `payment_complete()`** → tik `woocommerce_order_status_processing` kabliai. Gateway'ai: paysera (on), bacs (on), cod/cheque off.
- **Tema `flatsome-child/functions.php`:** `woocommerce_order_status_completed` prior. 5 → `petshop_generate_invoice_pdf` → `_petshop_completed_pdf` + `_petshop_avpn_number` (skaitiklis `petshop_avpn_counter`); completed laiško tekstą tema perrašo „Jūsų užsakymas išsiųstas ir jau kelyje“ (netinka pakartotiniam). Proforma tik bacs+pending/on-hold.
- **Varikliai ant `payment_complete`/`order_status_processing`:** AV_Order::fiksuoti (5; praleidžia jei `_ps_order_type`), AV_Reduce::mazinti (15; tik `_ps_source=av` su likučiu), Partijos::uzsakymo_nurasymas (25; tik `av_preke()`), Faktai::rasyti (35), Ivykiai (99), darbalaukis auto_rusiuoti (100). `Petshop_Siuntos::uzbaigimo_sargas` stabdo completed tik kai `_ps_shipments>1` be `_ps_uzbaigti_be_siuntu`. Magic-login (`class-magic-login.php`, 15 min) — nenaudotas.
- Testiniai #35434/#35436 — svečio (cid 0). Virtualių prekių nėra. PVM 21 % (standartinė klasė, kainos su PVM). Juosta v1.5: `uzs`=processing+on-hold, `neapm`=pending+failed (SQL `wc_orders`).

#### S1617 — `mu-plugins/petshop-darbalaukis.php` v3.20 → **v3.21 → v3.22** (339 494 B, md5 `99533f9116ad805ee39aa9c1b5240cc3`; v3.21 `c6a2232b…` 338 188 B; kopijos `ps-backups/petshop-darbalaukis-v320-BACKUP-2026-09-05.php`, `-v321-BACKUP-2026-09-05.php`; repo `deploy/petshop-darbalaukis.php` + `.php.b64`). `mu-plugins/petshop-juosta.php` v1.5 → **v1.6** (19 029 B, md5 `5b0be0fdff1fe31f5212493081d94e0c`; kopija `ps-backups/petshop-juosta-v15-BACKUP-2026-09-05.php`; repo `deploy/petshop-juosta.php`) — `uzs`/`neapm` SQL `AND id NOT IN (… meta_key='_ps_pakartotinis')`. Deploy gzip+b64 2 dalimis (kaip S1616), `token_get_all` + md5 sargai + gyvo failo md5 sargas.

**#6 „Pakartotinis užsakymas“ (spec §12.5).** Klausimų kortelėje „Siunta grįžta“ vietoj „Siųsti iš naujo“ (kol pakartotinis neapmokėtas) — mygtukas **„Pakartotinis užsakymas“** → forma kortelėje `Suma [Y] € · 4,22 + 3,99 — taisyk, jei reikia · Sukurti ir siųsti nuorodą · Be mokesčio · atgal` (Y iš S1616 `grizta_sumos()`; POST `admin_post_ps_dl_pakartotinis`, nonce `ps_dl_pakart_{id}`, lock; suma 0,50–200). **`pakartotinis_sukurti()`:** naujas mažas užsakymas `wc_create_order` — **SVEČIO** (`customer_id 0`, `_ps_pakartotinis_klientas` = pradinio kliento ID; Claude prielaida — žr. žemiau), tas pats adresas / el. paštas / telefonas, `created_via=darbalaukis`, viena virtuali paslaugos prekė **„Pakartotinis siuntimas“** (`pakartotinis_preke()`: `private`, `hidden`, virtual, be likučio, `_ps_sandelis=paslauga`, `_ps_paslauga=pakartotinis`; opcija `ps_pakartotinio_preke`; dev'e #35790), eilutė „Pakartotinis siuntimas už užsakymą #N“, suma su PVM = Y (`wc_get_price_excluding_tax` neto + `calculate_totals(true)`, sargas ±0,011), meta `_ps_pakartotinis`=pradinio ID, `_ps_pakart_ikainis`, `_ps_pakart_nuoroda`, `_ps_uzbaigti_be_siuntu=1`; **laiškas klientui su WC apmokėjimo nuoroda** (`get_checkout_payment_url`; `pakartotinis_laiskas()`); pradiniame `_ps_pakartotinis_id` + pastaba + įvykis `pakartotinis`; „siųsti nuorodą dar kartą“ (GET `pakart_nuoroda`). Apmokėjimo puslapyje (`order-pay`) **tik Paysera** (`pakartotinis_vartai`, filtras `woocommerce_available_payment_gateways`); svečio „ačiū“ be el. pašto patvirtinimo (`woocommerce_order_email_verification_required` → false). **`pakartotinis_apmoketas()`** (`woocommerce_order_status_processing` + `payment_complete`, prior. 110, po variklių): naujas → `completed` su `laiskai_off`, temos kablys išrašo AVPN (`_petshop_completed_pdf`), **darbalaukio laiškas „Apmokėjimas gautas — užsakymo Nr. N siuntą išsiųsime iš naujo“ su PVM sąskaita PDF**; žymė `_ps_pakart_ivykdyta`; pradiniame pastaba + įvykis `pakart_apmoketa` (kanalas paysera) → kortelėje „Siųsti iš naujo“. **v3.22:** VISI WC laiškai pakartotiniam užsakymui (klientui „vykdomas“/„įvykdytas“/„laukiama“, administratoriui „naujas užsakymas“) išjungti per `woocommerce_email_enabled_{id}` (`pakartotinis_wc_laiskai`) — WC „vykdomas“ šaudo prior. 10, prieš 110; e1d P testas rado 2 perteklinius laiškus (klientui „apmokėjimas gautas“ WC + admin „naujas užsakymas“). `grizta_is_naujo` leidžiamas tik kai pakartotinis **apmokėtas** arba pažymėta **„Be mokesčio“** (`_ps_pakartotinis_nemokamai` = laikas|kas; mūsų / vežėjo kaltė, spec 6.10) — panaudojus, į istoriją `_ps_pakartotiniai` (`[{id,b,suma,laikas}]`), `_ps_pakartotinis_id` nuimamas. „Atšaukti — prekės grįžo į AV“ / „Be mokesčio“ atšaukia neapmokėtą pakartotinį (be laiškų). Būsena (`pakartotinis_bukle()`, public) kortelėje `.dl-pk` ir skydelio pastaboje: „laukia kliento apmokėjimo — nuoroda išsiųsta … · siųsti nuorodą dar kartą“ / „apmokėtas … — galima „Siųsti iš naujo““ / „be mokesčio — mūsų / vežėjo kaltė“. Naujas užsakymas darbalaukyje NERODOMAS: `faktu_sarasas()` ir `auto_rusiuoti()` praleidžia `_ps_pakartotinis`; juosta v1.6 neskaičiuoja. Varikliai ant paslaugos prekės paleidžiami be efekto (AV_Reduce — be likučio; Partijos — `av_preke` false; AV_Order `MAIN` + `_ps_groups av 1` — triukšminės pastabos „Vykdymas: MAIN — AV 1 vnt“, žalos nėra); **faktuose** eilutė `sandelis=legacy` (AV_Source `paslauga` → legacy → av), pajamos 6,79 be PVM, PVM 1,42, „TESTINIS“ dev'e.

**Testas (e1d D/T1/T2/T3/P; e2d D/Z/Y1/Y2; Venipak/LP nekviesti, grįžta simuliuota `_ps_siunta_grizta`; Paysera sim = `update_status('processing', …, true)` kaip pluginas):**
- **T1:** kortelė #35436 su forma (suma 8.21, mygtukai) → POST sukurti → **#35791** pending, cid 0, `created_via=darbalaukis`, eilutė „Pakartotinis siuntimas už užsakymą #35436“ 6,785124 + PVM 1,42 = **8,21**, meta OK; laiškas „Jūsų užsakymo Nr. 35436 siunta grįžo — pakartotinis siuntimas 8,21 €“ (dev-pastas 5→6); kortelė „laukia … nuoroda išsiųsta 09-05 00:52 · siųsti nuorodą dar kartą“, formos nėra; #35791 Klausimuose NĖRA.
- **T2:** POST nemokamai → #35791 **cancelled** (be laiškų), pradiniame `_ps_pakartotinis_nemokamai`, kortelėje „Siųsti iš naujo“ + „be mokesčio“ pastaba. Žymė nuimta testui.
- **T3:** POST sukurti → **#35792**; apmokėjimo puslapis be slapukų: 200, Paysera yra, bacs nėra, prisijungimo formos nėra, klaidų nėra; #35792 nei Neapmokėtuose, nei Klausimuose; juosta `uzs 20 / neapm 0` (SQL pending 1).
- **P:** Paysera sim → #35792 **completed**, `paid`, **AVPN000324** PDF 33 714 B, faktai (1 eilutė), `_ps_pakart_ivykdyta`; pradiniame pastaba + įvykis `pakart_apmoketa`; kortelė „apmokėtas 09-05 00:52 — galima Siųsti iš naujo“ + mygtukas; skydelio pastaba tas pats. Laiškai 5→10: nuoroda ×2 + darbalaukio „Apmokėjimas gautas“ + **2 pertekliniai WC** (klientui „Jūsų užsakymas Nr. 35792 — apmokėjimas gautas“, admin „Naujas užsakymas Nr. 35792“) → **v3.22**. Playwright `s1617_e1_klausimai_apmoketa.png`, `s1617_e1_skydelis_35436.png` peržiūrėtos.
- **Z (v3.22):** „Siųsti iš naujo“ per tikrą kortelės nuorodą → 302 `dl_dalis` „siunta V…779 grįžo — AV dalis: Surinkti AV → naujas lipdukas“; `_ps_siunta_grizta`/`_ps_pakartotinis_id` nuimti, istorija `[{35792, apmoketa, 8,21}]`; #35436 Klausimuose lieka dėl S1616 „Grąžink klientui 13,01 €“ (tikėtasi). Re-sim grįžta (V…780) → Playwright `s1617_e2_klausimai_forma.png` (click `.dl-pk-b` → forma `display:flex`, suma 8.21, tekstas „Suma € 4,22 + 3,99 — taisyk, jei reikia · Sukurti ir siųsti nuorodą · Be mokesčio · atgal“; mygtukas „Pakartotinis užsakymas“).
- **Y1:** POST sukurti → **#35793**; Playwright SVEČIUI (be slapukų) `s1617_e2_apmokejimas_svecias.png`: „Apmokėjimas – Petshop.lt“, viena eilutė ×1 8,21 €, Viso 8,21 € (įskaičiuota 1,42 € PVM), **tik „Mokėjimas internetu Paysera“**, sutikimo varnelė, mygtukas „APMOKĖTI UŽSAKYMĄ“, prisijungimo formos/klaidų nėra.
- **Y2:** Paysera sim → #35793 completed, **AVPN000325**; filtras `has_filter(...)=20`; **laiškai +1 (tik darbalaukio „Apmokėjimas gautas“), pastabose „Email … sent“ 0**; kortelė „apmokėtas 09-05 01:05 — galima Siųsti iš naujo“. `s1617_e2_klausimai_apmoketa.png`, `s1617_e2_wc_uzsakymas_n3.png` peržiūrėtos.
- Laiškai: 0 išėjo (pre_wp_mail dev-pastas 5→12). TEMP snippet'ai — 1 neaktyvus (kito run'o trinamas).

**Laiškų tekstai (siūlo Claude — tvirtina Raimis):** (1) tema „Jūsų užsakymo Nr. N siunta grįžo — pakartotinis siuntimas Y €“: „Sveiki, {vardas}. Jūsų užsakymo Nr. N siunta grįžo mums neatsiimta / nepristatyta. Galime išsiųsti ją iš naujo tuo pačiu adresu. Pakartotinio siuntimo kaina — Y € (įkainis € pristatymas + 3,99 € siuntos grąžinimo išlaidos), pagal pirkimo taisyklių 6.10–6.11 p. [Apmokėti Y €] Gavę apmokėjimą, siuntą išsiųsime iš naujo ir atsiųsime sekimo numerį. Jei norite kito adreso ar paštomato — parašykite atsakydami į šį laišką prieš apmokėdami. Jei siųsti iš naujo nenorite — parašykite: užsakymą atšauksime ir grąžinsime už prekes ir pristatymą sumokėtą sumą, atskaičius 3,99 € siuntos grąžinimo išlaidas. Gražios dienos, Petshop.lt komanda“. (2) tema „Apmokėjimas gautas — užsakymo Nr. N siuntą išsiųsime iš naujo“: „Ačiū — apmokėjimą už pakartotinį siuntimą (Y €) gavome. Užsakymo Nr. N siuntą išsiųsime iš naujo tuo pačiu adresu ir atsiųsime sekimo numerį atskiru laišku. PVM sąskaita faktūra už pakartotinį siuntimą prisegta prie šio laiško.“

**Claude prielaidos (Raimiui patvirtinti):** (1) pakartotinis užsakymas — **svečio** (WC apmokėjimo puslapis registruoto kliento užsakymui reikalauja prisijungti; svečiui — tik nuoroda su raktu; paskyros „Užsakymų istorijoje“ jo nesimatys; norint — `user_has_cap` filtras arba magic-login); (2) apmokėjimo puslapyje **tik Paysera** (pavedimo patvirtinimui reiktų atskiro „Apmokėta“ mygtuko kortelėje — neįgyvendinta); (3) **VISI WC laiškai** pakartotiniam užsakymui išjungti, įskaitant admin „naujas užsakymas“ — vienintelis laiškas darbalaukio „apmokėjimas gautas“ su PVM sąskaita, administratoriui — pradinio kortelė/pastabos; (4) **„Be mokesčio“** mygtukas formoje (mūsų / vežėjo kaltė) — iškart „Siųsti iš naujo“ be naujo užsakymo; (5) paslaugos prekė `_ps_sandelis=paslauga` → faktuose `sandelis=legacy` (ataskaitose pakartotinio siuntimo pajamos eis kaip legacy/AV; jei reikia atskirti — atskira `paslauga` reikšmė faktų variklyje, neliesta); (6) laiškų tekstai aukščiau.

**Testiniai po S1617:** #35436 processing, `_ps_siunta_grizta` (sim 2, V…780), `_ps_pakartotinis_id`=35793 (**apmokėtas** — kortelėje „Siųsti iš naujo“), istorija #35792, Quattro dalis atšaukta (S1616 žymė 13,01); pakartotiniai **#35791 cancelled, #35792 completed (AVPN000324), #35793 completed (AVPN000325)** — cid 0, `created_via=darbalaukis`; paslaugos prekė **#35790** „Pakartotinis siuntimas“ (private) — lieka gamybai; AVPN skaitiklis 325. Trinti 6 etape: #35791–#35793 + faktų eilutės (`testinis`), `ps_s1617_test` opcija. Dev-pastas žurnalas 12.

**Lieka:** kreditinė pusiau automatinė (recon WCDN creditnote) → „Sąskaita“ langas → Pragma su kreditinėmis (klausti). Pakartotinis: Raimio prielaidų (1)–(6) patvirtinimas; jei (2) „ir pavedimu“ — mygtukas „Apmokėta“ kortelėje. J1, Venipak 4/5/7/8, V12/V13/V14 variklis, T10/V1 — kaip S1614.

#### S1617 — TĘSINYS (Raimio atsakymai 09-05 ~01:20): v3.22 → **v3.23 → v3.24** (345 956 B, md5 `95e5c3ba12581760dcae6b340e8f9571`; v3.23 `41cf2b8f…` 344 966 B; kopijos `ps-backups/petshop-darbalaukis-v322-BACKUP-2026-09-05.php`, `-v323-BACKUP-2026-09-05.php`; repo `deploy/`). Bridge: `irankiai/s1617_r4/r5.php` (recon), `s1617_e3a/e3d.php` (v3.23 + W1/W1B), `s1617_e4a/e4d.php` (v3.24 + W2/W3/V), `s1617_edit_v323/v324.py`. `screenshots/s1617_e3_aciu_pavedimas.png`, `s1617_e4_apmokejimas_onhold.png`, `s1617_e4_klausimai_pavedimas.png`, `s1617_e4_klausimai_apmoketa.png`.

**Raimio sprendimai (galioja):** (1) svečio užsakymas — **gerai**; (2) pavedimas — **reikia**; (3) WC laiškai jam išjungti — **taip**; (4) „Be mokesčio“ — **taip**; (5) **„Legacy niekur negali figūruoti“**; (6) laiškų tekstai — **gerai**.

**Recon r4/r5 (tik skaitymas):** `ps_fakt_eilutes.sandelis` reikšmės dev'e: ambrosia 9, belcor_tofu 1, **legacy 44**, prins 10, quattro 4, vf 22, zb 5 — `av` nėra: faktų variklis (`Fulfillment_Source::resolve` → `legacy`, label „Legacy (savas sandėlis)“) **VISAS savo sandėlio (AV) prekes rašo kodu `legacy`** — dokumentuota `petshop-fakt-siuntos.php` antraštėje (eil. 68–81: „`legacy` ten reiškia būtent AV … `sandelio_kodas()` verčia legacy → av“). Paslaugos prekė faktuose eina lygiai kaip kiekviena AV prekė; `_ps_sandelis=paslauga` → `AV_Source::resolve` = av („grynai AV prekė“). `ps_fakt_uzsakymai` stulpeliai: `uzsakymas_id, … statusas_galutinis, viso_ct, prekiu_suma_ct, mokejimo_budas, sandeliai, testinis …` (S1617 P/e2 „faktas null“ — mano SQL su neegzistuojančiu `statusas`; e4 W3 eilutė yra: viso 821, prekės 679, `mokejimo_budas=bacs`, `sandeliai=legacy`, testinis 1). **WC bacs:** enabled, „Bankinis pavedimas“, aprašymas yra, **sąskaitų (`woocommerce_bacs_accounts`) nėra** → WC „ačiū“/laiškas rekvizitų nerodytų; `process_payment` → `on-hold` („Awaiting BACS payment.“); tema `petshop_get_invoice_document_type`: proforma tik bacs + pending/on-hold, kitaip invoice. Rekvizitai temoje (`functions.php` 432 „gautas“ laiške; `base.php` 306–309, 479 IAPV): **UAB Avesa · AB Swedbank · LT127300010124940593**.

**v3.23 (pavedimas):** `pakartotinis_vartai` — apmokėjimo puslapyje **Paysera + Bankinis pavedimas**; laiške su nuoroda — „Apmokėti galite internetu (mygtukas) arba pavedimu“ + paragrafas „Arba pavedimu: UAB Avesa · AB Swedbank · LT12…593, suma Y €, paskirtis „Pakartotinis siuntimas, užsakymas Nr. N““ (`PAKART_BANKAS` — Claude prielaida: tie patys rekvizitai kaip temoje); klientui pasirinkus pavedimą (`on-hold`, pm bacs) „ačiū“ puslapyje `pakartotinis_aciu_bankas` (`woocommerce_thankyou_bacs`) rašo „Pavedimo rekvizitai“ + paskirtį; kortelėje, kol laukia, mygtukas **„Apmokėta pavedimu“** (`v t`, GET `pakart_apmoketa`, dialogas „Pavedimas Y € už pakartotinį užsakymą #M jau banke? …“) → `pakartotinis_pavedimas()`: pm=bacs (jei dar ne), pastaba, `update_status('processing', …, true)` kaip Paysera callback → tas pats `pakartotinis_apmoketas` → completed, AVPN, darbalaukio laiškas; įvykis `pakart_pavedimas` (web); `pakart_apmoketa` kanalas web (bacs) / paysera. `pakartotinis_bukle` laukia + bacs → „Klientas pasirinko pavedimą — kai pinigai banke, spausk „Apmokėta pavedimu““. **v3.24:** WC apmokėjimo puslapis `on-hold` užsakymui sako „apmokėjimas negalimas“ (rasta e3 W1B) → `woocommerce_valid_order_statuses_for_payment` += on-hold pakartotiniam (`pakartotinis_moketini_statusai`): pasirinkęs pavedimą klientas vis dar gali apmokėti Paysera per tą pačią nuorodą, kol darbuotojas nepažymėjo „Apmokėta pavedimu“.

**Testas (e3 W1/W1B v3.23, e4 W2/W3/V v3.24; Paysera/Venipak nekviesti):** „Siųsti iš naujo“ per tikrą nuorodą (#35793 → istorija), re-sim grįžta (V…781), POST sukurti → **#35794** (laiškas su nuoroda + rekvizitais išėjo į dev-pastas), sim klientas pasirinko pavedimą (pm bacs, `on-hold`, doc type `proforma`). Playwright SVEČIUI: „ačiū“ puslapis `s1617_e3_aciu_pavedimas.png` — „Pavedimo rekvizitai: Gavėjas UAB Avesa · AB Swedbank · LT12…593 · Suma 8,21 € · Paskirtis „Pakartotinis siuntimas, užsakymas Nr. 35436““, be el. pašto patvirtinimo; apmokėjimo puslapis on-hold v3.23 — WC klaida „Sustabdytas — apmokėjimas negalimas“ → v3.24 → `s1617_e4_apmokejimas_onhold.png`: Paysera + Bankinis pavedimas, be klaidų (prisijungusiam admin WC rodo „guest order“ įspėjimą — svečiui ne). Kortelė `s1617_e4_klausimai_pavedimas.png`: „…Klientas pasirinko pavedimą — kai pinigai banke, spausk „Apmokėta pavedimu““ + mygtukai „Apmokėta pavedimu · Atšaukti — prekės grįžo į AV · Atidaryti · Parašyti klientui“, skydelio pastaba tas pats. **„Apmokėta pavedimu“ per tikrą kortelės nuorodą** (nonce — tos pačios sesijos; pirmas bandymas su W2 nonce → 403, WP nonce rišamas prie sesijos žetono): 302 „#35794 (8,21 €) pažymėtas apmokėtu pavedimu, PVM sąskaita **AVPN000326** — dabar „Siųsti iš naujo““; #35794 `completed`, `paid`, `date_paid`, pm bacs, doc type **invoice**, PDF 33 638 B, faktų eilutė + užsakymo faktas (bacs), pastabos be „Email … sent“, dev-pastas **13 → 14** (tik darbalaukio „Apmokėjimas gautas“; skaičių tikrinti NAUJAME procese — opcijų kešas, V fazė); pradiniame pastaba „apmokėtas pavedimu (8,21 €, AVPN000326)“, įvykiai `pakart_pavedimas` + `pakart_apmoketa` (web); kortelė `s1617_e4_klausimai_apmoketa.png` — „apmokėtas 09-05 01:41 — galima „Siųsti iš naujo““ + mygtukas. Pamokos: (a) tame pačiame procese po išorinio POST `wc_get_order` grąžina seną objektą — meta skaityti SQL'u arba po `wp_cache_flush()`; (b) `get_option` žurnalą kešuoja — skaityti kitame run'e.

**(5) „Legacy“ — paaiškinimas Raimiui (kodo nekeista):** žodis „legacy“ niekur nerodomas — tai vidinis faktų lentelės kodas, kuriuo variklis nuo 08-06 žymi VISAS savo sandėlio prekes (ataskaitos jį skaito kaip AV). Paslaugos prekė eina kaip AV prekė, ne kitaip. Jei kodas `legacy` faktuose turi tapti `av` — tai variklio (`Fulfillment_Source::resolve` / `petshop-faktai.php`, `petshop-fakt-siuntos.php` `sandelio_kodas()`) keitimas + 44 esamų eilučių migracija — atskiras darbas, klausti.

**Testiniai po S1617 (galutiniai):** #35436 processing, `_ps_siunta_grizta` sim 3 (V…781), `_ps_pakartotinis_id`=**35794 apmokėtas pavedimu** (kortelėje „Siųsti iš naujo“), istorija #35792, #35793; pakartotiniai **#35791 cancelled, #35792 completed AVPN000324 (Paysera sim), #35793 completed AVPN000325 (Paysera sim), #35794 completed AVPN000326 (bacs, „Apmokėta pavedimu“)**; paslaugos prekė #35790 lieka; AVPN skaitiklis 326; dev-pastas žurnalas 14; TEMP 1 neaktyvus. Trinti 6 etape: #35791–#35794, faktų `testinis`, `ps_s1617_test`.

**Lieka:** ~~kreditinė~~ (v3.25) → ~~„Sąskaita“ langas~~ (v3.26–v3.29, juosta v1.7 — žr. „SĄSKAITA LANGAS“) → Pragma (klausti). Faktų kodas `legacy` → `av` (jei Raimis nori — variklis, klausti). J1, Venipak 4/5/7/8, V12/V13/V14, T10/V1 — kaip S1614.

#### S1617 — KREDITINĖ: RECON (r6/r7/r8, tik skaitymas; kodo nerašyta — laukia Raimio sprendimų K1–K3)
- **Temos `flatsome-child/woocommerce-delivery-notes/base.php` (v2.8) JAU TURI kreditinės šaką** (`$template === 'creditnote'`, eil. 22–48, 376–397, 424–427, 445–455): antraštė „Kreditinė PVM sąskaita faktūra“; numeris **`KR-` + pradinio užsakymo AVPN** (`_petshop_avpn_number`, jei nėra — generuojamas); eilutės — **TIK PASKUTINIO WC grąžinimo** (`$parent->get_refunds()[0]`) prekės: pavadinimas, SKU, kiekis „−n“, vnt. kaina, suma „−“; apačioje „Kreditinė data“ (generavimo diena), „Originali sąskaita: AVPN…“, „Užsakymo nr.“; sumos — to grąžinimo „Suma be PVM / PVM (21%) / Grąžinama suma“ (neigiamos). Mokėjimo dėžutės nėra. Visą užsakymą kreditinė NESPAUSDINA — tik WC refund eilutes (atsakymas į spec §12.5 „neišaiškinta“).
- **PDF:** tema AVPN daro `petshop_generate_invoice_pdf($order_id)` (functions.php 292–376): `$order` masyvas (id, orderNumber, documentDate, date, paymentMethod, billing), `$template='invoice'`, `$type='pdf'`, `include base.php` → Dompdf A4 → `uploads/wcdn/invoice/PVM-saskaita-AVPN…pdf`. Parametro `creditnote` nėra → darbalaukis gali daryti tą patį pats su `$template='creditnote'` (temos neliesdamas, tik `include` jos base.php), PDF į `uploads/wcdn/creditnote/` (dabar tuščias; 2026-08-26 testai #35088/#35090 turi senas `_wcdn_creditnote_pdf` žymes, failų nėra).
- **WCDN 7.3.0:** `wcdn_template_settings` raktai tik invoice/receipt/deliverynote/packingslip — `creditnote` NĖRA (po 7.1.2 → 7.3.0 migracijos); WCDN kreditinės renderis ima `$order['refund']['items']` (`generate_refund_preview` — iš `get_refunds()`), leidžiami statusai refunded/completed. Seno desk „Kreditinė“ mygtukas (`wcdn_print_creditnote`, `petshop-atsisakymas.php` „kreditinė — wcdn mygtuku“) eina per WCDN spausdinimą su temos base.php → tas pats „paskutinis refund“; ar veikia 7.3.0 — netikrinta (klasės `Tyche\WCDN\…`, `Templates::get` nerasta). Darbalaukiui nereikia — generuosim patys.
- **WC refund** (`wc_create_refund`): `amount` ≤ `get_remaining_refund_amount()` (S1616: po kiekio perrašymo mažesnis už realų — riba kreditinei!), `line_items` gali būti `line_item`, `fee`, `shipping` (pristatymo eilutė grąžinama); eilučių suma ir `amount` NESIEJAMI (dokumentas gali nesutapti). `refund_payment=false`, `restock_items=false` — kaip #4. Kabliukas `woocommerce_order_refunded` → `Petshop_Fakt_Grazinimai::rasyti($order_id,$refund_id)` (stulpeliai: kiekis, suma_ct, pvm_ct, savikaina_ct, priezastis_kodas/tekstas, atsargos_grizo, sandelis, testinis). Refund'ai dev'e: 35788 (#35414 −2,60), 35789 (#35425 −28,58), 35091–35093 (08-26).
- Skaitikliai: `petshop_avpn_counter` 327, `petshop_iapv_counter` 163; KR skaitiklio nėra.

**Sprendimai Raimiui (K1–K3), prieš kodą:**
- **K1 Numeracija.** Dabar KR = `KR-AVPN` + pradinės sąskaitos numeris → **vienam užsakymui visos kreditinės gautų tą patį numerį** (pvz., pirma #4 kiekio mažinimas, paskui grįžusi siunta). Variantai: (a) palikti KR-AVPN{pradinis} — viena kreditinė užsakymui, antra draudžiama (darbalaukis neleis); (b) sava eilė KR-AVPN{skaitiklis} (nuo 101 per T-0, kaip AVPN) — reikia **mažo temos base.php pakeitimo** (eil. 42; leidimas). Buhalterės klausimas.
- **K2 3,99 €.** Kreditinės eilutės = WC grąžinimo eilutės. (A) kreditinė visai grąžinamai daliai (prekės + pristatymas), o 3,99 siuntos grąžinimo išlaidos — **atskira PVM sąskaita** už paslaugą (paslaugos prekė kaip „Pakartotinis siuntimas“, naujas mažas užsakymas, apmokėta įskaitymu — klientui grąžinama suma − 3,99; du dokumentai, švaru); (B) viena kreditinė sumai − 3,99 su eilutėmis pilnomis — eilutės nesutaptų su suma (netinka); (C) kreditinė prekėms + pristatymui, 3,99 tik pastaboje „išskaičiuota 3,99, grąžinama X“ — 3,99 pajamos be dokumento. Claude siūlo A. Buhalterės klausimas.
- **K3 Kelias.** Kreditinę daryti per WC grąžinimo įrašą (`wc_create_refund`, pinigų nejudina, likučių neliečia): „Atšaukti — prekės grįžo į AV“ / apmokėto atšaukimas → Klausimo kortelėje „Grąžink klientui pinigus“ juodraštis (eilutės, pristatymas, suma; taisoma) → „Patvirtinti kreditinę“ → WC refund + KR PDF (temos šablonas, Dompdf) + laiškas klientui su kreditine? (ar be laiško) + pastaba „grąžink X rankomis“ → „Grąžinta“ nuima Klausimą. #4 kiekio refund'ams (`_ps_kiekis`) — tas pats „Patvirtinti kreditinę“ iš Klausimo kortelės (refund jau yra). Laiškas klientui su kreditine — taip / ne?

#### S1617 — KREDITINĖ ĮGYVENDINTA: darbalaukis v3.24 → **v3.25** (381 439 B, md5 `bd1bb760aa56182b003e6eadc939ba21`; kopija `ps-backups/petshop-darbalaukis-v324-BACKUP-2026-09-05.php`; repo `deploy/petshop-darbalaukis.php` + `.b64`) + **tema `flatsome-child/woocommerce-delivery-notes/base.php` v2.8 → v2.9 → v2.10** (24 883 B, md5 `1119082e28108554e5f5c9749803ef34`; kopijos `ps-backups/theme-wcdn-base-v28-BACKUP-2026-09-05.php`, `-v29-`; repo `deploy/wcdn-base.php`). Raimio sprendimai: **K1 taip** (sava KR eilė, tema liesti leista), **K2 A** („pritariu, logiška“), **K3 per WC įrašą, laiškas — ne automatiškai, darbuotojas**. Bridge: `irankiai/s1617_r9.php` (base.php nuskaitymas, WC/faktų kabliai), `s1617_e5a/e5d.php` (deploy D, S1, T1–T4, E, V), `s1617_edit_v325.py` + `s1617_v325_funcs.php`. `screenshots/s1617_e5_klausimai_kr_forma.png`, `s1617_e5_klausimai_kr_po.png`; PDF peržiūrėti lokaliai (b64 → pdftoppm).

**Raimio klausimas „per WC nieko nedarome“ — atsakyta:** WC lango darbuotojas nemato; WC grąžinimo įrašas = įrašas variklyje fone (kaip #4 kiekiai), nes (1) temos kreditinės šablonas skaito refund'ą, (2) faktų variklis (`Fakt_Grazinimai`) grąžinimo faktą rašo pats, (3) vienas šaltinis kreditinei/faktams/ataskaitoms/Pragmai. Savi laukai — viskam, ką darbuotojas mato ir sprendžia (spec R2).

**Tema `base.php` (leidimas K1; ne 1, o ~8 eilutės — visos kreditinės šakoje, sąskaitos/proformos neliestos):** v2.9 — eil. 42 numeris iš refund meta `_petshop_kravpn_number` (kitaip senas `KR-`+AVPN), eil. 379/448 kreditinė spausdina **tą** grąžinimą (`$maybe_order`, kai `$order['id']` = refund), o ne paskutinį, eil. 425 „Kreditinė data“ iš `_petshop_kravpn_date`, eil. 426 „Originali sąskaita“ = `$original_avpn` (buvo `str_replace('KR-')` — su sava eile rodytų neteisingai); v2.10 — pristatymo eilutė (`$refund_obj->get_items('shipping')`, „Pristatymas (VENIPAK Kurjeris) −1 3,99“) ir sumose „Pristatymas: −3,99“ (e5 T1 PDF: be jos 26,61 + 6,43 ≠ 37,03).

**Darbalaukis v3.25:** Klausimų kortelėje „Grąžink klientui pinigus“ kiekvienas `_ps_grazinti_rankomis` įrašas — atskira eilutė + mygtukas **„Kreditinė“** → forma kortelėje (`kreditine_juodrastis`): refund>0 (#4) — eilutės iš WC įrašo, fiksuotos; visas atšaukimas — eilutės be `_ps_atsaukta` ir be `_ps_kreditine` + pristatymas (varnelė, visa suma su PVM); dalis — eilutės su `_ps_atsaukta` be `_ps_kreditine`; kiekiai taisomi (0…q), varnelė „3,99 € — atskira PVM sąskaita“ (pagal `ka` „− 3,99“; nežymėti, kai mūsų/vežėjo kaltė), gyva suma (`krSum`) ir „klientui grąžinti X“ → **„Patvirtinti kreditinę“** (POST `ps_dl_kreditine`, nonce `ps_dl_kr_{id}`, dialogas). `kreditine_vykdyti()`: (1) refund 0 → `wc_create_refund` (eilutės `refund_total`/`refund_tax` proporcingai, pristatymo eilutė `qty 0`; `refund_payment=false`, `restock_items=false`; `laiskai_off`; `_restock_refunded_items += q` faktui; statusas nekinta — `woocommerce_order_fully_refunded_status` → false kol `self::$kr_kuriama`; WC riba `get_remaining_refund_amount` → klaida su suma „kreditinę tvarkyk su Raimiu“); refund meta `_ps_kreditine=1`; (2) **KR-AVPN numeris** iš `petshop_kravpn_counter` (KITAS, kaip AVPN; dev'e pradėta 101 → T-0 → 101) į refund meta `_petshop_kravpn_number` + `_petshop_kravpn_date`; (3) **PDF** `kreditine_pdf()` — tas pats `$order` masyvas / logotipas / Dompdf kaip temos `petshop_generate_invoice_pdf`, tik `$template='creditnote'`, `$order['id']`=refund → `uploads/wcdn/creditnote/Kreditine-saskaita-KR-AVPN000101.pdf` (`_petshop_kravpn_pdf`); (4) **3,99 €** (K2 A) `grazinimo_mokestis_sukurti()`: paslaugos prekė „Siuntos grąžinimo išlaidos“ (privati, `_ps_paslauga=grazinimo_islaidos`, `_ps_sandelis=paslauga`, opcija `ps_grazinimo_preke`; dev'e #35796), svečio užsakymas su `_ps_pakartotinis`=pradinis (nerodomas darbalaukyje/juostoje, WC laiškai off) + `_ps_paslauga`, „Įskaitymas iš grąžinamos sumos“, `processing` → `pakartotinis_apmoketas` šaka `grazinimo_islaidos` → completed, tema AVPN, be laiško; (5) įraše `kr{nr,refund,pdf,suma,laikas,kas,mokestis,mok_avpn,eil,laiskas}`, eilutėse `_ps_kreditine`=nr, įrašo `suma` = KR − 3,99 (jei mokestis); pastaba, įvykis `kreditine`. Kortelėje po: „Kreditinė **KR-AVPN000101** (37,03 €; 3,99 € sąskaita AVPN000327) · PDF (GET `kr_pdf`, srautas prieš užraktą) · siųsti klientui (GET `kr_laiskas` → `kreditine_laiskas`: laiškas su KR PDF + 3,99 € AVPN PDF, tema „Kreditinė sąskaita Nr. … už užsakymą Nr. N“; po — „siųsti dar kartą (išsiųsta …)“)“. „Grąžinta“ dialogas: „DĖMESIO: n įrašui kreditinė NEIŠRAŠYTA“; pastaboje KR numeriai. Eilutės click išimtis `.dl-kr-f`.

**Testas (e5; Paysera/Venipak nekviesti; laiškai per dev-pastas):**
- **S1** Playwright: #35434 kortelė, click „Kreditinė“ → forma: 2 eilutės (Animonda −2,60, Prins −29,60; kiekiai 1 iš 1), „Pristatymas (visa siunta grįžo) −4,83“ ✓, „Kreditinė iš viso −37,03“, 3,99 varnelė ✓, „klientui grąžinti 33,04“, „Patvirtinti kreditinę · atgal“ (`s1617_e5_klausimai_kr_forma.png`). #35414 forma: eilutė −1 −2,60 fiksuota, be 3,99.
- **T1 #35434 (cancelled, visa siunta grįžo):** POST → 302 „kreditinė KR-AVPN000101 (37,03 €) išrašyta · 3,99 € sąskaita AVPN000327 · grąžink klientui 33,04 €“; refund **#35795** −37,03 (2 prekės + pristatymas −3,99/−0,84), `_petshop_kravpn_number/_date/_pdf`; statusas liko **cancelled** (remaining 0,00, WC „refunded“ nepersijungė ✓); PDF 35 138 B; 3,99 € užsakymas **#35797** completed, „Įskaitymas iš grąžinamos sumos“, **AVPN000327** (PVM-saskaita-AVPN000327.pdf), faktai 3,30/0,69; grąžinimo faktai 3 eil. (2 prekės `atsargos_grizo 1`, pristatymas `kita`); eilutėse `_ps_kreditine`, `_restock_refunded_items 1`; įvykis `kreditine`; kortelė po: „Kreditinė KR-AVPN000101 (37,03 €; 3,99 € sąskaita AVPN000327) PDF siųsti klientui“, formos nėra.
- **T2 #35414 (#4 refund 35788):** → **KR-AVPN000102** 2,60 €, PDF 34 127 B, be 3,99; grąžinti 2,60.
- **T3 #35436:** pirma „Siųsti iš naujo“ per tikrą nuorodą (pk #35794 → istorija; #35436 Klausimuose lieka „Grąžink“) → POST → **KR-AVPN000103** 17,00 € (Quattro −14,05/−2,95), refund **#35798**, 3,99 € **#35799 AVPN000328**, grąžinti 13,01; statusas processing (remaining 6,61). `s1617_e5_klausimai_kr_po.png` — 3 kortelės su KR.
- **PDF KR-AVPN000101 (peržiūra):** „Kreditinė PVM sąskaita faktūra – KR-AVPN000101“, pardavėjas/pirkėjas, eilutės −1 (vnt. be PVM 2,15 / 24,46), „Kreditinė data 2026-09-05 · Originali sąskaita AVPN000303 · Užsakymo nr. 35434“, sumos −26,61 / PVM −6,43 / Grąžinama −37,03 — **pristatymo eilutės nebuvo** → base.php v2.10 (E: deploy + regeneravimas per Reflection): eilutė „Pristatymas (VENIPAK Kurjeris) −1 3,99“, sumos −26,61 / Pristatymas −3,99 / PVM −6,43 / −37,03 ✓. 3,99 € sąskaita AVPN000327: „Siuntos grąžinimo išlaidos už užsakymą #35434 · 1 · 3,30 · Mokėtina 3,99 · Apmokėjimo būdas: Įskaitymas iš grąžinamos sumos“ (rodo „Pristatymas: Nemokamas“ — šablono eilutė paslaugai, kaip ir pakartotiniam; paslėpti — jei Raimis norės).
- **T4:** `kr_pdf` GET → 200 `application/pdf`, `inline; filename="Kreditine-saskaita-KR-AVPN000101.pdf"`, 35 489 B; „siųsti klientui“ per tikrą nuorodą → 302 „išsiųsta (terra@petshop.lt)“, dev-pastas 16→17 su **2 priedais**, įraše `kr.laiskas`, pastaba. 0 laiškų išėjo. TEMP 1 neaktyvus.
- Skaitikliai po: `petshop_kravpn_counter` 104, `petshop_avpn_counter` 329. Dev-pastas žurnale 10:04 „[Petshop sargas] Fatal error“ — mano r9 payload (`shell_exec` uždrausta hostinge) — testinis, žalos nėra.

**Laiško su kreditine tekstas (Claude siūlo — Raimis tvirtina):** „Sveiki, {vardas}. Pridedame kreditinę PVM sąskaitą faktūrą Nr. KR-AVPN… (X €) už užsakymą Nr. N. [Siuntos grąžinimo išlaidos — 3,99 € (pirkimo taisyklių 6.10–6.11 p.) — PVM sąskaita faktūra Nr. AVPN… pridėta; jos apmokėjimą įskaitome iš grąžinamos sumos.] Grąžinama suma — Y €. Pinigus grąžinsime per 14 dienų tuo pačiu būdu, kuriuo mokėjote. Gražios dienos, Petshop.lt komanda“.

**Ribos / pastabos:** (a) WC `get_remaining_refund_amount` po #4 kiekio perrašymo mažesnis už realų → pilna kreditinė atšaukiant tokį užsakymą gali būti užblokuota — darbalaukis rodo klaidą su sumomis (retas atvejis, kelias — su Raimiu); (b) dalies atšaukimas be kreditinės prieš v3.25 + vėlesnis „Grąžinta“ — eilutės be `_ps_kreditine` liktų (senų testinių atvejis); (c) „Sąskaita“ langas — vėliau: kreditinės randamos per refund meta `_petshop_kravpn_number` (`wc_orders` type shop_order_refund + `wc_orders_meta`).

**Testiniai po kreditinės:** refund'ai **#35795** (35434, KR101), **#35798** (35436, KR103), #35788 (KR102); 3,99 € užsakymai **#35797** (AVPN000327), **#35799** (AVPN000328); paslaugos prekė **#35796** „Siuntos grąžinimo išlaidos“ — lieka gamybai; #35434 cancelled, refunded 37,03; #35436 processing, Surinkti AV (Exclusion), Klausimas „Grąžink 13,01“, KR103; #35414 processing, Klausimas „Grąžink 2,60“, KR102. Trinti 6 etape: #35795–#35799 (ne #35796), `wcdn/creditnote/*.pdf` testiniai, faktų `testinis`; skaitikliai KR/AVPN/IAPV → 101.

#### S1617 — „SĄSKAITA“ LANGAS ĮGYVENDINTAS: darbalaukis v3.25 → **v3.26 → v3.27 → v3.28 → v3.29** (397 454 B, md5 `d067b40cfbfd4ad520ddffd961b4af15`; kopijos `ps-backups/petshop-darbalaukis-v325…v328-BACKUP-2026-09-05.php`; repo `deploy/`) + **juosta v1.6 → v1.7** (19 526 B, md5 `d4a86860feb3fd0da6258338f72a36f4`; kopija `ps-backups/petshop-juosta-v16-BACKUP-2026-09-05.php`). Raimis: „tęsk“ (spec §12.5 „Sąskaita“ — sąrašas su filtrais, atsisiuntimas, skydelio mygtukas). Bridge: `irankiai/s1617_r10.php` (recon), `s1617_e6a/e6d.php` (v3.26 + juosta), `s1617_e7a/e7d.php` (v3.27 + T1 + S1 Playwright), `s1617_e8a/e8d.php` (v3.28), `s1617_e9a/e9d.php` (v3.29), `s1617_edit_v326.py` + `s1617_v326_funcs.php`. `screenshots/s1617_e6_saskaitos.png`, `s1617_e6_saskaitos_kr.png`, `s1617_e6_skydelis_35434.png`.

**Recon r10:** dokumentų meta — AVPN `_petshop_avpn_number` (50 užs.) + `_petshop_completed_pdf` (20 — senesni turi numerį be failo), IAPV `_petshop_iapv_number` (7) + `_petshop_order_pdf` (6, kasos metu), KR — refund meta `_petshop_kravpn_number/_date/_pdf` (3); datų meta nėra (AVPN — įvykdymo/apmokėjimo data iš HPOS `wc_order_operational_data`, IAPV — užsakymo, KR — refund sukūrimo). `uploads/wcdn/invoice/` 71 PDF (3 IAPV), `wcdn/creditnote/` 3. Juosta (R3): ⌂ · Užsakymai · Rytinė eiga · Prekės · Gavimas · Tiekimas · Rinkiniai · Akcijos · Laiškai · | paieška · Žurnalas · vartotojas. `petshop-pragma` mu-plugin'e NĖRA. Skydelio „Sąskaita“ — pilkas mygtukas footer'yje (`disabled title="dar nepadaryta"`). Rolė `ps_darbuotojas` turi `manage_woocommerce`.

**v3.26 (Sąskaitos):** (1) **skydelyje blokas „Sąskaitos“** (`dokumentai()`: IAPV, AVPN, kreditinės iš refund meta, susietų paslaugų užsakymų (`_ps_pakartotinis`=šis) AVPN — „Pakartotinis siuntimas (#N)“ / „Siuntos grąžinimo išlaidos (#N)“; kiekvienas: nr · tipas · data · suma · PDF (GET `dok_pdf&t=avpn|iapv|kr` — srautas prieš užraktą, `Content-Disposition inline`) arba „PDF nėra — sugeneruoti“ (GET `dok_gen` → temos `petshop_generate_invoice_pdf($id)` + `_petshop_completed_pdf`; tik apmokėtam su AVPN numeriu; tema neliesta) arba „PDF nėra“). **Pilkas „Sąskaita“ mygtukas nuimtas** — bloką pakeitė (Claude prielaida, „mažiau, ne daugiau“). (2) **langas `view=saskaitos`** (`saskaitos()`): HPOS SQL `UNION ALL` (AVPN/IAPV iš `wc_orders_meta`+`wc_orders`+`wc_order_operational_data`+`wc_order_addresses`; KR iš refund'ų + tėvo), `DISTINCT`, filtrai — tipas (Visi / PVM sąskaita / Išankstinė / Kreditinė), nuo–iki (`get_gmt_from_date` ribos), paieška (nr., užsakymo nr., klientas, el. paštas), 200 psl.; lentelė Data · Nr. · Tipas · Užsakymas (#, būsena; nuoroda į darbalaukį `atidaryti`) · Klientas (vardas · el. paštas) · Suma (KR minusu) · PDF / „nėra — sugeneruoti“ (AVPN, processing/completed); apačioje dok. skaičius ir **suma pagal filtrą** (kreditinės minusu). (3) **Juosta v1.7:** dešinėje prie „Žurnalas“ — **„Sąskaitos“** (`view=saskaitos`, `manage_woocommerce`; „Užsakymai“ neaktyvus, kai `view` rytas/saskaitos). Pastaba: rolė `ps_darbuotojas` TURI `manage_woocommerce` → nuorodą mato ir darbuotojas (žalos nėra — tik PDF). **v3.27:** e6 T1 „Visi tipai“ → 0 dok. — SQL klaida `Unknown column o.date_completed_gmt` (HPOS datos `wc_order_operational_data`) → `LEFT JOIN od`. **v3.28:** #35434 skydelio antraštė rodė „37,03 € 0,00 €“ (WC `get_formatted_order_total` su refund'u) → `savi_refundai()` += `_ps_kreditine` (kaip `_ps_kiekis`): kreditinės refund'as — savas dokumentas, WC sumose nerodomas (ir klientui). **v3.29:** rasta S1616 klaida — `sumu_eilutes` trynė `refund_{id}`, o WC `get_order_item_totals` raktas `refund_{indeksas}` (`get_refunds()` eilė) → savų refund'ų eilutės „Grąžinimas“ klientui (paskyra/laiškai) vis tiek rodėsi; dabar pagal indeksą — #35414/#35434/#35436 `rows` be `refund_*` ✓.

**Testas (e6–e9):** `view=saskaitos`: visi **60 dok. · 1 486,38 €**; `t=kr` 3 (−56,63 €); `t=iapv` 7 (285,59 €; 2 su PDF, 5 „nėra“ — senesni); `q=35434` 2 (KR −37,03 + AVPN000303 be PDF → suma 0,00); `nuo=iki=šiandien` 7 (−21,42 €). PDF GET: KR `application/pdf` inline 34 707 B, IAPV `Isankstine-saskaita-IAPV000162.pdf` 36 310 B ✓. **dok_gen** #35450 (processing, AVPN000314 be failo) → 302 „PDF PVM-saskaita-AVPN000314.pdf sugeneruotas“, grįžo į `view=saskaitos`, failas 35 578 B ✓. Skydelio `dok`: #35414 [AVPN000283 (PDF nėra — sugeneruoti), KR-AVPN000102]; #35434 [AVPN000303 (cancelled — be „sugeneruoti“), KR-AVPN000101, AVPN000327 „Siuntos grąžinimo išlaidos (#35797)“]; #35436 [AVPN000305, KR-AVPN000103, AVPN000324/325/326 „Pakartotinis siuntimas (#35792/3/4)“, AVPN000328]. Playwright (admin `bdz487`): `s1617_e6_saskaitos.png` — juosta su „Sąskaitos“ (aktyvi), filtrų juosta, lentelė 60 eil.; `s1617_e6_saskaitos_kr.png`; `s1617_e6_skydelis_35434.png` — blokas „Sąskaitos“ (3 dok.), footer be pilko mygtuko. Skydelio antraštė po v3.28 „37,03 €“ (buvo „37,03 € 0,00 €“). 0 laiškų. TEMP 1 neaktyvus.

**Pastebėta (ne šio darbo, fiksuoti):** atšaukto apmokėto užsakymo skydelis sako „neapmokėta · … Neapmokėtas — laukiam kliento pinigų. Gavus pavedimą — „Pažymėti apmokėtu““ (`faktai()['paid']` = `is_paid()` → cancelled → false) — klaidina; taisyti `paid` = `is_paid() || date_paid` (liečia `faktai()` — atskirai, su testais). Paslaugų sąskaitose „Pristatymas: Nemokamas“ (šablonas).

**Testiniai po Sąskaitų:** #35450 gavo PDF `PVM-saskaita-AVPN000314.pdf` (dok_gen testas). Kitų pokyčių nėra.

**Lieka:** Pragma eksportas su kreditinėmis (`petshop-pragma` — mu-plugin'e nėra; rasti, klausti Raimio). IAPV PDF senesniems (5) — negeneruojami (IAPV daroma kasoje, užsakymas jau apmokėtas → generatorius duotų AVPN); palikta „nėra“.

#### S1617 — BŪKLĖ SESIJOS PABAIGOJE (2026-09-05 vakaras)
- **Gyva dev'e:** `petshop-darbalaukis.php` **v3.31** (md5 `cc5b9d53208b590e15022ee13b06d6a6`), `petshop-juosta.php` **v1.7** (`d4a86860…`), tema `woocommerce-delivery-notes/base.php` **v2.11** (`24ca0be7…`), kliento-siuntos v1.3, dev-pastas v1.0; variklis nekeistas. Paslaugų prekės #35790, #35796. Skaitikliai: AVPN 329, IAPV 163, KR-AVPN 104 (T-0 → 101). Eilės: Gauti 1 · Neišrūšiuoti 2 · Laukiam 1 · Surinkti 7 · Dropshipping 8 · Paruošta 0 · Klausimai 6 · Neapmokėti 0 · Visi 51. Dev-pastas 18. TEMP 1 neaktyvus.
- **Repo:** `deploy/petshop-darbalaukis.php` (+ `.b64`) v3.31, `deploy/petshop-juosta.php` v1.7, `deploy/wcdn-base.php` v2.11, `deploy/petshop-darbalaukis-v329-ATSARGA.php`; `irankiai/s1617_r1…r11`, `e1a…e13d`, `edit_v321…v326`, `v325/v326/v331_funcs`; `analize/s1617_*.json`; `screenshots/s1617_*.png`.
- **Dokumentai:** log v1.9.23 (šis), spec v1.11, **STARTAS `dokumentai/STARTAS_2026-09-05_5_etapas_9_grazinimas_A_po_S1617.md`** (kitam langui — pradėti nuo A).
- **Kitas langas:** A „Grąžinimas“ (ne dėl grįžusios siuntos) → B „+ Naujas užsakymas“ → C „Atsiėmimas AV“ (išjungtas) — sprendimai log „RAIMIO ATSAKYMAI“; C laiškui reikės adreso ir valandų iš Raimio. Po jų: Pragma (recon + klausti), 6 etapas (valymas, faktų `legacy` → `av` su leidimu, skaitikliai → 101, tikra darbuotojo paskyra).

#### S1617 — RAIMIO ATSAKYMAI (09-05 vakaras) + v3.31 / base.php v2.11
- **Sprendimai:** **A** (grąžinimas ne dėl grįžusios siuntos) — visi Claude siūlymai GERAI: mygtukas „Grąžinimas“ Klausimo „Klientas atsisako“ kortelėje ir įvykdyto užsakymo skydelyje → prekės/kiekiai + priežastis (atsisakymas / brokas / mūsų klaida) → kreditinė tuo pačiu mechanizmu → „grąžink X rankomis“; likutis — varnelė „tinkama prekybai“ (numatyta taip, brokas — ne); pinigai: atsisakymas — prekės, pristatymas tik grąžinant VISĄ užsakymą, 3,99 netaikom; brokas / mūsų klaida — viskas be išskaitymų. **B** („Naujas užsakymas“ telefonu) — GERAI: „+ Naujas užsakymas“ Užsakymų lango viršuje; klientas (paieška tel./el. paštas arba naujas, be el. pašto leidžiama), prekės (paieška, kiekis), pristatymas (Venipak kurjeris / paštomatas / LP / Atsiėmimas AV), kainos parduotuvės + nuolaida su pastaba, apmokėjimas — pavedimu (išankstinė + laiškas su rekvizitais/nuoroda, Paysera galima) arba „Apmokėta vietoje“. **C** („Atsiėmimas AV“) — mintis gerai, **PADARYTI VISĄ, BET IŠJUNGTĄ** — kasoje neatsiranda, kol Raimis pats neįjungia (vienas jungiklis). **10** — gerai, bet **tekstas turi būti taisomas šablonas**, kreditinę siunčia TIK darbuotojas, automatiškai ne. **11** paslėpti. **12** tinka. **13** Pragma — rasti (nepamena). **14** — „legacy“ anksčiau reiškė AV + Prins + Ambrosia + Kauno grūdai + Belacor kartu, paskui išskaidyta pagal sandėlius; legacy kaip tokio neliko → faktų kodas `legacy` yra pasenęs pavadinimas (AV). Kodo keitimas — variklis (`Fulfillment_Source`, faktai, `sandelio_kodas()`); Claude siūlo daryti 6 etape (testiniai faktai vis tiek trinami — migracijos nereikia), prieš tai — leidimas.
- **13 Pragma RASTA (r11):** `wp-content/plugins/petshop-pragma/petshop-pragma.php` (22 662 B, aktyvus), cron `petshop_pragma_monthly_export` kas mėnesį 5 d. 11:00 (kitas 2026-10-05), `petshop_pragma_log` (paskutinis 2026-05: 2 užsakymai), `petshop_pragma_known_emails`; production mode OFF pagal Raimį (REGISTRAS G8 — teisinga būsena). Kreditinių pridėjimas — recon + klausti prieš liečiant.
- **v3.31** (403 843 B, md5 `cc5b9d53208b590e15022ee13b06d6a6`; kopija `ps-backups/petshop-darbalaukis-v3302-BACKUP-2026-09-05.php`): kreditinės laiško **šablonas** — opcija `ps_dl_kr_laiskas` {tema, tekstas, mokestis}, vietaženkliai `{vardas} {kr_nr} {kr_suma} {uzsakymas} {grazinama} {mokestis} {mok_suma} {mok_avpn}`, tuščia eilutė = pastraipa (`kr_laiskas_sudeti`); redagavimas — Sąskaitų lango apačioje `<details>` „Kreditinės laiško šablonas (numatytas/pakeistas)“ (tik `manage_woocommerce`; POST `ps_dl_kr_sablonas`, „Išsaugoti“ / „Atstatyti numatytą“; tuščias laukas → numatytas). Siunčia tik darbuotojas — nepakeista. **Tema base.php v2.11** (25 286 B, md5 `24ca0be71f45f910f253e663c2fff210`; kopija `ps-backups/theme-wcdn-base-v210-BACKUP-2026-09-05.php`): sumose „Pristatymas: Nemokamas“ tik kai užsakymas turi pristatymo eilutę (`$has_shipping`) — paslaugų sąskaitose (3,99 €, pakartotinis) eilutės nebėra.
- **Deploy e13d su naujais sargais:** `php -l` stabdiklis skripte; eilių skaičiai prieš (Gauti 1 · Neišrūšiuoti 2 · Laukiam 1 · Surinkti 7 · Dropshipping 8 · Paruošta 0 · Klausimai 6 · Neapmokėti 0 · Visi 51) = po; ping po įrašymo 200 be Fatal (kitaip atkuriama v3.30.2 + base v2.10 toje pačioje užklausoje). **Testas T4:** forma yra („numatytas“), POST išsaugoti testinį šabloną → opcija, „siųsti klientui“ #35434 → dev-pastas tema „TEST šablonas: kreditinė KR-AVPN000101 · užsakymas 35434“ (2 priedai) ✓, „Atstatyti numatytą“ → opcija ištrinta ✓; AVPN000327 PDF regeneruotas — be „Pristatymas“ eilutės ✓ (peržiūrėta). Dev-pastas 18. 0 išėjo.

#### S1617 — ⛔ INCIDENTAS 2026-09-05 ~14:05: DEV NUKRITO (v3.30 deploy su PHP 8 parse klaida)
- **Kas:** `petshop-darbalaukis.php` v3.30 (`paid` = `is_paid()||date_paid`, uždaryto pastaba, „Kaip mato klientas“) eil. 861 — įdėtas ternaras be skliaustų (`a ? b : c ? d : e`) → PHP 8 **compile fatal** kiekvienam WP užklausimui (mu-plugin). Lokalus `php -l` KLAIDĄ RADO, bet mano komandų grandinė (`php -l && … ; node … && …`) po nesėkmės vis tiek pastatė payload'ą ir paleido deploy; serverio sargas `token_get_all(TOKEN_PARSE)` tokios klaidos NEGAUDO (tik leksines). e10d D: `md5/token ok`, po įrašymo T3 → „Fatal error … line 861“, WP „Klaida“ puslapis. Bridge (REST + init) neveikia — WP nepakyla.
- **Atkūrimas (Raimis, hostingo failų tvarkytuvė / FTP):** nukopijuoti `/home/gyvunai2/domains/petshop.lt/public_html/wp-content/uploads/ps-backups/petshop-darbalaukis-v329-BACKUP-2026-09-05.php` → `/home/gyvunai2/domains/petshop.lt/public_html/wp-content/mu-plugins/petshop-darbalaukis.php` (v3.29, md5 `d067b40cfbfd4ad520ddffd961b4af15`, testuotas) ARBA įkelti repo `deploy/petshop-darbalaukis.php` (**v3.30.1**, pataisyta, `php -l` OK, md5 `c520a4c9beb8e70bfbc6906960f55886`). Po atkūrimo — Claude per bridge patikrina versiją ir (jei atkurta v3.29) įdiegia v3.30.1.
- **Atkūrimas 2026-09-05 ~14:50 (Raimis ištrynė failą → WP pakilo be darbalaukio, senas desk) → bridge veikia → e11d D: v3.30.1 įdiegta su NAUJU SARGU** (po `file_put_contents` — `wp_remote_get(admin-ajax heartbeat)`: 200, be „Fatal error“ → paliekama; kitaip failas ištrinamas atgal toje pačioje užklausoje) — ping OK, `Petshop_Darbalaukis::VERSIJA` 3.30.1, skydeliai OK (#35434 „apmokėta · Užsakymas atšauktas — buvo apmokėtas…“, #35799 „Užsakymas įvykdytas.“), Sąskaitos 60. **BET eilių regresija:** Klausimai 6 → 3, Neišrūšiuoti 2 → 4, Surinkti 7 → 8 — `faktai()` eilutėje mano `// v3.30` komentaras array VIDURYJE nurijo `'kl' => …, 'vez' => …` (PHP „Undefined array key kl/vez“ — klausimas ir vežėjas neapibrėžti; `php -l` to nemato). **v3.30.2** (398 536 B, md5 `9cc3a2da6a86a9177e6d57135425b78e`; e12d, kopija `ps-backups/petshop-darbalaukis-v3301-BACKUP-2026-09-05-*.php`) — komentaras `/* */`, raktai grįžo: Klausimai 6 (35436, 35100, 35414, 35423, 35434, 35439), Neišrūšiuoti 2, Surinkti 7, Dropshipping 8, Visi 51 — kaip prieš. Repo `deploy/petshop-darbalaukis.php` = v3.30.2.
- **Papildoma taisyklė:** po KIEKVIENO darbalaukio deploy tame pačiame run'e lyginti eilių skaičius prieš/po (Gauti · Neišrūšiuoti · Laukiam · Surinkti · Dropshipping · Paruošta · Klausimai · Neapmokėti · Visi) — bet koks poslinkis be paaiškinimo = regresija. Redaguoti tik VISĄ eilutę/bloką, niekada nekišti `//` į eilutės vidurį.
- **Pamoka (į taisykles):** (1) `php -l` — PRIVALOMAS stabdiklis: payload'ą statyti tik iš skripto, kuris po nesėkmingo `php -l` NUTRAUKIA (`subprocess` + `sys.exit`), ne shell grandinė; (2) serverio `token_get_all` ≠ sintaksės patikra — prieš `file_put_contents` PRIDĖTI antrą sargą: rašyti į laikiną failą `ps-backups/*.lint.php` ir bandyti `include` ne mu-plugins kontekste NEGALIMA (klasė persideklaruotų) → vienintelis tikras sargas — lokalus `php -l` su stabdikliu + `token_get_all`; (3) po deploy — TAME PAČIAME run'e antra HTTP užklausa į `/?ps_ping` prieš bet ką kitą; nepavykus — automatinis atkūrimas iš kopijos TOJE PAČIOJE deploy užklausoje neįmanomas (kita užklausa jau nepakyla) → deploy payload'as turi PATS po `file_put_contents` daryti `wp_remote_get` į savo puslapį ir, gavęs „Fatal error“, atkurti kopiją (deploy užklausa dar veikia su senu kodu). ĮDIEGTI kitam deploy.

---

### S1616 (2026-09-04, naktis → 2026-09-05)

> Pridėti po šio antraštės (virš S1615). Tema: **5 etapas #4 „kiekiai“ — B modelis (spec §12.5), darbalaukis v3.18.1 → v3.19 → v3.19.1.** Raimis: „darom“ (patvirtino apimtį: mažinti / išimti dabar, „pridėti“ per „naują mažą užsakymą + nuoroda“ vėliau). Variklis (registras A–J, `Petshop_Desk`, `Petshop_AV_Reduce`, `Petshop_AV_Tiekimas`, `Petshop_Fakt_Grazinimai`, WC refund f-ja, tema `functions.php`/`base.php`) NELIESTAS. Bridge: `irankiai/s1616_e1r/e2r.php` (recon), `s1616_e3a/e3d.php` (v3.19 deploy+testai), `s1616_e4a/e4d.php` (v3.19.1 deploy+patikra+„Grąžinta“), `s1616_e5z.php` („Grąžinta“ per tikrą kortelės nuorodą + skydelio forma). `analize/s1616_*.json`, `screenshots/s1616_e3_*, e4_*, e5_skydelis_35414_forma.png`.

#### S1616 — RECON (e1r/e2r, tik skaitymas; spec §6c, §11.3, §12.5)
- **WC 11.0.1 `wc_create_refund($args)`** (`wc-order-functions.php`): `line_items[$item_id]={qty, refund_total (BE PVM), refund_tax=[rate_id=>suma]}` → neigiama refund eilutė su meta `_refunded_item_id`; `refund_payment=false` (Paysera nekviečiama), `restock_items=false` (patys tvarkom likutį). Po `save()` kabliukas `woocommerce_order_refunded` (prior. 35) → `Petshop_Fakt_Grazinimai::rasyti($order_id,$refund_id)` skaito refund eilutes (kiekis, suma_ct, pvm_ct, savikaina iš fakto eilutės, `atsargos_grizo` iš `_restock_refunded_items`). Faktų korekcijos API nėra — grąžinimas = naujas faktas. `Fakt_Grazinimai::rasyti` — vienintelis kabliukas ant `order_refunded`.
- **WC kiekio rodymas** (`order-details-item.php` / `email-order-items.php`): jei `get_qty_refunded_for_item()` ≠ 0 → `<del>q</del> <ins>q−ref</ins>`; totaluose `add_order_item_totals_refund_rows()` prideda „Refund −X“ eilutę iš `get_refunds()`. Filtrai: `woocommerce_order_item_quantity_html`, `woocommerce_email_order_item_quantity`, `woocommerce_get_order_item_totals`, `woocommerce_get_formatted_order_total` — visi tušti (be temos override). Tema `base.php` sąskaitą skaičiuoja iš `get_items()`/`get_subtotal()`/`get_shipping_total()`/`get_total_tax()`/`get_total()` (po eilutės perrašymo — teisinga savaime; refundų neskaito). Snippet #653 (completed) regeneruoja PDF tuo pačiu AVPN iš esamų sumų.
- **Parašai:** `AV_Stock::qty($pid)` / `increase($pid,$by,$priez=?)` / `decrease(...)`, `AV_Dropship::perduotos($o)`, `AV_Tiekimas::ideti_eilute($o,$iid,$src=?)` / `isimti_eilute($o,$iid)` / `eilutes_bukle($order_id,$item_id)`, `Fakt_Grazinimai::rasyti($order_id,$refund_id)`, `Uzsakymu_Ivykiai::irasyti($a)`. `ps_fakt_grazinimai` stulpeliai: uzsakymas_id, refund_id, wc_eilutes_id, kiekis, suma_ct, pvm_ct, savikaina_ct, atsargos_grizo, sandelis, testinis, …

#### S1616 — `mu-plugins/petshop-darbalaukis.php` v3.18.1 → **v3.19 → v3.19.1** (300 706 B, md5 `c1a278f17c0dfd17e869bdf496d50b64`; v3.19 `29506f10…` 300 399 B; kopijos `ps-backups/petshop-darbalaukis-v3181-BACKUP-2026-09-04.php`, `-v319-BACKUP-2026-09-04.php`; repo `deploy/petshop-darbalaukis.php` + `.php.b64`). Deploy — **gzip+b64 2 dalimis** (v3.18.1→368 KB b64 su 5 dalimis; nuo v3.19 failas gzip'inamas prieš b64 → 111 KB, dalis 1 = 80 000 į `ps-backups/dl-vXXXX.part1` su md5 sargu, dalis 2 payload'e; `gzdecode` + bendras md5 + `token_get_all` + gyvo failo md5 sargas). Testai naujame procese, realūs `admin-post ps_dl_kiekis` / `ps_dl_veiksmas v=grazinta` kaip `testuotojas`; laiškai — `pre_wp_mail` + dev-pastas (0 išėjo); Venipak/LP nekviesti (refund `refund_payment=false`, Paysera nekviesta).

**#4 „kiekiai“ — B modelis (spec §12.5).** Skydelio eilutės „q×“ tampa spaudžiamas (`a.dl-kk`), kai eilutė be užrakto (tas pats `lock` kaip keliui: nėra lipduko, tiekėjui nepranešta, kaupiama partija ne užsakyta, nesurinkta) IR užsakymas apmokėtas IR q≥1 (`kiekio_gal()`). Paspaudus eilutėje atsiveria forma `Kiekis [n] × iš q · Išsaugoti · Išimti · atgal` (JS; `q<2` → „Kiekis/Išsaugoti“ užrakinti, lieka tik „Išimti“); prieš POST — dialogas su tekstu (kiekio / išėmimo variantas). **Eiga (`kiekis_vykdyti`, POST `admin_post_ps_dl_kiekis`, nonce `ps_dl_kiekis_{id}`, lock):** (1) likutis grįžta — AV per `likutis()` pagal `_ps_av_reduced_qty` (perrašomas/nuimamas), WC veidrodis per `wc_update_product_stock` pagal `_reduced_stock` (perrašomas), `_restock_refunded_items += d` (faktui „atsargos grįžo“); (2) `wc_create_refund` su eilute (`refund_payment=false`, `restock_items=false`, refund meta `_ps_kiekis=1`) — `laiskai_off`; **nepavykus — likutis ir žymės atstatomi, niekas nekeista**; (3) eilutė perrašoma (`set_quantity`, proporcingos `subtotal`/`total`/`taxes`, `_ps_kiekis_keistas`, `update_taxes()` + `calculate_totals(false)` — v3.19.1 taisymas, žr. žemiau; pristatymas nekinta) ARBA išimama (`remove_item`; vienintelė prekė → klaida „naudok Atšaukti“); kaupiama Tiekimo partija — eilutė perdedama (`isimti_eilute`+`ideti_eilute`); grupės/planas perskaičiuojami; `_ps_surinkta` nuimama; (4) žymė `_ps_grazinti_rankomis` (`[{laikas,kas,suma,refund,ka}]`) → **Klausimas „Grąžink klientui pinigus“** (kortelė su suma + mygtukas „Grąžinta“ = `v=grazinta`, nuima žymę; galioja ir įvykdytam/atšauktam — `atviri()` juos įtraukia per `GRAZINTI_META EXISTS`), pastaba, įvykis `kiekis` (eilutė, prieš/po). Klientui laiško nėra. Partijos — rankomis (pastaba). „Pridėti prekę“ — vėliau per „naują mažą užsakymą + nuoroda“.
**WC refundą po perrašymo rodytų dukart** (eilutė jau perrašyta): 4 filtrai nuima `_ps_kiekis` refundus — `kiekio_html` / `kiekio_laiske` (`_ps_kiekis_keistas` eilutė rodoma be `<del>`), `sumu_eilutes` (išima `refund_{id}` totalų eilutę), `suma_be_savu` (formatuota suma be savų refundų). **Dokumentuota pasekmė:** WC `get_remaining_refund_amount()` po keitimo mažesnis už realų (#35425 −21,23) — WC admin refundų nenaudojam; kreditinė (5 etapo „Sąskaita“) skaičiuos iš savų sumų.

**v3.19 → v3.19.1 (e3d radinys):** po `set_quantity` vien `calculate_totals(false)` PVM iš eilučių NESUMUOJA → #35414 rodė 19,49 € (PVM 3,75 vietoj 3,30). Taisymas: `update_taxes()` PRIEŠ `calculate_totals(false)`. Testinių #35414/#35425 sumos pertaisytos e4d D fazėje.

**Testas (e3d/e4d/e5z, D/V/Z/Y + Playwright):**
- **#35414** (MIXED, apmokėtas Paysera): eil. 781 Animonda (AV, `_ps_av_reduced_qty=2`) **2 → 1** → 302 „kiekis 2 → 1 · grąžink 2,60 € · AV +1 → 19“; po v3.19.1: `total` **19,04 €** (sub 13,96 + pristatymas 2,15 su PVM + PVM 2,93+0,37), eil. 781 q=1 `_ps_kiekis_keistas`, `_reduced_stock`=0→`_restock_refunded_items=1`, refund #35788 (−1, −2,15, `_ps_kiekis=1`), faktas `ps_fakt_grazinimai` #4 (kiekis 1, suma 215 ct, pvm 45 ct, sav 170 ct, atsargos_grizo 1); **likutis 19708 `_stock` 18 → 19**; Klausimas „Grąžink klientui pinigus“ (kortelė 2,60 €); paskyra 5787 (view-order 35414) — kiekis be `<del>`, be „Refund“ eilutės; completed laiško eilutės — „×1 … 2,60 €“, be `<del>`.
- **#35425** (MIXED): eil. 819 Ambrosia (tiesiai, `_reduced_stock=2`) **išimti** → 302 „išimta · grąžink 28,58 € · WC veidrodis +2 → 14“; eilutė pašalinta (`remove_item`), liko tik Animonda, `type` MIXED→MAIN, `ship_n` 2→1; `total` **7,35 €**; refund #35789 (−2, `_ps_kiekis=1`), faktas #5 (kiekis 2, suma 2362 ct, pvm 496 ct, sav 1554 ct); **likutis 19756 product_qty +2 → 14**; „Grąžinta“ (e5z, per tikrą kortelės nuorodą, nonce kaip UI): 302 „pažymėta: grąžinta 28,58 € — Klausimas nuimtas“, žymė nuimta, skydelis „Neišrūšiuoti“, įvykis `grazinta`, pastaba; pakartotinai → „grąžintinų pinigų nebėra“.
- **Neigiami:** #35421 eil. 806 (kaupiama partija #10 užsakyta) → „kiekio keisti negalima — jau užsakyta iš Quattro (užsakymas tiekėjui #10)“; #35423 eil. 813 „Išimti“ → „tai vienintelė prekė — naudok „Atšaukti““; #35423 eil. 813 n=5 (q=3) → „blogas kiekis — dabar 3, galima 1…2 arba „Išimti““. Nė vienas nesukūrė refundo/pakeitimo (35421 3 eilutės/0 refundų, 35423 q813=3/0 refundų).
- **Playwright** (e5z, palaukus `#skEil`): #35414 skydelyje `kk=2` (abi eilutės spaudžiamos), atverta forma (`n`, „Išsaugoti“, „Išimti“, „atgal“; Animonda po keitimo `1×`, todėl „Kiekis/Išsaugoti“ užrakinti — lieka „Išimti“), Klausimas „Grąžink klientui pinigus“, `kur` „Klausimai · toliau: Spręsti“. Klausimų kortelės (`s1616_e4_klausimai`): #35414 „Grąžink klientui 2,60 €“, #35425 „Grąžink klientui 28,58 €“, abi su „Grąžinta“ (grazinta=2). Ekrano nuotraukos peržiūrėtos.
- **Laiškai 0** (pre_wp_mail + dev-pastas 5→5). TEMP snippet'ai trinami run'o pradžioje; e5z pabaigoje `temp_liko=1` (aktyvus paties run'o snippet'as, deaktyvuojamas mjs pabaigoje — norma).

**Kaip mato darbuotojas (S1616):** skydelyje prekės „q×“ dabar spaudžiamas (kol nėra lipduko / tiekėjui nepranešta / nesurinkta) — atsiveria „Kiekis [n] · Išsaugoti · Išimti“; sumažinus arba išėmus prekę: likutis grįžta pats, lapas ir laiškai rodo naują kiekį, o Klausimuose atsiranda „Grąžink klientui pinigus“ su suma — grąžinęs per Paysera/pavedimu, spaudi „Grąžinta“ ir Klausimas dingsta. Vienintelės prekės išimti negalima (naudok „Atšaukti“). Klientui laiškas nesiunčiamas. **Klientas:** paskyroje/laiške mato tik galutinį kiekį (be perbrauktų senų).

**Testiniai po S1616 (trinti 6 etape):** #35414 processing (eil. 781 q=1 `_ps_kiekis_keistas`, refund #35788 `_ps_kiekis`, `_ps_grazinti_rankomis` LIEKA — Klausimas „Grąžink 2,60 €“), total 19,04; #35425 processing (eil. 819 išimta, liko 820, refund #35789, `_ps_grazinti_rankomis` NUIMTA e5z — „grąžinta“), total 7,35, type MAIN; likučiai **19708 `_stock` 18 → 19, 19756 product_qty 13→14** (own 1 + stock 13). Kiti nepakito. Dev-pastas 5.

#### S1616 (tęsinys) — `petshop-darbalaukis.php` v3.19.1 → **v3.20** (306 853 B, md5 `f4d32aefb19a27b3db85bb464b6b82e9`; kopija `ps-backups/petshop-darbalaukis-v3191-BACKUP-2026-09-04.php`; repo `deploy/`). Tema: **spec §12.5 „Siunta grįžta“ kortelės sumos.** Raimis 09-05: (1) sumos kortelėje; (2) 3,99 nuo grįžusios dalies ar nuo visos — „skirtumo nėra, suma nedidelė“; (3) šiame žingsnyje tik suma, pavadinimas **„Pakartotinis užsakymas“** (ne „pristatymas“). Bridge: `irankiai/s1616_e6r.php` (recon), `s1616_e7a/e7d.php` (deploy + sim + patikra), `s1616_e8z.php` (atšaukimai per tikras nuorodas + Y). `analize/s1616_e6r/e7d/e8z.json`, `screenshots/s1616_e7_klausimai_sumos.png`, `s1616_e8_klausimai_po.png`.

**Recon (e6r):** pristatymo zona „Lietuva“: Venipak paštomatas inst 3 `fee` 1,78 (+PVM = 2,15; nemokamas nuo 30 €), Venipak kurjeris inst 2 `fee` 3,30 (≤50 kg → 3,99), inst 6 6,60 (50–70), inst 7 9,90 (70–100), inst 8 20,65 (100–200); LP Express terminal inst 12/13 `fixed_cost` „1,78“; LP kurjeris inst 15 pagal svorį. Užsakymo pristatymo eilutė: `method_id`/`instance_id`/`cost`/`tax` (testiniuose ir `flat_rate` be instance — S1614 „Venipak kurjeris“ 3,49). `Petshop_Desk::vezejas()` — lp / venipak_pastomatas / venipak_kurjeris / kita.

**Padaryta:** `GRAZINIMO_MOKESTIS = 3.99` (su PVM). `grizta_sumos($f)`: **visa siunta grįžta** (kitų aktyvių dalių nėra) → sumokėta = `get_total()` (su pristatymu); **dalis** → tik grįžusios dalies prekės su PVM (pristatymas lieka su likusia dalimi — Claude prielaida pagal „skirtumo nėra“); X = sumokėta − 3,99. `pristatymo_ikainis($o,$vez)`: pasirinkto metodo instance `fee`/`fixed_cost` (+PVM pagal užsakymo pristatymo PVM santykį, 0,21 jei nemokamas) → kitaip kaip sumokėta užsakyme → kitaip 2,15 paštomatas / 3,99 kurjeris → kitaip „įkainį nustatyk pats“; Y = įkainis + 3,99. Klausimų kortelėje „Siunta grįžta“ — naujas `<p class="dl-sumos">` po tekstu: „Klientui grąžink: **S − 3,99 = X €** (bazė)“ ir „Pakartotinis užsakymas: **įkainis + 3,99 = Y €** (paaiškinimas; siunčiama tik apmokėjus)“. Dialogai „Atšaukti…“ sako tikslią sumą. **`grizta_atsaukti` (visas ir dalinis) įrašo `_ps_grazinti_rankomis` (suma X, `refund` 0, `ka` „siunta … grįžo → atšaukta: S − 3,99“)** → po atšaukimo perima Klausimas „Grąžink klientui pinigus“ (#4 mechanizmas; `atviri()` įtraukia ir atšauktą) — spec §12.5 kreditinės pastraipos „pastaba + Klausimas, kol nepažymėta“. Pastabos ir 302 žinutės su suma. Naujų mygtukų nėra. Kortelės „Grąžink“ suffix'as „Užsakymas atšauktas — likusią sumą…“ nuimtas (klaidintų).

**Testas (e7d D/T, e8z Z/Y; Venipak nekviestas — grįžta simuliuota meta `_ps_siunta_grizta`):**
- **#35434** (visas, AV dalis, kurjeris inst 2, total 37,03): kortelė „Klientui grąžink: 37,03 − 3,99 = **33,04 €** (visa siunta grįžta — sumokėta su pristatymu) · Pakartotinis užsakymas: 3,99 + 3,99 = **7,98 €** (VENIPAK Kurjeris įkainis)“; dialogas „grąžink klientui 33,04 € rankomis (37,03 − 3,99; Klausimas primins)“. „Atšaukti — prekės grįžo į AV“ per tikrą nuorodą → 302 „atšauktas … grąžink klientui 33,04 € rankomis (Klausimai)“; `cancelled`, variklis grąžino AV (**19708 `_stock` 19 → 20** „grynai AV +1“, `_ps_av_restored`), WC veidrodis **16889 `_stock` 12 → 13**; žymė `[{suma 33.04, refund 0, ka „siunta V…777 grįžo → atšaukta: 37,03 − 3,99“}]`; Klausimų kortelė atšauktam užsakymui „Grąžink klientui 33,04 €“ su „Grąžinta“ (be „Atšaukti“).
- **#35436** (MIXED: Exclusion AV + Quattro tiesiai; `flat_rate` 3,49+0,73; total 23,61): kortelė „17,00 − 3,99 = **13,01 €** (tik grįžusios dalies prekės; kita dalis lieka: AV) · Pakartotinis užsakymas: 4,22 + 3,99 = **8,21 €** (kaip sumokėta užsakyme)“. „Atšaukti tik grįžusią dalį“ → 302 „Quattro dalis atšaukta … AV +1 → 1 · kita dalis lieka: AV · grąžink klientui 13,01 €“; **16727 `_own_stock_qty` 0 → 1**, eil. 857 `_ps_atsaukta`, `_ps_dalys_atsaukta.quattro`, processing liko; žymė 13,01; skydelis „Klausimai · Grąžink klientui pinigus“; kortelė su „Grąžinta“ + „Atšaukti“.
- Playwright `s1616_e7_klausimai_sumos.png` (abi kortelės su sumomis) ir `s1616_e8_klausimai_po.png` (po atšaukimų — trys „Grąžink“ kortelės: #35436 13,01, #35414 2,60, #35434 33,04) peržiūrėtos. Laiškai 0 (pre_wp_mail, dev-pastas 5→5).

**Kaip mato darbuotojas:** „Siunta grįžta“ kortelėje iškart matosi „Klientui grąžink: … = X €“ ir „Pakartotinis užsakymas: … = Y €“ — skaičiuoti nereikia; atšaukus grįžusią siuntą, Klausimas „Grąžink klientui X €“ lieka, kol paspaus „Grąžinta“.

**Testiniai po S1616 (2):** #35434 **cancelled** (žymė 33,04, Klausimas „Grąžink“); #35436 processing, Quattro dalis atšaukta (žymė 13,01, Klausimas „Grąžink“); likučiai **19708 `_stock` 20, 16889 `_stock` 13 (own 1), 16727 `_own_stock_qty` 1**. Ankstesni S1616: #35414 (Klausimas 2,60), #35425 (grąžinta).

**Lieka:** „Pakartotinis užsakymas“ — naujas mažas užsakymas (įkainis + 3,99, tas pats klientas/adresas, nuoroda į pradinį) + apmokėjimo nuoroda + po apmokėjimo „Siųsti iš naujo“ (spec §12.5; pirmas „Naujas užsakymas“) → kreditinė pusiau automatinė (recon WCDN creditnote) → „Sąskaita“ langas → Pragma su kreditinėmis (klausti). J1, Venipak 4/5/7/8, V12/V13/V14 variklis, T10/V1 — kaip S1614.
