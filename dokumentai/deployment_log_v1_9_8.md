...
#### S1607 — ETAPAS 2: `mu-plugins/petshop-darbalaukis.php` v2.3 (127 066 B, md5 `61c3ce3479f523544d598644bdd4759a`; repo `deploy/petshop-darbalaukis.php` + `.b64`)
...
**Nauja meta:** užsakymo `_ps_rusiuota` (laikas|vartotojas | `auto`), `_ps_dalys_issiusta` (JSON), `_ps_surinkta`, `_ps_sekimo_siusta` (dubliuoja siuntų-laiškų žymę — V12); eilutės `_ps_kelias` (av|tiesiai|i_av — ketinimas; rodomas kelias iš `_ps_source`+partijos). Žurnalo veiksmai: `kelias`, `rusiuoti`, `issiusta`, `issiusta_dalis`, `nesurinkta`.
