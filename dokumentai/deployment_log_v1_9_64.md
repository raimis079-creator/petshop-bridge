...
| 9 | **Kelio taisyklė (§17.2) PAKEISTA** — `petshop-av-source.php` v1.3 (`7d8e67eb…`), `petshop-av-order.php` v1.2 (`fd7ed335…`), `petshop-gavimo-perrusiavimas.php` v1.2 (`d7f60f74…`), bak `.bak_s1676b` | Raimis: „dropship prekės eina iš dropship sandėlių pirmiausia; jei jie neturi ar užsakymas mišrus — AV.“ Nauja `parinkti($pid,$qty,$misrus)` + `ar_misrus($order)` (mišrus = yra grynai AV prekių); `resolve()` NEKEISTA — ji atsako „ar AV turi“ (darbalaukio/katalogo faktas, rankinis „→ AV“ veikia), `parinkti()` — „iš kur siųsti“. Tiekėjo likutis iš `Petshop_Sources::gyvi`; nežinomas = laikom, kad turi. Testai: Josera vienas → VF (2 089), Josera+AV prekė → MIXED (Josera VF, nes AV neturi), Quattro (AV 2, tiek. 49) vienas → Quattro, mišrus → AV, Tofu pakas (Belacor 0) → AV. Rankinis permetimas (`keisti_kelia`) patikrintas — likutis seka kelią, nekeista. |
