...
- **Klausimas Raimiui (sprendimas):** ar darbuotojai (Inga) turi matyti pinigines ataskaitas („Pardavimai ir pelnas“, „Mėnesio uždarymas“, „Klientų analizė“)? Dabar — taip (rolės `manage_woocommerce`, kaip ir patys puslapiai). Jei ne — atskira teisė `ps_ataskaitos` rolei / punktams (juosta + puslapių `capability`).
