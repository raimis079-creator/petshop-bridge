...
#### S1620 — 6 ETAPAS: RECON (e6r R1/R2 + e6r3; tik skaitymas, NIEKAS netrinta)
- **Užsakymai DB (HPOS + sync ON):** 64 `shop_order` (cancelled 14 · completed 20 · on-hold 3 · processing 27 = darbalaukio „Visi 64“) + 8 `shop_order_refund`. **Testiniai 60** (#35414–#35444, #35450, #35771–#35772, #35774–#35780, #35791–#35795, #35797–#35813; visi terra@petshop.lt / s1609.klientas@avesa.lt / *@dev.avesa.lt / terra@gyvunai.lt (#35441/35442, customer 1)) + **Raimio 4** (#35087 cancelled, #35088 completed, #35090, #35100 processing; 08-25/26, terra@gyvunai.lt, AVPN000272–275, IAPV000155–158, 59 pastabos) — ne darbalaukio testai, **klausti**. Refund'ai: 35788 (←35414, kiekis), 35789 (←35425, kiekis), 35795 (←35434), 35798 (←35436), 35800 (←35777) + 3 senesni. **NE užsakymai sąraše:** #35773 nėra; **#35781–#35784 — PREKĖS** (09-04: „Konservų dėžė 400/800 g Įvairovė“, 2 TRIXIE draskyklės draft) — kito projekto, **NELIESTI**; #35785–#35787 revizijos. Paslaugų prekės #35790/#35796 (`ps_pakartotinio_preke`/`ps_grazinimo_preke`) — NETRINTI. Pastabų (comments) testiniams — 752.
- **Susijusios lentelės (order_id):** `ps_fakt_uzsakymai` 67 (56 testinių + Raimio 4 + 7 našlaičiai 35240/35262–35266/35288 — seniai ištrintų), `ps_fakt_eilutes` 109 (84 + 24 + 1), `ps_fakt_grazinimai` 11 (8 + 35088/35090×2), `ps_fakt_siuntos` 25 (visi testiniai), `ps_shipments` 25 (visi), `ps_uzsakymu_ivykiai` 630 (303 testinių + 35090 ×4 + `0` ×3 + po 1 keistiems ID 35457…35766 — palikti), `ps_tiekimas` #10–#15 + `ps_tiekimas_eil` 5 (visi testiniai, 09-02/03), `ps_refill_tracking` #42–#48 (last_order_id testiniai; user 1 ir 5787), `ps_plan_events` order_id 0 (palikti). WC: `wc_order_stats` 76 (65 test + 3300–3304, 35087–35100, 35091–35093 našlaičiai), `wc_order_product_lookup` 96/126, `wc_orders_meta` 1580/1755, `woocommerce_order_items` 217/263, `wc_customer_lookup` #85–#96 testiniai svečiai + #86 = 5787. **`testinis`=1 — VISOSE faktų eilutėse** (67/67, kainos 1351, atsargos_d 31 765, web_ivykiai 1104) — tai aplinkos žymė (dev), ne testinio užsakymo; `fakt_eilutes.sandelis`: legacy 51 · vf 29 · prins 10 · ambrosia 9 · zb 5 · quattro 4 · belcor_tofu 1 (→ „legacy → av“ — su leidimu). `ps_partijos` (#1–#19, rugpjūtis, 34889 „TESTAS naršyklė“) — partijų projekto, ne darbalaukio — palikti. `ps_dim_klientai` / `ps_kl_suvestine` — perskaičiuojami cron (`ps_dim_klientu_perskaiciavimas` 20:40, `ps_klientai_perskaiciuoti` 02:30) — po faktų valymo susitvarko patys.
- **Failai `uploads/wcdn/`:** invoice 91 (AVPN 283–341 + IAPV 160–167 (5 PDF) + WCDN „Invoice -354xx-…“ testiniams + seni „Invoice -35262…35308“ ištrintiems), creditnote 4 (KR-AVPN 101–104), receipt 2 (PPK 101–102). Raimio 4: `PVM-saskaita-AVPN000273.pdf`.
- **Skaitikliai:** `petshop_avpn_counter` 342, `petshop_iapv_counter` 168, `petshop_kravpn_counter` 105, `petshop_ppk_counter` 103 → visi 101 (T-0 sąrašas; galima anksčiau, kai užsakymų nelieka). `wcdn_invoice_number_counter` 364 (WCDN savas, AVPN nenaudoja — palikti, nebent Raimis kitaip).
- **Likučiai:** 19708 Animonda (legacy, `_stock`) **16**; AV žurnale paskutinė teisėta `_stock` reikšmė **48** (2026-08-07 gavimas 43→48), po to tik testų nurašymai per pastabas (žurnale nėra) → grąžinti į 48 (**Raimis tvirtina / pasako tikrą**). 16727 Quattro `_stock` 6 / AV 1, 16889 Prins `_stock` 14 / AV 1, 19756 Ambrosia `_stock` 14 / AV 1 — AV žurnale įrašų nėra, pradinės AV reikšmės nežinomos (**Raimis pasako**). 35357 HAP (VF) `_stock` 6 = `_vf_qty` 6 — **VF sync kas val. atstato pats** (buvo 4 po S1619, dabar 6) ✓.
- **Vartotojai:** `testuotojas` #75 (`ps_darbuotojas`, testuotojas@dev.avesa.lt); testinis klientas #5787 (s1609klientas, 6 užs.); tikri darbuotojai — tik #1 bdz487 (Raimis, administrator). #5757–#5785 (08-31 15:35) — tikri klientai iš importo, neliesti. **Opcijos testinės:** `ps_s1617_test`, `ps_audit_ids` (35414–35434), `ps_audit_mail` (3), `ps_e3_oid`=35450, `ps_e3_oid2`=35771, `ps_dev_pastas_zurnalas` (42); `ps_test_product_id`=34889 (partijų projekto — palikti). Cron su užsakymais: `ps_venipak_sekimas`, `ps_velavimo_laiskai`, `ps_fakt_siuntu_sutikrinimas`, `ps_dropship_sargas` — po valymo dirbs su tuščiu (norma).
- **Kaskadų nėra:** WC `$order->delete(true)` (HPOS + sync) trina wc_orders/meta/addresses/operational/items/itemmeta + placeholder post + pastabas; WC Analytics lookup'ai — per `woocommerce_delete_order` (Action Scheduler) arba rankiniu DELETE; **mūsų lentelės, PDF, opcijos, skaitikliai, likučiai, refill, customer_lookup — tik rankomis, kiekvienas su savo sargu ir skaičiais prieš/po.** `shell_exec` nėra → mysqldump negalimas → prieš trynimą: (1) Raimis daro hostingo DB atsarginę kopiją (valdymo skydas), (2) payload'as eksportuoja trinamų eilučių JSON į `ps-backups/valymas-2026-09-06/` (užsakymai su items/meta/pastabomis, faktų/shipments/įvykių eilutės), PDF perkelia (ne trina) į tą patį katalogą.
- **Planas (Raimiui tvirtinti — STARTAS „6 ETAPAS — SPRENDIMAI“):** V0 hostingo backup (Raimis) → V1 eksportas į ps-backups → V2 refund'ai (8) → V3 užsakymai (60 [+4]) po vieną su patikra → V4 lentelės (faktai, shipments, įvykiai, tiekimas #10–15, refill #42–48, WC lookup'ai/našlaičiai, customer_lookup #85–96) → V5 PDF perkėlimas → V6 opcijos → V7 skaitikliai → 101 → V8 likučiai (19708 → 48, AV — Raimio) → V9 vartotojai (5787; testuotojas — po E2E) → V10 dim/kl perskaičiavimas (cron arba rankinis) → Q eilės 0/0/…/Visi 0, Playwright. **E2E** (darbuotojo dienoraštis visam keliui) — Claude siūlymas: PRIEŠ valymą (dar 1–2 užsakymai, tas pats valymas paima), arba po valymo su tikra paskyra ir antru mažu valymu.

#### S1620 — BŪKLĖ SESIJOS PABAIGOJE
- **Gyva dev'e:** darbalaukis **v3.35.1** (`4e8c1366…`), tema base.php **v2.12**, juosta v1.7, kliento-siuntos v1.3, **dev-pastas v1.1** (`ed7e80a8…`), **vertimai v1.2** (`8654dcc7…`); variklis ir Pragma nekeisti. Instancija #16 išjungta. Skaitikliai AVPN 341 · IAPV 167 · KR-AVPN 105 · PPK 103 (T-0 → 101). Eilės Visi 64. Testiniai +#35813 (atsiėmimas, completed, AVPN000341) — trinti 6 etape.
