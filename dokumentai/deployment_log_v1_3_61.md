# deployment_log.md — v1.3.61

## IRASAI (naujausi virsuje)

---

## 2026-07-14 — REMINDERS (M13) [S194]

Augintinio priminimai. petshop-core v0.8.0. Svarus — 0 fatal, visi testai pirmu bandymu. S194.

### S194-A — Petshop_Reminders (272 eil.)

DB gaj6_ps_reminders: user_id, pet_id, reminder_type(vaccination|deworming|flea_tick|vet_checkup|grooming|custom), reminder_label, due_date, repeat_interval_days, status(active|done|cancelled), notified_at, completed_at.

REST /petshop/v1/reminders: POST create, GET list, POST /{id} update, DELETE /{id}. require_login + IDOR (tikrina user_id).

### S194-B — Cron + M6 tokenai

wp_schedule_event ps_reminders_daily_check daily 09:00. check_due(): active + notified_at IS NULL + due_date <= today+3d -> fire pet_reminder_due per Registry. Payload: reminder_id, pet_id, pet_name, reminder_type, due_date, confirm_token, reschedule_token.

confirm_token + reschedule_token: M6 Action Tokens (purpose_group='reminder_{id}', TTL 7d). Panaudojus viena — kitas invaliduojamas (M6 purpose_group mechanizmas).

### S194-C — Repeat

Kai status='done' + repeat_interval_days > 0 -> automatiskai sukuria nauja priminima su due_date + interval. Pvz. vaccination 365d -> kitas metai.

### S194-D — Empirinis testas 7/7

create / list / cron_fire (vaccination+Reksas+both_tokens) / notified_at / repeat(2->2027-07-16) / IDOR(BLOCKED). Visi ✅ pirmu bandymu.
