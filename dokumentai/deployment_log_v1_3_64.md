# deployment_log.md — v1.3.64

## IRASAI (naujausi virsuje)

---

## 2026-07-14 — M8: PET-DASHBOARD AGREGATORIUS + REFILL-FEEDBACK [S197]

Profilio ekrano duomenu agregatorius + refill grizt. rysys. petshop-core v0.11.0. 0 fatal. S197.

### S197-A — Recon

refill_tracking: predicted_empty_date, avg_interval_days, confidence, purchase_count, last_purchase_date. reminders: reminder_type, reminder_label, due_date, status. Uzteko dashboard'ui.

### S197-B — Nauja klase Petshop_Pet_Dashboard (326 eil.)

GET /pet-dashboard/{pet_id} — VISKAS vienam ekranui:
- pet: info + photo_url + primary_need
- refill: days_left, color(green/yellow/orange — NE raudona), avg_interval, product info
- reminders: iki 5 artimiausiu (laiko juosta) su days_left
- shelf: ps_pet_products su produktu nuotraukom/kainom/stock (FIELD order primary_food first)
- food_rhythm: avg intervalas jei 2+ pirkimai
- completeness: % + missing[] (show tik jei <100%)

Refill spalvos logika: <=3d oranzine, <=10d gelsva, kitaip zalia. Raudona NENAUDOJAMA (tik klaidoms).

### S197-C — refill-feedback endpoint

POST /refill-feedback (pet_id, product_id, feedback: similar|sooner|later).
MVP korekcija: sooner *0.8 (min 7), later *1.2 (max 180), similar nekeicia.
Perskaiciuoja predicted_empty_date. Fire'ina refill_feedback_submitted event.

### S197-D — Testas 9/9

Dashboard 200. Reksas. Refill 22d/green. Priminimai(1, Skiepai, 11d). Lentynele(1). Food_rhythm(has_data). Completeness(83%). Feedback sooner: 30->24 (-20%). Visi ✅.

### S197 — LIVE

petshop-core v0.11.0. Profilio ekranas turi duomenu saltini. IDOR apsauga visur (pet.user_id patikra).

### S197 — SEKANTIS

- Profilio ekrano HTML/CSS/JS (naudos pet-dashboard) — pagal galutinius vizualus
- render_account_page: jei augintinis yra -> profilis, jei ne -> anketa
- pet-photo upload/serve endpoint
- iliustracijos (dizaineris)
