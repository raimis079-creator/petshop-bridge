# deployment_log.md — v1.3.65

## IRASAI (naujausi virsuje)

---

## 2026-07-14 — M8: PET-PHOTO SAUGUS UPLOAD/SERVE [S198]

Augintinio nuotraukos ikelimas + saugus pateikimas. petshop-core v0.12.0. 0 fatal. S198. **M8 BACKEND 100% BAIGTAS.**

### S198-A — Recon

GD yra (JPEG/PNG/WebP). finfo yra. Imagick nera (nereikia). Uploads rasomas.

### S198-B — Nauja klase Petshop_Pet_Photo (276 eil.)

Pagal MASTER 17.8 saugumas:
- Tikras MIME (finfo, ne plotmuo) — leidzia tik jpeg/png/webp
- EXIF pasalinamas (re-encode per GD)
- Resize + center-crop 400x400
- Privatus katalogas uploads/ps-pets/ su .htaccess (Deny from all) + index.php
- Random-hash failo pavadinimas (32 simb.)
- Pateikimas per autentifikuota proxy su nuosavybes patikra
- photo hash saugomas user_meta 'ps_pet_photo_{pet_id}' (BIGINT photo_file_id netiktu hash'ui)

REST:
- POST /pet-photo/{id} — multipart (field 'photo'), max 5MB
- GET /pet-photo/{id} — auth proxy (readfile, Content-Type image/jpeg)
- DELETE /pet-photo/{id} — pasalina

### S198-C — Dashboard integracija

format_pet naudoja Pet_Photo::has_photo + serve_url (ne WP attachment). completeness skaiciuoja has_photo.

### S198-D — Testas 10/10

Katalogas+htaccess. Upload 600x400 PNG -> 200. Hash saved. Resize 400x400. JPEG re-encode (EXIF gone). serve_url+dashboard photo. Blogo MIME atmetimas (tekstas kaip png -> 400). DELETE + failas dingsta. Visi ✅.

### S198 — LIVE + M8 BACKEND BAIGTAS

petshop-core v0.12.0. Visi M8 backend endpointai gyvi:
- pet-profile (POST/GET/PATCH/DELETE)
- brands autocomplete
- pet-product-link (POST/GET)
- pet-dashboard (agregatorius)
- refill-feedback
- pet-photo (POST/GET/DELETE)

### S198 — SEKANTIS (tik frontend liko)

M8 profilio ekrano HTML/CSS/JS (naudos pet-dashboard). render_account_page conditional (anketa vs profilis). Iliustracijos (dizaineris). Viskas priklauso nuo galutiniu vizualu.
