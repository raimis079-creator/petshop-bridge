...
> Pridėti po šio antraštės (virš S1617). Tema: **5 etapas A „GRĄŽINIMAS“ (ne dėl grįžusios siuntos; Raimio sprendimas log S1617 „RAIMIO ATSAKYMAI“ A) — darbalaukis v3.31 → v3.32.** Variklis neliestas; `kreditine_vykdyti` neliesta (nauja `grazinimas_vykdyti` naudoja tas pačias pagalbines `kreditine_numeris` / `kreditine_pdf` / `kreditine_kortele`). Bridge: `irankiai/s1618_edit_v332.py` (tikslūs `replace` su sargais ant pilno v3.31 + `php -l` STABDIKLIS `subprocess`+`sys.exit`) + `s1618_v332_funcs.php`; `s1618_e1a.php` (gz+b64 dalis 1 → `ps-backups/dl-v332.part1`), `s1618_e1d.php` (D deploy su sargais + ping + eilės prieš · Q eilės po · T1 · T2), `s1618_e2s.php` (S Playwright kadrai · C valymas). `analize/s1618_e1a/e1d/e2s/e2c.json`, `screenshots/s1618_e2_kortele_35450_forma.png, s1618_e2_kortele_35450_atsisakymas_dalinis.png, s1618_e2_skydelis_35777_forma.png, s1618_e2_klausimai_35777.png`.

#### S1618 — RECON (tik skaitymas, iš repo v3.31 `?ref=4fd00d2b`)
