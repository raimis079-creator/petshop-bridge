...
const head=async u=>{ const c=new AbortController(); const t=setTimeout(()=>c.abort(),15000); try{ const r=await fetch(u,{method:'HEAD',redirect:'manual',headers:UA,signal:c.signal}); return {s:r.status,l:r.headers.get('location')||''}; }catch(e){ return {s:0,l:String(e).slice(0,60)}; } finally{ clearTimeout(t);} };
