/**
 * PETSHOP ADS PMAX NUOTRAUKŲ MAINAI v1.1 (S1672)
...
function main() {
  var out = { v: 'ADS ASSET SWAP v1.1', dry: DRY, kada: new Date().toISOString(), klaidos: [], grupes: {} };
  ...
  try { var qa = AdsApp.search('SELECT campaign.name, asset_group.name, asset_group_asset.field_type, asset_group_asset.status, asset_group_asset.performance_label, ' +
      ...
      'FROM asset_group_asset WHERE campaign.status = "ENABLED" AND segments.date DURING LAST_90_DAYS');
