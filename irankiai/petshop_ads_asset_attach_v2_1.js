/**
 * PETSHOP ADS PMAX NUOTRAUKŲ PRIKABINIMAS v2.1 (S1672) — +5 s pauzė tarp prikabinimų; leisti kelis kartus, kol pries = 17/17/16 ir 11/11/11 — antras žingsnis po swap v1.3.
...
function main() {
  var out = { v: 'ADS ASSET ATTACH v2.1', dry: DRY, kada: new Date().toISOString(), klaidos: [], grupes: {} };
  ...
  for (var key in GRUPES) {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      try { var a = AdsApp.adAssets().assets().withIds([lib[an]]).get(); if (!a.hasNext()) { rep.klaidos.push(an + ' nerastas'); continue; }
        g.addAsset(a.next(), t); rep.prikabinti[t]++; Utilities.sleep(5000); } catch (e) { rep.klaidos.push(an + ': ' + e); }
    }
