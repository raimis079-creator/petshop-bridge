  ...
  if($faze==='alter'){
    ...
    foreach($pids as $pid=>$ru){ $pr=wc_get_product($pid); $d=array('pid'=>$pid,'rusis'=>$ru,'busena'=>'gone');
      if($pr){ $d['pav']=$pr->get_name(); $d['kaina']=(float)wc_get_price_to_display($pr); $d['url']=$pr->get_permalink(); $iid=$pr->get_image_id(); $d['img']=$iid?wp_get_attachment_image_url($iid,'medium'):'';
        if('publish'!==$pr->get_status()||!$pr->is_purchasable()) $d['busena']='unavailable'; elseif(!$pr->is_in_stock()) $d['busena']='out_of_stock'; else { $d['busena']='product'; if(isset($SV[$ru])){ $eil=array(); $ok=true; foreach($SV[$ru] as $kg){ $r=Petshop_Feeding_Service::evaluate(array('product_id'=>$pid,'quantity'=>1,'pet_input'=>array('current_weight_kg'=>$kg,'conditions'=>array()))); if(!isset($r['status'])||$r['status']!=='ok'||empty($r['eur_per_day'])||empty($r['duration_days'])){ $ok=false; $d['calc_issue']=isset($r['status'])?$r['status']:'?'; break; } $eil[]=array('kg'=>$kg,'g'=>$r['portion_g'],'eur'=>$r['eur_per_day'],'d'=>$r['duration_days']); } if($ok){ $d['busena']='calc'; $d['eilutes']=$eil; } } }
      }
