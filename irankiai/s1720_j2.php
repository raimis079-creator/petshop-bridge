<?php
/** Plugin Name: TEMP PS S1720j2 — 2.17 deploy: 1 = plugin (DATA) + schema handlingTime 1–2 + heartbeat; 2 = /pristatymas/ sausas (rodo atitikmenis); 4 = /pristatymas/ rašyti; 3 = testai; 9 = atstatyti viską */
add_action('wp_loaded', function(){
  if(!isset($_GET['ps_s1720j2'])) return; $f=$_GET['ps_s1720j2']; $r=['v'=>'S1720j2','faze'=>$f,'t'=>date('Y-m-d H:i:s')];
  ...
  if($f==='1'){
    ...
    if(!isset($_GET['d_petshop_pristatymo_pazadas_v1_0_1_php_txt'])) throw new Exception('nėra DATA media id');
    $mid=(int)$_GET['d_petshop_pristatymo_pazadas_v1_0_1_php_txt']; $path=get_attached_file($mid); if(!$path||!file_exists($path)) throw new Exception('media nerastas');
  ...
  if($f==='5'){ if(function_exists('wp_cache_clear_cache')){ wp_cache_clear_cache(); $r['super_cache']='išvalytas'; } else $r['super_cache']='funkcijos nėra'; $r['heartbeat']=$hb(); }
  if($f==='9'){
