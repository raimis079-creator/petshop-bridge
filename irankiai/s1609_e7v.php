<?php
/** TEMP PS S1609 run e7v — v3.10.2 patikra: amžius, dialogai, valymas */
add_action('init', function(){
  if (!isset($_GET['ps_e7v'])) return;
  $o=array('v'=>'run e7v'); global $wpdb; $p=$wpdb->prefix; set_time_limit(280);
  ...
  foreach(array('ps_e5_mail','ps_e5_oid') as $k){ $o['istrinta_'.$k]=delete_option($k); }
  ...
  $o['shots']=array(
    array('n'=>'e7_visi','u'=>$B.'&eile=visi','eval'=>"({eil:[...document.querySelectorAll('tr.eil')].slice(0,3).map(x=>x.innerText.replace(/\\s+/g,' ').slice(0,90))})"),
    array('n'=>'e7_dialogas_av','u'=>$B.'&eile=paruosta','click'=>'tr.eil[data-id=\"35429\"] td.d a.v.p','eval'=>"({dlg:(document.querySelector('.dl-dlg.on')||{}).innerText,opt:(document.querySelector('#dlDlgOpt')||{}).checked})"),
    array('n'=>'e7_paruosta_mob','u'=>$B.'&eile=paruosta','w'=>420,'h'=>900),
  );
