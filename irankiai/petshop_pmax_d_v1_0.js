/**
 * PMax D v1.0 (S1674) — nauja struktūra: pervadina šunų kampaniją, €40/d, URL expansion OFF,
...
var G = [
 {n:'D1 Exclusion LTV 7-12 kg', url:'https://petshop.lt/prekes-zenklas/exclusion/', tema:'Exclusion LTV (7/12 kg)', f:{brand:['Exclusion'], cl1:['reklamuoti_ltv']}},
 ...
 {n:'D3 Josera sunims', url:'https://petshop.lt/prekes-zenklas/josera/', tema:'Josera šunims (sausas)', f:{brand:['Josera'], cl2:['sausas-maistas-sunims'], cl0:['A','B']}},
 ...
 {n:'D5 Josera katems', url:'https://petshop.lt/prekes-zenklas/josera/', tema:'Josera katėms', f:{brand:['Josera'], cl2:['sausas-maistas-katems'], cl0:['A','B']}},
...
function main() {
  var out = { v:'PMAX D v1.0', dry:DRY, kada:new Date().toISOString(), klaidos:[], zingsniai:[] };
  try {
    ...
    var cops = [{ campaignOperation:{ update:{ resourceName:'customers/'+C+'/campaigns/'+DOG, name:NEW_NAME, urlExpansionOptOut:true }, updateMask:'name,url_expansion_opt_out' } },
    ...
    G.forEach(function(g){
      ...
      if (!DRY) z.rez = res(AdsApp.mutateAll(ops, {partialFailure:false}));
      out.zingsniai.push(z);
    });
    // 5. kačių kampanija PAUSE
    var cc = AdsApp.performanceMaxCampaigns().withIds([Number(CAT)]).get().next();
    out.zingsniai.push({k:'kates PAUSE', dabar:cc.isEnabled()}); if (!DRY) cc.pause();
...
function listing(ag, f) {
  ...
  function node(parent, k, val, type) {
    var rn = 'customers/'+C+'/assetGroupListingGroupFilters/'+(id--);
