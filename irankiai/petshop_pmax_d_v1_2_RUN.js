/**
 * PMax D v1.2 (S1676: tekstai ribojami 5/3/4, nuotraukos atskiru mutate partialFailure, be urlExpansion) — nauja struktūra: pervadina šunų kampaniją, €40/d, URL expansion OFF,
...
function main() {
  var out = { v:'PMAX D v1.2', dry:DRY, kada:new Date().toISOString(), klaidos:[], zingsniai:[] };
  try {
    ...
    G.forEach(function(g){
      ...
      copy.forEach(function(x){ ops.push({ assetGroupAssetOperation:{ create:{ assetGroup:ag, asset:x.a, fieldType:x.ft } } }); });
      ...
      if (!DRY) {
        ...
        if (agRn) { var iops = pick.map(function(x){ return { assetGroupAssetOperation:{ create:{ assetGroup:agRn, asset:x.a, fieldType:x.ft } } }; });
          z.nuotr = res(AdsApp.mutateAll(iops, {partialFailure:true})); }
      }
