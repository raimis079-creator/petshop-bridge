/**
 * PETSHOP ADS KONVERSIJŲ RECON v1.1 (S1673, C etapas) — TIK SKAITO, nieko nekeičia.
...
var NUO = '2026-09-01', IKI = '2026-09-11';

function main() {
  var out = { v: 'ADS KONV RECON v1.1', kada: new Date().toISOString(), klaidos: [] };
  ...
  out.pagal_diena = gaql('SELECT segments.date, segments.conversion_action_name, metrics.conversions, metrics.conversions_value, metrics.all_conversions FROM customer WHERE segments.date BETWEEN "' + NUO + '" AND "' + IKI + '" ORDER BY segments.date',
  ...
  out.pagal_kampanija = gaql('SELECT campaign.id, campaign.name, campaign.status, segments.date, metrics.cost_micros, metrics.clicks, metrics.conversions, metrics.conversions_value FROM campaign WHERE segments.date BETWEEN "' + NUO + '" AND "' + IKI + '" AND campaign.status != "REMOVED" ORDER BY segments.date',
