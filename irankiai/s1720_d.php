<?php
/** Plugin Name: TEMP PS S1720d — petshop-paieska v1.1 deploy (1 = bak+rašyti+heartbeat, 2 = testai, 3 = HTML patikra, 9 = atstatyti v1.0) */
add_action('wp_loaded', function(){
  if(!isset($_GET['ps_s1720d'])) return; $f=$_GET['ps_s1720d']; $r=['v'=>'S1720d','faze'=>$f,'t'=>date('Y-m-d H:i:s')];
  ...
  if($f==='1'){
    ...
    if(!isset($_GET['d_petshop_paieska_v1_1_php_txt'])) throw new Exception('nėra DATA media id');
    $mid=(int)$_GET['d_petshop_paieska_v1_1_php_txt']; $path=get_attached_file($mid); if(!$path||!file_exists($path)) throw new Exception('media failas nerastas '.$mid);
