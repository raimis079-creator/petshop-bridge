<?php
/** TEMP PS S1676 run j — darbalaukis v3.41 DEPLOY iš media (fazė D) + patikra (fazė T). */
add_action('init', function(){
  if (!isset($_GET['ps_s1676j'])) return; global $wpdb; $p=$wpdb->prefix; $F=$_GET['ps_s1676j']; $o=array('v'=>'S1676 j','faze'=>$F);
  $o['temp_istrinta']=(int)$wpdb->query("DELETE FROM {$p}snippets WHERE name LIKE 'TEMP%' AND active=0");
  $f=WPMU_PLUGIN_DIR.'/petshop-darbalaukis.php'; $bk=wp_upload_dir()['basedir'].'/ps-backups/petshop-darbalaukis.php.bak_s1676';
  if($F==='D'){
    $mid=isset($_GET['d_dl_v341_txt'])?(int)$_GET['d_dl_v341_txt']:0; $mf=$mid?get_attached_file($mid):''; $o['media']=array($mid,$mf?basename($mf):'NĖRA');
    ...
    if($o['senas_md5']!=='2ee69c4eb0102243f8a1b8c8df25a70f'){ $o['STOP']='gyvas ne v3.40'; }
    elseif($o['naujas_md5']!=='586da938edf9e0b2f6705bc71786b987'){ $o['STOP']='naujas md5'; }
