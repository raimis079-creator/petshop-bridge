<?php
/** TEMP PS S1633 run g — G: SARGŲ BŪKLĖ (cron'ai, sekimas, SLA, importai, laiškai, eilės, PHP klaidos, 301, GA4). READ-ONLY. */
add_action('init', function(){
  if (!isset($_GET['ps_s1633g'])) return;
  $o=array('v'=>'S1633 g'); global $wpdb; $p=$wpdb->prefix;
  ...
  $cs=array(new WP_Http_Cookie(array('name'=>LOGGED_IN_COOKIE,'value'=>wp_generate_auth_cookie($uid,$exp,'logged_in',$tok))));
  $r=wp_remote_get(admin_url('admin.php?page=ps-desk&eile=visi'),array('cookies'=>$cs,'timeout'=>90,'sslverify'=>false));
  $h=(string)wp_remote_retrieve_body($r); preg_match('/<nav[^>]*class="[^"]*dl-nav[^"]*"[^>]*>(.*?)<\/nav>/su',$h,$mm);
