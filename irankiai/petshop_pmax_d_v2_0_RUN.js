/**
 * PMax D v2.0 (S1676: NAUJA retail PMax kampanija su MC 5321054797, D1–D6 joje; listing filtrų sudėtiniai temp ID -1~-N; tekstai ribojami 5/3/4, nuotraukos atskiru mutate partialFailure, be urlExpansion) — nauja struktūra: pervadina šunų kampaniją, €40/d, URL expansion OFF,
...
var NEW_NAME = 'P-max | LT | Prekės (MC)', MC = '5321054797', NEW_BUDGET = 25000000, OLD_BUDGET = 15000000;
var G = [
...
function main() {
  var out = { v:'PMAX D v2.0', dry:DRY, kada:new Date().toISOString(), klaidos:[], zingsniai:[] };
  try {
    ...
    // 3. nauja retail PMax kampanija (PAUSED) + senos biudžetas €15
    var CB = 'customers/'+C+'/campaignBudgets/-2', CN = 'customers/'+C+'/campaigns/-1';
    var cops = [
      { campaignBudgetOperation:{ create:{ resourceName:CB, name:NEW_NAME+' biudžetas', amountMicros:String(NEW_BUDGET), deliveryMethod:'STANDARD', explicitlyShared:false } } },
      { campaignOperation:{ create:{ resourceName:CN, name:NEW_NAME, status:'PAUSED', advertisingChannelType:'PERFORMANCE_MAX', campaignBudget:CB,
          maximizeConversionValue:{}, shoppingSetting:{ merchantId:MC }, containsEuPoliticalAdvertising:'DOES_NOT_CONTAIN_EU_POLITICAL_ADVERTISING' } } },
      { campaignCriterionOperation:{ create:{ campaign:CN, location:{ geoTargetConstant:'geoTargetConstants/2440' } } } },
      { campaignBudgetOperation:{ update:{ resourceName:AdsApp.search('SELECT campaign_budget.resource_name FROM campaign WHERE campaign.id = '+DOG).next().campaignBudget.resourceName, amountMicros:String(OLD_BUDGET) }, updateMask:'amount_micros' } }
    ];
    out.zingsniai.push({k:'nauja kampanija', ops:cops.length});
    var NEWC = null;
    if (!DRY) { var rc = AdsApp.mutateAll(cops, {partialFailure:false}); out.zingsniai[out.zingsniai.length-1].rez = res(rc); if (rc[1].isSuccessful()) NEWC = rc[1].getResourceName(); out.nauja_kampanija = NEWC; if (!NEWC) throw new Error('kampanija nesukurta'); }
    // 4. grupės
    G.forEach(function(g){
      ...
      var ag = 'customers/'+C+'/assetGroups/-1', ops = [{ assetGroupOperation:{ create:{ resourceName:ag, campaign:(NEWC||CN), name:g.n, finalUrls:[g.url], status:'PAUSED' } } }];
...
function listing(ag, f) {
  ...
  function node(parent, k, val, type) {
    ...
    var c = { resourceName:rn, assetGroup:ag, type:type, listingSource:'SHOPPING' };
