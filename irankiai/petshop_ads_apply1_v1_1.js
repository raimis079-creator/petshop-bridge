/**
 * PETSHOP ADS APPLY #1 v1.1 (S1666)
...
function main() {
  var out = { v: 'APPLY1 v1.1', kada: new Date().toISOString(), klaidos: [] };
  ...
  // 2) Brand_Exclusion -> abu PMax (mutate)
  try {
    var ops = [
      { campaignSharedSetOperation: { create: {
          campaign: 'customers/7541530584/campaigns/21472017542',
          sharedSet: 'customers/7541530584/sharedSets/11895318074' } } },
      { campaignSharedSetOperation: { create: {
          campaign: 'customers/7541530584/campaigns/21565450990',
          sharedSet: 'customers/7541530584/sharedSets/11895318074' } } }
    ];
    var res = AdsApp.mutateAll(ops);
    out.priskyrimas_mutate = [];
    for (var i = 0; i < res.length; i++) {
      out.priskyrimas_mutate.push(res[i].isSuccessful() ? 'OK ' + res[i].getResourceName()
        : 'FAIL ' + res[i].getErrorMessages().join('; '));
    }
