<?php
/** TEMP PS S1609 run e6t — #5 sekimo laiškas po kiekvienos siuntos: testai darbuotojo paskyra per realius endpoint'us + laiškų gaudyklė */
...
add_action('init', function(){
  ...
  $f=strtoupper(sanitize_key($_GET['ps_e6t'])); $o=array('v'=>'run e6t','f'=>$f); global $wpdb; $p=$wpdb->prefix; set_time_limit(280);
  ...
  try{
   ...
   if($f==='R'){ // #35774 aklavietė: pakartoti „Prins išsiuntė“ — turi užbaigti (apėjimas), laiško nekartoti
    update_option('ps_e5_mail',array(),false); $o['pries']=$snap(35774);
    $o['prins_dar_karta']=$dl('issiusta',35774,array('dalis'=>'prins','sekimo'=>1)); $o['po']=$snap(35774); $o['mail']=$mails(2);
   }
   ...
   if($f==='N'){ // naujas: VF + Prins dropship (be Venipak) — nuo atėjimo iki įvykdyto
    ...
    $adr=array('first_name'=>'AUDITAS','last_name'=>'Testas 33','email'=>'terra@petshop.lt','phone'=>'+37060000000','address_1'=>'Testų g. 33','city'=>'Vilnius','postcode'=>'01100','country'=>'LT');
    ...
    $oid=$ord->get_id(); update_option('ps_e5_oid',$oid,false); $ord->payment_complete('AUDITAS-T33'); $o['oid']=$oid; $o['po_apmok']=$snap($oid);
   ...
   if($f==='I'){ // VF išsiuntė → 1 iš 2 (be numerio); Prins išsiuntė → 2 iš 2 + įvykdytas
    ...
    $sh=array(); foreach($l as $i=>$m){ if(strpos($m['tema'],'užsakymas')!==false||strpos($m['tema'],'Užsakymas')!==false) $sh[]=array('n'=>'e6_laiskas_'.$i,'u'=>home_url('/?ps_e6t=H&i='.$i),'w'=>760,'h'=>1000,'full'=>true); }
    $sh[]=array('n'=>'e6_paruosta','u'=>$B.'&eile=paruosta','full'=>true,'eval'=>"({pastaba:[...document.querySelectorAll('.pastaba')].map(x=>x.innerText),btn:[...document.querySelectorAll('td.d a.v')].map(x=>x.innerText.trim()).filter(Boolean)})");
    $sh[]=array('n'=>'e6_dialogas','u'=>$B.'&eile=paruosta','click'=>'tr.eil[data-id="35418"] td.d a.v.p','eval'=>"({dlg:(document.querySelector('.dl-dlg.on')||{}).innerText,opt:(document.querySelector('#dlDlgOpt')||{}).checked,href:(document.querySelector('.dl-dlg.on a.v.p')||{}).href})");
    $sh[]=array('n'=>'e6_visi','u'=>$B.'&eile=visi','eval'=>"({eil:[...document.querySelectorAll('tr.eil')].slice(0,6).map(x=>x.innerText.replace(/\\s+/g,' ').slice(0,160))})");
    if(false) $sh[]=array('n'=>'e5_istorija','u'=>$B.'&eile=visi&atidaryti='.$oid,'eval'=>"new Promise(function(r){setTimeout(function(){r({kur:(document.querySelector('.dl-kur')||{}).textContent,zing:[...document.querySelectorAll('#skEil .zingsneliai')].map(x=>x.innerText.replace(/\\s+/g,' '))})},1500)})");
    $o['shots']=$sh;
   }
