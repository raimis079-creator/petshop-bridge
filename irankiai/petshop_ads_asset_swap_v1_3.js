/**
 * PETSHOP ADS PMAX NUOTRAUKŲ MAINAI v1.3 (S1672)
...
function main() {
  var out = { v: 'ADS ASSET SWAP v1.3', dry: DRY, kada: new Date().toISOString(), klaidos: [], grupes: {} };
  ...
  out.archyvas_90d = [];
  try { var qa = AdsApp.search('SELECT campaign.name, asset_group.name, asset_group_asset.field_type, asset_group_asset.status, ' +
    ...
    while (qa.hasNext()) { var ra = qa.next(); out.archyvas_90d.push({ c: ra.campaign.name, g: ra.assetGroup.name, ft: ra.assetGroupAsset.fieldType, st: ra.assetGroupAsset.status, 
