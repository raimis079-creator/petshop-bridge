<?php
/** TEMP PS S1686 mj — READ-ONLY: (1) product-calc.js — svorio įvestis, localStorage, init; (2) feeding-calc REST — permission_callback (svečias?); (3) Feeding_Service::evaluate parašas ir pavyzdinis skaičiavimas 3 svoriams; (4) Feeding_UI render vieta. */
add_action('init', function(){
  ...
  foreach(array(18014,12466) as $pid){ $pr=wc_get_product($pid); foreach(array(10,20,30) as $kg){ try{ $r=Petshop_Feeding_Service::evaluate(array('product_id'=>$pid,'weight_kg'=>$kg)); }catch(Throwable $e){ $r='ERR '.$e->getMessage(); } $o['pvz'][$pid.'/'.$pr->get_name()][$kg]=is_array($r)?array_intersect_key($r,array_flip(array('portion_g','duration_days','eur_per_day','eur_per_month','issues'))):$r; } }
