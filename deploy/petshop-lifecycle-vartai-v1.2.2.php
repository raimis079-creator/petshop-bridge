...
class Petshop_Lifecycle_Vartai {
	...
	/** S1688: klientas pats paprašė priminimo prekės puslapyje (feedback_cycle='relaunch') — holdout netaikomas. */
	...
	public static function eligibility( $result, $flow, $flow_class, $email, $context = array() ) {
		...
		if ( self::holdout( $email ) && ! self::prasyta( $email, $context ) ) return array( 'allowed' => false, 'reason' => 'holdout_10', 'terminal' => true );
		if ( 'soft' === $vartai ) {
