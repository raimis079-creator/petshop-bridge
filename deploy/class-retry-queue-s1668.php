 ...
 *    - S1668: be el. pašto → status='skipped' (į Sender nesiunčiam, eventas lieka analitikai)
 *    - S1668: Sender 404 „Subscriber not found" → status='skipped' be DLQ alert
 *      (pirkėjo nėra Sender kontaktuose; užsakymo laiškus siunčia WC — Raimio sprendimas B)
...
/*
 * MIGRACIJA (S186): Perkelta iš petshop-esp/includes/class-retry-queue.php.
 * Klases pervadintos Petshop_ESP_* -> Petshop_*. DB nekeičiam.
 * ps_esp_adapter() kvietimas paliktas kol kas — petshop-esp v0.4.0 ta funkcija tampa
 * Sender-specifiniu helper'iu. Kai atsiras antras provider (SMS), pereisim
 * i apply_filters('petshop_get_message_provider') mechanizma.
 */
class Petshop_Retry_Queue {
	...
	public static function handle_process_event( $log_id ) {
		...
		$row = $wpdb->get_row( $wpdb->prepare(
			"SELECT * FROM `" . Petshop_Event_Log::table_name() . "` WHERE id = %d LIMIT 1",
			(int) $log_id
		) );
		if ( ! $row ) {
		...
		// S1668: be el. pašto į Sender nesiunčiam (svečių krepšeliai) — eventas lieka žurnale analitikai
		if ( '' === trim( (string) $row->email ) ) {
			Petshop_Event_Log::update( $row->id, array(
				'status'     => 'skipped',
				'last_error' => 'skipped: no_email',
			) );
			return;
		}
		...
		if ( ! $retriable ) {
			// S1668: kontakto nėra Sender'yje — ne klaida, praleidžiam be DLQ alert (Raimio sprendimas B)
			$klaida = isset( $result['error'] ) ? ( is_scalar( $result['error'] ) ? (string) $result['error'] : wp_json_encode( $result['error'] ) ) : '';
			$atsak  = isset( $result['esp_response'] ) ? ( is_scalar( $result['esp_response'] ) ? (string) $result['esp_response'] : wp_json_encode( $result['esp_response'] ) ) : '';
			if ( false !== stripos( $klaida . ' ' . $atsak, 'Subscriber not found' ) ) {
				Petshop_Event_Log::update( $row->id, array(
					'status'       => 'skipped',
					'attempts'     => $attempts,
					'last_error'   => 'skipped: subscriber_not_in_sender',
					'esp_response' => isset( $result['esp_response'] ) ? $result['esp_response'] : null,
				) );
				return;
			}
	...
	public static function process_pending_batch( $limit = 50 ) {
		...
		$pending = Petshop_Event_Log::get_pending( $limit );
		$enqueued = 0;
