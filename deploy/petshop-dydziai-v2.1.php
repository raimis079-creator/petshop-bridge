 ...
 * Version: 2.1
 * 2.0.1: mygtukas „Rinktis“ kiekvienoje eilutėje (Raimis: klientai nuorodos nepastebės), visa eilutė spaudžiama.
 */
...
class Petshop_Dydziai {
	...
	/** Pardavimai per narį: [365 d., 90 d., 30 d.] vnt. (istorija + WC), transient 12 val. */
	public static function pardavimai( $seima, $ids ) {
		$key = 'ps_dydz_pard2_' . md5( $seima . implode( ',', $ids ) );
		$c = get_transient( $key ); if ( is_array( $c ) ) { return $c; }
		global $wpdb; $out = array_fill_keys( $ids, array( 0, 0, 0 ) ); $in = implode( ',', array_map( 'intval', $ids ) );
		try {
			foreach ( array( 'ps_ist_fakt_eilutes', 'ps_fakt_eilutes' ) as $t ) {
				...
				foreach ( (array) $rows as $r ) { $id = (int) $r['preke_id']; $o = $out[ $id ] ?? array( 0, 0, 0 ); $out[ $id ] = array( $o[0] + (int) $r['q'], $o[1] + (int) $r['q90'], $o[2] + (int) $r['q30'] ); }
			}
		} catch ( \Throwable $e ) {}
	...
	/** „Miręs" narys (S1721, R 18:35): likutis 0 ir 0 pardavimų per 30 d. (bonusinis „N+M kg") arba 90 d. (kiti) → nerodomas lentelėje/čipsuose/veide. */
	public static function mires( $uzr, $yra, $pard ) {
		if ( $yra ) { return false; }
		$p = is_array( $pard ) ? $pard : array( (int) $pard, (int) $pard, (int) $pard );
		return ( strpos( (string) $uzr, '+' ) !== false ) ? $p[2] <= 0 : $p[1] <= 0;
	}
	...
	public static function eilutes( $seima, $pid = 0 ) {
		...
		foreach ( $ids as $id ) {
			...
			$yra = $p->is_in_stock(); $pd = $pard[ $id ] ?? array( 0, 0, 0 );
			if ( $id !== (int) $pid && self::mires( $u, $yra, $pd ) ) { continue; }
			$eil[] = array( 'id' => $id, 'uzr' => $u, 'url' => get_permalink( $id ), 'yra' => $yra, 'kaina' => $price,
				'kaina_html' => $p->get_price_html(), 'kg' => $kg, 'l' => $l, 'perkg' => ( $kg > 0 && $price > 0 ) ? $price / $kg : 0,
				'dp' => (int) get_post_meta( $id, '_dp_pack_qty', true ) > 1, 'pard' => (int) $pd[0], 'sort' => self::rikiavimas( $u ), 'pav' => $p->get_name() );
		}
	...
	public static function pieskime() {
		...
		echo '<div class="ps-pak__wrap"><table class="ps-pak__t"><thead><tr><th>Dydis</th><th class="n">Kaina</th>' . ( $su_kg ? '<th class="n">€/kg</th><th class="n">Sutaupai</th>' : '' ) . '<th class="n ps-pak__dcol" hidden>Užtenka</th><th class="n"></th></tr></thead><tbody>';
		foreach ( $eil as $x ) {
			...
			echo '<tr class="' . esc_attr( $cls ) . '" data-url="' . ( $cur ? '' : esc_url( $x['url'] ) ) . '" data-id="' . (int) $x['id'] . '" data-kg="' . esc_attr( $x['kg'] ) . '" data-kaina="' . esc_attr( $x['kaina'] ) . '"><td>' . $uz . $dp . $zym . ( $x['yra'] ? '' : '<span class="ps-pak__nera">neturime</span>' ) . '</td>';
			...
			echo '<td class="n ps-pak__d" hidden></td><td class="n ps-pak__act">' . ( $cur ? '<span class="ps-pak__cur">pasirinkta</span>' : '<a class="ps-pak__btn" href="' . esc_url( $x['url'] ) . '">Rinktis</a>' ) . '</td></tr>';
		}
...
.ps-pak__btn{display:inline-block;font-size:11.5px;font-weight:600;line-height:1;padding:7px 11px;border:1px solid #1f7a4d;border-radius:4px;color:#1f7a4d;text-decoration:none;white-space:nowrap}
.ps-pak__btn:hover{background:#1f7a4d;color:#fff;text-decoration:none}
.ps-pak__cur{font-size:11px;color:#7d8983;white-space:nowrap}
.ps-pak__act{vertical-align:middle}
tr.ps-pak__r[data-url]:not([data-url=""]){cursor:pointer}tr.ps-pak__r[data-url]:not([data-url=""]):hover td{background:#f6faf7}
...
P.addEventListener("click",function(e){var tr=e.target.closest("tr.ps-pak__r");if(tr&&tr.getAttribute("data-url")&&!e.target.closest("a")){location.href=tr.getAttribute("data-url");return}var a=e.target.closest(".ps-pak__calc");if(!a)return;var c=document.getElementById("ps-calc");if(c){e.preventDefault();c.scrollIntoView({behavior:"smooth",block:"center"});var i=document.getElementById("ps-calc-w");if(i)setTimeout(function(){i.focus()},400)}});
})();</script>';
	}
}
