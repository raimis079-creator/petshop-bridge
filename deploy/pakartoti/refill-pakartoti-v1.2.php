<?php
/**
 * Šablonas: refill_due — „Pakartoti tą patį" v1.2 (S1685; be įdėtos lentelės — Outlook ją išmeta už paraščių): visos paskutinio užsakymo sekamos maisto prekės, kiekiai, kainos, suma; vienas mygtukas į kasą (Petshop_Pakartoti::url).
...
if ( $n ) {
	$eil = '';
	foreach ( $d['prekes'] as $pr ) { $eil .= '<div style="padding:7px 0;border-bottom:1px solid #EEEEEE;"><a href="' . esc_url( $pr['url'] ) . '" style="color:#1F2A24;text-decoration:none;">' . esc_html( $pr['pav'] ) . '</a><br><span style="color:#7A867C;">' . (int) $pr['kiekis'] . ' vnt. · ' . esc_html( $eur( $pr['kaina'] ) ) . '</span></div>'; }
