...
class Petshop_Pakartoti {
	...
	public static function grupe( $uid, $pid ) {
		...
		$vaikai = array(); foreach ( $o->get_items() as $it ) { if ( $it->get_meta( '_mnm_container' ) ) $vaikai[ (int) $it->get_meta( '_mnm_container' ) ][] = (int) $it->get_product_id(); }
		foreach ( $o->get_items() as $it ) {
			if ( $it->get_meta( '_mnm_container' ) ) continue; // rinkinio komponentas — eina su konteineriu
			$ipid = (int) ( $it->get_variation_id() ? $it->get_variation_id() : $it->get_product_id() ); $ppid = (int) $it->get_product_id();
			$konteineris = ( '' !== (string) $it->get_meta( '_mnm_config' ) ) || isset( $vaikai[ (int) $it->get_id() ] );
			$sekamas = in_array( $ipid, $sek, true ) || in_array( $ppid, $sek, true ) || ( $konteineris && isset( $vaikai[ (int) $it->get_id() ] ) && array_intersect( $vaikai[ (int) $it->get_id() ], $sek ) );
			if ( ! $sekamas ) continue;
			$pr = wc_get_product( $ipid ); $k = max( 1, (int) $it->get_quantity() );
			if ( ! $pr || 'publish' !== $pr->get_status() || ! $pr->is_purchasable() || ! $pr->is_in_stock() ) { $nera[] = $it->get_name(); continue; }
			$kaina = (float) wc_get_price_to_display( $pr ) * $k;
			if ( $konteineris && $kaina <= 0 && isset( $vaikai[ (int) $it->get_id() ] ) ) { $kaina = (float) $o->get_line_total( $it, true ); foreach ( $o->get_items() as $v ) { if ( (int) $v->get_meta( '_mnm_container' ) === (int) $it->get_id() ) { $vp = wc_get_product( (int) ( $v->get_variation_id() ? $v->get_variation_id() : $v->get_product_id() ) ); $kaina += $vp ? (float) wc_get_price_to_display( $vp ) * max( 1, (int) $v->get_quantity() ) : (float) $o->get_line_total( $v, true ); } } }
			$suma += $kaina;
	...
	public static function endpoint() {
		...
		if ( WC()->session ) { WC()->session->set_customer_session_cookie( true ); WC()->session->save_data(); } if ( method_exists( WC()->cart, 'maybe_set_cart_cookies' ) ) WC()->cart->maybe_set_cart_cookies();
