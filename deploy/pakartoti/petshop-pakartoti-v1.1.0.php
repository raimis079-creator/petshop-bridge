...
class Petshop_Pakartoti {
	...
	public static function grupe( $uid, $pid ) {
		...
		foreach ( $o->get_items() as $it ) {
			...
			if ( ! in_array( $ipid, $sek, true ) && ! in_array( $ppid, $sek, true ) ) continue;
			...
			$kaina = (float) wc_get_price_to_display( $pr ) * $k; $suma += $kaina;
			$prekes[] = array( 'pid' => $ipid, 'pav' => $pr->get_name(), 'kiekis' => $k, 'kaina' => $kaina, 'url' => $pr->get_permalink() );
		}
	...
	public static function endpoint() {
		...
		foreach ( $g['prekes'] as $pr ) { $pr_obj = wc_get_product( $pr['pid'] ); $vid = $pr_obj && $pr_obj->is_type( 'variation' ) ? $pr['pid'] : 0; $ppid = $vid ? $pr_obj->get_parent_id() : $pr['pid']; if ( WC()->cart->add_to_cart( $ppid, $pr['kiekis'], $vid ) ) $n++; }
