<?php
/**
 * Plugin Name: Petshop SEO šablonai v1.3 (S1717/S1718, 2.14)
 ...
 *   (3) v1.1 (S1718): WC Brands (`WC_Brands::brand_description`) gamintojo aprašymą išveda VIRŠ prekių, o `petshop-kategorijos-aprasymas`
 *   tą patį aprašymą rodo PO prekių (suskleistą) — viršutinis nuimamas, kad nesidubliuotų ir nestumtų prekių žemyn.
 *   (4) v1.2 (S1718b): gamintojo puslapio JSON-LD papildomas `Brand` mazgu (name, url, description iš term aprašymo, iki 300 simb.),
 *   o Rank Math `CollectionPage` gauna `about` → Brand — aiškesnis subjektas Google ir AI botams.
...
final class Petshop_SEO_Sablonai {
	...
	public static function init() {
		...
		add_action( 'wp', array( __CLASS__, 'nuimti_brand_virsu' ), 1 );
		add_filter( 'rank_math/json_ld', array( __CLASS__, 'brand_ld' ), 20, 2 );
	}

	/** v1.2: Brand mazgas gamintojo puslapio JSON-LD + CollectionPage.about. */
	public static function brand_ld( $data, $jsonld ) {
		if ( ! is_array( $data ) ) { return $data; }
		$t = self::terminas();
		if ( ! $t || 'product_brand' !== $t->taxonomy ) { return $data; }
		$url = get_term_link( $t );
		if ( is_wp_error( $url ) ) { return $data; }
		$id   = $url . '#brand';
		$desc = preg_replace( '#<h[1-6][^>]*>.*?</h[1-6]>#si', ' ', (string) $t->description );
		$desc = preg_replace( '/\s+/u', ' ', trim( wp_strip_all_tags( $desc ) ) );
		if ( mb_strlen( $desc ) > 300 ) { $desc = mb_substr( $desc, 0, 297 ) . '…'; }
		$b = array( '@type' => 'Brand', '@id' => $id, 'name' => html_entity_decode( $t->name, ENT_QUOTES, 'UTF-8' ), 'url' => $url );
		if ( '' !== $desc ) { $b['description'] = $desc; }
		$data['ps_brand'] = $b;
		foreach ( $data as $k => $n ) {
			if ( is_array( $n ) && isset( $n['@type'] ) && 'CollectionPage' === $n['@type'] ) { $data[ $k ]['about'] = array( '@id' => $id ); break; }
		}
		return $data;
	}

	/** v1.1: nuima WC Brands aprašymą nuo archyvo viršaus (apačioje jį rodo petshop-kategorijos-aprasymas). */
	public static function nuimti_brand_virsu() {
		if ( is_admin() || ! class_exists( 'WC_Brands' ) ) { return; }
		global $wp_filter;
		if ( empty( $wp_filter['woocommerce_archive_description'] ) ) { return; }
		foreach ( $wp_filter['woocommerce_archive_description']->callbacks as $prio => $cbs ) {
			foreach ( $cbs as $cb ) {
				$f = $cb['function'];
				if ( is_array( $f ) && isset( $f[0], $f[1] ) && $f[0] instanceof WC_Brands && 'brand_description' === $f[1] ) {
					remove_action( 'woocommerce_archive_description', $f, $prio );
				}
			}
		}
	}
