<?php
/**
 * Plugin Name: Petshop paieška v1.1 (S1699 → S1720)
...
final class Petshop_Paieska {
	...
	const STOP = array( 'ir', 'su', 'be', 'iš', 'is', 'the', 'and', 'for', 'with', 'of', 'a', 'sausas', 'sausasmaistas', 'maistas', 'pašaras', 'pasaras', 'food', 'dry', 'wet' );
	/**
	...
	private static function kiek( $w ) {
		...
		$c[ $w ] = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->posts} WHERE post_type='product' AND post_status='publish' AND " . self::salyga( $w ) );
		return $c[ $w ];
	}
	...
	public static function posts_search( $search, $q ) {
		...
		if ( count( $zodziai ) >= 2 ) {
			...
			if ( $zin && $nez ) { $naudoti = $zin; $atmesti = $nez; }
		}
