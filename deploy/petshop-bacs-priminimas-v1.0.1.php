<?php
/**
 * Petshop Bacs Priminimas v1.0.1 (S1676, 2026-09-12)
...
final class Petshop_Bacs_Priminimas {
	...
	public static function priminimo_html( $order ) {
		...
		$iki = wp_date( 'Y-m-d H:i', $sukurta + self::ATSAUK_PO );
		...
		$body  = Petshop_Email_Layout::p( $greet . ' jūsų užsakymą Nr. ' . esc_html( $nr ) . ' gavome, tačiau apmokėjimo pavedimu dar nesulaukėme.' );
		$body .= '<tr><td style="font-size:14px;line-height:1.7;padding-bottom:18px;">'
			...
			. ( $rk['iban'] ? '<strong>Sąskaita:</strong> ' . esc_html( $rk['iban'] ) . '<br>' : '' )
			. ( $rk['bankas'] ? '<strong>Bankas:</strong> ' . esc_html( $rk['bankas'] ) . '<br>' : '' )
			. '<strong>Mokėjimo paskirtis:</strong> ' . esc_html( $nr ) . '</td></tr>';
		$body .= Petshop_Email_Layout::p( 'Jei jau apmokėjote — nieko daryti nereikia. Jei mokėjimo negausime iki ' . esc_html( $iki ) . ', užsakymą atšauksime, o prekės grįš į prekybą.' );
		...
		return array( 'subject' => 'Užsakymas Nr. ' . $nr . ' laukia apmokėjimo', 'html' => Petshop_Email_Layout::wrap( array(
			'subject' => 'Užsakymas Nr. ' . $nr . ' laukia apmokėjimo', 'preheader' => 'Apmokėjimo pavedimu dar nesulaukėme — prekes laikome jums.',
