 ...
 * Author: Petshop.lt
 *
 * v2.6.0 (2026-09-20, S1697) — TIK Kaina24 / Kainos.lt: <manufacturer> = WC prekes zenklas (product_brand),
 *   _legacy_manufacturer tik kai zenklo nera (anksciau atvirksciai: 'Josera petfood GmbH', 'EXCL MEDITER',
 *   'Animonda Gran Carno' ejo kaip skirtingi gamintojai). Google feed'as nekeistas. Raimio 'daryk'.
 *
 * v2.5.0 (2026-09-12, S1677) — TIK Google feed'as: custom_label_1 = 'pigu', kai kaina (su PVM) < pigu_iki (€12) ir
 *   nera kitos zymes (ne_reklamai / reklamuoti_ltv turi pirmenybe). Tikslas: pigios prekes ne i mokama Shopping
 *   (marza € neapmoka kliko), tik free listings / krepselio priedas. Raimio sprendimas S1677.
 * v2.4.1 (2026-09-11, S1672) — TIK Google feed'as: g:shipping_weight VISADA (MC atmete 715
 *   prekes 'missing shipping weight', nes MC tarifai pagal svori): WC _weight > 0, kitaip is
 *   pavadinimo (kg/g/l/ml, paskutinis atitikmuo), kitaip numatytasis 0.5 kg (Raimio 'daryk').
 *   ps_feeds_google_svoris(). Kaina24 / Kainos.lt NEKEISTI.
 *
 * v2.4.0 (2026-09-11, S1671, PMax pertvarka B etapas) — TIK Google feed'as:
 *   - g:availability is realaus likucio (in_stock / out_of_stock), anksciau visada in_stock
 *     (136 publish prekes su likuciu 0 ejo kaip prieinamos);
 *   - g:price = reguliari, g:sale_price = akcijos kaina, kai preke akcijoje;
 *   - g:custom_label_0 = marzos pakopa A(>=35%) B(28-35) C(20-28) D(<20) X(be savikainos),
 *     marza = (kaina be PVM - savikaina) / kaina be PVM; savikaina _cost_price -> _vf_cost -> _zb_cost;
 *   - g:custom_label_1 = reklamuoti_ltv (Exclusion 7/12 kg — Raimio sprendimas) | ne_reklamai (Quattro) | '';
 *   - g:custom_label_2 = giliausios kategorijos slug; g:custom_label_3 = sandelis (_ps_sandelis);
 *   - taisykles opcijoje ps_feeds_google_taisykles (JSON), numatytosios ps_feeds_google_taisykles();
 *   - g:shipping_weight pagal _weight, kaip buvo. Kaina24 / Kainos.lt NEKEISTI.
 *
 * v2.3.0 (2026-09-10, S1669) — Kaina24 / Kainos.lt prekiu nuorodoms prideta
 *   utm_source=kaina24 / utm_source=kainos (Raimio sprendimas): kanalas
 *   atpazistamas ps_fakt_uzsakymai (petshop-kanalai klasifikuoja kaip mokamas).
 *   Google feed'as nekeistas.
 *
...
function ps_feeds_preke( int $id ) {
	...
	if ( null === $likutis ) {
		$likutis = $pr->is_in_stock() ? 1 : 0; // valdymas isjungtas — pagal statusa
	}

	// v2.4.0: savikaina (kaskada) ir marza nuo kainos be PVM; reguliari kaina akcijoms.
	$sav = 0.0;
	foreach ( array( '_cost_price', '_vf_cost', '_zb_cost' ) as $sk ) {
		$sv = (float) str_replace( ',', '.', (string) get_post_meta( $id, $sk, true ) );
		if ( $sv > 0 ) {
			$sav = $sv;
			break;
		}
	}
	$kaina_be = (float) $kaina / 1.21;
	$marza    = ( $sav > 0 && $kaina_be > 0 ) ? round( 100 * ( $kaina_be - $sav ) / $kaina_be, 1 ) : null;
	$reg      = $pr->get_regular_price();
	$akcija   = $pr->is_on_sale() && '' !== (string) $reg && (float) $reg > (float) $kaina;
	...
	return array(
		...
		'savikaina'   => $sav,
		'marza'       => $marza,
		'reg_kaina'   => $akcija ? number_format( (float) $reg, 2, '.', '' ) : '',
		'sandelis'    => (string) get_post_meta( $id, '_ps_sandelis', true ),
		'kat_slug'    => $kat ? $kat->slug : '',
		'specs'       => $specs,
...
/** v2.3.0: kanalo zyme prekes nuorodai (kaina24 / kainos). */
function ps_feeds_utm( $url, string $saltinis ): string {
	return (string) add_query_arg( 'utm_source', $saltinis, (string) $url );
}
...
function ps_feeds_kaina24_eilute( array $p ): string {
	...
	if ( $p['gamintojas'] || $p['brendas'] ) {
		$x .= '    <manufacturer>' . ps_feeds_cdata( $p['brendas'] ?: $p['gamintojas'] ) . '</manufacturer>' . "\n";
	}
	if ( $p['sku'] ) {
	...
	$x .= '    <product_url>' . ps_feeds_cdata( ps_feeds_utm( $p['nuoroda'], 'kaina24' ) ) . '</product_url>' . "\n";
	$x .= '    <purchase_url>' . ps_feeds_cdata( ps_feeds_utm( $p['nuoroda'], 'kaina24' ) ) . '</purchase_url>' . "\n";
...
function ps_feeds_kainos_eilute( array $p ): string {
	...
	$x .= '      <product_url>' . esc_url( ps_feeds_utm( $p['nuoroda'], 'kainos' ) ) . '</product_url>' . "\n";
	$x .= '      <purchase_url>' . esc_url( ps_feeds_utm( $p['nuoroda'], 'kainos' ) ) . '</purchase_url>' . "\n";
...
/** v2.4.0: Google feed'o taisykles — numatytosios, perrasomos opcija ps_feeds_google_taisykles (JSON). */
function ps_feeds_google_taisykles(): array {
	$d = array(
		'pakopos'        => array( 'A' => 35, 'B' => 28, 'C' => 20 ), // >= riba; kitaip D; be savikainos X
		'ltv_brand'      => 'Exclusion',
		'ltv_min_kg'     => 6.5,
		'ltv_kg_regex'   => '/\\b(7|12)\\s*kg\\b/iu',
		'ne_reklamai'    => array( 'Quattro' ),
		'pigu_iki'       => 12.0, // v2.5.0: kaina su PVM < riba -> custom_label_1 'pigu'
	);
	$o = json_decode( (string) get_option( 'ps_feeds_google_taisykles', '' ), true );
	return is_array( $o ) ? array_replace_recursive( $d, $o ) : $d;
}

/** v2.4.0: custom_label_0..3 — [pakopa, reklamos zyme, kategorijos slug, sandelis]. */
function ps_feeds_google_labels( array $p ): array {
	$t = ps_feeds_google_taisykles();
	$m = $p['marza'];
	if ( null === $m ) {
		$pak = 'X';
	} elseif ( $m >= $t['pakopos']['A'] ) {
		$pak = 'A';
	} elseif ( $m >= $t['pakopos']['B'] ) {
		$pak = 'B';
	} elseif ( $m >= $t['pakopos']['C'] ) {
		$pak = 'C';
	} else {
		$pak = 'D';
	}
	$z = '';
	if ( in_array( $p['brendas'], (array) $t['ne_reklamai'], true ) ) {
		$z = 'ne_reklamai';
	} elseif ( $p['brendas'] === $t['ltv_brand'] ) {
		$kg = (float) str_replace( ',', '.', $p['svoris'] );
		if ( ( $kg >= (float) $t['ltv_min_kg'] ) || preg_match( $t['ltv_kg_regex'], $p['pavadinimas'] ) ) {
			$z = 'reklamuoti_ltv';
		}
	}
	if ( '' === $z && (float) $p['kaina'] < (float) $t['pigu_iki'] ) {
		$z = 'pigu'; // v2.5.0
	}
	return array( $pak, $z, (string) $p['kat_slug'], (string) $p['sandelis'] );
}

/** v2.4.1: siuntimo svoris kg — WC _weight > 0, kitaip is pavadinimo (kg/g/l/ml), kitaip 0.5. */
function ps_feeds_google_svoris( array $p ): string {
	$w = (float) str_replace( ',', '.', (string) ( $p['svoris'] ?? '' ) );
	if ( $w <= 0 ) {
		if ( preg_match_all( '/(\\d+(?:[.,]\\d+)?)\\s*(kg|g|l|ml)\\b/iu', (string) ( $p['pavadinimas'] ?? '' ), $m, PREG_SET_ORDER ) ) {
			$last = end( $m );
			$n    = (float) str_replace( ',', '.', $last[1] );
			$u    = strtolower( $last[2] );
			$w    = ( 'kg' === $u || 'l' === $u ) ? $n : $n / 1000;
		}
	}
...
function ps_feeds_google_eilute( array $p ): string {
	...
	$x .= '      <g:availability>' . ( $p['likutis'] > 0 ? 'in_stock' : 'out_of_stock' ) . '</g:availability>' . "\n";
	$x .= '      <g:condition>new</g:condition>' . "\n";
	if ( '' !== $p['reg_kaina'] ) {
		$x .= '      <g:price>' . $p['reg_kaina'] . ' EUR</g:price>' . "\n";
		$x .= '      <g:sale_price>' . $p['kaina'] . ' EUR</g:sale_price>' . "\n";
	} else {
		$x .= '      <g:price>' . $p['kaina'] . ' EUR</g:price>' . "\n";
	}
	$lb = ps_feeds_google_labels( $p );
	foreach ( $lb as $i => $lv ) {
		if ( '' !== $lv ) {
			$x .= '      <g:custom_label_' . $i . '>' . esc_html( $lv ) . '</g:custom_label_' . $i . '>' . "\n";
		}
	}
	...
	$x .= '      <g:shipping_weight>' . ps_feeds_google_svoris( $p ) . ' kg</g:shipping_weight>' . "\n";
