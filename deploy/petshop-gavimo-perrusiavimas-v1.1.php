<?php
/**
 * Petshop Gavimo Perrūšiavimas v1.1 (S1676, 2026-09-12, Raimis: „prekės atvažiavo, suvedžiau per Gavimą, turiu iš karto rinkti užsakymus“)
 *
...
final class Petshop_Gavimo_Perrusiavimas {
	...
	public static function perrusiuoti( $pid, $dry = false ) {
		...
		foreach ( $kand as $k ) {
			...
			if ( $dry ) { $e['dry'] = 'kandidatas'; $rez['eil'][] = $e; continue; }
			// Prieš keičiant — ar AV dar užtenka (ankstesnės eilutės galėjo nurašyti)?
			$r = class_exists( 'Petshop_AV_Source' ) ? Petshop_AV_Source::resolve( $k['pid'], $k['q'] ) : null;
			if ( ! $r || empty( $r['av_uztenka'] ) ) { $e['praleista'] = $r ? $r['reason'] : 'resolverio nėra'; $rez['eil'][] = $e; continue; }
			try {
