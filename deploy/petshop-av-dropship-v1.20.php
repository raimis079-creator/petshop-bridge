<?php
/**
 * Petshop AV Dropship v1.20 (S1670, 2026-09-11) — 🔴 KELI TIEKĖJO ADRESAI PER KABLELĮ NEIŠEIDAVO.
 *
 * RADINYS: `ps_tiekeju_pastai['vf']` = „ieva…@vetfarmas.lt,karolina…@vetfarmas.lt" buvo
 * perduodamas wp_mail() kaip VIENAS masyvo elementas → PHPMailer adresą atmeta tyliai,
 * laiškas išeina tik kopijai (terra@), o pastaba užsakyme rašo visus adresus (#1000/#1001).
 * DABAR: tiekėjo adresai skaidomi per , arba ;, tikrinami is_email(); neteisingas/tuščias
 * adresas → laiškas NESIUNČIAMAS, aiški klaida. Pastaboje ir pranešime — TIKRI gavėjai
 * (PHPMailer To po phpmailer_init). Jei tiekėjas tarp tikrų gavėjų neatsidūrė → ne „išsiųsta".
 *
...
class Petshop_AV_Dropship {
	...
	public static function siusti() {
		...
		$tiek_geri = [];
		if ( $ln['tiekejui'] ) {
			/* v1.20 (S1670): keli adresai per kablelį — skaidom ir tikrinam; kitaip wp_mail visą eilutę tyliai išmeta. */
			$tiek_adr  = array_values( array_filter( array_map( 'trim', preg_split( '/[,;]+/', (string) $pastas ) ) ) );
			$tiek_blogi = array_values( array_filter( $tiek_adr, function ( $a ) { return ! is_email( $a ); } ) );
			$tiek_geri  = array_values( array_filter( $tiek_adr, function ( $a ) { return (bool) is_email( $a ); } ) );
			if ( $tiek_blogi || ! $tiek_geri ) {
				wp_die( 'Laiškas NEIŠSIŲSTAS: ' . esc_html( $vardas ) . ' el. pašto adresas ' . ( $tiek_blogi ? 'neteisingas (' . esc_html( implode( ', ', $tiek_blogi ) ) . ')' : 'nenurodytas' ) . '. Pataisyk tiekėjo adresą ir siųsk dar kartą.' );
			}
			$gav = $tiek_geri;
		}
		...
		/* v1.20: tikri gavėjai — tai, ką PHPMailer realiai įdėjo į „Kam". */
		$tikri    = [];
		$gaudykle = function ( $pm ) use ( &$tikri ) { foreach ( (array) $pm->getToAddresses() as $a ) { $tikri[] = strtolower( (string) $a[0] ); } };
		add_action( 'phpmailer_init', $gaudykle, 1 );
		$ok   = wp_mail( $gav, $tema, $h, $antraste, $priedai );
		remove_action( 'phpmailer_init', $gaudykle, 1 );
		$tikri = array_values( array_unique( $tikri ) );
		if ( $ok && $tiek_geri && ! array_intersect( array_map( 'strtolower', $tiek_geri ), $tikri ) ) {
			$ok = false;
			self::$pasto_klaida = 'tiekėjo adresai į laišką nepateko (' . implode( ', ', $tiek_geri ) . ')';
		}
		if ( $tikri ) { $gav = $tikri; }
