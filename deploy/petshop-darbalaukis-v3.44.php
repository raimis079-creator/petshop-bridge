<?php
/**
 * Petshop Darbalaukis v3.44 (S1709, Raimis 09-23 „daryk“): LP Express siuntos DYDIS darbalaukyje — „Lipdukas LP“ ir „Lipdukas iš naujo“ dialoguose pasirinkimas XS/S/M/L/XL
 *   (numatytas: sukurtos siuntos arba plugino apskaičiuotas). Pakeitus: be siuntos → kuriama su dydžiu (filtras `woo_lithuaniapost_size_service_resolve_order_size` prio 99);
 *   siunta be numerio → perrašoma (`create_parcel` overwriteIdRef); su numeriu → `woo_lithuaniapost__order_action_cancel_labels`, tada nauja. Dialogas: `opt.tipas='sel'`.
 * Petshop Darbalaukis v3.43 (S1680, Raimis 09-14): atšaukto NEAPMOKĖTO užsakymo atkūrimas, kai klientas vėliau sumokėjo pavedimu — kortelėje „Gavome pavedimą — atkurti“ (GET `atkurti`):
 *   bacs, AV žymės išvalomos, `payment_complete()` → processing, eina į darbą; laiškai įprasti.
 * Petshop Darbalaukis v3.42 (S1680, Raimis 09-14 „padaryk tokį principą kaip Venipak“): LP Express lipdukas iš darbalaukio — kortelės veiksmas „Lipdukas LP“ (GET `lp_lipdukas`) →
 *   LP plugino filtras `woo_lithuaniapost_order_action_generate_stickers` (inicijuoja siuntimą, barcode į plugino meta, PDF į buferį) → `uploads/ps-lipdukai/lp/`, `_ps_lp_lipdukas_pdf`,
 *   kortelėje „Lipdukas LP (PDF)“ (GET `lp_pdf`) + „Lipdukas iš naujo“. Anksčiau LP lipdukas buvo tik per WC užsakymų sąrašą (J1), į kurį darbuotojas nebeturi kelio.
 * Petshop Darbalaukis v3.41.2 (S1680, Raimis 09-14): kortelėje prie pristatymo — paštomato pavadinimas ir adresas (Venipak/LP), ne vien ID (`vietos_pav`).
 * Petshop Darbalaukis v3.41.1 (S1676, Raimis „taip“): Venipak kodas 5 (paštomate, laukia kurjerio) laikomas išsiųsta — VENIPAK_PAEME + 5.
 * Petshop Darbalaukis v3.41 (S1676, Raimis 09-12 „neapmokėtas — nematau mokėjimo būdo“): visur, kur „Neapmokėtas“ — sąrašo eilutė, žymė „Visuose“, kortelės takelis — pridėtas būdas: „· pavedimu / · Paysera / · grynais“ (`mok_trumpai()`), naujų stulpelių nėra.
 * Petshop Darbalaukis v3.40 (S1670, Raimis 09-11 „padaryk, kad galėčiau redaguoti kliento el. paštą“): „Redaguoti“ — laukas **El. paštas**
 *   (`set_billing_email`, is_email, prieš/po pastaboje ir įvykyje, laiškas klientui nesiunčiamas). Po lipduko / išsiųsta / uždaryta, kai adreso keisti
 *   nebegalima, „Redaguoti“ lieka aktyvus su `tipas=tik_el` — keičiamas TIK el. paštas (kad išeitų „Išsiųsta“ / sąskaitos laiškai). Adreso taisyklė (v3.18) nepakeista.
 *
...
class Petshop_Darbalaukis {
	...
	const EILES = array(
		...
		'laukiam'    => array( 'Laukiam iš tiekėjų', 'Užsakyta iš tiekėjo į AV — atvažiavus spausk „Gauta“. AV siunta nerenkama, kol visos jos prekės neatvyko.', 'g' ),
		'surinkti'   => array( 'Surinkti AV',        'Visos AV siuntos prekės vietoje — Surinkti → Lipdukas → Kurjeris paėmė.', 'z' ),
		'laiskai'    => array( 'Dropshipping',       'Viena kortelė tiekėjui: siunčia klientui (lipdukai), veža į AV, į atsargas — vienas laiškas „Užsakyti iš [T]“ (laišką gali peržiūrėti).', 'm' ),
	...
	public static function grizti_cia( $location, $status ) {
		...
		if ( isset( $q['tk'] ) && isset( $q['page'] ) && 'ps-tiekimas' === $q['page'] ) {
			...
			$m = array(
				...
				'issaugota'         => array( 'dl_info', 'prekės į AV sudėtos į užsakymą tiekėjui #' . $pid ),
				'prideta'           => array( 'dl_info', 'prekė pridėta į atsargas — užsakymas tiekėjui #' . $pid . ' (išeis su „Užsakyti iš ' . $v . '“)' ), // v3.37
				'nerasta'           => array( 'dl_klaida', $v . '|prekė nerasta — įrašyk pavadinimą iš sąrašo arba SKU' ), // v3.37
	...
	protected static function laisku_langas() { return isset( $_GET['view'] ) && 'laiskai' === $_GET['view'] && ! self::senas(); } // v3.38
...
#wpcontent{padding-left:0!important}#wpbody-content{padding-bottom:0!important}html.wp-toolbar{padding-top:32px!important}body{background:#EEF1EF}.psj-2{display:none!important}</style>';
	...
	/** v3.41: mokėjimo būdas trumpai — pavedimu / Paysera / grynais / (pavadinimas). */
	protected static function mok_trumpai( $o ) {
		$pm = (string) $o->get_payment_method();
		if ( 'bacs' === $pm ) { return 'pavedimu'; }
		if ( false !== stripos( $pm, 'paysera' ) ) { return 'Paysera'; }
		if ( 'cod' === $pm ) { return 'grynais'; }
		$t = trim( (string) $o->get_payment_method_title() );
		return $t !== '' ? $t : 'būdas nežinomas';
	}
	...
	protected static function busena( $f ) {
		...
		if ( in_array( 'neapmoketi', $f['eiles'], true ) ) { return array( 'Neapmokėtas · ' . self::mok_trumpai( $f['o'] ), 'b-amber' ); } // v3.41
	...
	protected static function mygtukas_eilei( $f, $eile ) {
		$leid = array( 'nauji' => array( 'rusiuoti' ), 'surinkti' => array( 'lapai', 'lipdukas' ), 'laukiam' => array( 'tiekimas' ), 'laiskai' => array( 'kons' ), 'paruosta' => array( 'issiusta' ), 'neapmoketi' => array( 'apmoketa' ), 'klausimai' => array( 'spresti' ) );
	...
	protected static function faktai( $o, $z = array() ) {
		...
		foreach ( $o->get_items() as $iid => $it ) {
			...
			if ( $it->get_meta( '_mnm_config' ) || $it->get_meta( '_mnm_container_size' ) ) { $reduced = true; } // S1683: rinkinio (Mix-and-Match) konteineris — likutis komponentuose, trūkumo neskaičiuojam
		...
		elseif ( $neapm ) { $T[] = array( 'apmoketa', 'neapmokėtas · ' . self::mok_trumpai( $o ), 'now', '', 'apmoketa' ); $eiles['neapmoketi'] = 1; } // v3.41
		...
		if ( ! $atsauk && ! $neapm ) {
			...
			if ( $av_side ) {
				...
				foreach ( array_keys( $i_av_src ) as $s ) {
					...
					if ( isset( $i_av_laukia[ $s ] ) ) { $T[] = array( 'lauk_' . $s, 'laukiam iš ' . self::vardas( $s ), $nz ? 'todo' : 'wait', $s, $nz ? null : 'tiekimas' ); if ( $rus && ! $baigta ) { $eiles[ $nz ? 'laiskai' : 'laukiam' ] = 1; } } // v3.37: neužsakyta → Dropshipping kortelė
	...
	protected static function mygtukas( $t, $f ) {
		...
		switch ( $t[4] ) {
			...
			case 'kons':
				...
				return array( 'Užsakyti iš ' . ( count( $v ) > 1 ? implode( ' ir ', $v ) : $v[0] ) . ' į AV', self::url( array( 'eile' => 'laiskai', 'view' => null, 'q' => null, 'b' => null ) ), null, 'p', 0 ); // v3.37: kortelė Dropshipping eilėje
			...
			case 'lipdukas':
				if ( 'lp' === $f['vez'] ) { // v3.42 (S1680, Raimis 09-14 „toks principas kaip Venipak“): LP siunta + lipdukas per plugino API čia pat
					$lpv = self::vietos_pav( $o, $f ); $sv = self::d( 'uzsakymo_svoris', $o );
					return array( 'Lipdukas LP', self::dl_url( 'lp_lipdukas', $id ), array( 'antraste' => $antraste, 'tekstas' => 'Registruoti LP Express siuntą ir suformuoti lipduką? Paštomatas: ' . ( $lpv ? $lpv : '—' ) . '.' . ( $sv > 0 ? ' Svoris ' . number_format( (float) $sv, 2, ',', '' ) . ' kg.' : '' ) . ' Numeris atsiras kortelėje, PDF — nuoroda „Lipdukas LP“.', 'ok' => 'Lipdukas LP', 'opt' => self::lp_dydzio_opt( $o ) ), 'p', 0 ); // v3.44: siuntos dydis
				}
	...
	protected static function veiksmo_url( $v, $id, $g = '' ) {
		return self::dek( wp_nonce_url( admin_url( 'admin-post.php?action=ps_desk_veiksmas&v=' . rawurlencode( $v ) . '&id=' . (int) $id . '&g=' . rawurlencode( $g ?: self::url() ) ), 'ps_desk_' . $v . '_' . (int) $id ) );
	}

	protected static function dek( $u ) { return html_entity_decode( (string) $u, ENT_QUOTES, 'UTF-8' ); }

	protected static function dl_url( $v, $id, $extra = array() ) {
		return self::dek( wp_nonce_url( admin_url( 'admin-post.php?' . http_build_query( array_merge( array( 'action' => 'ps_dl_veiksmas', 'v' => $v, 'id' => (int) $id, 'g' => self::url( array( 'atidaryti' => (int) $id ) ) ), $extra ) ) ), 'ps_dl_' . $v . '_' . (int) $id ) );
	}
	...
	protected static function skydelis( $f ) {
		...
		foreach ( $f['eil'] as $e ) {
			...
			$tiek_url = ( 'i_av' === $e['k'] && $e['b'] && 'gauta' !== $e['b']['busena'] ) ? self::url( array( 'eile' => 'uzsakyta' === $e['b']['busena'] ? 'laukiam' : 'laiskai', 'view' => null, 'q' => null, 'b' => null, 'atidaryti' => null ) ) : ''; // v3.37: kaupiama → Dropshipping kortelė
		...
		$lp_pdf = ''; if ( 'lp' === $f['vez'] ) { $pf = (string) $o->get_meta( '_ps_lp_lipdukas_pdf' ); $lp_pdf = ( $pf && file_exists( $pf ) ) ? self::dl_url( 'lp_pdf', $id ) : ''; if ( $lp_pdf && ! $f['uzdarytas'] ) { $perreg = self::dl_url( 'lp_lipdukas', $id ); $perreg_d = array( 'antraste' => sprintf( 'Užsakymas #%s', $o->get_order_number() ), 'tekstas' => 'Suformuoti LP lipduką iš naujo? Jei pakeisi dydį — sena LP siunta bus atšaukta ir užregistruota nauja (naujas numeris; seną lipduką išmesk).', 'ok' => 'Lipdukas iš naujo', 'opt' => self::lp_dydzio_opt( $o ) ); } } // v3.42; v3.44 dydis
		$b = $f['btn'];
		...
		return array(
			...
			'vezejas' => $f['ats'] ? 'Atsiėmimas AV' : self::d( 'vezejo_vardas', $o ), 'vieta' => self::vietos_pav( $o, $f ), /* v3.41.2 */ 'pastaba_kl' => $o->get_customer_note(), /* v3.35 (C) */
			'eil' => $eil, 'pastaba' => $pastaba, 'nr_siuntos' => $nr, 'pak' => $pak, 'perreg' => $perreg, 'perreg_d' => isset( $perreg_d ) ? $perreg_d : null, 'lp_pdf' => $lp_pdf, /* v3.42; v3.44 perreg_d */
			'klausimas' => $f['kl'],
			...
			'atkurti' => self::atkuriamas( $o ) ? self::dl_url( 'atkurti', $id ) : '', // v3.43: atšauktas neapmokėtas, klientas vėliau sumokėjo pavedimu → atkurti
	...
	/** v3.42 (S1680): LP Express siunta + lipdukas iš darbalaukio — kaip Venipak, bet per LP plugino kelią (filtras `woo_lithuaniapost_order_action_generate_stickers`:
	 *  inicijuoja siuntimą (barcode į `_woo_lithuaniapost_barcode`) ir atsiunčia lipdukų PDF į išvestį). PDF gaudomas buferiu, saugomas `uploads/ps-lipdukai/lp/`,
	 *  `_ps_lp_lipdukas_pdf`; jei siunta LP sistemoje dar nesukurta — pirma `woo_lithuaniapost_order_action_create_parcel`. Registras `_ps_siuntos` neliečiamas (numeris — plugino meta, kaip iki šiol). */
	protected static function lp_lipdukas( $o, $u ) {
		...
		$id = $o->get_id();
		// v3.44 (S1709, Raimis 09-23): siuntos dydis iš dialogo (XS…XL). Pluginas dydį parenka pats pagal matmenis ir siuntą dažnai sukuria jau kasoje —
		// pakeitus dydį: nėra siuntos → kuriama su pasirinktu; siunta yra be numerio → perrašoma (overwriteIdRef, kaip pluginas daro po klaidos);
		// jau su numeriu (lipdukas buvo) → sena atšaukiama plugino `cancel_labels`, tada nauja. Plugino kodas neliečiamas — tik jo filtrai.
		$dyd = strtoupper( sanitize_key( (string) ( $_GET['dydis'] ?? '' ) ) ); if ( ! in_array( $dyd, self::LP_DYDZIAI, true ) ) { $dyd = ''; }
		$dyd_buvo = strtoupper( (string) $o->get_meta( '_woo_lithuaniapost_shipping_item_size' ) );
		if ( $dyd && $dyd !== $dyd_buvo ) {
			$bc0 = $o->get_meta( '_woo_lithuaniapost_barcode' ); $bc0 = is_array( $bc0 ) ? implode( ',', array_filter( $bc0 ) ) : trim( (string) $bc0 );
			if ( $bc0 ) {
				if ( ! has_filter( 'woo_lithuaniapost__order_action_cancel_labels' ) ) { return array( 'dl_klaida', 'LP plugino atšaukimo veiksmo nėra — dydžio keisti negalima' ); }
				$cr = apply_filters( 'woo_lithuaniapost__order_action_cancel_labels', array( $id ) ); $o = wc_get_order( $id );
				if ( ( is_array( $cr ) && ! empty( $cr['errors'] ) ) || false === $cr || $o->get_meta( '_woo_lithuaniapost_barcode' ) ) { return array( 'dl_klaida', 'senos LP siuntos ' . $bc0 . ' atšaukti nepavyko (gal jau paimta?) — dydis nepakeistas, lipdukas liko senas' ); }
				$o->add_order_note( 'Darbalaukis: LP siunta ' . $bc0 . ' (' . ( $dyd_buvo ?: '?' ) . ') atšaukta dėl dydžio keitimo į ' . $dyd . ' (' . $u->display_name . ').', false, true );
			}
			$uzd = function () use ( $dyd ) { return $dyd; };
			add_filter( 'woo_lithuaniapost_size_service_resolve_order_size', $uzd, 99, 0 );
			do_action( 'woo_lithuaniapost_order_action_create_parcel', $o, true );
			remove_filter( 'woo_lithuaniapost_size_service_resolve_order_size', $uzd, 99 );
			$o = wc_get_order( $id );
			if ( ! $o->get_meta( '_woo_lithuaniapost_shipping_item_id' ) || $dyd !== strtoupper( (string) $o->get_meta( '_woo_lithuaniapost_shipping_item_size' ) ) ) { $e = self::siuntos_klaida( $o, 'lp' ); return array( 'dl_klaida', 'LP siuntos su dydžiu ' . $dyd . ' sukurti nepavyko' . ( $e ? ' — ' . $e['t'] : '' ) ); }
		}
		...
		$dz = strtoupper( (string) $o->get_meta( '_woo_lithuaniapost_shipping_item_size' ) );
		$o->add_order_note( 'Darbalaukis: LP Express siunta ' . ( $bc ? $bc : '(numerio negauta)' ) . ( $dz ? ' (dydis ' . $dz . ')' : '' ) . ' užregistruota, lipdukas suformuotas (' . $u->display_name . ').', false, true ); $o->save();
		return array( 'dl_info', 'LP Express siunta ' . ( $bc ? $bc : '' ) . ( $dz ? ' (dydis ' . $dz . ')' : '' ) . ' užregistruota — lipdukas: kortelėje „Lipdukas LP (PDF)“' );
	}

	/** v3.44 (S1709): LP paštomato (TERMINAL) dydžiai — plugino `get_bp_sizes`. */
	const LP_DYDZIAI = array( 'XS', 'S', 'M', 'L', 'XL' );

	/** v3.44: dialogo pasirinkimas „Dydis“ — numatytas: jau sukurtos siuntos dydis arba plugino apskaičiuotas (pagal matmenis/svorį). */
	protected static function lp_dydzio_opt( $o ) {
		$dab = strtoupper( (string) $o->get_meta( '_woo_lithuaniapost_shipping_item_size' ) ); $kaip = 'dabartinis';
		if ( ! $dab ) { $kaip = 'apskaičiuotas'; try { $m = apply_filters( 'woo_lithuaniapost__order_action_get_lp_shipping_method', $o ); $x = $m ? apply_filters( 'woo_lithuaniapost_size_service_resolve_order_size', $o, $m ) : ''; $dab = is_string( $x ) ? strtoupper( $x ) : ''; } catch ( Throwable $e ) { $dab = ''; } }
		if ( ! in_array( $dab, self::LP_DYDZIAI, true ) ) { $dab = 'M'; $kaip = 'numatytas'; }
		$var = array(); foreach ( self::LP_DYDZIAI as $d ) { $var[] = array( $d, $d . ( $d === $dab ? ' (' . $kaip . ')' : '' ) ); }
		return array( 'vardas' => 'dydis', 'tekstas' => 'Siuntos dydis', 'def' => $dab, 'tipas' => 'sel', 'var' => $var );
	}
	...
	protected static function redagavimas( $f ) {
		$r = self::red_pilnas( $f ); $o = $f['o']; $id = $f['id'];
		if ( $r ) { $r['laukai']['el'] = (string) $o->get_billing_email(); return $r; }
		// v3.40: adreso keisti nebegalima — bet el. paštą galima visada (laiškai klientui).
		return array( 'u' => admin_url( 'admin-post.php' ), 'n' => wp_create_nonce( 'ps_dl_red_' . $id ), 'g' => self::url( array( 'atidaryti' => $id ) ), 'tipas' => 'tik_el', 'laukai' => array( 'el' => (string) $o->get_billing_email() ), 'vieta' => array( 'id' => '', 't' => '' ) );
	}
	protected static function red_pilnas( $f ) {
		$o = $f['o']; $id = $f['id'];
	...
	/** v3.41.2 (S1680, Raimis 09-14 „noriu matyti į kokį paštomatą užsakė“): kortelei — paštomato pavadinimas ir adresas (Venipak per plugino `venipak_resolve_order_pickup`, LP — plugino meta), ne vien ID. */
	protected static function vietos_pav( $o, $f ) {
		if ( 'lp' === $f['vez'] ) { $t = (string) $o->get_meta( '_woo_lithuaniapost_lpexpress_terminal' ); return $t ? $t : (string) $o->get_meta( '_woo_lithuaniapost_lpexpress_terminal_id' ); }
		if ( 'venipak_pastomatas' !== $f['vez'] ) { return ''; }
		$id = (string) $o->get_meta( 'venipak_pickup_point' ); if ( ! $id ) { return ''; }
		$pt = function_exists( 'venipak_resolve_order_pickup' ) ? venipak_resolve_order_pickup( $o ) : false;
		return $pt ? self::vietos_tekstas( $pt ) : $id;
	}
	...
	public static function redaguoti_vykdyti() {
		...
		try {
			...
			if ( 'tik_el' !== $red['tipas'] && $tel !== (string) $red['laukai']['tel'] ) { if ( '' === $tel ) { $baigti( 'dl_klaida', 'telefonas privalomas' ); } $pries['tel'] = $red['laukai']['tel']; $po['tel'] = $tel; $zin[] = 'Tel.: „' . $red['laukai']['tel'] . '“ → „' . $tel . '“'; $o->set_billing_phone( $tel ); $o->set_shipping_phone( $tel ); }
			// v3.40: kliento el. paštas.
			if ( array_key_exists( 'el', $_POST ) ) {
				$el_raw = trim( (string) wp_unslash( $_POST['el'] ) ); $el_buvo = (string) $o->get_billing_email();
				if ( $el_raw !== $el_buvo ) {
					if ( '' === $el_raw ) { $baigti( 'dl_klaida', 'el. paštas negali būti tuščias' ); }
					$el = sanitize_email( $el_raw ); if ( ! is_email( $el ) || $el !== $el_raw ) { $baigti( 'dl_klaida', 'neteisingas el. pašto adresas: „' . $el_raw . '“' ); }
					$pries['el'] = $el_buvo; $po['el'] = $el; $zin[] = 'El. paštas: „' . $el_buvo . '“ → „' . $el . '“'; $o->set_billing_email( $el );
				}
			}
			...
			$nuimta = '';
			if ( $f['siuntos_klaida'] && 'tik_el' !== $red['tipas'] ) {
			...
			$o->add_order_note( sprintf( 'Darbalaukis: ' . ( 'tik_el' === $red['tipas'] ? 'kliento el. paštas pakeistas' : 'pristatymo duomenys pakeisti' ) . ' (%s): %s. %sKlientui laiškas nesiųstas (Raimis 09-04: jei reikia — darbuotojas rašo pats).', $u->display_name, implode( '; ', $zin ), $nuimta ), false, true ); $o->save();
			...
			$baigti( 'dl_info', ( 'tik_el' === $red['tipas'] ? 'el. paštas pakeistas: ' : 'pristatymo duomenys pakeisti: ' ) . implode( '; ', $zin ) . ( $nuimta ? ' · ' . trim( $nuimta ) : '' ) );
	...
	public static function vykdyti() {
		...
		if ( 'lp_pdf' === $v ) { self::lp_pdf_faila( $o ); exit; } // v3.42
		...
		try {
			...
			elseif ( 'lp_lipdukas' === $v ) { $rez = self::lp_lipdukas( $o, $u ); } // v3.42
			...
			elseif ( 'atkurti' === $v ) { $rez = self::atkurti_vykdyti( $o, $u ); } // v3.43
	...
	public static function tiekimas_vykdyti() {
		...
		if ( ! $src || ! class_exists( 'Petshop_AV_Tiekimas' ) || ! in_array( $ka, array( 'uzsakyti', 'priimti', 'prideti', 'eilute' ), true ) ) { $klaida( 'nežinomas veiksmas' ); }
		if ( 'eilute' === $ka ) { // v3.38: atsargų eilutės kiekis / išėmimas (tik kaupiama partija, tik `order_id IS NULL`) — kaip variklio `veiksmas()` qty[] / trinti[]
			global $wpdb; $eid = absint( $_POST['eid'] ?? 0 ); $e = $eid ? $wpdb->get_row( $wpdb->prepare( "SELECT e.*, p.busena, p.tiekejas FROM {$wpdb->prefix}ps_tiekimas_eil e JOIN {$wpdb->prefix}ps_tiekimas p ON p.id = e.partija_id WHERE e.id = %d", $eid ) ) : null;
			if ( ! $e || $e->order_id || 'kaupiama' !== $e->busena || $e->tiekejas !== $src ) { $klaida( 'eilutės keisti negalima (jau užsakyta arba tai užsakymo prekė)' ); }
			if ( ! empty( $_POST['trinti'] ) ) { $wpdb->delete( $wpdb->prefix . 'ps_tiekimas_eil', array( 'id' => $eid ) ); $z = 'prekė išimta iš užsakymo tiekėjui #' . (int) $e->partija_id; }
			else { $q = max( 1, min( 999, absint( $_POST['qty'] ?? 0 ) ) ); $wpdb->update( $wpdb->prefix . 'ps_tiekimas_eil', array( 'qty' => $q ), array( 'id' => $eid ) ); $z = 'kiekis ' . $q . ' — užsakymas tiekėjui #' . (int) $e->partija_id; }
			do_action( 'ps_juosta_isvalyti' ); wp_safe_redirect( add_query_arg( array( 'pd_ok' => 'dl_info', 'pd_nr' => rawurlencode( self::vardas( $src ) . '|' . $z ) ), $atgal ) ); exit;
		}
		if ( 'prideti' === $ka && ! empty( $_POST['prekes'] ) && is_array( $_POST['prekes'] ) ) { // v3.38: rinkiklis — kelios prekės iš karto (kaip variklio 'pridėti', sujungiant su esama eilute)
			global $wpdb; $pid = Petshop_AV_Tiekimas::atvira_partija( $src ); $n = 0; $vardai = array();
			foreach ( $_POST['prekes'] as $id_ => $q_ ) { $id_ = absint( $id_ ); $q_ = max( 1, min( 999, absint( $q_ ) ) ); $pr_ = $id_ ? wc_get_product( $id_ ) : null; if ( ! $pr_ ) { continue; }
				$yra = $wpdb->get_row( $wpdb->prepare( "SELECT id, qty FROM {$wpdb->prefix}ps_tiekimas_eil WHERE partija_id = %d AND product_id = %d AND order_id IS NULL LIMIT 1", $pid, $id_ ) );
				if ( $yra ) { $wpdb->update( $wpdb->prefix . 'ps_tiekimas_eil', array( 'qty' => (int) $yra->qty + $q_ ), array( 'id' => (int) $yra->id ) ); } else { $wpdb->insert( $wpdb->prefix . 'ps_tiekimas_eil', array( 'partija_id' => $pid, 'product_id' => $id_, 'order_id' => null, 'qty' => $q_ ) ); }
				$n++; if ( count( $vardai ) < 3 ) { $vardai[] = $q_ . '× ' . mb_substr( $pr_->get_name(), 0, 40 ); } }
			if ( ! $n ) { $klaida( 'nepažymėta nė viena prekė' ); }
			do_action( 'ps_juosta_isvalyti' ); wp_safe_redirect( add_query_arg( array( 'pd_ok' => 'dl_info', 'pd_nr' => rawurlencode( self::vardas( $src ) . '|į atsargas pridėta ' . $n . ' prek. (' . implode( '; ', $vardai ) . ( $n > 3 ? '…' : '' ) . ') — užsakymas tiekėjui #' . $pid ) ), $atgal ) ); exit;
		}
		if ( 'prideti' === $ka ) { // v3.37: „į atsargas“ pagal SKU / ID — variklio `veiksmas()` 'pridėti' (nauja_sku, nauja_qty)
			$pid = Petshop_AV_Tiekimas::atvira_partija( $src ); $sku = sanitize_text_field( wp_unslash( $_POST['nauja_sku'] ?? '' ) ); if ( '' === $sku ) { $klaida( 'įrašyk prekę' ); }
			$_POST['ka'] = 'pridėti'; $_POST['partija'] = $pid; $_POST['_wpnonce'] = wp_create_nonce( 'ps_tiekimas_' . $pid ); $_REQUEST['_wpnonce'] = $_POST['_wpnonce'];
			do_action( 'ps_juosta_isvalyti' ); do_action( 'admin_post_ps_tiekimas' ); exit;
		}
	...
	/** v3.37: „Užsakyti iš [T]“ iš vienos kortelės — pažymėtos „veža į AV“ eilutės į kaupiamą partiją (`ideti_eilute`, `_ps_konsolidacija`), partijos pristatymas/svoris/dėžės,
	 *  tada: yra pažymėtų Dropshipping užs. → variklio `ps_dropship_send` su `su_partija` (VIENAS laiškas); nėra → variklio `ps_tiekimas` `uzsakyti` (savas laiškas; ZB — be laiško). Variklis nekeistas. */
	public static function uzsakyti_vykdyti() {
		if ( ! current_user_can( 'edit_shop_orders' ) ) { wp_die( 'Nepakanka teisių' ); }
		$src = sanitize_key( wp_unslash( $_POST['tiekejas'] ?? '' ) ); check_admin_referer( 'ps_dl_uzs_' . $src );
		$atgal = wp_validate_redirect( wp_unslash( $_POST['ps_dl_g'] ?? '' ), admin_url( 'admin.php?page=' . self::SLUG . '&eile=laiskai' ) );
		$klaida = function ( $t ) use ( $atgal, $src ) { wp_safe_redirect( add_query_arg( array( 'pd_ok' => 'dl_klaida', 'pd_nr' => rawurlencode( self::vardas( $src ) . '|' . $t ) ), $atgal ) ); exit; };
		$tk = class_exists( 'Petshop_AV_Tiekimas' ); if ( ! $src || ! $tk ) { $klaida( 'nežinomas tiekėjas' ); }
		$csv = function ( $k ) { return isset( $_POST[ $k ] ) ? array_values( array_filter( array_map( 'absint', explode( ',', sanitize_text_field( wp_unslash( $_POST[ $k ] ) ) ) ) ) ) : array(); };
		$ids_ds = $csv( 'uzsakymai' ); $ids_av = $csv( 'ids_av' ); $rank = Petshop_AV_Tiekimas::rankinis( $src ); $u = wp_get_current_user(); $pirmas = 0;
		if ( 'zb' === $src ) { $ids_ds = array(); } // ZB dropship — atskiras „Pažymėti ZB perduotais“
		foreach ( $ids_av as $oid ) {
			$o = wc_get_order( $oid ); if ( ! $o || ! $o->is_paid() ) { continue; }
			$spr = self::d( 'misrus_sprendimas', $o );
			foreach ( $o->get_items() as $iid => $it ) {
				list( $k, $s ) = self::eilutes_kelias( $o, $iid, $it, $spr );
				if ( 'i_av' !== $k || $s !== $src || Petshop_AV_Tiekimas::eilutes_bukle( $oid, (int) $iid ) ) { continue; }
				if ( $pid_ = Petshop_AV_Tiekimas::ideti_eilute( $o, (int) $iid, $src ) ) {
					$it->update_meta_data( '_ps_konsolidacija', 1 ); $it->save(); if ( ! $pirmas ) { $pirmas = $oid; }
					if ( class_exists( 'Petshop_Uzsakymu_Ivykiai' ) ) { Petshop_Uzsakymu_Ivykiai::irasyti( array( 'uzsakymas' => $oid, 'eilute' => (int) $iid, 'sritis' => 'desk', 'veiksmas' => 'kons', 'rezultatas' => 'ok', 'kanalas' => 'web', 'po' => array( 'partija' => $pid_, 'tiekejas' => $src ), 'pastaba' => mb_substr( $it->get_name(), 0, 60 ) . ' → užsakymas tiekėjui #' . $pid_ ) ); }
				}
			}
		}
		$kaup = Petshop_AV_Tiekimas::atvira_su_eilutemis( $src ); $pid = $kaup ? (int) $kaup['part']->id : 0;
		if ( $pid ) {
			$upd = array(); $b = sanitize_key( wp_unslash( $_POST['pristatymas'] ?? '' ) );
			if ( isset( Petshop_AV_Tiekimas::PRISTATYMAI[ $b ] ) ) { $upd['pristatymas'] = $b; } elseif ( ! $rank && empty( $kaup['part']->pristatymas ) ) { $klaida( 'pasirink „Kaip atkeliaus į AV“' ); }
			if ( isset( $_POST['svoris'] ) ) { $w = (float) str_replace( ',', '.', sanitize_text_field( wp_unslash( $_POST['svoris'] ) ) ); $upd['svoris'] = $w > 0 ? round( $w, 2 ) : null; }
			if ( isset( $_POST['dezes'] ) ) { $upd['dezes'] = max( 1, min( 20, absint( $_POST['dezes'] ) ) ); }
			if ( isset( $_POST['pastaba'] ) ) { $upd['pastaba'] = mb_substr( sanitize_textarea_field( wp_unslash( $_POST['pastaba'] ) ), 0, 1000 ); } // v3.38.1: prierašas → savas Tiekimo laiškas
			if ( $upd ) { global $wpdb; $wpdb->update( $wpdb->prefix . 'ps_tiekimas', $upd, array( 'id' => $pid ) ); } // partijos savybės — kaip variklio `veiksmas()` 'issaugoti' (H238)
			if ( ! $pirmas ) { foreach ( Petshop_AV_Tiekimas::partijos_eilutes( $pid ) as $e ) { if ( $e->order_id ) { $pirmas = (int) $e->order_id; break; } } }
		}
		do_action( 'ps_juosta_isvalyti' );
		if ( $ids_ds ) {
			$_POST['uzsakymai'] = implode( ',', $ids_ds ); if ( $pid ) { $_POST['su_partija'] = $pid; $_POST['partija_dezes'] = $upd['dezes'] ?? 0; } else { unset( $_POST['su_partija'] ); }
			$_POST['_wpnonce'] = wp_create_nonce( 'ps_dropship_send' ); $_REQUEST['_wpnonce'] = $_POST['_wpnonce'];
			do_action( 'admin_post_ps_dropship_send' ); // variklis: laiškas + lipdukai + partija (`uzdaryti_po_laisko`), grįžimas per `grizti_cia()`
			exit;
		}
		if ( $pid ) {
			$_POST['partija'] = $pid; $_POST['ka'] = 'uzsakyti'; $_POST['ps_dl_pirmas'] = $pirmas; $_POST['_wpnonce'] = wp_create_nonce( 'ps_tiekimas_' . $pid ); $_REQUEST['_wpnonce'] = $_POST['_wpnonce'];
			do_action( 'admin_post_ps_tiekimas' ); // variklis: savas laiškas (ZB — be laiško), grįžimas per `grizti_cia()`
			exit;
		}
		$klaida( 'nėra ko užsakyti — nepažymėtas nė vienas užsakymas ir prekių į AV nėra' );
	}
	 ...
	 *  v3.41.1 (S1676, Raimis „taip“): 5 At pickup point waiting for courier — siunta įdėta į paštomatą mūsų pusėje = išsiųsta (#35872/#1002 atvejai). */
	const VENIPAK_PAEME = array( 1, 2, 3, 5, 6, 9 );
	...
	protected static function kreditine_laiskas( $o, $u, $idx ) {
		...
		list( $tema, $h ) = self::kr_laiskas_sudeti( $o, $kr, (float) ( $g[ $idx ]['suma'] ?? 0 ) ); // v3.31: šablonas iš opcijos (Sąskaitų lange), Raimis #10
	...
	protected static function saskaitos() {
		...
		$viso = $wpdb->get_results( $args ? $wpdb->prepare( $sql, $args ) : $sql, ARRAY_A ); $n = count( $viso ); $suma = 0.0; foreach ( $viso as $r ) { if ( 'ppk' === $r['t'] ) { continue; } $suma += (float) $r['s']; } // v3.34: kvitai (grynų priėmimas) į sąskaitų sumą neskaičiuojami
		...
		echo '<main class="dl-main"><h1 class="dl-h1">Sąskaitos <small>visos PVM sąskaitos, išankstinės, kreditinės ir kvitai — PDF čia, WC lango nereikia · ' . (int) $n . ' dok.</small></h1>';
...
(function(){ var F=document.getElementById('dlNu'); if(!F) return; var RATE=parseFloat(F.getAttribute('data-rate')||'21')/100, PM=JSON.parse(F.getAttribute('data-pm')||'{}'), N=F.getAttribute('data-n'); var $=function(i){return document.getElementById(i);};
	...
	/** v3.38: AJAX `ps_dl_atsargos` — tiekėjo prekės rinkikliui: src (`_ps_sandelis`), zenklas (product_brand term_id), isparduota (WC likutis ≤ 0 = katalogo „reikia užsakyti“), q (filtras). ≤ 300 eil. */
	public static function ajax_atsargos() {
		if ( ! current_user_can( 'edit_shop_orders' ) || ! check_ajax_referer( 'ps_dl_zurnalas', 'n', false ) ) { wp_send_json_error( 'teisės', 403 ); }
		global $wpdb; $src = sanitize_key( wp_unslash( $_GET['src'] ?? '' ) ); $z = absint( $_GET['zenklas'] ?? 0 ); $isp = ! empty( $_GET['isparduota'] ); $q = trim( (string) wp_unslash( $_GET['q'] ?? '' ) );
		if ( ! $src || ( ! $z && ! $isp && mb_strlen( $q ) < 2 ) ) { wp_send_json_success( array() ); }
		$sql = "SELECT p.ID FROM {$wpdb->posts} p JOIN {$wpdb->postmeta} m ON m.post_id = p.ID AND m.meta_key = '_ps_sandelis' AND m.meta_value = %s LEFT JOIN {$wpdb->prefix}wc_product_meta_lookup l ON l.product_id = p.ID"; $w = array( $src );
		if ( $z ) { $sql .= " JOIN {$wpdb->term_relationships} r ON r.object_id = p.ID JOIN {$wpdb->term_taxonomy} tt ON tt.term_taxonomy_id = r.term_taxonomy_id AND tt.taxonomy = 'product_brand' AND tt.term_id = %d"; $w[] = $z; }
		$sql .= " WHERE p.post_status = 'publish' AND p.post_type = 'product'"; if ( $isp ) { $sql .= " AND ( l.stock_quantity IS NULL OR l.stock_quantity <= 0 OR l.stock_status = 'outofstock' )"; }
		if ( mb_strlen( $q ) >= 2 ) { $like = '%' . $wpdb->esc_like( $q ) . '%'; $sql .= ' AND ( p.post_title LIKE %s OR l.sku LIKE %s )'; $w[] = $like; $w[] = $like; }
		$sql .= ' ORDER BY p.post_title ASC LIMIT 300';
		$ids = $wpdb->get_col( $wpdb->prepare( $sql, $w ) ); if ( ! $ids ) { wp_send_json_success( array() ); }
		$pask = array(); if ( class_exists( 'Petshop_Partijos' ) && $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->prefix}ps_partijos'" ) ) { foreach ( (array) $wpdb->get_results( "SELECT product_id, gauta, kiekis_gautas, savikaina_eur FROM {$wpdb->prefix}ps_partijos WHERE atsaukta = 0 AND product_id IN (" . implode( ',', array_map( 'intval', $ids ) ) . ') ORDER BY gauta ASC, id ASC' ) as $r ) { $pask[ (int) $r->product_id ] = $r; } }
		$tk = 'vf' === $src ? '_vf_qty' : ''; $ck = array( 'vf' => '_vf_cost', 'zb' => '_zb_cost' ); $out = array();
		foreach ( $ids as $id ) { $p = wc_get_product( (int) $id ); if ( ! $p ) { continue; } $pr = $pask[ (int) $id ] ?? null; $sav = $pr ? (float) $pr->savikaina_eur : ( isset( $ck[ $src ] ) ? (float) $p->get_meta( $ck[ $src ] ) : (float) $p->get_meta( '_cost_price' ) );
			$out[] = array( 'id' => (int) $id, 'n' => $p->get_name(), 'sku' => (string) $p->get_sku(), 'av' => class_exists( 'Petshop_AV_Stock' ) ? (int) Petshop_AV_Stock::qty( (int) $id ) : (int) $p->get_stock_quantity(), 'tiek' => $tk ? (int) $p->get_meta( $tk ) : null, 'stock' => (int) $p->get_stock_quantity(), 'pask' => $pr ? mb_substr( (string) $pr->gauta, 0, 10 ) . ' · ' . (int) $pr->kiekis_gautas . ' vnt.' : '', 'sav' => $sav > 0 ? round( $sav, 2 ) : null, 'kaina' => round( (float) wc_get_price_including_tax( $p ), 2 ) ); }
		wp_send_json_success( $out );
	}
	 ...
	 *  LP terminalas plugino meta. Apmokėjimas: `pavedimu` → bacs + on-hold (WC / temos srautas kaip kasoje: laiškas su išankstine; v3.34: WC likutis nurašomas tik apmokėjus — `telefonu_likutis`); `grynais` (v3.34, buvo `vietoje`) → „Apmokėta grynais“ (`cod`) + processing (varikliai kaip po Paysera); kvitas PPK — atskirai skydelyje. */
	public static function naujas_vykdyti() {
		...
		$mok = sanitize_key( $_POST['mok'] ?? 'pavedimu' ); if ( ! in_array( $mok, array( 'pavedimu', 'grynais' ), true ) ) { $klaida( 'nežinomas apmokėjimo būdas' ); } // v3.34: `vietoje` → `grynais`
		...
		try {
			...
			$n->add_order_note( sprintf( 'Darbalaukis: užsakymas priimtas telefonu / vietoje (%s). Prekės: %s. Pristatymas: %s%s — %s €. %sApmokėjimas: %s.', $u->display_name, implode( ', ', $eil_t ), $pm[ $pk ]['t'], $vp_t ? ' „' . ( $vp_t['name'] ?? '' ) . ', ' . ( $vp_t['city'] ?? '' ) . '“' : ( $lp_t ? ' „' . $lp_t['name'] . ', ' . $lp_t['city'] . '“' : '' ), self::eur( $prist_g ), $nuol > 0 ? 'Nuolaida ' . self::eur( $nuol ) . ' € — ' . $nuol_p . '. ' : '', 'grynais' === $mok ? 'apmokėta grynais (kvitas PPK — skydelyje, kai klientas moka)' : 'pavedimu — laukiam' ), false, true );
			...
			$n = wc_get_order( $id );
			if ( class_exists( 'Petshop_Uzsakymu_Ivykiai' ) ) { Petshop_Uzsakymu_Ivykiai::irasyti( array( 'uzsakymas' => $id, 'sritis' => 'desk', 'veiksmas' => 'naujas', 'rezultatas' => 'ok', 'kanalas' => 'web', 'kas' => $u->ID, 'kas_vardas' => $u->display_name, 'po' => array( 'mok' => $mok, 'prist' => $pk, 'vieta' => $vieta, 'nuolaida' => $nuol, 'suma' => (float) $n->get_total(), 'klientas' => $uid, 'prekes' => $eil_t ), 'pastaba' => 'telefoninis užsakymas ' . self::eur( $n->get_total() ) . ' € (' . ( 'grynais' === $mok ? 'apmokėta grynais' : 'pavedimu' ) . ')' ) ); }
			do_action( 'ps_juosta_isvalyti' );
	...
	/** v3.43 (S1680, Raimis 09-14 „klientė pasirinko Paysera, neapmokėjo, sistema po 90 min atšaukė, o ji apmokėjo pavedimu“): ar atšauktą NEAPMOKĖTĄ užsakymą galima atkurti. */
	public static function atkuriamas( $o ) {
	...
	/** v3.43: GET `atkurti` — mokėjimo būdas → bankinis pavedimas (bacs), AV mažinimo žymės išvalomos (jei buvo sumažinta ir grąžinta — kad `Petshop_AV_Reduce::mazinti` suveiktų iš naujo),
	 *  `payment_complete()` (WC: cancelled → processing, apmokėjimo data, sandėlis; variklio/AV kabliai suveikia kaip įprastam apmokėjimui). Laiškai NEišjungiami — klientas sumokėjo, „vykdomas“ laiškas tinka. */
	protected static function atkurti_vykdyti( $o, $u ) {
		if ( ! self::atkuriamas( $o ) ) { return array( 'dl_klaida', 'atkurti negalima — tik atšauktas NEAPMOKĖTAS užsakymas be grąžinimų ir PVM sąskaitos' ); }
		$id = $o->get_id(); $sen = $o->get_payment_method_title();
		$gw = WC()->payment_gateways() ? WC()->payment_gateways()->payment_gateways() : array(); $t = isset( $gw['bacs'] ) ? $gw['bacs']->get_title() : 'Bankinis pavedimas';
		$o->set_payment_method( 'bacs' ); $o->set_payment_method_title( $t );
		$isv = array(); foreach ( array( '_ps_av_reduced', '_ps_av_restored' ) as $k ) { if ( $o->get_meta( $k ) ) { $o->delete_meta_data( $k ); $isv[] = $k; } }
		$o->add_order_note( sprintf( 'Darbalaukis: atšauktas užsakymas ATKURTAS — klientas sumokėjo bankiniu pavedimu (%s €). Mokėjimo būdas: %s → %s.%s (%s)', self::eur( $o->get_total() ), $sen ? $sen : '—', $t, $isv ? ' AV žymės išvalytos: ' . implode( ', ', $isv ) . '.' : '', $u->display_name ), false, true );
		$o->save();
		$o->payment_complete();
		$o = wc_get_order( $id );
		if ( ! $o->is_paid() ) { return array( 'dl_klaida', 'atkurti nepavyko (būsena ' . $o->get_status() . ') — pasakyk Raimiui' ); }
		return array( 'dl_info', 'užsakymas atkurtas ir apmokėtas pavedimu — būsena ' . $o->get_status() . ( 'processing' === $o->get_status() ? ', eina į darbą („Nauji“ — rūšiuoti)' : ' — pažiūrėk „Klausimuose“' ) );
	}
	...
	protected static function url( $args = array() ) {
		...
		foreach ( array( 'eile', 'q', 'data', 'nuo', 'iki', 'vykdymas', 'vezejas', 'b', 'r', 'psl', 'mok', 'st', 'suma_nuo', 'suma_iki' ) as $k ) { if ( isset( $_GET[ $k ] ) && '' !== $_GET[ $k ] ) { $b[ $k ] = sanitize_text_field( wp_unslash( $_GET[ $k ] ) ); } }
	...
	public static function puslapis() {
		...
		if ( self::naujas_langas() ) { self::stilius(); echo '<div class="dl" id="dl" data-eile="naujas" data-atid="0" data-n="' . esc_attr( wp_create_nonce( 'ps_dl_zurnalas' ) ) . '">'; self::pranesimas(); self::naujas(); self::skydelio_html(); self::dialogas(); self::skriptas(); self::naujas_skriptas(); echo '</div>'; return; } // v3.33 (B)
		...
		if ( self::laisku_langas() ) { self::stilius(); echo '<div class="dl" id="dl" data-eile="laiskai_arch" data-atid="0" data-n="' . esc_attr( wp_create_nonce( 'ps_dl_zurnalas' ) ) . '">'; self::pranesimas(); self::laisku_archyvas(); self::skydelio_html(); self::dialogas(); self::skriptas(); echo '</div>'; return; } // v3.38
	...
	protected static function filtru_juosta( $eile, $f ) {
		$akt = $f['vykdymas'] || $f['vezejas'] || $f['data'] || $f['r'] || $f['mok'] || $f['st'] || '' !== $f['suma_nuo'] || '' !== $f['suma_iki']; // v3.36
	...
	protected static function lentele( $rows, $eile ) {
		...
		foreach ( $rows as $r ) {
			...
			echo '<br>' . esc_html( $vardas ?: '—' ) . ' <span class="pilkas maz">· ' . esc_html( self::d( 'vezejo_vardas', $o ) ) . ( $miestas ? ', ' . esc_html( $miestas ) : '' ) . ' · ' . esc_html( wp_strip_all_tags( $o->get_formatted_order_total() ) ) . ( $r['paid'] ? '' : ' · <b class="raud">neapmokėta · ' . esc_html( self::mok_trumpai( $o ) ) . '</b>' ) . '</span>';
			...
			foreach ( $gr as $k => $es ) { $e0 = $es[0]; $n = 0; foreach ( $es as $e ) { $n += $e['q']; } $bad = array(); foreach ( $es as $e ) { if ( false === $e['av_ok'] ) { $bad[] = 'Avesoje ' . (int) $e['av_qty'] . ', reikia ' . $e['q']; } }
				$pav = $e0['n']; // v3.35.2: pilnas pavadinimas — svoris/pakuotė privalo matytis
	...
	protected static function laisku_korteles( $rows ) {
		...
		$faktai = array(); $iav = array();
		foreach ( $rows as $r ) { $faktai[ $r['id'] ] = $r; foreach ( $r['eil'] as $e ) { if ( 'i_av' === $e['k'] && $e['src'] && ( ! $e['b'] || 'kaupiama' === $e['b']['busena'] ) ) { $iav[ $e['src'] ][ $r['id'] ][] = $e; } } }
		$kaup_src = $tk ? (array) $wpdb->get_col( "SELECT DISTINCT p.tiekejas FROM {$wpdb->prefix}ps_tiekimas p JOIN {$wpdb->prefix}ps_tiekimas_eil e ON e.partija_id = p.id WHERE p.busena='kaupiama'" ) : array();
		$tiek_pasirinktas = isset( $_GET['tiek'] ) ? sanitize_key( wp_unslash( $_GET['tiek'] ) ) : ''; // v3.38: tuščia kortelė prekėms į atsargas
		$visi_tiek = array( 'vf', 'zb', 'prins', 'belcor_tofu', 'quattro', 'ambrosia' );
		echo '<div class="dl-ats-tiek"><span class="pilkas maz">Užsakyti į atsargas iš:</span>'; foreach ( $visi_tiek as $t_ ) { echo ' <a class="v t' . ( $t_ === $tiek_pasirinktas ? ' p' : '' ) . '" href="' . esc_url( self::url( array( 'eile' => 'laiskai', 'tiek' => $t_, 'view' => null, 'q' => null, 'b' => null, 'atidaryti' => null ) ) ) . '">' . esc_html( self::vardas( $t_ ) ) . '</a>'; } echo '</div>';
		$srcs = array_unique( array_merge( array_keys( $g ), array_keys( $iav ), $kaup_src, $tiek_pasirinktas && in_array( $tiek_pasirinktas, $visi_tiek, true ) ? array( $tiek_pasirinktas ) : array() ) );
		if ( ! $srcs ) { echo '<div class="dl-tuscia">Laiškų tiekėjams nėra — viskas išsiųsta. Prekių į atsargas — pasirink tiekėją viršuje.</div>'; self::laisku_archyvo_nuoroda(); return; }
		$ln = class_exists( 'Petshop_AV_Dropship' ) ? Petshop_AV_Dropship::laisko_nust() : array( 'tiekejui' => false, 'man' => true );
		$pastai = (array) get_option( 'ps_tiekeju_pastai', array() ); $prist = $tk ? Petshop_AV_Tiekimas::PRISTATYMAI : array();
		$cia = self::url(); $n_ = wp_create_nonce( 'ps_dl_zurnalas' );
		usort( $srcs, function ( $a, $b ) { $ra = self::riba( $a ); $rb = self::riba( $b ); return ( $ra ? $ra[2] : PHP_INT_MAX ) <=> ( $rb ? $rb[2] : PHP_INT_MAX ); } );
		$siandien = array(); foreach ( (array) get_option( 'ps_laisku_archyvas', array() ) as $la ) { if ( substr( (string) ( $la['laikas'] ?? '' ), 0, 10 ) === current_time( 'Y-m-d' ) && preg_match( '/Perdavimas tiekėjui: (.+?) ·/u', (string) ( $la['kont'] ?? '' ), $mm ) ) { $siandien[ mb_strtolower( $mm[1] ) ][] = substr( (string) $la['laikas'], 11, 5 ); } }
		foreach ( (array) $wpdb->get_results( "SELECT tiekejas, uzsakyta FROM {$wpdb->prefix}ps_tiekimas WHERE DATE(uzsakyta)=CURDATE()" ) as $pt ) { $siandien[ mb_strtolower( self::vardas( $pt->tiekejas ) ) ][] = substr( (string) $pt->uzsakyta, 11, 5 ); }
		foreach ( $srcs as $src ) {
			$uzs = $g[ $src ] ?? array(); $vardas = self::vardas( $src ); $rank = $tk && Petshop_AV_Tiekimas::rankinis( $src ); list( $rk, $rt ) = self::riba_tekstas( $src );
			...
			/* ---- veža į AV + į atsargas ---- */
			$kaup = $tk ? Petshop_AV_Tiekimas::atvira_su_eilutemis( $src ) : null; $part = $kaup ? $kaup['part'] : null;
			$av_uzs = $iav[ $src ] ?? array(); $n_prek = 0; $prev_eil = array(); $kg = 0.0; $be_svorio = 0; $n_ats = 0;
			foreach ( $av_uzs as $oid => $es ) { foreach ( $es as $e ) { $n_prek++; $pr = wc_get_product( $e['pid'] ); $w = $pr ? (float) $pr->get_weight() : 0; if ( $w > 0 ) { $kg += $w * $e['q']; } else { $be_svorio++; } $prev_eil[] = (object) array( 'product_id' => $e['pid'], 'qty' => $e['q'] ); } }
			if ( $kaup ) { foreach ( $kaup['eilutes'] as $e ) { if ( $e->order_id ) { continue; } $n_ats++; $pr = wc_get_product( $e->product_id ); $w = $pr ? (float) $pr->get_weight() : 0; if ( $w > 0 ) { $kg += $w * $e->qty; } else { $be_svorio++; } $prev_eil[] = $e; } }
			$m_prek = $n_prek + $n_ats;
			echo '<div class="dl-kortele dl-tk"><h2>Užsakyti iš ' . esc_html( $vardas ) . ' <span class="pilkas">· ' . count( $uzs ) . ' užs.' . ( $m_prek ? ' · ' . $m_prek . ' prek. į AV' : '' ) . ( $rt ? ' · <span class="dl-riba-' . esc_attr( $rk ) . '">' . esc_html( $rt ) . '</span>' : '' ) . '</span></h2>';
			if ( ! empty( $siandien[ mb_strtolower( $vardas ) ] ) ) { echo '<p class="pastaba dl-antras"><b>' . esc_html( $vardas ) . ' šiandien jau užsakyta ' . esc_html( implode( ', ', array_unique( $siandien[ mb_strtolower( $vardas ) ] ) ) ) . '</b> — tai bus antras laiškas tą pačią dieną. Siunti tik jei tikrai reikia.</p>'; }
			if ( $uzs ) {
				echo '<h3 class="dl-tk-h3">Siunčia klientui</h3>';
			...
			foreach ( $uzs as $oid => $u ) {
				...
				echo '<tr class="eil" data-id="' . (int) $oid . '"' . ( $fx ? ' data-sk="1"' : '' ) . '><td><label class="dl-cb"><input type="checkbox" class="dl-uzs-cb" data-form="dlf_' . esc_attr( $src ) . '" data-n="1" value="' . (int) $oid . '" data-lip="' . ( $nr ? 1 : 0 ) . '" checked title="Nuimk — šis užsakymas į laišką nepateks"></label><span class="nr">#' . esc_html( $u['nr'] ) . '</span>' . ( $fx && ! empty( $fx['naujas'] ) ? ' <b class="dl-nz" title="dar neatidarytas">N</b>' : '' ) . '<br><span class="pilkas maz">' . esc_html( $u['klientas'] ) . ' · ' . esc_html( $u['metodas'] ) . '</span></td><td>';
				$tsv = '';
			...
			echo '<h3 class="dl-tk-h3">Veža į AV' . ( $part ? ' <span class="pilkas">· užsakymas tiekėjui #' . (int) $part->id . '</span>' : '' ) . '</h3>';
			echo '<table class="dl-tbl dl-tbl-k"><tbody>';
			foreach ( $av_uzs as $oid => $es ) {
				$fx = $faktai[ $oid ] ?? null; $o_ = $fx ? $fx['o'] : wc_get_order( $oid );
				echo '<tr class="eil" data-id="' . (int) $oid . '"' . ( $fx ? ' data-sk="1"' : '' ) . '><td><label class="dl-cb"><input type="checkbox" class="dl-av-cb" data-form="dlf_' . esc_attr( $src ) . '" data-n="' . count( $es ) . '" value="' . (int) $oid . '" checked title="Nuimk — šio užsakymo prekės į užsakymą tiekėjui nepateks"></label><span class="nr">#' . esc_html( $o_ ? $o_->get_order_number() : $oid ) . '</span>' . ( $fx && ! empty( $fx['naujas'] ) ? ' <b class="dl-nz" title="dar neatidarytas">N</b>' : '' ) . '<br><span class="pilkas maz">' . esc_html( $o_ ? trim( $o_->get_billing_first_name() . ' ' . $o_->get_billing_last_name() ) : '' ) . '</span></td><td>';
				foreach ( $es as $e ) { echo '<div>' . (int) $e['q'] . '× ' . esc_html( $e['n'] ) . ( $e['sku'] ? ' <span class="pilkas maz">' . esc_html( $e['sku'] ) . '</span>' : '' ) . ' <span class="pilkas maz">· ' . esc_html( $e['b'] ? 'sudėta' : 'neužsakyta' ) . '</span></div>'; }
				echo '</td><td class="d"></td></tr>';
			}
			if ( $kaup ) { foreach ( $kaup['eilutes'] as $e ) { if ( $e->order_id ) { continue; } $pr = wc_get_product( $e->product_id );
				echo '<tr><td><span class="pilkas">į atsargas</span></td><td><div><form method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '" class="dl-eil-f">' . wp_nonce_field( 'ps_dl_tiek_' . $src . '_0', '_wpnonce', true, false ) . '<input type="hidden" name="action" value="ps_dl_tiekimas"><input type="hidden" name="tiekejas" value="' . esc_attr( $src ) . '"><input type="hidden" name="partija" value="0"><input type="hidden" name="ka" value="eilute"><input type="hidden" name="eid" value="' . (int) $e->id . '"><input type="hidden" name="ps_dl_g" value="' . esc_url( $cia ) . '">'
					. '<input type="number" name="qty" min="1" max="999" value="' . (int) $e->qty . '" class="dl-eil-q" onchange="this.form.submit()" title="kiekis — pakeitus išsaugoma">× ' . esc_html( $pr ? $pr->get_name() : '#' . $e->product_id ) . ( $pr && $pr->get_sku() ? ' <span class="pilkas maz">' . esc_html( $pr->get_sku() ) . '</span>' : '' )
					. ' <button type="submit" name="trinti" value="1" class="v t dl-eil-x" title="išimti iš užsakymo tiekėjui" onclick="return confirm(\'Išimti prekę iš užsakymo tiekėjui?\')">×</button></form></div></td><td class="d"></td></tr>'; } } // v3.38: kiekis / išimti
			if ( ! $av_uzs && ! $n_ats ) { echo '<tr><td colspan="3"><span class="pilkas maz">Į AV iš ' . esc_html( $vardas ) . ' šiuo metu nieko — skydelyje prekei pasirink „' . esc_html( $vardas ) . ' veža į AV“ arba pridėk į atsargas čia.</span></td></tr>'; }
			echo '</tbody></table>';
			// v3.38: prekių rinkiklis vietoj SKU laukelio — ženklas · išparduota · paieška · lentelė su varnelėmis ir kiekiais
			$zenklai = $wpdb->get_results( $wpdb->prepare( "SELECT t.term_id, t.name, COUNT(*) n FROM {$wpdb->term_relationships} r JOIN {$wpdb->term_taxonomy} tt ON tt.term_taxonomy_id = r.term_taxonomy_id AND tt.taxonomy = 'product_brand' JOIN {$wpdb->terms} t ON t.term_id = tt.term_id JOIN {$wpdb->postmeta} m ON m.post_id = r.object_id AND m.meta_key = '_ps_sandelis' AND m.meta_value = %s JOIN {$wpdb->posts} po ON po.ID = r.object_id AND po.post_status = 'publish' AND po.post_type = 'product' GROUP BY t.term_id, t.name ORDER BY t.name", $src ) );
			echo '<div class="dl-rink" data-src="' . esc_attr( $src ) . '" data-n="' . esc_attr( $n_ ) . '"><button type="button" class="v t dl-rink-atv">Pridėti prekių iš ' . esc_html( $vardas ) . ' ▾</button>';
			echo '<form method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '" class="dl-rink-f" style="display:none">' . wp_nonce_field( 'ps_dl_tiek_' . $src . '_0', '_wpnonce', true, false ) . '<input type="hidden" name="action" value="ps_dl_tiekimas"><input type="hidden" name="tiekejas" value="' . esc_attr( $src ) . '"><input type="hidden" name="partija" value="0"><input type="hidden" name="ka" value="prideti"><input type="hidden" name="ps_dl_g" value="' . esc_url( $cia ) . '">';
			echo '<div class="dl-rink-filtrai"><label>Ženklas <select class="dl-rink-z"><option value="">— visi ' . esc_html( $vardas ) . ' (' . count( $zenklai ) . ' ženklai) —</option>'; foreach ( $zenklai as $z ) { echo '<option value="' . (int) $z->term_id . '">' . esc_html( $z->name ) . ' (' . (int) $z->n . ')</option>'; } echo '</select></label>';
			echo '<label><input type="checkbox" class="dl-rink-isp" value="1"> išparduota (reikia užsakyti)</label><label>ieškoti sąraše <input type="text" class="dl-rink-q" placeholder="pvz. festival" autocomplete="off"></label><span class="pilkas maz dl-rink-b">pasirink ženklą arba „išparduota“</span></div>';
			echo '<div class="dl-rink-lent"></div><div class="dl-zingsniai-k"><button class="v p dl-rink-prid" type="submit" disabled data-tpl="Pridėti pažymėtas (%n)">Pridėti pažymėtas (0)</button> <button type="button" class="v t dl-rink-uzd">Uždaryti</button></div></form></div>';
			$ids_csv = implode( ',', array_keys( $uzs ) ); $ids_av_csv = implode( ',', array_keys( $av_uzs ) );
			echo '<div class="dl-zingsniai-k">';
			if ( $uzs ) { if ( $be_paprasti ) { echo '<span class="zn">1</span><a class="v p" href="' . esc_url( self::veiksmo_url( 'vp_reg', 0, $cia ) . '&ids=' . implode( ',', $be_paprasti ) . '&sandelis=' . rawurlencode( $src ) ) . '">Lipdukai (' . count( $be_paprasti ) . ')</a>'; } else { echo '<span class="zn">1</span><span class="v" style="opacity:.6">Lipdukai ✓</span>'; } }
			if ( 'zb' === $src && $uzs ) {
			...
			echo '<form method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '" class="dl-inl dl-laiskas-f dl-tk-f" id="dlf_' . esc_attr( $src ) . '">' . wp_nonce_field( 'ps_dl_uzs_' . $src, '_wpnonce', true, false ) . '<input type="hidden" name="action" value="ps_dl_uzsakyti"><input type="hidden" name="tiekejas" value="' . esc_attr( $src ) . '"><input type="hidden" name="uzsakymai" value="' . esc_attr( $ids_csv ) . '" class="dl-uzs-ids"><input type="hidden" name="ids_av" value="' . esc_attr( $ids_av_csv ) . '" class="dl-av-ids"><input type="hidden" name="ps_dl_g" value="' . esc_url( $cia ) . '"><input type="hidden" name="laisk_zyme" value="1">';
			if ( $m_prek ) {
				echo '<div class="dl-tk-prist"><span class="pilkas maz">Kaip atkeliaus į AV:</span>';
				foreach ( $prist as $k => $v ) { echo '<label><input type="radio" name="pristatymas" value="' . esc_attr( $k ) . '"' . checked( $bud, $k, false ) . '> ' . esc_html( $v ) . '</label>'; }
			...
			if ( 'zb' === $src ) {
				if ( $m_prek ) { echo '<span class="zn">' . ( $uzs ? 3 : 1 ) . '</span><button class="v p" type="submit" data-tpl="Užsakyti iš ZB į AV (%m prek.)" onclick="return confirm(' . esc_attr( wp_json_encode( 'Užsakyti iš ZB į AV? Laiško nebus — sąrašą suvesi į ZB sistemą („Kopijuoti“ atsiras Laukiam iš tiekėjų, kol prekės atvažiuos).' ) ) . ')">Užsakyti iš ZB į AV (' . $m_prek . ' prek.)</button>'; }
			} else {
				$patv = 'Užsakyti iš ' . $vardas . '? Laiškas išeina iš karto — vienas laiškas su pažymėtais Dropshipping užsakymais (lipdukai priede) ir prekėmis į AV' . ( in_array( $bud, array( 'kurjeris', 'pastomatas' ), true ) ? '; siunta į AV registruojama Venipak ir kainuoja' : '' ) . '.';
				echo '<span class="zn">' . ( $uzs ? 2 : 1 ) . '</span><button class="v p" type="submit" data-tpl="Užsakyti iš ' . esc_attr( $vardas ) . ' (%n užs. + %m prek. į AV)" onclick="return confirm(' . esc_attr( wp_json_encode( $patv ) ) . ')">Užsakyti iš ' . esc_html( $vardas ) . ' (' . count( $uzs ) . ' užs. + ' . $m_prek . ' prek. į AV)</button>';
				echo ' <button type="button" class="v t dl-perz">Peržiūrėti laišką</button>';
				echo '<div class="dl-laisko-nust"><label class="dl-prier">Prierašas tiekėjui <span class="pilkas maz">(įeis į laišką po prekių lentele)</span><textarea name="pastaba" rows="2" placeholder="pvz.: prašome pristatyti iki penktadienio; Festival — jei nėra, pakeiskite 3 kg pakuotėmis"></textarea></label>';
				if ( $uzs ) { echo '<label><input type="checkbox" name="su_lipdukais" value="1" checked> lipdukai</label><label><input type="checkbox" name="su_manifestu" value="1" checked> kurjerio sąrašas</label>'; }
				echo '<label><input type="checkbox" name="laisk_tiekejui" value="1"' . checked( ! empty( $ln['tiekejui'] ), true, false ) . '> siųsti tiekėjui' . ( ! empty( $pastai[ $src ] ) ? ' (' . esc_html( $pastai[ $src ] ) . ')' : ' <span class="raud">— el. pašto nėra</span>' ) . '</label><label><input type="checkbox" name="laisk_man" value="1"' . checked( ! empty( $ln['man'] ), true, false ) . '> kopija man</label></div>';
				$prev_part = (object) array( 'pristatymas' => $bud, 'dezes' => (int) max( 1, (int) ( $part->dezes ?? 1 ) ) );
				$perz_av = $m_prek && $tk ? Petshop_AV_Tiekimas::laisko_dalis( $prev_part, $prev_eil, '' ) : '';
				$perz = $uzs && class_exists( 'Petshop_AV_Dropship' ) ? str_replace( '<p style="margin:14px 0">{{PRIERASAS}}</p>', '<p class="dl-perz-pastaba" style="margin:14px 0;display:none"></p>', Petshop_AV_Dropship::laisko_html( $src, $uzs, '{{PRIERASAS}}', $perz_av ) ) : '<p>Laba diena,</p><p>prašome paruošti šias prekes.</p>' . $perz_av . '<p class="dl-perz-pastaba" style="margin:14px 0;display:none"></p><p>Ačiū,<br>UAB Avesa · petshop.lt<br>terra@petshop.lt</p>'; // v3.38.1: prierašo vieta peržiūroje
				echo '<div class="dl-perz-t" style="display:none">' . $perz . '</div>';
			}
			echo '</form></div>';
			if ( $be ) { echo '<p class="pastaba">Be lipduko pažymėti užsakymai į laišką neis — pirma lipdukai, arba nuimk jų varneles ir siųsk likusius (tiekėjui — vienas laiškas dienai; antras — tik darbuotojo sprendimu).</p>'; }
			echo '</div>';
		}
		self::laisku_archyvo_nuoroda();
	}

	/** v3.38: nuoroda į išsiųstų laiškų archyvą (vietoj juostos „Laiškai“). */
	protected static function laisku_archyvo_nuoroda() { $n = count( (array) get_option( 'ps_laisku_archyvas', array() ) ); echo '<p class="pastaba dl-arch-n"><a href="' . esc_url( admin_url( 'admin.php?page=' . self::SLUG . '&view=laiskai' ) ) . '">Išsiųsti laiškai tiekėjams (' . $n . ')</a></p>'; }

	/** v3.38: `view=laiskai` — išsiųstų laiškų tiekėjams archyvas (`ps_laisku_archyvas`, 200 paskutinių; variklio `Petshop_AV_Dropship::archyvuoti`). */
	protected static function laisku_archyvas() {
		$a = (array) get_option( 'ps_laisku_archyvas', array() );
		echo '<main class="dl-main"><div class="dl-kortele"><h2>Išsiųsti laiškai tiekėjams <span class="pilkas">· ' . count( $a ) . '</span></h2>';
		if ( ! $a ) { echo '<div class="dl-tuscia">Laiškų dar nebuvo.</div></div></main>'; return; }
		echo '<table class="dl-tbl dl-tbl-k"><tbody>';
		foreach ( $a as $i => $l ) { echo '<tr><td class="pilkas maz" style="white-space:nowrap">' . esc_html( mb_substr( (string) ( $l['laikas'] ?? '' ), 0, 16 ) ) . '</td><td><div><b>' . esc_html( (string) ( $l['tema'] ?? '' ) ) . '</b></div><div class="pilkas maz">' . esc_html( (string) ( $l['kont'] ?? '' ) ) . ' · ' . esc_html( (string) ( $l['kam'] ?? '' ) ) . ( ! empty( $l['priedai'] ) ? ' · priedai: ' . esc_html( implode( ', ', (array) $l['priedai'] ) ) : '' ) . '</div><div class="dl-arch-t" style="display:none">' . wp_kses_post( (string) ( $l['html'] ?? '' ) ) . '</div></td><td class="d"><button type="button" class="v t dl-arch-atv">Rodyti</button></td></tr>'; }
	...
	protected static function laukiam_korteles( $rows, $ds = array() ) { // v3.37: tik užsakyta → „Gauta“; užsakymas iš tiekėjo — Dropshipping kortelėje
		global $wpdb; $tk = class_exists( 'Petshop_AV_Tiekimas' );
		$g = array(); $faktai = array();
		foreach ( $rows as $r ) { $faktai[ $r['id'] ] = $r; foreach ( $r['eil'] as $e ) { if ( 'i_av' === $e['k'] && $e['src'] && $e['b'] && 'uzsakyta' === $e['b']['busena'] ) { $g[ $e['src'] ]['uzs'][ $r['id'] ][] = $e; } } }
		...
		if ( ! $g ) { echo '<div class="dl-tuscia">Nieko nelaukiam iš tiekėjų — viskas AV. Užsakyti iš tiekėjo — Dropshipping kortelėje.</div>'; return; }
		...
		foreach ( $g as $src => $x ) {
			...
			foreach ( $x['part'] ?? array() as $p ) {
				...
				foreach ( $eil as $e ) {
					...
					$sav_ = class_exists( 'Petshop_Partijos' ) && method_exists( 'Petshop_Partijos', 'paskutine_savikaina' ) ? Petshop_Partijos::paskutine_savikaina( (int) $e->product_id ) : null; if ( ! $sav_ && $pr ) { $sav_ = (float) $pr->get_meta( 'vf' === $src ? '_vf_cost' : ( 'zb' === $src ? '_zb_cost' : '_cost_price' ) ); } // v3.38
					echo '<td class="d dl-tk-gauta"><label>Gauta <input type="number" min="0" name="gauta[' . (int) $e->id . ']" value="' . (int) $e->qty . '"></label> <label class="pilkas maz">savikaina <input type="number" step="0.01" min="0" name="savikaina[' . (int) $e->id . ']" value="' . esc_attr( $sav_ > 0 ? number_format( (float) $sav_, 2, '.', '' ) : '' ) . '" placeholder="€ be PVM" title="Savikaina be PVM — kuria gavimo partiją (savikaina, marža). Tuščia — tik likutis."> €</label> <label class="pilkas maz">galioja iki <input type="text" name="galioja[' . (int) $e->id . ']" placeholder="YYYY-MM" pattern="\d{4}-\d{2}" title="Neprivaloma — pildyk, kai prekė lieka sandėlyje"> <span class="pilkas maz">(jei lieka sandėlyje)</span></label></td></tr>';
				}
	...
	protected static function klausimu_korteles( $rows ) {
		foreach ( $rows as $r ) {
			...
			if ( 0 === strpos( $kl, 'Trūksta' ) ) {
				...
				$pastaba = 'Dažniausiai paštomatas nebegalioja arba adresas / pašto kodas neteisingas. „Taisyti adresą“ — pataisyk skydelyje ir išsaugok; klaida nusiims, tada registruok lipduką iš naujo.' . ( ( $sk['red'] && 'tik_el' !== $sk['red']['tipas'] ) ? '' : ' Redaguoti nebegalima (išsiųsta / uždaryta).' );
				$veiksmai = ( ( $sk['red'] && 'tik_el' !== $sk['red']['tipas'] ) ? '<button class="v p" data-atidaryti="1" data-redaguoti="1">Taisyti adresą</button> ' : '<button class="v p" data-atidaryti="1">Atidaryti</button> ' ) . $rasyti . ' ' . $atsaukti;
	...
	protected static function rytas( $atviri, $c ) {
		...
		$Z = array(
			...
			array( 'Dropshipping — lipdukai ir užsakymai tiekėjams (siunčia klientui · veža į AV · į atsargas)', $c['laiskai'], $tiek_t ? implode( ' · ', $tiek_t ) : 'kortelė per tiekėją: 1 Lipdukai → 2 Užsakyti iš [T] — vienas laiškas', 'laiskai', 'Atidaryti' ),
	...
	protected static function dialogas() {
		...
		<div class="dl-dlg" id="dlDlg" role="dialog" aria-modal="true"><h3 id="dlDlgH"></h3><p id="dlDlgT"></p>
			<label id="dlDlgOptL" class="dl-opt"><input type="checkbox" id="dlDlgOpt"><input type="number" id="dlDlgN" min="1" max="20" style="display:none;width:60px;font:inherit;border:1px solid var(--linija);border-radius:5px;padding:3px 6px"><select id="dlDlgS" style="display:none;font:inherit;border:1px solid var(--linija);border-radius:5px;padding:3px 6px"></select> <span id="dlDlgOptT"></span></label>
...
.dl-f{display:flex;gap:6px;align-items:center;flex-wrap:wrap;font-size:12px}.dl-f input[type=date],.dl-f input[type=number]{font:inherit;font-size:12px;border:1px solid var(--linija);border-radius:5px;padding:2px 6px;background:#fff;width:auto;min-height:0;line-height:1.4}.dl-f input[type=number]{width:72px}.dl-f-nuoiki{display:inline-flex;gap:4px;align-items:center}.dl-f select{font:inherit;font-size:12px;border:1px solid var(--linija);border-radius:5px;padding:2px 6px;background:#fff;color:var(--pilka);max-width:200px;height:26px}.dl-x{font-size:12px}
...
.dl a.dl-e-naujas{margin-left:auto;background:var(--zalia,#2d6a35);color:#fff;border-color:transparent}.dl a.dl-e-naujas:hover{opacity:.9;text-decoration:none}
...
(function(){
	...
	function dlg(a){ var d; try{ d=JSON.parse(a.getAttribute('data-d')); }catch(e){ return false; } if(!d) return false;
		...
		var L=$('dlDlgOptL'),C=$('dlDlgOpt'),N=$('dlDlgN'),S=$('dlDlgS'); N.style.display='none'; S.style.display='none'; C.style.display='';
		if(d.opt&&d.opt.tipas==='sel'){ L.style.display='flex'; C.style.display='none'; S.style.display=''; S.innerHTML=(d.opt['var']||[]).map(function(v){ return '<option value="'+esc(v[0])+'"'+(v[0]===d.opt.def?' selected':'')+'>'+esc(v[1])+'</option>'; }).join(''); $('dlDlgOptT').textContent=d.opt.tekstas; S.onchange=function(){ ok.href=url+'&'+encodeURIComponent(d.opt.vardas)+'='+encodeURIComponent(S.value); }; S.onchange(); } /* v3.44 */
		else if(d.opt&&d.opt.tipas==='n'){ L.style.display='flex'; C.style.display='none'; N.style.display=''; N.value=d.opt.def||1; $('dlDlgOptT').textContent=d.opt.tekstas; N.oninput=function(){ ok.href=url+'&'+encodeURIComponent(d.opt.vardas)+'='+encodeURIComponent(N.value||1); }; N.oninput(); }
	...
	function rodyti(o,r){ skO=o;
		...
		var S=$('skSiunta'); if(o.pak||o.nr_siuntos.length||o.perreg||o.lp_pdf){ S.style.display='block'; $('skSiuntaT').innerHTML=(o.nr_siuntos.length?'<div>'+o.nr_siuntos.map(esc).join('<br>')+'</div>':'<div class="pilkas maz">siunta dar neregistruota</div>')
			...
			+(o.lp_pdf?'<div style="margin-top:6px"><a class="v" href="'+esc(o.lp_pdf)+'" target="_blank">Lipdukas LP (PDF)</a>'+((o.perreg&&!o.pak)?' <a class="v" href="'+esc(o.perreg)+'"'+(o.perreg_d?' data-d="'+esc(JSON.stringify(o.perreg_d))+'"':'')+'>Lipdukas iš naujo</a>':'')+'</div>':''); /* v3.42; v3.44 dialogas su dydžiu */
		...
		var D=$('skDok'); if(o.dok&&o.dok.length){ D.style.display='block'; $('skDokT').innerHTML=o.dok.map(function(x){ if(x.btn){ return '<div class="dl-dok">'+esc(x.z)+' · '+esc(x.s)+' € · <a class="v p maz" href="'+esc(x.btn)+'" data-d="'+esc(JSON.stringify(x.dlg||{}))+'">'+esc(x.bt)+'</a></div>'; } return '<div class="dl-dok"><b>'+esc(x.nr)+'</b> · '+esc(x.z)+(x.d?' · '+esc(x.d):'')+' · '+esc(x.s)+' € · '+(x.u?'<a href="'+esc(x.u)+'" target="_blank">PDF</a>':(x.gen?'<a class="pilkas maz" href="'+esc(x.gen)+'">PDF nėra — sugeneruoti</a>':'<span class="pilkas maz">PDF nėra</span>'))+(x.kas?' <span class="pilkas maz">priėmė '+esc(x.kas)+'</span>':'')+'</div>'; }).join(''); /* v3.34: PPK mygtukas / kvitas */ } else D.style.display='none';
		...
		if(o.atkurti) f+='<a class="v p" href="'+esc(o.atkurti)+'" data-d="'+esc(JSON.stringify({antraste:'Gavome pavedimą · #'+o.nr,tekstas:'Klientas sumokėjo bankiniu pavedimu už jau atšauktą užsakymą? Užsakymas atkuriamas: mokėjimo būdas keičiamas į bankinį pavedimą, pažymimas apmokėtu ('+o.suma+'), prekės rezervuojamos, eina į darbą; klientui išeina įprastas „užsakymas vykdomas“ laiškas. Prieš spausdamas patikrink, ar suma banke sutampa.',ok:'Gavome pavedimą — atkurti'}))+'">Gavome pavedimą — atkurti</a>'; /* v3.43 */
		...
		f+='<span style="margin-left:auto"></span>'+(o.red?'<button class="v t" id="skRed"'+(o.red.tipas==='tik_el'?' title="lipdukas jau užregistruotas / išsiųsta / uždaryta — keičiamas tik kliento el. paštas"':'')+'>Redaguoti</button>':'<button class="v t" disabled title="lipdukas jau užregistruotas / išsiųsta / uždaryta — keisk rankiniu būdu">Redaguoti</button>')+(o.nesurinkta?'<a class="v t" href="'+esc(o.nesurinkta)+'" title="Grąžinti į „Surinkti“">Atšaukti surinkimą</a>':'')+(o.sekimo?'<a class="v t" href="'+esc(o.sekimo)+'">Sekimo numeriai klientui</a>':'')+(o.velavimas?'<a class="v t" href="'+esc(o.velavimas.u)+'" data-d="'+esc(JSON.stringify(o.velavimas.d))+'">Pranešti klientui apie vėlavimą</a>':'')+(o.klientas_url?'<a class="v t" target="_blank" rel="noopener" href="'+esc(o.klientas_url)+'" title="Kliento užsakymo puslapis — kaip jį mato klientas (siuntos, sąskaita)">Kaip mato klientas</a>':'')+(o.graz?'<button class="v t" id="skGraz" title="Klientas grąžina prekes — kreditinė, likutis">Grąžinimas</button>':'')+(o.mail?'<a class="v t" href="mailto:'+esc(o.mail)+'?subject='+encodeURIComponent('Užsakymas #'+o.nr+' — petshop.lt')+'">Parašyti klientui</a>':'')+(o.atsaukti?'<a class="v t raud" href="'+esc(o.atsaukti.u)+'" data-d="'+esc(JSON.stringify(o.atsaukti.d))+'">Atšaukti</a>':'');
	...
	function redaguoti(o){ var r=o.red; if(!r) return; var P=$('skPr'); var L=r.laukai; var h='<form method="post" action="'+esc(r.u)+'" class="dl-red" id="skRedF"><input type="hidden" name="action" value="ps_dl_redaguoti"><input type="hidden" name="id" value="'+o.id+'"><input type="hidden" name="_wpnonce" value="'+esc(r.n)+'"><input type="hidden" name="g" value="'+esc(r.g)+'">';
		if(r.tipas==='tik_el'){ h+='<div class="pilkas maz">Lipdukas jau užregistruotas / išsiųsta / uždaryta — adreso keisti nebegalima, keičiamas tik kliento el. paštas</div>'; } else {
		...
		h+='<label>El. paštas<input type="email" name="el" value="'+esc(L.el||'')+'"></label>'; /* v3.40 */
		...
		if(r.tipas!=='kurjeris'&&r.tipas!=='tik_el'){ fetch(ajaxurl+'?action=ps_dl_vietos&vez='+encodeURIComponent(r.tipas)+'&n='+encodeURIComponent(o.zn),{credentials:'same-origin'}).then(function(x){return x.json();}).then(function(j){ if(!j||!j.success){ $('skRedN').textContent='sąrašo įkelti nepavyko'; return; } var V=j.data, S=$('skRedS'), Q=$('skRedQ');
	...
	function dlPrierasas(f){ if(!f) return; var t=f.querySelector('textarea[name=pastaba]'), p=f.querySelector('.dl-perz-pastaba'); if(!t||!p) return; var v=t.value.trim(); p.textContent=v; p.style.display=v?'':'none'; }
	document.addEventListener('input',function(e){ var t=e.target.closest('textarea[name=pastaba]'); if(t) dlPrierasas(t.closest('form')); });
	function dlPerskaiciuoti(fid){ var f=document.getElementById(fid); if(!f) return; var ids=[],n=0,be=0,av=[],m=0; document.querySelectorAll('.dl-uzs-cb[data-form="'+fid+'"]').forEach(function(c){ if(c.checked){ ids.push(c.value); n+=parseInt(c.getAttribute('data-n'),10)||1; if(c.getAttribute('data-lip')==='0') be++; } }); document.querySelectorAll('.dl-av-cb[data-form="'+fid+'"]').forEach(function(c){ if(c.checked){ av.push(c.value); m+=parseInt(c.getAttribute('data-n'),10)||1; } }); var h=f.querySelector('.dl-uzs-ids'); if(h) h.value=ids.join(','); var ha=f.querySelector('.dl-av-ids'); if(ha) ha.value=av.join(','); var ats=0; f.closest('.dl-kortele').querySelectorAll('td .pilkas').forEach(function(x){ if(x.textContent==='į atsargas') ats++; }); f.querySelectorAll('button[data-tpl]').forEach(function(b){ b.textContent=b.getAttribute('data-tpl').replace('%n',n).replace('%m',m+ats); b.disabled=(!ids.length&&!(m+ats))||be>0; b.title=be>0?'pažymėti užsakymai be lipduko — pirma lipdukai arba nuimk varneles':''; }); }
	document.addEventListener('change',function(e){ var cb=e.target.closest('.dl-uzs-cb,.dl-av-cb'); if(!cb) return; dlPerskaiciuoti(cb.getAttribute('data-form')); });
	document.querySelectorAll('.dl-laiskas-f[id^="dlf_"]').forEach(function(f){ dlPerskaiciuoti(f.id); });
	document.addEventListener('click',function(e){ var b=e.target.closest('.dl-rink-atv,.dl-rink-uzd'); if(b){ var r=b.closest('.dl-rink'); var f=r.querySelector('.dl-rink-f'); f.style.display=b.classList.contains('dl-rink-atv')?'':'none'; if(b.classList.contains('dl-rink-atv')) r.querySelector('.dl-rink-z').focus(); return; }
		var a=e.target.closest('.dl-arch-atv'); if(a){ var t=a.closest('tr').querySelector('.dl-arch-t'); var on=t.style.display==='none'; t.style.display=on?'':'none'; a.textContent=on?'Slėpti':'Rodyti'; } });
	function dlRinkKrauti(r){ var z=r.querySelector('.dl-rink-z').value, isp=r.querySelector('.dl-rink-isp').checked?1:0, q=r.querySelector('.dl-rink-q').value.trim(), l=r.querySelector('.dl-rink-lent'), b=r.querySelector('.dl-rink-b'); if(!z&&!isp&&q.length<2){ l.innerHTML=''; b.textContent='pasirink ženklą arba „išparduota“'; dlRinkSk(r); return; } b.textContent='kraunama…';
		fetch(ajaxurl+'?action=ps_dl_atsargos&n='+encodeURIComponent(r.getAttribute('data-n'))+'&src='+encodeURIComponent(r.getAttribute('data-src'))+'&zenklas='+encodeURIComponent(z)+'&isparduota='+isp+'&q='+encodeURIComponent(q)).then(function(x){return x.json();}).then(function(j){ if(!j||!j.success){ b.textContent='klaida'; return; } var d=j.data||[]; b.textContent=d.length+(d.length===300?'+':'')+' prek.'; var h='<table><thead><tr><th></th><th>Prekė</th><th>SKU</th><th class="sk">AV</th><th class="sk">'+(d.length&&d[0].tiek!==null?'Tiek.':'')+'</th><th>Pask. gavimas</th><th class="sk">Savik.</th><th class="sk">Kaina</th><th class="sk">Kiekis</th></tr></thead><tbody>';
			d.forEach(function(p){ h+='<tr'+(p.stock<=0?' class="isp"':'')+'><td><input type="checkbox" class="dl-rink-cb" value="'+p.id+'"></td><td>'+esc(p.n)+'</td><td class="pilkas maz">'+esc(p.sku)+'</td><td class="sk av">'+p.av+'</td><td class="sk">'+(p.tiek===null?'':p.tiek)+'</td><td class="pilkas maz">'+esc(p.pask)+'</td><td class="sk">'+(p.sav===null?'—':p.sav.toFixed(2))+'</td><td class="sk">'+p.kaina.toFixed(2)+'</td><td class="sk"><input type="number" min="1" max="999" value="1" name="prekes['+p.id+']" disabled></td></tr>'; });
			l.innerHTML=h+'</tbody></table>'; dlRinkSk(r); }).catch(function(){ b.textContent='klaida'; }); }
	function dlRinkSk(r){ var n=0; r.querySelectorAll('.dl-rink-cb').forEach(function(c){ var q=c.closest('tr').querySelector('input[type=number]'); q.disabled=!c.checked; if(c.checked) n++; }); var p=r.querySelector('.dl-rink-prid'); if(p){ p.textContent=p.getAttribute('data-tpl').replace('%n',n); p.disabled=!n; } }
	document.addEventListener('change',function(e){ var r=e.target.closest('.dl-rink'); if(!r) return; if(e.target.classList.contains('dl-rink-cb')){ dlRinkSk(r); return; } if(e.target.classList.contains('dl-rink-z')||e.target.classList.contains('dl-rink-isp')) dlRinkKrauti(r); });
	document.addEventListener('input',function(e){ var q=e.target.closest('.dl-rink-q'); if(!q) return; var r=q.closest('.dl-rink'); clearTimeout(r._t); r._t=setTimeout(function(){ dlRinkKrauti(r); },300); });
	document.addEventListener('input',function(e){ var q=e.target.closest('.dl-ats-q'); if(!q||q.value.length<2||/^\d+$/.test(q.value)) return; clearTimeout(q._t); q._t=setTimeout(function(){ fetch(ajaxurl+'?action=ps_dl_prekes&n='+encodeURIComponent(q.getAttribute('data-n'))+'&q='+encodeURIComponent(q.value)).then(function(r){return r.json();}).then(function(j){ var dl=document.getElementById(q.getAttribute('list')); if(!dl||!j||!j.success) return; dl.innerHTML=''; (j.data||[]).forEach(function(p){ var o=document.createElement('option'); o.value=p.sku||String(p.id); o.label=p.n+' · AV '+p.av; o.textContent=p.n+' · AV '+p.av; dl.appendChild(o); }); }); },250); });
	document.addEventListener('click',function(e){ var k=e.target.closest('.dl-kopijuoti'); if(k){ e.stopPropagation(); var t=k.getAttribute('data-tsv'); (navigator.clipboard?navigator.clipboard.writeText(t):Promise.reject()).then(function(){ k.textContent='Nukopijuota'; setTimeout(function(){k.textContent='Kopijuoti';},1500); }).catch(function(){ window.prompt('Nukopijuok:',t); }); return; }
		var p=e.target.closest('.dl-perz'); if(p){ e.stopPropagation(); var f=p.closest('form'); var d=f&&f.querySelector('.dl-perz-t'); if(d){ d.style.display=d.style.display==='none'?'block':'none'; dlPrierasas(f); } return; }
