<?php
/**
 * Plugin Name: Petshop Dydžių katalogas v1.0 (viena kortelė šeimai)
 ...
 *   ir prekių skaičius teisingi. Prekių puslapiai, feed'ai, sitemap, admin — neliečiami. Šeimų sąrašas — transient ps_dk_seimos
...
class Petshop_Dydziu_Katalogas {
	...
	public static function seimos() {
		...
		try {
			...
			$pard = array_fill_keys( $ids, 0 );
			foreach ( array( 'ps_ist_fakt_eilutes', 'ps_fakt_eilutes' ) as $t ) {
				if ( ! $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->prefix}$t'" ) ) { continue; }
				foreach ( (array) $wpdb->get_results( "SELECT preke_id, SUM(kiekis) q FROM {$wpdb->prefix}$t WHERE preke_id IN ($in) AND COALESCE(testinis,0)=0 AND diena >= DATE_SUB(CURDATE(), INTERVAL 365 DAY) GROUP BY preke_id", ARRAY_A ) as $r ) { $pard[ (int) $r['preke_id'] ] += (int) $r['q']; }
			}
			$dyd = array(); $tax = array();
			...
			foreach ( $rows as $r ) {
				...
				$out[ $r['seima'] ][] = array( $id, (int) $pard[ $id ], (float) $r['kaina'], $dyd[ $id ] ?? array(), $kg, $tax[ $id ] ?? array() );
			}
