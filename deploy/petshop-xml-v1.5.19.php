...
function petshop_xml_zb_to_draft( int $post_id, string $reason, bool $set_konservas_price = false ): void {
    ...
    if ( $set_konservas_price ) {
        ...
        if ( $zb_cost > 0 && ! $already && ! $has_price && class_exists( 'Petshop_Pricing' ) ) {
            ...
            $result    = $pricing->calculate_final_price( $zb_cost, $cat_slugs, petshop_xml_get_brand_slug( $post_id ) );
            $final     = $result['price'];
...
function petshop_xml_initial_import( int $post_id, bool $is_update ): void {
    ...
    if ( ! $price_locked && ! $price_initialized && ! $has_valid_price ) {
        $price_result  = $pricing->calculate_final_price( $zb_cost, $cat_slugs, petshop_xml_get_brand_slug( $post_id ) );
...
/**
 * Prekes ZENKLO slug'as kainodarai (2026-08-26, savininko sprendimas).
 *
 * `Petshop_Pricing::get_markup()` nuo v1.5.0 moka zenklo taisykles, bet
 * zenklo nezino — ji mato tik kategorijas. Todel perduodam ji is cia.
 * Grazina TUSCIA eilute, jei zenklo nera — variklis tada dirba kaip anksciau.
 */
function petshop_xml_get_brand_slug( int $post_id ): string {
    $t = wp_get_object_terms( $post_id, 'product_brand', [ 'fields' => 'slugs' ] );
    if ( is_wp_error( $t ) || empty( $t ) ) { return ''; }
    return strtolower( (string) $t[0] );
}
