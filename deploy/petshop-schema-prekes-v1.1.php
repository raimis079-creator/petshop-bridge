<?php
/**
 * Plugin Name: Petshop schema — prekės v1.1 (S1701 → S1717)
 ...
 *   €2,15 su PVM, nemokamas nuo €30, kurjeris €3,99 su PVM; išsiuntimas 1–3 d. d.), offers.hasMerchantReturnPolicy (14 d. ne maisto prekėms,
 ...
 *   v1.1 (S1717, 2.14): pristatymo kainos schema'oje su PVM (2,15 / 3,99 — kasos kainos po S1716); Organization mazgas (logo, url,
 *   telefonas, adresas, legalName) papildomas VISUOSE puslapiuose, ne tik prekės; pradiniame puslapyje Rank Math „Article" mazgas
 *   (puslapio numatytoji schema) ir jo autoriaus Person išmetami — lieka WebPage/WebSite/Organization.
 *   Išjungti: opcija ps_schema_prekes_isjungta = 1. Nieko nekeičia, jei Rank Math grafo nėra.
 */
if ( ! defined( 'ABSPATH' ) ) { exit; }

final class Petshop_Schema_Prekes {
	const PASTOMATAS_CT = 215; const KURJERIS_CT = 399; const NEMOKAMAS_NUO_CT = 3000; // su PVM (kasos kainos, S1716)
	...
	/** Organization mazgas — visuose puslapiuose (v1.1). */
	private static function organizacija( $data, $logo ) {
	...
	/** Pradinis puslapis: ne straipsnis (v1.1) — Article ir jo Person mazgai išmetami. */
	private static function pradinis( $data ) {
		foreach ( $data as $k => $node ) {
			if ( ! is_array( $node ) || ! isset( $node['@type'] ) ) { continue; }
			$t = (array) $node['@type'];
			if ( in_array( 'Article', $t, true ) || in_array( 'Person', $t, true ) ) { unset( $data[ $k ] ); }
		}
		return $data;
	}
	...
	public static function json_ld( $data, $jsonld ) {
		...
		$logo = self::logo();
		$data = self::organizacija( $data, $logo );
		if ( is_front_page() ) { return self::pradinis( $data ); }
