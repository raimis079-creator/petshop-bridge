...
class Petshop_Sutikimai {
	...
	public static function laukas( $checkout ) {
		echo '<div class="ps-sutikimai" style="margin:10px 0 4px">';
		woocommerce_form_field( self::LAUKAS, array(
			'type'  => 'checkbox',
			'class' => array( 'form-row-wide', 'ps-similar-optout' ),
			'label' => esc_html( self::ETIKETE ),
			'required' => false,
		), $checkout->get_value( self::LAUKAS ) ? 1 : 0 );
		echo '<p class="ps-sutikimai-paaisk" style="font-size:.85em;opacity:.75;margin:-6px 0 10px">' . esc_html( self::PAAISKINIMAS ) . '</p></div>';
	}
