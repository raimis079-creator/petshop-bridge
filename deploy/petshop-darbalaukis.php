<?php
/**
 * Petshop Darbalaukis v3.2 (S1607, 2 etapas, Run 18 — 2+ sandėliai → Neišrūšiuoti, „Auto“) — SĄRAŠAS KAIP MAKETE v7 + SKYDELIS SU TRIMIS KELIAIS.
 *
...
#wpcontent{padding-left:0!important}#wpbody-content{padding-bottom:0!important}html.wp-toolbar{padding-top:32px!important}body{background:#EEF1EF}.psj-2{display:none!important}</style>';
	...
	protected static function kodel( $e ) {
		...
		elseif ( $t && 'i_av' === $e['k'] && ! $e['b'] && ( $pid = self::atvira_partija( $e['tiek'] ) ) ) { $s = 'AV ' . ( $e['av_qty'] ? 'tik ' . (int) $e['av_qty'] : 'nėra' ) . ', ' . $t . ' turi. Užsakymas į AV iš ' . $t . ' jau atviras (#' . $pid . ') — siūlom vežti į AV ir siųsti viena siunta. Jei skubu — „' . $t . ' siunčia klientui“.'; }
	...
	protected static function lentele( $rows, $eile ) {
		...
		foreach ( $rows as $r ) {
			...
			if ( $o->get_customer_note() ) { echo '<div class="dl-note">' . esc_html( $o->get_customer_note() ) . '</div>'; }
			echo '</td>';
