...
#wpcontent{padding-left:0!important}#wpbody-content{padding-bottom:0!important}html.wp-toolbar{padding-top:32px!important}body{background:#EEF1EF}.psj-2{display:none!important}</style>';
		...
		switch ( $t[4] ) {
			...
			case 'lipdukas':
				if ( 'lp' === $f['vez'] ) { // v3.42 (S1680, Raimis 09-14 „toks principas kaip Venipak“): LP siunta + lipdukas per plugino API čia pat
					...
					return array( 'Lipdukas LP', self::dl_url( 'lp_lipdukas', $id ), array( 'antraste' => $antraste, 'tekstas' => 'Registruoti LP Express siuntą ir suformuoti lipduką? Paštomatas: ' . ( $lpv ? $lpv : '—' ) . '.' . ( $sv > 0 ? ' Svoris ' . number_format( (float) $sv, 2, ',', '' ) . ' kg.' : '' ) . ' Numeris atsiras kortelėje, PDF — nuoroda „Lipdukas LP“.', 'ok' => 'Lipdukas LP' ), 'p', 0 );
				}
	...
	protected static function skydelis( $f ) {
		...
		$lp_pdf = ''; if ( 'lp' === $f['vez'] ) { $pf = (string) $o->get_meta( '_ps_lp_lipdukas_pdf' ); $lp_pdf = ( $pf && file_exists( $pf ) ) ? self::dl_url( 'lp_pdf', $id ) : ''; if ( $lp_pdf && ! $f['uzdarytas'] ) { $perreg = self::dl_url( 'lp_lipdukas', $id ); } } // v3.42
		$b = $f['btn'];
		...
		return array(
			...
			'eil' => $eil, 'pastaba' => $pastaba, 'nr_siuntos' => $nr, 'pak' => $pak, 'perreg' => $perreg, 'lp_pdf' => $lp_pdf, /* v3.42 */
			'klausimas' => $f['kl'],
...
(function(){
	...
	function rodyti(o,r){ skO=o;
		...
		var S=$('skSiunta'); if(o.pak||o.nr_siuntos.length||o.perreg||o.lp_pdf){ S.style.display='block'; $('skSiuntaT').innerHTML=(o.nr_siuntos.length?'<div>'+o.nr_siuntos.map(esc).join('<br>')+'</div>':'<div class="pilkas maz">siunta dar neregistruota</div>')
			...
			+(o.lp_pdf?'<div style="margin-top:6px"><a class="v" href="'+esc(o.lp_pdf)+'" target="_blank">Lipdukas LP (PDF)</a>'+((o.perreg&&!o.pak)?' <a class="v" href="'+esc(o.perreg)+'">Lipdukas iš naujo</a>':'')+'</div>':''); /* v3.42 */
