<?php
/**
 * Petshop Welcome Modal v1.4.2 (S1714, 2026-09-24; v1.4.1/2 — Complianz juosta įterpiama JS vėliau ir iš pradžių paslėpta,
 *   todėl DOM būsena netikrinama: kai Complianz skriptas yra, laukiama tik `cmplz_banner-status` slapuko) — (1) CTA „Pridėti augintinį" nustato psw_seen (anksčiau modalas
 *   antrą kartą atsidarydavo ant anketos); (2) nerodomas /augintinio-profilis/ ir /skaiciuokle/ puslapiuose;
 *   (3) svečiui rodomas tik kai Complianz juosta jau uždaryta (cmplz_banner-status slapukas arba juostos nėra);
 *   (4) svečio psw_seen tikrinamas ir JS pusėje — nuo S1713 puslapiai kešuojami (WP Super Cache), serverio
 *   $_COOKIE patikra svečiui nebegalioja.
 * v1.3 (S1636) — A tekstas suvienodintas su B tonu (Raimio sprendimas).
...
class Petshop_Welcome_Modal {
	...
	protected static function variantas() {
		...
		if ( is_page( array( 'augintinio-profilis', 'skaiciuokle' ) ) ) return '';
		if ( isset( $_COOKIE[ self::COOKIE ] ) ) return ''; // svečiui su kešu — žr. JS patikrą
	...
	public static function render() {
		...
		(function(){
		  ...
		  if(/(?:^|; )<?php echo esc_js( self::COOKIE ); ?>=/.test(document.cookie)){ ov.remove(); return; }
		  if(/\/(augintinio-profilis|skaiciuokle|kasa|krepselis)\//.test(location.pathname)){ ov.remove(); return; }
		  ...
		  ov.querySelectorAll('.psw-b1').forEach(function(a){ a.addEventListener('click',cookie); });
		  ...
		  function cmplzOk(){ if(/(?:^|; )cmplz_banner-status=/.test(document.cookie)) return true;
			return !document.getElementById('cmplz-cookiebanner-js'); /* Complianz yra — laukiam, kol lankytojas uždarys juostą (slapukas) */ }
		  var bandymai=0;
		  function planuok(){ if(cmplzOk()){ setTimeout(rodyk, <?php echo (int) $delsa * 1000; ?>);
			<?php if ( ! $b ) : ?>if(window.matchMedia('(min-width:768px)').matches) document.addEventListener('mouseout',exit);<?php endif; ?>
			} else if(++bandymai<300){ setTimeout(planuok,1000); } }
		  planuok();
		})();
		</script>
		<?php
