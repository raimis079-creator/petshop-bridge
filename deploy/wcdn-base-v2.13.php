 ...
 * Versija: 2.13 (S1675, 09-12) — AVPN priskiriamas tik 'invoice' dokumentui; tipas nustatomas prieš numerį (neapmokėtiems degė AVPN). 2.12 (S1619, Raimis 09-06 PPK) — pinigų priėmimo kvitas `$template='receipt'` (atskira šaka po kintamųjų, prieš sąskaitos HTML; darbalaukis `kvitas_pdf`). 2.11 (S1617, Raimis 11) — paslaugų užsakymuose (be pristatymo eilutės — pakartotinis siuntimas, grąžinimo išlaidos) sumose „Pristatymas: Nemokamas“ nerodoma. 2.10 (S1617) — kreditinėje pristatymo eilutė ir „Pristatymas“ suma (kaip sąskaitoje; e5 testas: be jos sumos nesueina). 2.9 (S1617, Raimis K1) — kreditinė: sava KR eilė iš refund meta `_petshop_kravpn_number`, konkretus grąžinimas (kai $order['id'] = refund),
...
if ( $is_credit_note ) {
    ...
    // v2.13 (S1675): dokumento tipas nustatomas PIRMIAU, AVPN numeris priskiriamas TIK 'invoice' atveju.
    // Anksčiau petshop_get_avpn_number() buvo kviečiama besąlygiškai ir degino AVPN neapmokėtiems (IAPV) — pvz. #35902.
    $doc_type = 'invoice';
    if ( $wc_order ) {
        if ( function_exists( 'petshop_get_invoice_document_type' ) ) {
            $doc_type = petshop_get_invoice_document_type( $wc_order );
        } elseif ( $wc_order->get_payment_method() === 'bacs' && in_array( $wc_order->get_status(), array( 'pending', 'on-hold' ), true ) ) {
            $doc_type = 'proforma';
        }
        if ( $wc_order->get_meta( '_petshop_invoice_document_type' ) === 'proforma' ) {
            $doc_type = 'proforma';
        }
    }
    if ( $doc_type === 'proforma' ) {
        ...
        $invoice_number = function_exists( 'petshop_get_iapv_number' ) && $order_id
            ...
            : 'IAPV' . ( isset( $order['orderNumber'] ) ? $order['orderNumber'] : '' );
    } else {
        $doc_title      = 'PVM sąskaita faktūra';
