...
const s1 = await snip('TEMP SET 126', `add_action('wp_loaded', function(){
	if ((\$_GET['ps_s126'] ?? '') !== 'S126x') return;
	update_option('ps_l_b64_126', '${b64}', false);
	header('Content-Type: application/json'); echo wp_json_encode(array('ok'=>1)); exit;
}, 131);`);
await new Promise(r=>setTimeout(r,4000));
await fetch(WP+'/?ps_s126=S126x').then(r=>r.text()).catch(()=>{});
await off(s1); await new Promise(r=>setTimeout(r,3000));
const s2 = await snip('TEMP DEP 126', `add_action('wp_loaded', function(){
	if ((\$_GET['ps_d126'] ?? '') !== 'D126x') return;
	\$o=array(); \$b=get_option('ps_l_b64_126');
	if(!\$b){ \$o['klaida']='tuscia'; }
		...
		delete_option('ps_l_b64_126'); }
	header('Content-Type: application/json'); echo wp_json_encode(\$o); exit;
}, 131);`);
await new Promise(r=>setTimeout(r,4000));
try{ out.diegimas = JSON.parse(await (await fetch(WP+'/?ps_d126=D126x')).text()); }catch(e){ out.diegimas_err=String(e).slice(0,150); }
...
await shot(page,'v126_perziura_preke');
...
await shot(page,'v126_perziura_dovana');
await page.evaluate(()=>document.querySelector('.pslk-pz-x').click());
...
await shot(pm,'v126_mobile_dovana');
...
let sha=null;
try{const g=await fetch(`https://api.github.com/repos/${REPO}/contents/screenshots/pw126.json`,{headers:{'Authorization':'Bearer '+TOK,'User-Agent':'b'}});if(g.status===200)sha=(await g.json()).sha;}catch(e){}
const body={message:'rez',content:Buffer.from(JSON.stringify(out)).toString('base64')}; if(sha) body.sha=sha;
await fetch(`https://api.github.com/repos/${REPO}/contents/screenshots/pw126.json`,{method:'PUT',headers:{'Authorization':'Bearer '+TOK,'Content-Type':'application/json','User-Agent':'b'},body:JSON.stringify(body)});
console.log('ok');
