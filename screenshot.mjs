...
try{
  const sA=await snip('TEMP P1F E2EA',A64);
  ...
  await put('rep_1_virsus.png', await p.screenshot({fullPage:false}), 'ataskaita virsus');
  await p.evaluate(()=>window.scrollTo(0,1400)); await new Promise(r=>setTimeout(r,700));
  await put('rep_2_vidurys.png', await p.screenshot({fullPage:false}), 'ataskaita vidurys');
  await p.evaluate(()=>window.scrollTo(0,document.body.scrollHeight)); await new Promise(r=>setTimeout(r,700));
  await put('rep_3_apacia.png', await p.screenshot({fullPage:false}), 'ataskaita apacia');
  await br.close();
}catch(e){ out.bendra=String(e).slice(0,300); }
await put('p1f.json', Buffer.from(JSON.stringify(out)), 'p1f screens');
console.log('ok');
