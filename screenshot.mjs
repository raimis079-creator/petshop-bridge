...
const H2=/Rekomenduojama paros doz\w* suaugusioms kat\u0117ms/i;
...
for(const id of IDS){let T="";try{T=(JSON.parse(fs.readFileSync('/tmp/c/'+id+'.json','utf8')).content||{}).raw||"";}catch(e){rep[id]={err:'read'};okAll=false;continue;}
  ...
  builds[id]={newT,sample:rows[0].lo+' g'};
}
console.log('PARSE:',JSON.stringify(rep,null,1));
if(!okAll){console.log('ABORT');commit("prins_catapply_"+Date.now()+".json",JSON.stringify({abort:1,rep}));process.exit(0);}
const wres={};
for(const id of IDS){const newT=builds[id].newT;
  ...
  wres[id]={lossless:md5(rb)===md5(newT),b2b:rb.includes('b2b-black'),tbl:rb.includes('<table>'),val:rb.includes(builds[id].sample),fe_b2b:fe.includes('b2b-black'),fe_tbl:fe.includes('<table'),fe_val:fe.includes(builds[id].sample)};
}
console.log('WRITE/VERIFY:',JSON.stringify(wres,null,1));
commit("prins_catapply_"+Date.now()+".json",JSON.stringify({rep,wres},null,1));
console.log('DONE');
