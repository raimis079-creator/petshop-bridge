...
for(const id of IDS){
  ...
  g.compSafe=!/(Sud\u0117t|Analitin|Metaboliz|Priedai)/i.test(removed);
  ...
  let code='000',rb='';
  try{code=execSync(`curl -sk -o /tmp/wr.json -w "%{http_code}" -X POST -u "$WP_USER:$WP_PASS_CLEAN" -H "Content-Type: application/json" --data @/tmp/w.json "https://dev.avesa.lt/wp-json/wp/v2/product/${id}?context=edit&_fields=id,content"`,{env,maxBuffer:200000000}).trim();}catch(e){code='ERR';}
...
commit("ont_A_apply2_"+Date.now()+".json", JSON.stringify(results,null,1));
console.log("DONE");
