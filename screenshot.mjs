...
function commit(name,str){ const url='https://api.github.com/repos/'+repo+'/contents/screenshots/'+name; let sha=''; try{ sha=JSON.parse(execSync('curl -s -H "Authorization: Bearer '+tok+'" "'+url+'?ref=main&t='+Date.now()+'"',{encoding:'utf8'})).sha||''; }catch(e){} const body={message:'fr3',branch:'main',content:Buffer.from(str,'utf8').toString('base64')}; if(sha) body.sha=sha; fs.writeFileSync('/tmp/cbfr3.json',JSON.stringify(body)); execSync('curl -s -o /dev/null -X PUT -H "Authorization: Bearer '+tok+'" -H "Accept: application/vnd.github+json" -d @/tmp/cbfr3.json "'+url+'"',{encoding:'utf8'}); }
...
(async()=>{
  ...
  var out = { len: html.length, has_naujausi: html.indexOf('NAUJAUSI'), has_footer_widgets_class: html.indexOf('footer-widgets') };
  var idx = html.indexOf('NAUJAUSI');
  out.ctx = idx>=0 ? html.slice(Math.max(0,idx-1200), idx+50) : '(NEBUVO RASTA)';
  ...
  commit('footer_ctx2.json', JSON.stringify(out));
  console.log('len',out.len,'idx',out.has_naujausi);
})();
