...
async function busena(pg,name){ await miegok(800); const html=await pg.content();
  ...
  if(name) o.put=await put('screenshots/h261_'+name+'.png',await pg.screenshot({fullPage:true}),'H261B'); return o; }
try{
  ...
  const d=await fx(WP+'/?ps_h260=RUN20260824O&deploy=1&sha='+SHA,{},'deploy');
  ...
  const p=await fx(WP+'/?ps_h260=RUN20260824O&po=1',{},'po'); try{ out.serveris=JSON.parse(await p.text()); }catch(e){ out.serveris='ne-json'; }
...
await put('screenshots/h261run.json', Buffer.from(JSON.stringify(out,null,1)), 'H261B');
