...
// === RECEPTAS ===
const RECEPTAS = {
  pavadinimas: "Churu įvairių skonių rinkinys katėms · 7 vnt.",
  sku: "RINK-CHURU-MIX-7",
  kaina: 21.29,
  dydis: 7,
  kategorija_slug: "skanestu-rinkiniai",
  trumpas_aprasymas: "Penkių skirtingų Churu formatų rinkinys katėms: Luxury, Pops, TidBits, Fun Bites ir kreminis. Idealus, norint išbandyti įvairius skonius ir tekstūras.",
  komponentai: [
    { id: 26077, kiekis: 2 }, // Luxury tunu+krevetės (2.79)
    { id: 21609, kiekis: 2 }, // Pops tunu+vištiena (2.49)
    { id: 26064, kiekis: 1 }, // TidBits tunu+lašiša (2.99)
    { id: 24009, kiekis: 1 }, // Fun Bites tunu+sūris (6.59)
...
function generateDescription(comp_details, recipe) {
  // Vienetas pagal kategoriją
  const isKonservai = recipe.kategorija_slug === 'konservu-rinkiniai';
  const vienetas = isKonservai ? 'skardinės' : 'vnt.';
  let html = `<h3>Rinkinyje rasite (${recipe.dydis} ${vienetas}):</h3>\n<ol>\n`;
  // Map id -> kiekis iš recepto
  const kiekisMap = {};
  recipe.komponentai.forEach(k => { kiekisMap[k.id] = k.kiekis; });
  comp_details.forEach(c => {
    if (c.name) {
      const kiekis = kiekisMap[c.id] || 1;
      const prefix = kiekis > 1 ? `${kiekis} × ` : '';
      html += `  <li>${prefix}${c.name}</li>\n`;
    }
  });
  html += `</ol>\n`;
  // Vandens tekstas tik konservams (maistui), ne skanėstams/kramtalams
  if (isKonservai) {
    html += `<p><em>Gyvūnas visuomet turi turėti šviežio geriamo vandens.</em></p>`;
  }
  return html;
}
...
(async () => {
  ...
  // === Description preview ===
  const desc_html = generateDescription(comp_details, RECEPTAS);
  out.aprasymo_preview = desc_html.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();

  // === Layout preview ===
  const layout = gridLayout(RECEPTAS.komponentai.length);
  ...
  const cr = api('POST', '/wp-json/wc/v3/products', {
    ...
    meta_data: [
      { key: '_petshop_component_quantities', value: JSON.stringify(
          Object.fromEntries(RECEPTAS.komponentai.map(k => [String(k.id), k.kiekis]))
        ) }
    ]
  });
  ...
  try {
    ...
    for (const c of comp_details) {
      ...
      if (fs.existsSync(file) && fs.statSync(file).size > 1000) {
        // Unikalus tile vienam komponentui (kiekis matomas kortelėje per "× N" žymeklį)
        dl.push(file);
      }
    }
