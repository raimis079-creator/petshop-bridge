...
const C16=[[18109,"Catelux","a"],[18106,"Catelux","a"],[18101,"Culinesse","a"],[18098,"Culinesse","a"],[18095,"DailyCat","a"],[18092,"DailyCat","a"],[18077,"Kitten","k"],[18074,"Kitten","k"],[18043,"Marinesse","a"],[18040,"Marinesse","a"],[18011,"NatureCat","a"],[18007,"Naturelle","a"],[18004,"Naturelle","a"],[17992,"Senior","a"],[17986,"SensiCat","a"],[17983,"SensiCat","a"]];
...
function removeBlock(T){const iR=T.indexOf("\u0160\u0117rimo rekomendacij");if(iR<0)return null;const iStart=T.lastIndexOf("<p",iR);if(iStart<0)return null;let pos=iStart,consumed=0,first=true;while(consumed<16){const ws=(T.slice(pos).match(/^\s*/)||[""])[0];const bstart=pos+ws.length;if(T.slice(bstart,bstart+2)!=="<p")break;const endP=T.indexOf("</p>",bstart);if(endP<0)break;const inner=stripTags(T.slice(bstart,endP+4));const keep=first||isFeed(inner)||isKitHdr(inner)||inner===""||inner==="\u2013";if(!keep)break;pos=endP+4;consumed++;first=false;}return {iStart,iEnd:pos,consumed};}
const results=[];
for(const [id,line,fmt] of C16){try{
  ...
  if(r.consumed<4){results.push({id,line,SKIP:"consumed_low",c:r.consumed});continue;}
  ...
  const g={rekom_gone:newT.indexOf("\u0160\u0117rimo rekomendacij")<0,sud:newT.indexOf("Sud\u0117tis")>-1,anal:newT.indexOf("Analitin")>-1,single:(newT.split(MARK).length-1)===1,probe:newT.indexOf(probe)>-1};
  if(!g.rekom_gone||!g.sud||!g.anal||!g.single||!g.probe){results.push({id,line,SKIP:"guard",g});continue;}
  const wc=writeRaw(id,newT);const after=readRaw(id);
  results.push({id,line,fmt,act:"CONVERTED",consumed:r.consumed,write:wc,lossless:after!==null&&md5(after)===md5(newT),ver_rekom_gone:after!==null&&after.indexOf("\u0160\u0117rimo rekomendacij")<0,ver_single:after!==null&&(after.split(MARK).length-1)===1,ver_probe:after!==null&&after.indexOf(probe)>-1});
}catch(e){results.push({id,line,ERR:String(e).slice(0,80)});}}
commit("catA_apply_"+Date.now()+".json", JSON.stringify(results,null,1));
console.log("DONE");
