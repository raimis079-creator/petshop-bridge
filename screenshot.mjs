...
const TOK=process.env.GH_TOKEN||''; const REPO=process.env.GH_REPO||'raimis079-creator/petshop-bridge'; const VER='dep-193002'; const out={v:VER};
...
try{ const t=await token('https://www.googleapis.com/auth/analytics.edit');
  ...
  const ack=await fetch('https://analyticsadmin.googleapis.com/v1beta/properties/346051580:acknowledgeUserDataCollection',{method:'POST',headers:A,body:JSON.stringify({acknowledgement:'I acknowledge that I have the necessary privacy disclosures and rights from my end users for the collection and processing of their data, including the association of such data with the visitation information Google Analytics collects from my site and/or app property.'})}); out.ack_status=ack.status; out.ack=(await ack.text()).slice(0,200);
