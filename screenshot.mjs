...
while(page<=3){
  ...
  acc=acc.concat(p.map(x=>({id:x.id,title:(x.title&&x.title.rendered||'').replace(/&amp;/g,'&').replace(/&#8211;/g,'-')})));
  if(p.length<100)break;page++;
}
const primeDog=acc.filter(x=>/PRIME/.test(x.title)&&/DOG/.test(x.title));
commit("farmina_prime_dog_list.json",JSON.stringify({total:primeDog.length,prods:primeDog},null,1));
console.log("LOOK DONE",primeDog.length);
