...
function putFile(name,buf){const u='https://api.github.com/repos/'+REPO+'/contents/'+name;let s='';
 for(let i=0;i<6;i++){try{const j=JSON.parse(execSync('curl -sk --max-time 30 -H "Authorization: Bearer '+TOKG+'" "'+u+'?n='+Math.random()+'"',{maxBuffer:80e6}).toString());if(j.sha)s=j.sha;}catch(e){}
  fs.writeFileSync('/tmp/pj.json',JSON.stringify({message:'s566',content:buf.toString('base64'),...(s?{sha:s}:{})}));
...
const O={VERSIJA_RUN:'run566'};
// auth cookie per snippeta
const PHP=`add_action('wp_loaded',function(){
 if(!isset($_GET['ps_ck'])||$_GET['ps_ck']!=='Ck566xQ') return;
...
let sid=null;
fs.writeFileSync('/tmp/sn.json',JSON.stringify({name:'TEMP S566 Cookie',code:PHP,scope:'global',active:true}));
...
const ck=sh('curl -sSk --max-time 60 "'+SITE+'/?ps_ck=Ck566xQ"');
...
if(C && C.logged_in){
 try{
  ...
  const puslapiai=[
    ['skydelis','/wp-admin/admin.php?page=ps-desk&eile=nauji'],
  ];
  O.psl={};
  for(const [v,u] of puslapiai){
    try{
      ...
      // atidarom misru uzsakyma #34882
      try{
        await p.click('tr:has(.pd-nr:text-is("#34882")) .pd-cust');
        await p.waitForTimeout(900);
      }catch(e){}
      const info=await p.evaluate(()=>{
        const T=e=>e&&e.innerText?e.innerText.replace(/\s+/g,' ').trim():'';
        return {
          nr:T(document.getElementById('pkNr')),
          eilutes:[...document.querySelectorAll('#pkBody .pd-line')].map(l=>({
            t:T(l).slice(0,120),
            mygtukas:T(l.querySelector('.pd-tiek a')),
            url:(l.querySelector('.pd-tiek a')||{}).href||''
          }))
        };
      });
      O.psl[v]={http:rp?rp.status():null, ...dbg, ...info};
      const el0=await p.$('#pdPeek');
      const png=el0?await el0.screenshot():await p.screenshot({fullPage:true});
      putFile('screenshots/s566_'+v+'.png', png);
      O.psl[v].png='screenshots/s566_'+v+'.png';
...
putFile('analize/s566.json', Buffer.from(JSON.stringify(O,null,1)));
console.log('OK');
