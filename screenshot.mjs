...
(async()=>{
  ...
  var s0 = await page.evaluate(()=>{
    var b=document.querySelector('.psc-cta-slot .single_add_to_cart_button');
    return {text:b?b.textContent.trim():'?', disabled:b?b.classList.contains('disabled'):null, bg:b?window.getComputedStyle(b).backgroundColor:'?'};
  });
  putBin('e5c_empty.png', await page.screenshot({fullPage:false}));
  // Pripildau iki 12 (12 vnt dydis) — pirmam konservui 12
  await page.evaluate(()=>{
    var form = Array.from(document.querySelectorAll('.psc-form')).find(f=>f.style.display!=='none');
    if(form){ var inp=form.querySelector('.mnm_child_products input.qty, input[type=number]'); if(inp){ inp.value=12; inp.dispatchEvent(new Event('change',{bubbles:true})); } }
  });
  await page.waitForTimeout(1500);
  var s12 = await page.evaluate(()=>{
    var b=document.querySelector('.psc-cta-slot .single_add_to_cart_button');
    return {text:b?b.textContent.trim():'?', disabled:b?b.classList.contains('disabled'):null, bg:b?window.getComputedStyle(b).backgroundColor:'?',
      sum:(document.querySelector('.psc-sum-selected')||{}).textContent, left:(document.querySelector('.psc-sum-left')||{}).textContent};
  });
  putBin('e5c_full.png', await page.screenshot({fullPage:false}));
  commit('e5c_check.json', JSON.stringify({empty:s0, full:s12},null,1));
  console.log(JSON.stringify({empty:s0, full:s12}));
