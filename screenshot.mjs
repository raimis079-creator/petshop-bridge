...
const idx=raw.indexOf('Šėrimo');
const ENC='&lt;/p&gt;';
const encClose=idx>=0?raw.indexOf(ENC,idx):-1;
const out={id:ID,status:j.status,rawLen:raw.length,idx,encClose};
if(idx<0||encClose<0){out.err='marker';commit("farmina_pilot_"+Date.now()+".json",JSON.stringify(out,null,1));console.log("ABORT marker");process.exit(0);}
const cut=encClose+ENC.length;
const ins='\n'+block;
const newT=raw.slice(0,cut)+ins+raw.slice(cut);
const g={
  markerOk:idx>=0&&encClose>idx,
