...
(async()=>{
  ...
  var probe = await page.evaluate(()=>{
    ...
    var st = cta ? window.getComputedStyle(cta) : null;
    return {
      cta_disabled_class: cta ? cta.classList.contains('disabled') : null,
      cta_bg: st ? st.backgroundColor : '?',
      dup_2930: (document.body.textContent.match(/29,30/g)||[]).length,
      has_old_note: document.body.textContent.includes('Fiksuota dėžės kaina')
    };
  });
  commit('e5b_check.json', JSON.stringify(probe,null,1));
  putBin('e5b_check.png', await page.screenshot({fullPage:false}));
