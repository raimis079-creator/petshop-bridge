...
function putFile(name,buf){const u='https://api.github.com/repos/'+REPO+'/contents/'+name;let s='';
 for(let i=0;i<6;i++){try{const j=JSON.parse(execSync('curl -sk --max-time 30 -H "Authorization: Bearer '+TOKG+'" "'+u+'?n='+Math.random()+'"',{maxBuffer:80e6}).toString());if(j.sha)s=j.sha;}catch(e){}
  fs.writeFileSync('/tmp/pj.json',JSON.stringify({message:'s631',content:buf.toString('base64'),...(s?{sha:s}:{})}));
...
let sid=null;
for(const s of (Array.isArray(lst)?lst:[])) if(String(s.name||'').indexOf('TEMP S631')>=0) sid=s.id;
fs.writeFileSync('/tmp/sn.json',JSON.stringify({name:'TEMP S631 Own patikra',code:PHP,scope:'global',active:true}));
...
const d1=sh('curl -sSk --max-time 120 "'+SITE+'/?ps_own=K631o"');
try{O.own=JSON.parse(d1);}catch(e){O.own={raw:String(d1).slice(0,1500)};}
// isjungiam po darbo
...
putFile('analize/s631.json', Buffer.from(JSON.stringify(O,null,1)));
console.log('OK');
