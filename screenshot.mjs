...
`;
const s2=await wp('/wp-json/code-snippets/v1/snippets',{method:'POST',body:JSON.stringify({name:'TEMP GPAIS Autologin v5',code:phpAuto,scope:'global',active:true,priority:5})});
const j2=js(s2.text);
...
try{
  ...
  let atidaryta=false;
  for(let b=0;b<3 && !atidaryta;b++){
    await pg.evaluate(()=>{
      const kart=document.querySelector('.kort-kartoti');
      if(kart){ kart.click(); return; }
      const a=document.querySelector('.pskat-t .atv'); if(a) a.click();
    });
    try{ await pg.waitForSelector('.kort-tabs button',{timeout:25000}); atidaryta=true; }
    catch(e){ await pg.waitForTimeout(3000); }
  }
  try{ if(!atidaryta) throw new Error('kortele neatsidare po 3 bandymu'); }
  catch(e){
...
const g=await fetch('https://api.github.com/repos/'+REPO+'/contents/screenshots/gp3.json',{headers:{'Authorization':'Bearer '+TOK}});
if(g.status===200){ body.sha=(await g.json()).sha; }
await fetch('https://api.github.com/repos/'+REPO+'/contents/screenshots/gp3.json',{method:'PUT',headers:{'Authorization':'Bearer '+TOK,'Content-Type':'application/json'},body:JSON.stringify(body)});
