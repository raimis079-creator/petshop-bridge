...
(async()=>{
  ...
  try{
    ...
    await p.goto(BASE+'/kategorija/katems/tualetai-kraikai-semtuveliai/?yith_wcan=1&filter_tipas=uzdaras-namelis&query_type_tipas=or&cb='+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
    ...
    const buf = await p.screenshot({fullPage:false}); commitB64('final_visual.png', buf.toString('base64'));
    await b.close();
  }catch(e){ out.err=e.message.slice(0,150); }
  commitB64('final_visual.json', Buffer.from(JSON.stringify(out),'utf8').toString('base64'));
