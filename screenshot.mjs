...
const out={};
for(const id of [12452,12928,12458]){const T=readRaw(id);if(T===null){out[id]={ERR:1};continue;}
  const iM=T.indexOf(MARK);
  out[id]={pre:T.slice(Math.max(0,iM-260), iM), feed:T.slice(iM, T.length)};}
commit("euk_pre_"+Date.now()+".json", JSON.stringify(out,null,1));
console.log("DONE");
