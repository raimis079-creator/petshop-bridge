...
(async()=>{
  try{
    ...
    // atidarau visas accordion sekcijas
    meta.opened=await page.evaluate(()=>{let n=0;document.querySelectorAll('.ps-desc-acc details').forEach(d=>{if(!d.open){d.open=true;n++;}});return n;});
    await page.waitForTimeout(800);
    meta.feeding_tables=await page.evaluate(()=>{let c=0;document.querySelectorAll('.ps-desc-acc table').forEach(t=>{if(/svoris/i.test(t.innerText))c++;});return c;});
    meta.serimo_text=await page.evaluate(()=>{for(const t of document.querySelectorAll('.ps-desc-acc table')){if(/svoris/i.test(t.innerText))return t.innerText.replace(/\s*\n\s*/g,' | ').slice(0,300);}return 'NERA';});
    const buf=await page.screenshot({fullPage:true,timeout:30000});
    meta.png=commit('verify_conv.png',buf.toString('base64'));
    meta.ok=true; await browser.close();
  }catch(e){meta.ok=false;meta.error=String(e&&e.stack||e).slice(0,300);}
  commit('verify_conv_meta.json',Buffer.from(JSON.stringify(meta,null,2)).toString('base64'));
  console.log('DONE');
})();
