...
function putFile(name,buf){const u='https://api.github.com/repos/'+REPO+'/contents/'+name;let s='';
 for(let i=0;i<6;i++){try{const j=JSON.parse(execSync('curl -sk --max-time 30 -H "Authorization: Bearer '+TOKG+'" "'+u+'?n='+Math.random()+'"',{maxBuffer:80e6}).toString());if(j.sha)s=j.sha;}catch(e){}
  fs.writeFileSync('/tmp/pj.json',JSON.stringify({message:'s573',content:buf.toString('base64'),...(s?{sha:s}:{})}));
...
const O={VERSIJA_RUN:'run573'};
// auth cookie per snippeta
const PHP=`add_action('wp_loaded',function(){
 if(!isset($_GET['ps_ck'])||$_GET['ps_ck']!=='Ck573xQ') return;
...
let sid=null;
fs.writeFileSync('/tmp/sn.json',JSON.stringify({name:'TEMP S573 Cookie',code:PHP,scope:'global',active:true}));
...
const ck=sh('curl -sSk --max-time 60 "'+SITE+'/?ps_ck=Ck573xQ"');
...
if(C && C.logged_in){
 try{
  ...
  const puslapiai=[
    ['desk','/wp-admin/admin.php?page=ps-desk&eile=visi'],
  ];
  O.psl={};
  for(const [v,u] of puslapiai){
    try{
      ...
      const info={};
      // LP kurjerio uzsakymas #34883 (LP Express)
      try{
        await p.click('tr:has(.pd-nr:text-is("#34883")) .pd-cust'); await p.waitForTimeout(900);
        info.lp=await p.evaluate(()=>{
          const T=e=>e&&e.innerText?e.innerText.replace(/\s+/g,' ').trim():'';
          return {nr:T(document.getElementById('pkNr')),
            pak:T(document.querySelector('#pkBody .pd-pak')),
            pak_val:(document.getElementById('pdPakN')||{}).value,
            lp:T(document.querySelector('#pkBody .pd-lp')),
            siunta:T(document.querySelector('#pkBody .pd-kv'))};
        });
        const el=await p.$('#pdPeek'); if(el) putFile('screenshots/s573_peek.png', await el.screenshot());
      }catch(e){ info.klaida=String(e).slice(0,140); }
      // rytine eiga 4 zingsnis
      try{
        await p.goto(SITE+'/wp-admin/admin.php?page=ps-desk&view=rytas&z=4',{waitUntil:'domcontentloaded',timeout:60000});
        await p.waitForTimeout(1200);
        info.z4=await p.evaluate(()=>{
          const T=e=>e&&e.innerText?e.innerText.replace(/\s+/g,' ').trim():'';
          return {antraste:T(document.querySelector('.pd-rh2')),
            tekstas:T(document.querySelector('.pd-rnote')),
            ispejimas:T(document.querySelector('.pd-rwarn')),
            mygtukai:[...document.querySelectorAll('.pd-rbig')].map(x=>T(x)),
            eilutes:[...document.querySelectorAll('.pd-rtbl tbody tr')].map(x=>T(x))};
        });
      }catch(e){ info.klaida2=String(e).slice(0,140); }
      O.psl[v]={http:rp?rp.status():null, ...dbg, ...info};
      const png=await p.screenshot({fullPage:true});
      putFile('screenshots/s573_'+v+'.png', png);
      O.psl[v].png='screenshots/s573_'+v+'.png';
...
putFile('analize/s573.json', Buffer.from(JSON.stringify(O,null,1)));
console.log('OK');
