...
try {
  out.check = [23320,20274,14211].map(id=>{
    ...
    return {id, t:(p.attributes||[]).map(a=>a.name+":"+(a.options||[]).join("/"))};
  });
} catch(e){ out.check_err=(e.stderr||String(e)).slice(0,80); }
fs.writeFileSync("screenshots/drask_apply2_"+Date.now()+".txt", JSON.stringify(out,null,2));
fs.writeFileSync("screenshots/drask_apply2.txt", JSON.stringify(out,null,2));
