...
const IDS=[20475,20461,20478,20472];
const out={};
for(const id of IDS){const T=readRaw(id);if(T===null){out[id]={ERR:1};continue;}
  const iR=T.indexOf("Rekomenduojamas kiekis per par");const iM=T.lastIndexOf(MARK);
  out[id]={iR,iM,len:T.length,dump:iR>=0?T.slice(Math.max(0,iR-80), iM>iR?iM+40:iR+1400):"NO_REKOM"};
}
commit("recon4_"+Date.now()+".json", JSON.stringify(out,null,1));
console.log("DONE");
