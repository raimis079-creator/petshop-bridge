...
const p=JSON.parse(execSync(`curl -sk --max-time 30 -u "$WP_USER:$WP_PASS_CLEAN" "https://dev.avesa.lt/wp-json/wc/v3/products/27130?_fields=permalink"`,{encoding:'utf8',env,maxBuffer:20000000}));
const url=p.permalink+(p.permalink.includes('?')?'&':'?')+'ps_desc=1&ps_desc_open=all';
const { chromium } = await import('playwright');
const TS=String(Date.now());
const browser=await chromium.launch({args:['--ignore-certificate-errors']});
const ctx=await browser.newContext({viewport:{width:1280,height:1700}, ignoreHTTPSErrors:true});
...
putBinary('kids27130_'+TS+'.png', buf);
console.log('done');
await browser.close();
