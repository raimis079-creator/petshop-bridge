...
try{
  ...
  await put('v865_800.png', await p.screenshot({fullPage:false}), 'v865 800');
  out.m700 = await tikr(1500,700,'700');
  await put('v865_700.png', await p.screenshot({fullPage:false}), 'v865 700');
  out.m1100 = await tikr(1600,1100,'1100');
  await br.close();
}catch(e){ out.klaida=String(e).slice(0,300); }
await put('vis865.json', Buffer.from(JSON.stringify(out)), 'vis865');
console.log('ok');
