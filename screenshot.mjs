...
const out={};
for(const id of [33370,14651,14516]){
  const j=cj(`${BASE}/wp/v2/product/${id}?context=edit&_fields=id,content`);
  const raw=(j.content&&j.content.raw)||"";
  const i=raw.indexOf('Pakuotės');
  out[id]={around:i>=0?raw.slice(Math.max(0,i-80),i+250):'(none)',rawTail:raw.slice(-300)};
}
commit("farmina_pak_inspect.json",JSON.stringify(out,null,1));
