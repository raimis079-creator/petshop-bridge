...
const COLS=["Kūno svoris","Mažas aktyvumas","Vidutinis aktyvumas","Didelis aktyvumas"];
const LRM={comp:"Kukurūzai, ryžiai (21,0%), ėrienos miltai (18,0%), jautienos taukai, kukurūzų glitimas, bulvių baltymas, bulvių krakmolas, džiovinta cikorijų masė (2,0%), celiuliozė, saulėgrąžų aliejus, fruktooligosacharidai (FOS) (0,5%), dumblių aliejus (0,2%), hidrolizuotos mielės.",
  ...
  energy:"3530 kcal/kg",cols:COLS,
  rows:[["1","30","35","40"],["2","50","55","65"],["3","65","75","85"],["4","80","90","105"],["5","90","105","125"],["6","105","120","145"],["7","120","135","165"],["8","130","150","180"],["9","140","165","195"],["10","155","180","210"],["12","175","205","240"],["14","195","230","270"],["16","220","250","295"],["18","240","275","325"],["20","255","295","350"]]};
const HRM={comp:"Silkių miltai (22,0%), kukurūzai, ryžiai (18,0%), žirniai, kukurūzų glitimas, jautienos taukai, bulvių krakmolas, džiovinta cikorijų masė (2,0%), celiuliozė, saulėgrąžų aliejus, fruktooligosacharidai (FOS) (0,5%), dumblių aliejus (0,2%), hidrolizuotos mielės.",
  ...
  energy:"3482 kcal/kg",cols:COLS,
  rows:[["1","30","35","40"],["2","50","55","65"],["3","65","75","90"],["4","80","90","110"],["5","90","110","125"],["6","105","125","145"],["7","120","140","165"],["8","130","155","180"],["9","145","165","195"],["10","155","180","210"],["12","180","205","245"],["14","200","230","275"],["16","220","255","300"],["18","240","280","330"],["20","260","300","355"]]};
const REC={16824:LRM,16821:LRM,16830:HRM,16827:HRM};
...
for(const id of IDS){let j={};try{j=JSON.parse(fs.readFileSync('/tmp/c/'+id+'.json','utf8'));}catch(e){rep[id]={err:'read'};ok=false;continue;}
  ...
  rep[id]={status:j.status,sku:j.sku,recipe:(r===LRM?'LR-MINI':'HR-MINI'),introLen:intro.length,newLen:newT.length,guards:g,pass:allg};
  if(!allg){ok=false;continue;}
  builds[id]={newT,sample:r.rows[4][1]+' g',status:j.status};
}
if(!ok){commit("prins_mini_"+Date.now()+".json",JSON.stringify({abort:1,rep},null,1));console.log("ABORT");process.exit(0);}
const wres={};
...
commit("prins_mini_"+Date.now()+".json",JSON.stringify({rep,wres},null,1));
