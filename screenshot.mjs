...
add_action('wp_loaded', function(){
  if(!isset($_GET['ps_idb']) || $_GET['ps_idb']!=='Idb25x') return;
  ...
  $o['ps_pets_engine']=$wpdb->get_var("SELECT ENGINE FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='{$pf}ps_pets'");
  header('Content-Type: application/json'); echo json_encode($o); exit;
});`;
try{
  const mk=wj('POST','code-snippets/v1/snippets',{name:'IDB (temp)',code:CODE,scope:'front-end',active:true,priority:5});
  let sid=null; try{sid=JSON.parse(mk).id;}catch(e){}
  execSync('sleep 4');
  const r=execSync('curl -sk "https://dev.avesa.lt/?ps_idb=Idb25x"',{maxBuffer:5e6,timeout:60000}).toString();
...
putB64('innodb.json', Buffer.from(JSON.stringify(o)).toString('base64'));
console.log('done');
