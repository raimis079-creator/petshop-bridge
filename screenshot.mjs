...
function putFile(name,buf){const u='https://api.github.com/repos/'+REPO+'/contents/'+name;let s='';
 for(let i=0;i<6;i++){try{const j=JSON.parse(execSync('curl -sk --max-time 30 -H "Authorization: Bearer '+TOKG+'" "'+u+'?n='+Math.random()+'"',{maxBuffer:80e6}).toString());if(j.sha)s=j.sha;}catch(e){}
  fs.writeFileSync('/tmp/pj.json',JSON.stringify({message:'s521',content:buf.toString('base64'),...(s?{sha:s}:{})}));
...
const O={VERSIJA_RUN:'run521'};
// auth cookie per snippeta
const PHP=`add_action('wp_loaded',function(){
 if(!isset($_GET['ps_ck'])||$_GET['ps_ck']!=='Ck521xQ') return;
...
let sid=null;
fs.writeFileSync('/tmp/sn.json',JSON.stringify({name:'TEMP S521 Cookie',code:PHP,scope:'global',active:true}));
...
const ck=sh('curl -sSk --max-time 60 "'+SITE+'/?ps_ck=Ck521xQ"');
...
if(C && C.logged_in){
 try{
  ...
  p.on('dialog',async dg=>{ O.dialogas=dg.message().slice(0,220); await dg.accept(); });
  ...
  for(const [v,u] of puslapiai){
    try{
      ...
      if(v==='neapmoketi'){
        O.pries=await p.evaluate(()=>[...document.querySelectorAll('.pd-tbl tbody tr')].map(r=>({
          nr:(r.querySelector('.pd-nr')||{}).innerText,
          bus:(r.querySelector('.pd-pill')||{}).innerText,
          vei:(r.querySelector('td.pd-act .pd-btn-p')||{}).innerText,
          url:(r.querySelector('td.pd-act a.pd-btn-p')||{}).href
        })));
        const png0=await p.screenshot({fullPage:true}); putFile('screenshots/s521_pries.png',png0);
        try{
          const t=await p.$('tr:has(.pd-nr:text-is("#34881")) td.pd-act a.pd-btn-p');
          if(t){ await t.click(); await p.waitForLoadState('domcontentloaded'); await p.waitForTimeout(2500); }
          else { O.mygtukas='NERASTAS'; }
        }catch(e){ O.klik=String(e).slice(0,160); }
        O.po=await p.evaluate(()=>({
          url:location.href,
          pranesimas:(document.querySelector('.pd-msg')||{}).innerText,
          rail:[...document.querySelectorAll('.pd-q')].map(x=>x.innerText.replace(/\s+/g,' ').trim()),
          eilutes:[...document.querySelectorAll('.pd-tbl tbody tr')].map(r=>(r.querySelector('.pd-nr')||{}).innerText)
        }));
        const png1=await p.screenshot({fullPage:true}); putFile('screenshots/s521_po.png',png1);
        try{
          await p.goto(SITE+'/wp-admin/admin.php?page=ps-desk&eile=visi',{waitUntil:'domcontentloaded',timeout:60000});
          await p.waitForTimeout(1500);
          O.busena_34881=await p.evaluate(()=>{
            const r=[...document.querySelectorAll('.pd-tbl tbody tr')].find(x=>(x.querySelector('.pd-nr')||{}).innerText==='#34881');
            return r?{bus:(r.querySelector('.pd-pill')||{}).innerText,
                      pay:(r.querySelector('.pd-pay')||{}).innerText,
                      vei:(r.querySelector('td.pd-act .pd-btn-p')||{}).innerText}:'NERASTA';
          });
        }catch(e){ O.po_klaida=String(e).slice(0,150); }
      }
      const png=await p.screenshot({fullPage:true});
      putFile('screenshots/s521_'+v+'.png', png);
      O.psl[v].png='screenshots/s521_'+v+'.png';
...
putFile('analize/s521.json', Buffer.from(JSON.stringify(O,null,1)));
console.log('OK');
