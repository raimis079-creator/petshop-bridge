...
// SVARBU: pre_wp_mail blokuoja REALU siuntima (testinis pastas neegzistuoja -> bounce),
// bet laiska uzfiksuoja, kad galetume istraukti nuoroda.
const php = `
...
fs.writeFileSync('/tmp/snip.json', JSON.stringify({ name:'TEMP M8 E207', code:php, scope:'global', active:true }));
...
await page.locator('input.pspet-input').first().fill('Anoniminė');
...
// --- 3. Ar laiskas su nuoroda sugeneruotas NAUJAM email? ---
...
try {
  ...
  const m = String(mj.body||'').match(/https:\/\/dev\.avesa\.lt\/petshop-login\?token=[^"']+/);
  link = m ? m[0].replace(/&amp;/g,'&') : null;
  out.s.link_found = !!link;
} catch(e){ out.s.mail_raw = mail.slice(0,200); }

// --- 4. Paspaudziam nuoroda ---
if (link) {
  await page.goto(link, { waitUntil:'domcontentloaded', timeout:45000 });
  await page.waitForTimeout(1500);
  out.s.confirm_page = await page.locator('body').innerText().catch(()=>'');
  await page.screenshot({ path:'/tmp/e207a.png' });
  await page.locator('button[type=submit]').first().click();
  await page.waitForTimeout(5000);
  out.s.landed_after_login = page.url();
  out.s.profile_text = await page.locator('#pspet-profile').innerText().catch(()=>'(nera #pspet-profile)');
  out.s.draft_after_login = await page.evaluate(() => { try { return localStorage.getItem('pspet_draft'); } catch(e){ return 'ERR'; } });
...
const kphp = `add_action('wp_loaded', function(){ if(!isset($_GET['ps_ki'])||$_GET['ps_ki']!=='Rr3Ww8Yy'){return;} global $wpdb; delete_option('ps_test_last_mail'); $n=$wpdb->query("DELETE FROM {$wpdb->prefix}snippets WHERE name LIKE 'TEMP M8%'"); echo wp_json_encode(array('d'=>$n)); exit; });`;
fs.writeFileSync('/tmp/k.json', JSON.stringify({ name:'TEMP M8 Kill vI', code:kphp, scope:'global', active:true }));
sh(`curl -sk -X POST -H "Authorization: Basic ${AUTH}" -H "Content-Type: application/json" -d @/tmp/k.json "${API}"`);
out.kill = sh('curl -sk --max-time 25 "https://dev.avesa.lt/?ps_ki=Rr3Ww8Yy"').slice(0,80);
ghPut('screenshots/m8_e207.json', Buffer.from(JSON.stringify(out)), 'e207 result');
console.log('DONE');
