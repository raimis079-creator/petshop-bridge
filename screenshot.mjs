...
(async()=>{
  ...
  // 1. Dabartinis 17057 turinys
  const p = api('GET','/wp-json/wc/v3/products/17057');
  out.current_17057 = {
    id: p.id,
    name: (p.name||'').slice(0,80),
    image_count: (p.images||[]).length,
    images: (p.images||[]).map(im => ({id:im.id, src:im.src, name:im.name, position:im.position}))
  };
  commit('inspect_17057.json', JSON.stringify(out,null,1));
  console.log("DONE");
})();
