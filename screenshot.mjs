...
function putFile(name,buf){const u='https://api.github.com/repos/'+REPO+'/contents/'+name;let s='';
 for(let i=0;i<6;i++){try{const j=JSON.parse(execSync('curl -sk --max-time 30 -H "Authorization: Bearer '+TOKG+'" "'+u+'?n='+Math.random()+'"',{maxBuffer:80e6}).toString());if(j.sha)s=j.sha;}catch(e){}
  fs.writeFileSync('/tmp/pj.json',JSON.stringify({message:'s487',content:buf.toString('base64'),...(s?{sha:s}:{})}));
...
const O={VERSIJA_RUN:'run487-admin'};
// auth cookie per snippeta
const PHP=`add_action('wp_loaded',function(){
 if(!isset($_GET['ps_ck'])||$_GET['ps_ck']!=='Ck487xQ') return;
...
let sid=null;
fs.writeFileSync('/tmp/sn.json',JSON.stringify({name:'TEMP S487 Cookie',code:PHP,scope:'global',active:true}));
...
const ck=sh('curl -sSk --max-time 60 "'+SITE+'/?ps_ck=Ck487xQ"');
...
if(C && C.logged_in){
 try{
  ...
  const puslapiai=[
    ['uzsakymai_pakuotojui','/wp-admin/admin.php?page=wc-orders'],
    ['prekiu_sarasas_AV','/wp-admin/edit.php?post_type=product&ps_av_filter=turi'],
  ];
  O.psl={};
  for(const [v,u] of puslapiai){
    try{
      ...
      putFile('screenshots/s487_'+v+'.png', png);
      O.psl[v].png='screenshots/s487_'+v+'.png';
...
putFile('analize/s487.json', Buffer.from(JSON.stringify(O,null,1)));
console.log('OK');
