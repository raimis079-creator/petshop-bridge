...
(async()=>{
  ...
  const ctx=await browser.newContext({ignoreHTTPSErrors:true,httpCredentials:{username:env.WP_USER,password:env.WP_PASS_CLEAN},viewport:{width:1200,height:1400}});
  const page=await ctx.newPage();
  try{await page.goto(URL,{waitUntil:'domcontentloaded',timeout:60000});}catch(e){out.gotoErr=String(e).slice(0,150);}
  await page.waitForTimeout(3500);
  // try click description tab
  ...
  // extract description rendering
  try{
    out.desc=await page.evaluate(()=>{
      const el=document.querySelector('#tab-description')||document.querySelector('.woocommerce-Tabs-panel--description')||document.querySelector('.product-short-description')||document.querySelector('.entry-content');
      if(!el)return {found:false};
      return {found:true, hasTableEl: !!el.querySelector('table'), hasB2b: !!el.querySelector('.b2b-black'),
        text: (el.innerText||'').replace(/\s+/g,' ').slice(0,500),
        showsLiteralTag: (el.innerText||'').includes('<p>')||(el.innerText||'').includes('&lt;')};
    });
  }catch(e){out.descErr=String(e).slice(0,150);}
  // screenshot description element if found, else full page
  try{
    ...
    out.shot=putBin('farmina_shot_14535.png',buf);
  }catch(e){out.shotErr=String(e).slice(0,150);}
  await browser.close();
  commit('farmina_shot_meta.json',JSON.stringify(out,null,1));
