...
const r=cs('PUT','snippets/512',{name:'Petshop Aprasymu Accordion PROTO v3 (test ps_desc)',scope:'global',active:true,code:php});
out.snippet={id:r.id,active:r.active};
execSync('sleep 2');
const PRODS=[ {id:33394,n:'Farmina (ZB) maistas'},{id:13869,n:'Monge maistas'},{id:33900,n:'Trixie narvas (ZB)'},{id:27879,n:'Nobleza tualetas (VF)'} ];
...
for(const p of PRODS){
  ...
  try{
    ...
    rec.titles = await page.evaluate(()=>{ const a=document.querySelector('.ps-desc-acc'); return a?[...document.querySelectorAll('.ps-desc-acc summary')].map(s=>s.textContent.trim()):'FALLBACK(plain)'; });
    const el=await page.$('#tab-description');
    if(el){ await el.scrollIntoViewIfNeeded(); await page.waitForTimeout(400); rec.png=putResult('proto3_'+p.id+'_'+TS+'.png', await el.screenshot()); }
  }catch(e){ rec.err=e.message.slice(0,80); }
  out.tests.push(rec);
}
await browser.close();
putResult('proto3_test_'+TS+'.txt', out);
