...
const o={};
const mk2=wj('POST','code-snippets/v1/snippets',{name:'VIS221 (temp)',code:Buffer.from(PREP,'base64').toString('utf8'),scope:'front-end',active:true,priority:5});
...
try{
  ...
  o.typed=await page.evaluate(()=>{
    const i=[...document.querySelectorAll('input')].find(x=>x.placeholder==='Įveskite pavadinimą arba prekės ženklą');
    if(!i) return 'NO INPUT';
    const setter=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set;
    setter.call(i,'Josera'); i.dispatchEvent(new Event('input',{bubbles:true})); return 'OK';
  });
  await page.waitForTimeout(3000);
  o.res_n=await page.evaluate(()=>document.querySelectorAll('.pspet-res-item').length);
  o.clicked=await page.evaluate(()=>{const b=document.querySelector('.pspet-res-item'); if(b){const t=b.textContent.trim().slice(0,60); b.click(); return t;} return 'NONE';});
  await page.waitForTimeout(5000);
  o.po=await page.evaluate(()=>{const n=document.querySelector('.pspet-profile'); return n? n.innerText.replace(/\s+/g,' ').slice(0,350):'';});
  o.modulyje=await page.evaluate(()=>/Grįžti į augintinio puslapį/.test(document.body.innerText) && /Dabartinis maistas/.test(document.body.innerText));
  const b1=await page.screenshot({fullPage:true}); putB64('y_2_liekam_modulyje.png', b1.toString('base64'));
  await browser.close();
...
putB64('ver221.json', Buffer.from(JSON.stringify(o)).toString('base64'));
console.log('done');
