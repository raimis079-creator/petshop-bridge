...
const SHA='6014760f628c900a25861e857093ff6f16b8392e';
const MD5='cbef34facf5b876dcabda3d98513f52c';
const out={v:'H243B'};
...
let sid=null;
try{
  const c=await fx(SNIP,{method:'POST',headers:A,body:JSON.stringify({name:'TEMP H243 v1 (desk 3.38 deploy)',code:Buffer.from(B64,'base64').toString('utf8'),scope:'global',active:true,priority:5})},'snip');
  ...
  if(j&&j.id){
    ...
    if(cookies.length && out.deploy && out.deploy.md5===MD5){
      ...
      out.nauji={http:r.status(),
        ...
        put:await put('screenshots/h243_nauji.png',await pg.screenshot({fullPage:true}),'H243B')};
      const r2=await pg.goto(WP+'/wp-admin/admin.php?page=ps-desk&eile=nauji&zvilgsnis=neperduota',{waitUntil:'networkidle',timeout:60000});
      await miegok(900);
      out.zvilgsnis={http:r2.status(),
        eiluciu:await pg.$$eval('tbody tr',ns=>ns.length).catch(()=>-1)};
      const r3=await pg.goto(WP+'/wp-admin/admin.php?page=ps-desk&eile=misrus',{waitUntil:'networkidle',timeout:60000});
      await miegok(900);
      out.misrus={http:r3.status(),
        sekcijos:await pg.$$eval('.pd-msec',ns=>ns.map(n=>n.textContent.trim())),
        bukles:await pg.$$eval('.pd-mbukle',ns=>ns.map(n=>n.textContent.trim())),
        put:await put('screenshots/h243_misrus.png',await pg.screenshot({fullPage:true}),'H243B')};
...
await put('screenshots/h243.json', Buffer.from(JSON.stringify(out,null,1)), 'H243B');
