...
add_action('init', function(){
  ...
  if(!$eil){ $o['rasta']=false; $eil=array('product_id'=>19715,'id'=>0,'kiekis_gautas'=>0); }
  {
    $pid=(int)$eil['product_id'];
    $o['rasta']=array('id'=>$eil['id'],'pid'=>$pid,'kiekis'=>$eil['kiekis_gautas']);
    if((int)$eil['id']) { $wpdb->delete($t, array('id'=>(int)$eil['id']), array('%d')); }
    /* Tiesiogiai i meta ir lookup: AV saugiklis gali atmesti CRUD rasyma. */
    update_post_meta($pid,'_stock','8');
    update_post_meta($pid,'_stock_status','instock');
    $lt=$p.'wc_product_meta_lookup';
    $wpdb->update($lt, array('stock_quantity'=>8,'stock_status'=>'instock'), array('product_id'=>$pid));
...
`;
const s1=await wp('/wp-json/code-snippets/v1/snippets',{method:'POST',body:JSON.stringify({name:'TEMP Valymas v73b',code:php,scope:'global',active:true,priority:5})});
const j1=js(s1.text);
await new Promise(r=>setTimeout(r,4000));
try{
  const res=execSync(`curl -sk "${B}/?ps_v=Vl3tC2" --max-time 120`,{encoding:'utf8',maxBuffer:20*1024*1024});
...
const g=await fetch(`https://api.github.com/repos/${REPO}/contents/screenshots/val73b.json`,{headers:{'Authorization':'Bearer '+TOK}});
if(g.status===200){ body.sha=(await g.json()).sha; }
await fetch(`https://api.github.com/repos/${REPO}/contents/screenshots/val73b.json`,{method:'PUT',headers:{'Authorization':'Bearer '+TOK,'Content-Type':'application/json'},body:JSON.stringify(body)});
console.log('ok');
