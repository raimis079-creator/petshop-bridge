...
function putFile(name,buf){const u='https://api.github.com/repos/'+REPO+'/contents/'+name;let s='';
 for(let i=0;i<6;i++){try{const j=JSON.parse(execSync('curl -sk --max-time 30 -H "Authorization: Bearer '+TOKG+'" "'+u+'?n='+Math.random()+'"',{maxBuffer:80e6}).toString());if(j.sha)s=j.sha;}catch(e){}
  fs.writeFileSync('/tmp/pj.json',JSON.stringify({message:'s585',content:buf.toString('base64'),...(s?{sha:s}:{})}));
...
const O={VERSIJA_RUN:'run585'};
// auth cookie per snippeta
const PHP=`add_action('wp_loaded',function(){
 if(!isset($_GET['ps_ck'])||$_GET['ps_ck']!=='Ck585xQ') return;
...
let sid=null;
fs.writeFileSync('/tmp/sn.json',JSON.stringify({name:'TEMP S585 Cookie',code:PHP,scope:'global',active:true}));
...
const ck=sh('curl -sSk --max-time 60 "'+SITE+'/?ps_ck=Ck585xQ"');
  ...
  for(const [v,u] of puslapiai){
    try{
      ...
      const info=await p.evaluate(()=>{
        ...
        const wcsub=[...document.querySelectorAll('#adminmenu a')].map(x=>x.getAttribute('href')||'')
          .filter(h=>h.indexOf('wc-orders')>-1);
      ...
      putFile('screenshots/s585_'+v+'.png', png);
      O.psl[v].png='screenshots/s585_'+v+'.png';
...
putFile('analize/s585.json', Buffer.from(JSON.stringify(O,null,1)));
console.log('OK');
