...
async function busena(pg,name){ await miegok(800); const html=await pg.content();
  ...
  if(name) o.put=await put('screenshots/h265_'+name+'.png',await pg.screenshot({fullPage:true}),'H265B'); return o; }
try{
  ...
  const d=await fx(WP+'/?ps_h265=RUN20260824X&deploy=1&sha='+SHA,{},'deploy');
...
await put('screenshots/h265run.json', Buffer.from(JSON.stringify(out,null,1)), 'H265B');
