...
const s1 = await snip('TEMP SET 118', `add_action('wp_loaded', function(){
	if ((\$_GET['ps_s118'] ?? '') !== 'S118x') return;
	update_option('ps_l_b64_118', '${b64}', false);
	header('Content-Type: application/json'); echo wp_json_encode(array('ok'=>1)); exit;
}, 131);`);
await new Promise(r=>setTimeout(r,4000));
await fetch(WP+'/?ps_s118=S118x').then(r=>r.text()).catch(()=>{});
await off(s1); await new Promise(r=>setTimeout(r,3000));

const s2 = await snip('TEMP DEP 118', `add_action('wp_loaded', function(){
	if ((\$_GET['ps_d118'] ?? '') !== 'D118x') return;
	\$o=array(); \$b=get_option('ps_l_b64_118');
	if(!\$b){ \$o['klaida']='tuscia'; }
		...
		delete_option('ps_l_b64_118'); }
	header('Content-Type: application/json'); echo wp_json_encode(\$o); exit;
}, 131);`);
await new Promise(r=>setTimeout(r,4000));
try{ out.diegimas = JSON.parse(await (await fetch(WP+'/?ps_d118=D118x')).text()); }catch(e){ out.diegimas_err=String(e).slice(0,150); }
await off(s2); await new Promise(r=>setTimeout(r,2500));

/* ===== dezes ===== */
const s3 = await snip('TEMP DEZ 118', `add_action('wp_loaded', function(){
	if ((\$_GET['ps_z118'] ?? '') !== 'Z118x') return;
...
await new Promise(r=>setTimeout(r,4000));
try{ out.paruosimas = JSON.parse(await (await fetch(WP+'/?ps_z118=Z118x')).text()); }catch(e){ out.paruosimas_err=String(e).slice(0,200); }
...
let sha=null;
try{const g=await fetch(`https://api.github.com/repos/${REPO}/contents/screenshots/pw118.json`,{headers:{'Authorization':'Bearer '+TOK,'User-Agent':'b'}});if(g.status===200)sha=(await g.json()).sha;}catch(e){}
const body={message:'rez',content:Buffer.from(JSON.stringify(out)).toString('base64')}; if(sha) body.sha=sha;
const p=await fetch(`https://api.github.com/repos/${REPO}/contents/screenshots/pw118.json`,{method:'PUT',headers:{'Authorization':'Bearer '+TOK,'Content-Type':'application/json','User-Agent':'b'},body:JSON.stringify(body)});
