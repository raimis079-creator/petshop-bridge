...
(async()=>{
  try{
    ...
    meta.expanded=await page.evaluate(()=>{let n=0;const w=['Sud\u0117tis','Analitin','\u0160\u0117rimo'];document.querySelectorAll('*').forEach(el=>{const t=(el.childNodes.length===1&&el.firstChild&&el.firstChild.nodeType===3)?el.textContent.trim():'';if(t&&w.some(x=>t.startsWith(x))&&t.length<40){try{el.click();n++;}catch(e){}}});return n;});
    await page.waitForTimeout(1000);
    meta.serTable=await page.evaluate(()=>{for(const t of document.querySelectorAll('table')){if(/svoris/i.test(t.innerText))return t.innerText.replace(/\s*\n\s*/g,' | ').slice(0,400);}return 'NERA';});
    const buf=await page.screenshot({fullPage:true,timeout:30000});
    meta.png=commit('w1shot_'+TS+'.png',buf.toString('base64')); meta.png_name='w1shot_'+TS+'.png';
    meta.ok=true; await browser.close();
  }catch(e){meta.ok=false;meta.error=String(e&&e.stack||e).slice(0,300);}
  commit('w1meta_'+TS+'.json',Buffer.from(JSON.stringify(meta,null,2)).toString('base64'));
