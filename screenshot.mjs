...
const targets=[
  {id:19751, url:"https://dev.avesa.lt/product/ambrosia-begrudis-su-eriena-ir-sviezia-lasisa-sausas-maistas-sunims-grain-free-lamb-fresh-salmon-12-kg/?ps_desc=1"}
];
...
for(const t of targets){
  ...
  let buf;
  try{ buf=await page.locator('.ps-desc-acc').first().screenshot({timeout:8000}); }
  catch(e){ buf=await page.screenshot({fullPage:false}); }
  putBinary('ambfinal_'+t.id+'_'+TS+'.png', buf);
  await ctx.close();
}
console.log('done');
await browser.close();
