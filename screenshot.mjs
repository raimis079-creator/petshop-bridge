...
function putResult(name,txt){const u='https://api.github.com/repos/'+REPO+'/contents/analize/'+name;let s='';
 for(let i=0;i<6;i++){try{const j=JSON.parse(execSync('curl -sk --max-time 30 -H "Authorization: Bearer '+TOKG+'" "'+u+'?n='+Math.random()+'"',{maxBuffer:20e6}).toString());if(j.sha)s=j.sha;}catch(e){}
  fs.writeFileSync('/tmp/pj.json',JSON.stringify({message:'s497',content:Buffer.from(txt).toString('base64'),...(s?{sha:s}:{})}));
...
const O={VERSIJA_RUN:'run497-v1'}; let sid=null;
...
fs.writeFileSync('/tmp/sn.json',JSON.stringify({name:'TEMP S497 Pakuotes v2',code:PHP,scope:'global',active:true}));
...
putResult('s497.json', JSON.stringify(O,null,1));
console.log('OK');
