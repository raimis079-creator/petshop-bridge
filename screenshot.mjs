...
(async()=>{
  ...
  // === 1. HTTP patikra visų 17 URL PRIEŠ deploy ===
  const urls = ['/apie-mus/','/kontaktai/','/mano-paskyra/','/uzsakymai/','/pristatymas/','/apmokejimas/','/grazinimas/','/taisykles/','/privatumo-politika/','/slapuku-politika/','/kategorija/sunims/','/kategorija/katems/','/kategorija/grauzikams/','/kategorija/pauksciams/','/kategorija/zuvims/','/akcijos/','/pasiulymai/'];
  ...
  if(!allOk){
    out += '\n!!! KAZKAS NE 200 - deploy STABDOMAS !!!\n';
    putFile('deploy_footer.txt', out);
    return;
  }
  out += '  visi 17 = 200 ✅\n\n';

  // === 2. Backup i WP options ===
  const backups = {};
  for(const wid of Object.keys(PAYLOADS)){
    const r = api('/wp-json/wp/v2/widgets/'+wid+'?context=edit');
    try{
      const j = JSON.parse(r);
      backups[wid] = j.instance ? j.instance.raw : null;
    }catch(e){}
  }
  out += 'Backup padarytas (widgets_backup_20260709.json guli output\'e)\n\n';

  // === 3. PUT kiekvieno widget'o ===
  out += '=== PUT widgets ===\n';
  ...
  const chk = await p.evaluate(()=>{
    ...
    for(const id of cols){
      ...
      result[id] = { title, link_count: links.length, links };
    }
    return result;
  });
  out += '=== VERIFIKACIJA ===\n'+JSON.stringify(chk, null, 2)+'\n';
  putBin('footer_desktop.png', await p.screenshot({ fullPage:false, clip:{x:0, y:900-500, width:1280, height:500} }));
  // Scroll iki footer
  await p.evaluate(()=>window.scrollTo(0,document.body.scrollHeight));
  await p.waitForTimeout(1000);
  putBin('footer_full.png', await p.screenshot({ fullPage:false }));
  await ctx.close();

  // === 5. Kitas puslapis - kad globaliai atsinaujino ===
  ...
  const chk2 = await p2.evaluate(()=>{
    ...
    return { klientams_links: kl ? kl.querySelectorAll('a').length : 0 };
  });
  ...
  putBin('footer_mobile.png', await pm.screenshot({ fullPage:false }));
  await cm.close();
