...
const skuShaUrl=`https://api.github.com/repos/${repo}/contents/screenshots/animonda_sku_html.json?ref=main&t=${Date.now()}`;
...
for(const [sku,obj] of Object.entries(SKU_HTML)){
  ...
  loopCnt++;
  if(loopCnt%10===0) commit('animonda_progress.json',JSON.stringify({loopCnt,stages,planned:planned.slice(-3),apply:apply.slice(-3)},null,1));
  try{
    ...
    // Insert priež "Šėrim" marker'į - rasti pradžia paragrapho
    ...
    planned.push({id:sku,title:j.title?.rendered||'',pass,guards,oldLen:c.length,newLen:newT.length,type:obj.type});
    stages.planned++;
    ...
    apply.push({id:sku,verLen,newLen:newT.length,lossless,oldLen:c.length,type:obj.type});
    stages.applied++;
  }catch(e){planned.push({id:sku,err:String(e).slice(0,100)});stages.failed++;}
}

const rep={loopCnt,stages,planned,apply};
commit(`animonda_apply_${Date.now()}.json`,JSON.stringify(rep,null,1));
console.log("done");
