...
(async()=>{
  ...
  const probe = await page.evaluate(()=>{
    ...
    return {
      ...
      mnm_status: (document.querySelector('.mnm_status, .mnm_message')||{}).textContent?.trim().slice(0,90) || '?'
    };
  });
  commit('vitrina_v4_check.json', JSON.stringify(probe,null,1));
  // Scrollinu iki MnM statuso
  await page.evaluate(()=>{ var e=document.querySelector('.mnm_status,.mnm_message'); if(e) e.scrollIntoView({block:'center'}); });
  await page.waitForTimeout(1000);
  putBin('vitrina_v4.png', await page.screenshot({fullPage:false}));
