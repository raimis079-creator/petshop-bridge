...
const out={versija:'PARUOSTI-M1'}; const NL=String.fromCharCode(10);
...
for (let i=0;i<FAILAI.length;i++){
  ...
  const sS=await snip('TEMP Q SET '+i,["add_action('wp_loaded', function(){"," if ((\$_GET['ps_s'] ?? '') !== 'Q"+i+"x') return;"," update_option('ps_x_b64_"+i+"', '"+b64+"', false);"," header('Content-Type: application/json'); echo wp_json_encode(array('ok'=>1)); exit;","}, 131);"].join(NL));
  await new Promise(r=>setTimeout(r,4500));
  try{ await fetch(WP+'/?ps_s=Q'+i+'x'); }catch(e){}
  await off(sS); await new Promise(r=>setTimeout(r,2500));
}
const sW=await snip('TEMP Q WRITE',[
"add_action('wp_loaded', function(){",
" if ((\$_GET['ps_w'] ?? '') !== 'QWx') return;",
...
"}, 131);"].join(NL));
await new Promise(r=>setTimeout(r,4500));
try{ out.diegimas=JSON.parse(await (await fetch(WP+'/?ps_w=QWx')).text()); }catch(e){ out.e1=String(e).slice(0,150); }
await off(sW); await new Promise(r=>setTimeout(r,3000));

const sL=await snip('TEMP Q LOGIN',["add_action('init', function(){"," if ((\$_GET['ps_login'] ?? '') !== 'QLx') return;"," \$a=get_users(array('role'=>'administrator','number'=>1,'fields'=>'ID')); if(!\$a) return;"," wp_set_current_user((int)\$a[0]); wp_set_auth_cookie((int)\$a[0], false, is_ssl());"," wp_safe_redirect(admin_url('admin.php?page=petshop-reports-paruosti')); exit;","}, 1);"].join(NL));
await new Promise(r=>setTimeout(r,4500));
...
await page.goto(WP+'/?ps_login=QLx',{waitUntil:'domcontentloaded',timeout:60000});
