...
for(const id of IDS){
  try{
    ...
    // Surask markeriu kandidatus
    const markers=[];
    for(const m of ['Šėrim','Rekomenduo','Dien','Patari','rekomenduo','šerimo','sėrim','feeding']) {
      let idx=c.indexOf(m);
      if (idx>=0) markers.push({m,idx});
    }
    out[id]={title:(j.title&&j.title.rendered)||'',cLen:c.length,markers};
  }catch(e){out[id]={err:String(e).slice(0,100)};}
}
commit('monge_rest_analysis.json',JSON.stringify(out,null,1));
console.log("done");
