...
const out={marker:'KOPIJA PATIKRA v4', ts:new Date().toISOString()};
...
`;
const s2=await wp('/wp-json/code-snippets/v1/snippets',{method:'POST',body:JSON.stringify({name:'TEMP Kopija Autologin v4',code:phpAuto,scope:'global',active:true,priority:5})});
...
try{
  ...
  const eil = await pg.evaluate(()=>document.querySelectorAll('.pskat-t .atv').length);
  out.eiluciu=eil;
  if (eil) {
...
out.put=await putResult('kopija_patikra4.json', out);
