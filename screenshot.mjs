...
(async()=>{
  ...
  const ctx=await browser.newContext({ignoreHTTPSErrors:true, viewport:{width:760,height:820}});
  const out={};
  ...
  if(s1) putBin('sav3_skan.png', await s1.screenshot()); else putBin('sav3_skan.png', await page1.screenshot());
  ...
  commit('sav3_check.json', JSON.stringify(out,null,1));
