...
let sid=null;
try{
  const c=await fx(SNIP,{method:'POST',headers:A,body:JSON.stringify({name:'TEMP H242 v2 (desk 3.37 deploy)',code:Buffer.from(B64,'base64').toString('utf8'),scope:'global',active:true,priority:5})},'snip');
  ...
  if(j&&j.id){
    ...
    const d=await fx(WP+'/?ps_h242=RUN20260823A',{},'deploy');
    ...
    if(cookies.length){
      ...
      out.misrus={http:r.status(),
        ...
        put:await put('screenshots/h242_misrus.png',await pg.screenshot({fullPage:true}),'H242C')};
