...
for(const D of DATA){
  ...
  for(const id of D.ids){
    try{
      ...
      // PRIDEDU GALE - garantuoja pilna issaugojima
      const newT = T + ser;
      // guards: visas T issaugotas + Sudetis nepakitusi + analitines vis dar yra + lentele prideta
      const preserved = newT.startsWith(T);
      const sm2=newT.match(/Sud\u0117tis:[\s\S]*?<\/p>/);
      const sudOk = sm2 && md5(sm2[0])===sud_md5;
      const analOk = newT.indexOf("Analitin")>-1 === analPresent;
      if(!preserved || !sudOk || !analOk || !/<table>/.test(newT)){ results.push({id,recipe:D.recipe,SKIP:"guard",preserved,sudOk,analOk}); continue; }
...
commit("wave1b_"+TS+".json", JSON.stringify(results,null,2));
console.log("DONE "+TS);
