...
const IDS=[18084,18065];
const out={};
for(const id of IDS){const T=readRaw(id);if(T===null){out[id]={ERR:1};continue;}
  const iR=T.indexOf("\u0160\u0117rimo rekomendacij");
  out[id]={iR,len:T.length,dump:iR>=0?T.slice(Math.max(0,iR-120),iR+1500):"NO"};
}
commit("recon4cat_"+Date.now()+".json", JSON.stringify(out,null,1));
console.log("DONE");
