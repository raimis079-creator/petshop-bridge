...
const sh=JSON.parse(execSync(`curl -s -H "Authorization: Bearer ${tok}" -H "Accept: application/vnd.github.raw" "https://api.github.com/repos/${repo}/contents/screenshots/monge_sku_html.json?ref=main&t=${Date.now()}"`,{encoding:'utf8',maxBuffer:200000000}));
const IDS=Object.keys(sh).map(k=>parseInt(k));
