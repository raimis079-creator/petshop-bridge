...
try{
  ...
  const ctx=await br.newContext({viewport:{width:1500,height:2200},ignoreHTTPSErrors:true,userAgent:UA});
  const pg=await ctx.newPage(); const js=[]; pg.on('pageerror',e=>js.push(String(e).slice(0,140)));
  await pg.goto(WP+'/?ps_lg=LG20260826',{waitUntil:'domcontentloaded',timeout:45000});
  await miegok(1200);
  await pg.goto(WP+'/wp-admin/admin.php?page=ps-pardavimai&preset=30&testiniai=1',{waitUntil:'domcontentloaded',timeout:60000});
  await miegok(2500);
  out.h1=await pg.$eval('h1',n=>n.textContent.trim()).catch(()=>'NERA');
  out.js=js;
  out.tekstas=await pg.evaluate(()=>document.querySelector('.psru')?document.querySelector('.psru').innerText.slice(0,4000):'NERA .psru');
  out.p1=await put('screenshots/pard_full.png', await pg.screenshot({fullPage:true}), VER);
  /* HUB */
  await pg.goto(WP+'/wp-admin/admin.php?page=petshop-reports',{waitUntil:'domcontentloaded',timeout:45000});
  await miegok(1500);
  out.p2=await put('screenshots/hub.png', await pg.screenshot({fullPage:true}), VER);
  await br.close();
...
await put('deploy/shot.json', Buffer.from(JSON.stringify(out,null,1)), VER);
