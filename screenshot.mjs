...
const REC={16854:HR,16851:HR,16824:LRM,16821:LRM};
...
for(const id of IDS){let j={};try{j=JSON.parse(fs.readFileSync('/tmp/c/'+id+'.json','utf8'));}catch(e){rep[id]={err:'read'};ok=false;continue;}
  ...
  let intro=T.slice(0,b)
    ...
    .replace(/la\u0161i\u0161\u0173\s+tauk[a-z\u0105-\u017E]*/gi,'dumbli\u0173 aliejumi').replace(/la\u0161i\u0161os\s+aliej[a-z\u0105-\u017E]*/gi,'dumbli\u0173 aliejumi')
  ...
  builds[id]={newT,sample:r.rows[r.rows.length-1][3]+' g'};
}
if(!ok){commit("prins_b3_"+Date.now()+".json",JSON.stringify({abort:1,rep},null,1));console.log("ABORT");process.exit(0);}
const wres={};
for(const id of IDS){const newT=builds[id].newT;
  ...
  wres[id]={lossless:md5(rb)===md5(newT),fe_tbl:fe.includes('<table'),fe_sample:fe.includes(builds[id].sample)};
}
commit("prins_b3_"+Date.now()+".json",JSON.stringify({rep,wres},null,1));
console.log("B3 DONE");
