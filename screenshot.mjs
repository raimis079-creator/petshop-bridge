...
try{
  ...
  const c=await fx(SNIP,{method:'POST',headers:A,body:JSON.stringify({name:'TEMP PS Katalogo Diegimas v1.0 (ID paieska)',
    code:Buffer.from(B64,'base64').toString('utf8'),scope:'global',active:true,priority:5})},'create');
  sid=JSON.parse(await c.text()).id; await miegok(9000);
  const d=await fx(WP+'/?ps_dieg=DRY',{headers:UA},'dry');
  const dt=await d.text(); let DJ=null; try{ DJ=JSON.parse(dt); }catch(e){ out.dry_zalias=dt.slice(0,900); }
  out.dry=DJ;
  if(DJ && DJ.veiksmas==='BUTU IRASYTA'){
    await miegok(2500);
  ...
  // patikra: ar langas atsidaro ir ar paieska pagal ID randa
  await miegok(4000);
  for(const [k,u] of Object.entries({be_id:WP+'/wp-admin/admin.php?page=ps-katalogas&kruva=visos&q=35102',
                                     sku:WP+'/wp-admin/admin.php?page=ps-katalogas&kruva=visos&q=187218-25l'})){
    try{ const h=await fx(u,{headers:{...UA,Authorization:AUTH}},k); const t=await h.text();
      out['patikra_'+k]={http:h.status, fatal:/Fatal error|Parse error/.test(t),
        nerasta:/prekių nėra|Rasta prekių<\/[^>]+>\s*0/i.test(t), baitu:t.length};
    }catch(e){ out['patikra_'+k]='klaida'; }
  }
}catch(e){ out.klaida=String(e).slice(0,600); }
try{ if(sid) await fetch(SNIP+'/'+sid,{method:'POST',headers:A,body:JSON.stringify({id:sid,active:false})}); }catch(e){}
await put('analize/dieg_status.json', Buffer.from(JSON.stringify(out,null,1)), VER);
console.log('ok');
