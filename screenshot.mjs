...
for(let i=0;i<ids.length;i+=50){
  ...
  for(let t=0;t<3;t++){ try{ const r=wc('products?include='+chunk+'&per_page=50&_fields=id,sku,name,description,short_description,date_modified'); prods=prods.concat(r); break; }catch(e){ execSync('sleep 2'); } }
}
putResult('backup_'+TS+'.json', JSON.stringify(prods));
