...
const NAME='Petshop Aprasymu Accordion PROTO v1 (test ps_desc)';
let sid=null; try{ const lst=cs('GET','snippets?per_page=100'); if(Array.isArray(lst)){ const e=lst.find(s=>s.name===NAME); if(e) sid=e.id; } }catch(e){}
let r; if(sid){ r=cs('PUT','snippets/'+sid,{name:NAME,scope:'global',active:true,code:php}); } else { r=cs('POST','snippets',{name:NAME,scope:'global',active:true,code:php}); }
out.snippet={id:r.id,active:r.active,name:r.name};
execSync('sleep 2');
// test prekes
const PRODS=[ {id:33394,src:'ZB/Farmina',cat:'maistas'},{id:13869,src:'Monge',cat:'maistas'},
              {id:33900,src:'ZB',cat:'narvas'},{id:27879,src:'VF',cat:'tualetas'} ];
const { chromium } = await import('playwright');
const browser = await chromium.launch({ args:['--no-sandbox','--disable-setuid-sandbox'] });
const ctx = await browser.newContext({ viewport:{ width:1180, height:1600 }, ignoreHTTPSErrors:true });
...
for(const p of PRODS){
  ...
  try{
    ...
    // aktyvuoti description tab jei reikia
    ...
    rec.dom = await page.evaluate(()=>{
      ...
      const tabtext=tab?tab.innerText.replace(/\s+/g,' ').trim():'';
      const junk=/\.b2b-|color:#000|margin:0 !important/.test(tab?tab.innerHTML:'');
      return {has_accordion:!!acc, section_titles:titles, tab_text_sample:tabtext.slice(0,180), junk_visible:junk};
    });
    const el=await page.$('#tab-description');
    if(el){ await el.scrollIntoViewIfNeeded(); await page.waitForTimeout(400); rec.png=putResult('proto_'+p.id+'_'+TS+'.png', await el.screenshot()); }
    else { rec.png=putResult('proto_'+p.id+'_'+TS+'.png', await page.screenshot()); rec.note='no-tab-el'; }
  }catch(e){ rec.err=e.message.slice(0,80); }
  out.tests.push(rec);
}
await browser.close();
putResult('proto_test_'+TS+'.txt', out);
