...
(async()=>{
  ...
  const probe = await page.evaluate(()=>{
    ...
    return {
      ...
      has_lt_pasirinkite: bodyText.includes('Pasirinkite') || bodyText.includes('Pasirinkote'),
      mnm_status: (document.querySelector('.mnm_status, .mnm_message, .mnm_price')||{}).textContent?.trim().replace(/\s+/g,' ').slice(0,100) || '?'
    };
  });
  commit('vitrina_v5_check.json', JSON.stringify(probe,null,1));
  await page.evaluate(()=>{ var e=document.querySelector('.mnm_status,.mnm_message,.mnm_price'); if(e) e.scrollIntoView({block:'center'}); });
  await page.waitForTimeout(1000);
  putBin('vitrina_v5.png', await page.screenshot({fullPage:false}));
