...
let sid=null;
try{
  const c=await fx(SNIP,{method:'POST',headers:A,body:JSON.stringify({name:'TEMP H249 v2 (desk 3.41)',code:Buffer.from(B64,'base64').toString('utf8'),scope:'global',active:true,priority:5})},'snip');
  ...
  if(j&&j.id){ sid=j.id; await miegok(9000);
    ...
    if(cookies.length && out.deploy && out.deploy.md5===MD5){
      ...
      out.visi={http:r.status(), fatal:(await pg.content()).includes('Fatal error'),
        ...
        put:await put('screenshots/h249_visi.png',await pg.screenshot({fullPage:true}),'H249B')};
...
await put('screenshots/h249run.json', Buffer.from(JSON.stringify(out,null,1)), 'H249B');
