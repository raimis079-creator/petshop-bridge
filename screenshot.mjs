...
const out={sources:{zb:0,vf:0,'vf+legacy':0,legacy:0,unknown:0}, zb_ids:[], unknown_ids:[], done:0, fail:0};
for(const ch of chunks){
  ...
  for(let t=0;t<2 && !ok;t++){
    try{ const r=execSync(`curl -sk --max-time 90 -u "$WP_USER:$WP_PASS_CLEAN" "https://dev.avesa.lt/?ps_imprisk=1&k=ps2026&ids=${ch}"`,{encoding:'utf8',env,maxBuffer:50000000});
      ...
      (ex.zb||[]).forEach(e=>out.zb_ids.push(e.id+':'+(e.t||'').slice(0,22)));
      (ex.unknown||[]).forEach(e=>out.unknown_ids.push(e.id));
      out.done++; ok=true;
...
putResult('imprisk5_'+TS+'.json', JSON.stringify(out));
