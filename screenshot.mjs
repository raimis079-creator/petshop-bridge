...
(async()=>{
  ...
  const probe = await page.evaluate(()=>{
    ...
    var hasProduct = false, hasQuantity = false;
    document.querySelectorAll('th.product-details, th.product-quantity').forEach(function(el){
      if(el.textContent.trim()==='Product') hasProduct=true;
      if(el.textContent.trim()==='Quantity') hasQuantity=true;
    });
    return {
      still_has_Product: hasProduct, still_has_Quantity: hasQuantity,
      has_please_select_en: bodyText.includes('Please select'),
      has_items_en: /\d+ items/.test(bodyText)
    };
  });
  commit('v8_check.json', JSON.stringify(probe,null,1));
  await page.evaluate(()=>{ var e=document.querySelector('.mnm_child_products th'); if(e) e.scrollIntoView({block:'center'}); });
  await page.waitForTimeout(500);
  putBin('v8_check.png', await page.screenshot({fullPage:false}));
