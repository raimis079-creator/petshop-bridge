...
// patikrinam WC produkto svorį ir matmenis
const w=cj(`${BASE}/wc/v3/products/14535`);
const out={
  id:w.id, name:w.name,
  weight:w.weight, dimensions:w.dimensions,
  shipping_class:w.shipping_class, sku:w.sku,
  // visi kiti svorį/dydį liečiantys
  attrs:(w.attributes||[]).map(a=>({name:a.name,options:a.options}))
};
commit("farmina_dims_14535.json",JSON.stringify(out,null,1));
console.log("DIMS DONE");
