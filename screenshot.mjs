...
const URL='https://dev.avesa.lt/product/churu-kreminiu-skanestu-rinkinys-katems-%c2%b7-9-vnt/?nc='+Date.now();
(async()=>{
  ...
  const probe = await page.evaluate(()=>{
    ...
    rows.forEach(function(row){
      ...
      data.push({
        ...
        kiekis: row.getAttribute('data-kiekis-rodyti')
      });
    });
  ...
  commit('churu_probe.json', JSON.stringify(probe,null,1));
  putBin('churu_visual.png', await page.screenshot({fullPage:false}));
  console.log(JSON.stringify(probe).slice(0,800));
  await ctx.close();
  await browser.close();
})();
