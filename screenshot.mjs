...
const out={};
let cats=[],seen={};
for(const q of ["sampun","sampunai","sampunas"]){
...
out.cats=cats;
// imti visas su 'sampun' slug
const targets=cats.filter(c=>/sampun/i.test(c.slug)||/ampun/i.test(c.name));
...
fs.writeFileSync("screenshots/sampunai_recon.txt", JSON.stringify(out,null,1));
