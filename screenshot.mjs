...
const out={versija:'VERIFY-E1'}; const NL=String.fromCharCode(10);
async function irasyk(){
  ...
  const b={message:'verify',content:Buffer.from(JSON.stringify(out)).toString('base64')}; if(sha) b.sha=sha;
...
try{
const sA=await snip('TEMP VERIFY AGR',[
"add_action('wp_loaded', function(){",
" if ((\$_GET['ps_v'] ?? '') !== 'Vfy1x') return;",
" global \$wpdb; \$o=array();",
" \$d=current_time('Y-m-d');",
" delete_transient('ps_ata_siandien_'.\$d);",
" \$o['siandien']=\$d;",
" \$o['gyvos_eilutes']=count(Petshop_Ataskaitu_Agregavimas::siandien());",
" \$e=Petshop_Ataskaitu_Agregavimas::eilutes(gmdate('Y-m-d', strtotime(\$d.' -29 day')), \$d, array('laukai','pardavimai'));",
" \$t=array(); foreach(\$e as \$x){ \$k=\$x['sritis'].':'.\$x['tipas']; if(!isset(\$t[\$k])) \$t[\$k]=array('kiek'=>0,'ses'=>0); \$t[\$k]['kiek']+=(int)\$x['kiekis']; \$t[\$k]['ses']+=(int)\$x['sesiju']; }",
" \$o['pjuvis']=\$t;",
" header('Content-Type: application/json'); echo wp_json_encode(\$o); exit;",
"}, 131);"].join(NL));
await new Promise(r=>setTimeout(r,4500));
try{ out.duomenys=JSON.parse(await (await fetch(WP+'/?ps_v=Vfy1x')).text()); }catch(e){ out.e=String(e).slice(0,150); }
await off(sA); await new Promise(r=>setTimeout(r,3000));

const sL=await snip('TEMP VERIFY LOGIN',[
"add_action('init', function(){",
" if ((\$_GET['ps_login'] ?? '') !== 'VfyLx') return;",
" \$a=get_users(array('role'=>'administrator','number'=>1,'fields'=>'ID')); if(!\$a) return;",
...
await page.goto(WP+'/?ps_login=VfyLx',{waitUntil:'domcontentloaded',timeout:60000});
await page.waitForTimeout(4500);
out.piltuvelis=await page.locator('.psru-zings .sk').allTextContents().catch(()=>[]);
out.kelias=(await page.locator('.psru-veiksmai.trys').innerText().catch(()=>'')).slice(0,400);
out.prekes=(await page.locator('#psru-prekes tbody').innerText().catch(()=>'')).slice(0,500);
out.js=jsErr;
