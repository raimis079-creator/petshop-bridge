...
async function busena(pg,name){ await miegok(800); const html=await pg.content();
  ...
  if(name) o.put=await put('screenshots/h259_'+name+'.png',await pg.screenshot({fullPage:true}),'H259C'); return o; }
try{
  ...
  const d=await fx(WP+'/?ps_h258=RUN20260824L&deploy=1&sha='+SHA,{},'deploy');
  ...
  if(visi && cookies.length){
    ...
    const a=await pg.$('tr:has-text("#35061") a:has-text("Perduoti")');
  ...
  const p=await fx(WP+'/?ps_h258=RUN20260824L&po=1',{},'po'); try{ out.serveris=JSON.parse(await p.text()); }catch(e){ out.serveris='ne-json'; }
...
await put('screenshots/h259run.json', Buffer.from(JSON.stringify(out,null,1)), 'H259C');
