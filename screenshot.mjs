...
let sid=null;
try{
  const c=await fx(SNIP,{method:'POST',headers:A,body:JSON.stringify({name:'TEMP E2 Beacon v4 (UTM)',code:Buffer.from(B64,'base64').toString('utf8'),scope:'global',active:true,priority:5})},'snip');
  sid=JSON.parse(await c.text()).id; out.sukurta=sid; await miegok(9000);

  const s=await fx(WP+'/?ps_e2b4='+KEY+'&faze=start',{},'start');
  ...
  await eik(WP+'/?utm_source=test&utm_medium=cpc&utm_campaign=e2test',1800);
  ...
  out.put=await put('screenshots/e2_beacon4.png', await pg.screenshot({fullPage:false}), VER);
  await br.close();

  const r=await fx(WP+'/?ps_e2b4='+KEY+'&faze=rez',{},'rez');
  const txt=await r.text();
  try{ const j=JSON.parse(txt); await put('deploy/e2_beacon4.json', Buffer.from(JSON.stringify(j,null,1)), VER); out.irasyta=1; }
...
await put('deploy/e2_beacon4run.json', Buffer.from(JSON.stringify(out,null,1)), VER);
