...
const O={VERSIJA_RUN:'run350-v1'}; let sid=null;
try{
...
if(!sid){ putB64('s350.json',Buffer.from(JSON.stringify(O,null,1)).toString('base64')); process.exit(0); }
sh('sleep 5');

// ---- 1. PATCH (atskiras requestas) ----
O.ver = q('ver');
const patchOK = O.ver && O.ver.pet_form && O.ver.pet_form.fetch_fpw===0 && O.ver.pet_form.s348_zyme>=1
  && O.ver.mu && O.ver.mu.gmdate_svoris===1 && O.ver.mu.s348_zyme>=1;
O.patchOK = patchOK;
if(patchOK){
sh('sleep 2');
...
putB64('s350.json',Buffer.from(JSON.stringify(O,null,1)).toString('base64'));
console.log('done');
