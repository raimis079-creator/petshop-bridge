...
const out={};
// kategoriju medis po 89
const all=gj('https://dev.avesa.lt/wp-json/wc/v3/products/categories?per_page=100&_fields=id,name,slug,parent,count');
out.tree=all.filter(c=>c.id===89||c.parent===89).map(c=>c.id+' '+c.name+' ('+c.slug+') parent='+c.parent+' count='+c.count);
function dump(cid){ const p=gj('https://dev.avesa.lt/wp-json/wc/v3/products?category='+cid+'&per_page=100&status=any&_fields=id,name,status,categories'); return p.map(x=>({id:x.id,n:x.name,s:x.status[0],c:(x.categories||[]).map(c=>c.id+':'+c.name).join(' | ')})); }
out['90_Lesalas']=dump(90);
out['98_Skanestai']=dump(98);
out['89_TIESIOG']=dump(89);
out.fin=putResult('pauks_audit_'+TS+'.txt', out);
