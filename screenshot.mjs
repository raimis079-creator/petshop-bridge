...
O.sid=sid;
if(!sid){ putB64('twopets.json',Buffer.from(JSON.stringify(O,null,1)).toString('base64')); console.log('no sid'); process.exit(0); }
sh('sleep 5');
function uzk(n){
...
if (A && A.cookie_value) {
 try{
  ...
  async function anketa(vardas, zyme){
    ...
    if (o.rusis_matoma) await suo.click({timeout:12000}).catch(e=>{o.e1=String(e).slice(0,100);});
  ...
  const pridBtn = page.getByText(/Pridėti augintinį|Sukurti profilį|Naujas augintinis/i).first();
...
putB64('twopets.json',Buffer.from(JSON.stringify(O,null,1)).toString('base64'));
console.log('done');
