...
const MD5='cc82a77f1de59939d894c6e3a355626e';
const out={versija:'S715'};
...
try{
  ...
  const s=await snip('TEMP S715',B64);
  ...
  /* PATIKRA: ar kasdien() tyli malones metu */
  const s2=await snip('TEMP S715B', Buffer.from(
    "add_action('wp_loaded',function(){ if((isset($_GET['ps_mg'])?$_GET['ps_mg']:'')!=='MG') return;"
    +" global $wpdb; $P=$wpdb->prefix; $o=array();"
    +" $o['start']=get_option('ps_sargas_start');"
    +" $o['amzius_h']=$o['start']?round((time()-$o['start'])/3600,2):null;"
    +" $o['siusta_pries']=count((array)get_option('ps_sargas_siusta',array()));"
    +" if(class_exists('Petshop_Sargas')) Petshop_Sargas::kasdien();"
    +" $o['siusta_po']=count((array)get_option('ps_sargas_siusta',array()));"
    +" $o['tylejo']=($o['siusta_po']===$o['siusta_pries']);"
...
await put('s715.json.gz', zlib.gzipSync(Buffer.from(JSON.stringify(out))), 's715 sargas v1.2');
