...
const NAUJAS_MD5='ee9b485d4d544810a8ebae1b26b1d974';
const out={versija:'DEP903'};
...
try{
  // 1. failas is repo
  const fr=await fetch(`https://api.github.com/repos/${REPO}/contents/deploy/kat_v871.php`,{headers:{'Authorization':'Bearer '+TOK,'Accept':'application/vnd.github.raw','User-Agent':'b'}});
  ...
  await put('v871_virsus.png', await p.screenshot({fullPage:false}), 'v87 virsus');
  ...
  await put('v871_apacia.png', await p.screenshot({fullPage:false}), 'v87 apacia');
  ...
  out.vidurys = await p.evaluate(()=>{
    ...
    const TH=document.querySelector('.pskat-lent-lauk thead th');
    const rb=B?B.getBoundingClientRect():null, rr=R?R.getBoundingClientRect():null, rt=TH?TH.getBoundingClientRect():null;
    return { bar_top: rb?Math.round(rb.top):null, rail_top: rr?Math.round(rr.top):null,
      thead_top: rt?Math.round(rt.top):null, thead_matomas: rt? (rt.top>=0 && rt.bottom>rt.top) : null,
      psvirs: getComputedStyle(document.documentElement).getPropertyValue('--ps-virsus').trim() };
  });
  await put('v871_vidurys.png', await p.screenshot({fullPage:false}), 'v87 vidurys');
  await br.close();
}catch(e){ out.klaida=String(e).slice(0,400); }
await put('dep903.json', Buffer.from(JSON.stringify(out)), 'dep902');
