...
function commit(name,str){ const url='https://api.github.com/repos/'+repo+'/contents/screenshots/'+name; let sha=''; try{ sha=JSON.parse(execSync('curl -s -H "Authorization: Bearer '+tok+'" "'+url+'?ref=main&t='+Date.now()+'"',{encoding:'utf8'})).sha||''; }catch(e){} const body={message:'mr2',branch:'main',content:Buffer.from(str,'utf8').toString('base64')}; if(sha) body.sha=sha; fs.writeFileSync('/tmp/cbm2.json',JSON.stringify(body)); execSync('curl -s -o /dev/null -X PUT -H "Authorization: Bearer '+tok+'" -H "Accept: application/vnd.github+json" -d @/tmp/cbm2.json "'+url+'"',{encoding:'utf8'}); }
...
(async()=>{
  fs.writeFileSync('/tmp/b557.json', JSON.stringify({name:'PSC MENU2', code:code, scope:'global', active:true}));
  ...
  var m=r.match(/(\{.*\})/s); commit('menu2.json', m?m[0]:r.slice(0,500)); console.log('done');
