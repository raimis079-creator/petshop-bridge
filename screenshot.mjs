...
function stripLegacy(T){const iH4=T.indexOf("Rekomenduojamas kiekis per par");if(iH4<0)return{base:T,removed:false};const iStart=T.lastIndexOf("<div",iH4);const anchor="geriamo vandens.</p></div></div>";const iA=T.indexOf(anchor,iH4);if(iStart<0||iA<0)return{base:T,removed:false};return{base:T.slice(0,iStart)+T.slice(iA+anchor.length),removed:true};}
const ADULT=[
...
for(const r of ADULT){const block=buildWetS(r.c);const p5='7px;">'+r.c[0]+' g</td>';
 for(const id of r.ids){try{
   ...
   const hadLeg=T.indexOf("Rekomenduojamas kiekis per par")>-1;
   const sl=stripLegacy(T);
   if(hadLeg&&!sl.removed){results.push({id,recipe:r.n,SKIP:"anchor_missing"});continue;}
   let base0=sl.base;
   ...
   const g={rekom_gone:newT.indexOf("Rekomenduojamas kiekis per par")<0,sud:newT.indexOf("Sud\u0117tis")>-1,anal:newT.indexOf("Analitin")>-1,single:(newT.split(MARK).length-1)===1,p5:newT.indexOf(p5)>-1,note:newT.indexOf("geriamojo vandens")>-1};
   if(!g.rekom_gone||!g.sud||!g.anal||!g.single||!g.p5||!g.note){results.push({id,recipe:r.n,SKIP:"guard",g});continue;}
   const wc=writeRaw(id,newT);const after=readRaw(id);
   results.push({id,recipe:r.n,act:sl.removed?"REMOVED+STYLED":"STYLED",write:wc,lossless:after!==null&&md5(after)===md5(newT),ver_rekom_gone:after!==null&&after.indexOf("Rekomenduojamas kiekis per par")<0,ver_single:after!==null&&(after.split(MARK).length-1)===1,ver_p5:after!==null&&after.indexOf(p5)>-1});
...
for(const id of JUN_IDS){try{
  ...
  const newT=base+jb;
  const g={single:(newT.split(MARK).length-1)===1,p5:newT.indexOf(jp5)>-1,anal:newT.indexOf("Analitin")>-1};
  if(!g.single||!g.p5||!g.anal){results.push({id,recipe:"Junior",SKIP:"guard",g});continue;}
  const wc=writeRaw(id,newT);const after=readRaw(id);
  results.push({id,recipe:"Junior",act:"STYLED",write:wc,lossless:after!==null&&md5(after)===md5(newT),ver_single:after!==null&&(after.split(MARK).length-1)===1,ver_p5:after!==null&&after.indexOf(jp5)>-1});
}catch(e){results.push({id,recipe:"Junior",ERR:String(e).slice(0,90)});}}
commit("restyle_apply_"+Date.now()+".json", JSON.stringify(results,null,1));
console.log("DONE");
