...
try{
  ...
  const ctx=await browser.newContext({ignoreHTTPSErrors:true,viewport:{width:380,height:2600}});
  ...
  await page.evaluate(()=>{const els=[...document.querySelectorAll('th,summary,p')];const f=els.find(e=>/Am\u017eius \(m\u0117n/.test(e.textContent||''));if(f)(f.closest('details')||f).scrollIntoView({block:'center'});});
  await page.waitForTimeout(600);
  let tgt=await page.$('.ps-desc-acc');
  const buf= tgt? await tgt.screenshot() : await page.screenshot({fullPage:false});
  out.http=putFile("vj_mobile2.png", buf.toString('base64'));out.bytes=buf.length;out.ok=true;
}catch(e){out.ok=false;out.err=String(e).slice(0,250);}
finally{if(browser)await browser.close();}
{
  const url='https://api.github.com/repos/'+repo+'/contents/screenshots/vj_mobile2_result.json';
  let sha='';try{sha=JSON.parse(execSync('curl -s -H "Authorization: Bearer '+tok+'" -H "User-Agent: r" "'+url+'?ref=main&t='+Date.now()+'"',{encoding:'utf8'})).sha||'';}catch(e){}
  const body={message:'r',branch:'main',content:Buffer.from(JSON.stringify(out)).toString('base64')};if(sha)body.sha=sha;
  fs.writeFileSync('/tmp/rj.json',JSON.stringify(body));
  execSync('curl -s -o /dev/null -X PUT -H "Authorization: Bearer '+tok+'" -H "User-Agent: r" -d @/tmp/rj.json "'+url+'"');
}
