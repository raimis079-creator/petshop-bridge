...
const B64=''; const VER='SM';
...
try{
  ...
  const c=await fx(SNIP,{method:'POST',headers:A,body:JSON.stringify({name:'TEMP L301',code:Buffer.from(B64,'base64').toString('utf8'),scope:'global',active:true,priority:5})},'create');
  const cr=JSON.parse(await c.text()); sid=cr.id; out.zingsniai.push('snip_id:'+sid);
  await miegok(9000);
  /* 3. skaitom */
  const r=await fx(WP+'/?ps_l3=GO',{headers:{'Cache-Control':'no-cache'}},'get');
...
await put('deploy/l301.json', Buffer.from(JSON.stringify(out,null,1)), VER);
