...
function putFile(name,buf){const u='https://api.github.com/repos/'+REPO+'/contents/'+name;let s='';
 for(let i=0;i<6;i++){try{const j=JSON.parse(execSync('curl -sk --max-time 30 -H "Authorization: Bearer '+TOKG+'" "'+u+'?n='+Math.random()+'"',{maxBuffer:80e6}).toString());if(j.sha)s=j.sha;}catch(e){}
  fs.writeFileSync('/tmp/pj.json',JSON.stringify({message:'s551',content:buf.toString('base64'),...(s?{sha:s}:{})}));
...
const O={VERSIJA_RUN:'run551'};
// auth cookie per snippeta
const PHP=`add_action('wp_loaded',function(){
 if(!isset($_GET['ps_ck'])||$_GET['ps_ck']!=='Ck551xQ') return;
...
let sid=null;
fs.writeFileSync('/tmp/sn.json',JSON.stringify({name:'TEMP S551 Cookie',code:PHP,scope:'global',active:true}));
...
const ck=sh('curl -sSk --max-time 60 "'+SITE+'/?ps_ck=Ck551xQ"');
...
if(C && C.logged_in){
 try{
  ...
  const puslapiai=[
    ['z1','/wp-admin/admin.php?page=ps-desk&view=rytas&z=1&naujai=1'],
  ...
  for(const [v,u] of puslapiai){
    try{
      ...
      putFile('screenshots/s551_'+v+'.png', png);
      O.psl[v].png='screenshots/s551_'+v+'.png';
...
putFile('analize/s551.json', Buffer.from(JSON.stringify(O,null,1)));
console.log('OK');
