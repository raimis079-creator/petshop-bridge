...
(async()=>{
  ...
  var probe = await page.evaluate(()=>{
    ...
    return {
      ...
      disabled_attr: cta.hasAttribute('disabled'),
      aria_disabled: cta.getAttribute('aria-disabled'),
      is_disabled_prop: cta.disabled,
      type: cta.tagName,
      // Ar MnM turi kitą būsenos indikatorių?
      form_has_pass: !!document.querySelector('.psc-form:not([style*="none"]) .mnm_form.pass, .psc-form:not([style*="none"]) .validation_passed')
    };
  });
  commit('cta_state.json', JSON.stringify(probe,null,1));
