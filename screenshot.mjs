...
add_action('wp_loaded', function(){
	if(!isset($_GET['ps_v5'])||$_GET['ps_v5']!=='V5Qq7Zz'){return;}
	if(!isset($_GET['confirm'])||$_GET['confirm']!=='REBUILD_V5'){echo 'NO CONFIRM';exit;}
	@set_time_limit(900); global $wpdb; $pf=$wpdb->prefix; $o=array('errors'=>array());

	// ===== 0. TRINAM VISKA (sauga irodyta S212) =====
	$o['before']=array('t'=>(int)$wpdb->get_var("SELECT COUNT(*) FROM {$pf}ps_feeding_tables"),
	                   'r'=>(int)$wpdb->get_var("SELECT COUNT(*) FROM {$pf}ps_feeding_rows"),
	                   'm'=>(int)$wpdb->get_var("SELECT COUNT(*) FROM {$pf}ps_feeding_map"));
	$wpdb->query("DELETE FROM {$pf}ps_feeding_rows");
	$wpdb->query("DELETE FROM {$pf}ps_feeding_map");
	$wpdb->query("DELETE FROM {$pf}ps_feeding_tables");
	$wpdb->query("ALTER TABLE {$pf}ps_feeding_tables AUTO_INCREMENT=1");
	$o['wiped']=array('t'=>(int)$wpdb->get_var("SELECT COUNT(*) FROM {$pf}ps_feeding_tables"),
	                  'r'=>(int)$wpdb->get_var("SELECT COUNT(*) FROM {$pf}ps_feeding_rows"),
	                  'm'=>(int)$wpdb->get_var("SELECT COUNT(*) FROM {$pf}ps_feeding_map"));
	...
	$tabs=array(); $stat=array('scanned'=>0,'phrase'=>0,'table_found'=>0,'vet'=>0,'no_table'=>0);
	foreach($ids as $id){
		if(get_post_meta($id,'_stock_status',true)!=='instock') continue;
		$stat['scanned']++;
		$c=get_post_field('post_content',$id);
		$pos = mb_stripos($c,'Šėrimo instrukcij');          // <<< TEISINGAI
		if($pos===false) continue;
		$stat['phrase']++;
		$full = mb_substr($c, $pos);                          // <<< SIMBOLIAI + SIMBOLIAI
		// apribojam iki kitos antrastes
		$scope = $full;
		...
		if(!$tbl){ if(mb_stripos($scope,'veterinar')!==false) $stat['vet']++; else $stat['no_table']++; continue; }
		$stat['table_found']++;
		...
		if($shape==='simple'||$shape==='by_age'){
			...
			if($shape==='simple' && mb_strpos($h0,'suaugusio')!==false) $wb='adult_expected';
			$ws=array();$as=array();
	...
	$o['scan']=$stat; $o['unique_tables']=count($tabs);

	// SAUGIKLIS
	foreach($tabs as $t) if($t['status']==='verified' && $t['shape']!=='by_age' && !$t['weight_basis']){
		$o['ABORTED']='verified be weight_basis'; header('Content-Type: application/json'); echo wp_json_encode($o); exit; }
	...
	foreach($tabs as $cs=>$t){
		if(!$wpdb->insert($pf.'ps_feeding_tables',array('checksum'=>$cs,'source_hash'=>$t['source_hash'],
			...
			'source_version'=>'post_content_v5_mb_2026-07-16','row_count'=>count($t['rows']),'parsed_at'=>$now,
			'verified_at'=>$t['status']==='verified'?$now:null,'verified_by'=>$t['status']==='verified'?'auto_parser_v5':null,
	...
	// gyvas testas
	$o['live_15kg_current']=$wpdb->get_results("SELECT t.brand,t.shape,r.amount_from_g,r.amount_to_g,r.condition_dimensions
	...
	$o['live_15kg_BLOCKED']=(int)$wpdb->get_var("SELECT COUNT(*) FROM {$pf}ps_feeding_rows r
		JOIN {$pf}ps_feeding_tables t ON t.id=r.feeding_table_id WHERE t.status='verified' AND t.species='dog'
		AND t.weight_basis='adult_expected' AND r.weight_from_kg<=15 AND r.weight_to_kg>=15");
	$o['dist']=$wpdb->get_results("SELECT shape,weight_basis,row_dimension,status,COUNT(*) n FROM {$pf}ps_feeding_tables GROUP BY shape,weight_basis,row_dimension,status ORDER BY shape,status",ARRAY_A);
	$o['reasons']=$wpdb->get_results("SELECT reason,COUNT(*) n FROM {$pf}ps_feeding_tables WHERE status='ambiguous' GROUP BY reason ORDER BY n DESC",ARRAY_A);
	$o['brands']=$wpdb->get_results("SELECT t.brand,SUM(t.status='verified') v,SUM(t.status='ambiguous') a,
		COUNT(DISTINCT m.product_id) sku FROM {$pf}ps_feeding_tables t LEFT JOIN {$pf}ps_feeding_map m ON m.feeding_table_id=t.id
		GROUP BY t.brand ORDER BY sku DESC LIMIT 10",ARRAY_A);
	header('Content-Type: application/json'); echo wp_json_encode($o); exit;
});`;
fs.writeFileSync('/tmp/snip.json',JSON.stringify({name:'TEMP M8 V5',code:php,scope:'global',active:true}));
sh(`curl -sk -X POST -H "Authorization: Basic ${AUTH}" -H "Content-Type: application/json" -d @/tmp/snip.json "${API}"`);
const r=sh('curl -sk --max-time 800 "https://dev.avesa.lt/?ps_v5=V5Qq7Zz&confirm=REBUILD_V5"');
try{out.p=JSON.parse(r);}catch(e){out.raw=r.slice(0,800);}
const k=`add_action('wp_loaded',function(){if(!isset($_GET['ps_kv5'])||$_GET['ps_kv5']!=='Rr3Ww8Yy'){return;}global $wpdb;$n=$wpdb->query("DELETE FROM {$wpdb->prefix}snippets WHERE name LIKE 'TEMP M8%'");echo wp_json_encode(array('d'=>$n));exit;});`;
fs.writeFileSync('/tmp/k.json',JSON.stringify({name:'TEMP M8 Kill V5',code:k,scope:'global',active:true}));
sh(`curl -sk -X POST -H "Authorization: Basic ${AUTH}" -H "Content-Type: application/json" -d @/tmp/k.json "${API}"`);
out.cleanup=sh('curl -sk --max-time 25 "https://dev.avesa.lt/?ps_kv5=Rr3Ww8Yy"').slice(0,40);
}catch(e){out.FATAL=String(e&&e.message?e.message:e).slice(0,300);}
ghPut('screenshots/m8_v5.json',Buffer.from(JSON.stringify(out)),'parser v5 rebuild');
console.log('DONE');
