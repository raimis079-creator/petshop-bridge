...
function commitB64(name,b64){ const url='https://api.github.com/repos/'+repo+'/contents/screenshots/'+name; let sha=''; try{ sha=JSON.parse(execSync('curl -s -H "Authorization: Bearer '+tok+'" "'+url+'?ref=main&t='+Date.now()+'"',{encoding:'utf8'})).sha||''; }catch(e){} const body={message:'rects',branch:'main',content:b64}; if(sha) body.sha=sha; fs.writeFileSync('/tmp/cbrc.json',JSON.stringify(body)); execSync('curl -s -o /dev/null -X PUT -H "Authorization: Bearer '+tok+'" -H "Accept: application/vnd.github+json" -d @/tmp/cbrc.json "'+url+'"',{encoding:'utf8'}); }
...
(async()=>{
  ...
  const getRects = async (page) => {
    return await page.evaluate(() => {
      ...
      var cols = document.querySelectorAll('.psc-sol-cols');
      var gridCols = cols[1] ? cols[1].querySelectorAll('.psc-sol-col') : [];
      var dogGrid = gridCols[0] ? gridCols[0].getBoundingClientRect() : null;
      var catGrid = gridCols[1] ? gridCols[1].getBoundingClientRect() : null;
      var intro = rect('.psc-sol-intro');
      var warning = rect('.psc-sol-warning');
      var docHeight = document.body.scrollHeight;
      return {
        h2map: h2map,
        intro: intro,
        dogGrid: dogGrid ? {top:dogGrid.top,bottom:dogGrid.bottom,left:dogGrid.left,right:dogGrid.right} : null,
        catGrid: catGrid ? {top:catGrid.top,bottom:catGrid.bottom,left:catGrid.left,right:catGrid.right} : null,
        warning: warning,
        docHeight: docHeight
      };
    });
  ...
  try{
    ...
    commitB64('rv2_desktop_full.png', bufD.toString('base64'));
    await cD.close();
    ...
    commitB64('rv2_mobile_full.png', bufM.toString('base64'));
    await cM.close();
  ...
  commitB64('rects.json', Buffer.from(JSON.stringify(out),'utf8').toString('base64'));
