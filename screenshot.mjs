...
// Petshop MnM Sutaupymo žymeklis v6
// A) Produkto puslapyje: "Sutaupote X,XX € (Y%)" po kaina
// B) Kategorijoje: Flatsome onsale badge "-Y%" ant nuotraukos
// C) Kainų rodymas: ~~senoji~~ naujoji (perbraukta komponentų suma + rinkinio kaina)
...
// === B) Sale badge: Flatsome onsale badge "-Y%" ===
...
// === C) MnM = "on sale" (kad badge atsirastų) ===
add_filter('woocommerce_product_is_on_sale', function($on_sale, $product) {
    ...
    // Statinis cache per request, kad neskaitytų DB kiekvieną kartą
    static $cache = array();
    $pid = $product->get_id();
    if (!isset($cache[$pid])) {
...
// === D) "Senoji kaina" = komponentų suma (kad ~~23,41~~ 21,29 atrodytų teisingai) ===
...
(async()=>{
  ...
  const u = api('PUT','/wp-json/code-snippets/v1/snippets/535', {
    name:'Petshop MnM Sutaupymo žymeklis v6 (badge + perbraukta kaina)',
    code: SNIPPET_V6,
    desc:'v6: onsale badge + perbraukta komponentų suma + sutaupymo tekstas. Statinis cache per request.',
    scope: 'global', active: true
  });
  out.update = u && u.id ? ('updated len='+(u.code||'').length) : (u.__raw||'?');
  commit('snip535_v6.json', JSON.stringify(out,null,1));
  console.log("DONE");
})();
