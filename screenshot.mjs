...
let sid=null;
try{
  const c=await fx(SNIP,{method:'POST',headers:A,body:JSON.stringify({name:'TEMP H247 v2 (tiekimas 1.6)',code:Buffer.from(B64,'base64').toString('utf8'),scope:'global',active:true,priority:5})},'snip');
  ...
  if(j&&j.id){ sid=j.id; await miegok(9000);
    ...
    if(cookies.length && out.deploy && out.deploy.md5===MD5){
      ...
      out.kaupiama={http:r.status(), fatal:(await pg.content()).includes('Fatal error'),
        ...
        put:await put('screenshots/h247_kaupiama.png',await pg.screenshot({fullPage:true}),'H247C')};
      ...
      out.adresai={http:r2.status(), fatal:(await pg.content()).includes('Fatal error'),
        ...
        put:await put('screenshots/h247_adresai.png',await pg.screenshot({fullPage:true}),'H247C')};
...
await put('screenshots/h247run.json', Buffer.from(JSON.stringify(out,null,1)), 'H247C');
