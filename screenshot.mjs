...
(async()=>{
  ...
  var probe = await page.evaluate(()=>({
    ...
    has_group_buttons: document.querySelectorAll('.psc-group-btn').length,
    gram_buttons: document.querySelectorAll('.psc-gram-btn').length,
    size_buttons: document.querySelectorAll('.psc-size-btn').length,
    box_price: (document.querySelector('.psc-box-price')||{}).textContent,
    has_proxy: !!document.querySelector('.psc-proxy-cta')
  }));
  commit('v1_compat.json', JSON.stringify(probe,null,1));
