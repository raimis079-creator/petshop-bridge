...
let sid=null;
try{
  const c=await fx(SNIP,{method:'POST',headers:A,body:JSON.stringify({name:'TEMP H251 v1 (3 failai)',code:Buffer.from(B64,'base64').toString('utf8'),scope:'global',active:true,priority:5})},'snip');
  ...
  if(j&&j.id){ sid=j.id; await miegok(9000);
    ...
    if(cookies.length && visi){
      ...
      let r2=null;
      for(let k=0;k<3;k++){
        try{ r2=await pg.goto(WP+'/wp-admin/admin.php?page=ps-laiskai',{waitUntil:'domcontentloaded',timeout:60000}); break; }
        catch(e){ out.laisk_retry=(out.laisk_retry||0)+1; await miegok(5000); }
      }
      await miegok(1200);
      if(!r2){ out.laiskai={klaida:'goto nepavyko'}; }else{
      out.laiskai={http:r2.status(), fatal:(await pg.content()).includes('Fatal error'),
        h1:await pg.$eval('h1',n=>n.textContent.trim()).catch(()=>'?'),
        skirtukai:await pg.$$eval('.nav-tab',ns=>ns.map(n=>n.textContent.trim())),
        blokai:await pg.$$eval('.ps-tiek-h h2',ns=>ns.map(n=>n.textContent.trim())),
        mygtukai:await pg.$$eval('.ps-tiek .button',ns=>ns.map(n=>n.textContent.trim())),
        eiluciu:await pg.$$eval('tbody tr',ns=>ns.length).catch(()=>0),
        put:await put('screenshots/h250_laiskai.png',await pg.screenshot({fullPage:true}),'H251A')}; }
      ...
      out.dropship={http:r3.status(), fatal:(await pg.content()).includes('Fatal error'),
        ...
        put:await put('screenshots/h250_dropship.png',await pg.screenshot({fullPage:true}),'H251A')};
...
await put('screenshots/h250run.json', Buffer.from(JSON.stringify(out,null,1)), 'H251A');
