...
(async()=>{let ok=0;for(const it of items){try{const b=execSync('curl -s -k --max-time 25 "'+it.img+'"',{encoding:'buffer',maxBuffer:20000000});if(b.length>500){putBinary('g'+it.id+'.png',b);ok++;out+='ok '+it.id+'\n';}}catch(e){out+='err '+it.id+'\n';}}putText('_grauzdl.txt',out+'DONE '+ok);})();
