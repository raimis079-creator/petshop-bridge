...
add_action('wp_loaded', function(){
  if(!isset($_GET['ps_gui']) || $_GET['ps_gui']!=='Gui25x') return;
  $f=WP_PLUGIN_DIR.'/petshop-core/includes/class-pet-ui.php';
  header('Content-Type: application/json'); echo json_encode(array('b64'=>base64_encode(file_get_contents($f)))); exit;
});`;
try{
  const mk=wj('POST','code-snippets/v1/snippets',{name:'GUI (temp)',code:CODE,scope:'front-end',active:true,priority:5});
  let sid=null; try{sid=JSON.parse(mk).id;}catch(e){}
  execSync('sleep 4');
  const r=execSync('curl -sk "https://dev.avesa.lt/?ps_gui=Gui25x"',{maxBuffer:10e6,timeout:60000}).toString();
  const a=r.indexOf('{'),b=r.lastIndexOf('}'); o.b64=(a>=0&&b>a)?JSON.parse(r.slice(a,b+1)).b64:null;
  if(sid!=null){ try{execSync('curl -sk '+AUTH+' -X DELETE "https://dev.avesa.lt/wp-json/code-snippets/v1/snippets/'+sid+'"');}catch(e){} }
}catch(e){o.err=String(e).slice(0,200);}
putB64('getui.json', Buffer.from(JSON.stringify(o)).toString('base64'));
console.log('done');
