...
(async()=>{
  ...
  try{
    ...
    const ctxD=await browser.newContext({ignoreHTTPSErrors:true,viewport:{width:1280,height:760}});
    ...
    log+='desktop '+bD.length+'\n'; putBinary('pop2_desktop.png',bD);
    await ctxD.close();
    const ctxM=await browser.newContext({ignoreHTTPSErrors:true,viewport:{width:390,height:760},isMobile:true});
    ...
    log+='mobile '+bM.length+'\n'; putBinary('pop2_mobile.png',bM);
    await ctxM.close();
  }catch(e){log+='ERR '+e+'\n';}
  finally{if(browser)await browser.close();putText('_shot3_log.txt',log);}
})();
