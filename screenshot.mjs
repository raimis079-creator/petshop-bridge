...
const out={v:'S1577'}; const PREKE='/product/royal-canin-cat-fussy-exigent-10-kg-sausas-pasaras-isrankioms-katems/'; const sl=ms=>new Promise(r=>setTimeout(r,ms));
...
async function goto(pg,path,o){ for(let i=0;i<3;i++){ const r=await pg.goto(WP+path,{waitUntil:'domcontentloaded',timeout:60000}); const st=r?r.status():0; const len=(await pg.content()).length; (o.docs||(o.docs=[])).push(path+' '+st+' '+len); if(st===200&&len>5000) return true; await sl(8000); } return false; }
async function scen(label,dev,veiksmas){
  ...
  try{
    ...
    if(veiksmas==='niekas'){ await sl(5000); await goto(pg,'/kasa/',o); await pg.waitForTimeout(6000);
      ...
      await put('screenshots/s1577_kasa_baneris.png',await pg.screenshot(),'S1577 kasa baneris'); }
    await put('screenshots/s1577_'+label+'.png',await pg.screenshot(),'S1577 '+label);
...
await br.close(); await put('analize/s1577_consent2.json',Buffer.from(JSON.stringify(out,null,1)),'S1577 consent2'); console.log('ok');
