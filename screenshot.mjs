...
const id=26414;
const ser=build2([[5,85,95],[10,140,165],[20,240,275],[30,320,375],[40,400,465],[60,540,625],[80,670,780]]);
let res;
try{
  const T=readRaw(id);
  if(T===null) res={id,ERR:"read"};
    ...
    else{const wc=writeRaw(id,newT); const after=readRaw(id);res={id,recipe:"Salmon+Potato A/S",write:wc,ver_table:after!==null&&/<td>5 kg<\/td>/.test(after)&&/<td>670 g<\/td>/.test(after),ver_sud:after!==null&&md5((after.match(/Sud\u0117tis:[\s\S]*?<\/p>/)||[""])[0])===sud,ver_anal:after!==null&&after.indexOf("Analitin")>-1,lossless:after!==null&&md5(after)===md5(newT)};}
  }
}catch(e){res={id,ERR:String(e).slice(0,100)};}
commit("waveJ_"+Date.now()+".json", JSON.stringify(res,null,2));
console.log("DONE");
