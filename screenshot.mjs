...
(async()=>{
  ...
  var probe = await page.evaluate(()=>{
    ...
    var visible=0, hidden=0, hasBeefChicken=false;
    rows.forEach(function(r){
      if(r.querySelector('th')) return;
      if(r.style.display==='none') hidden++;
      else { visible++; if(/Beef \+ Chicken/i.test(r.textContent)) hasBeefChicken=true; }
    });
    return {visible_cans:visible, hidden_cans:hidden, beef_chicken_still_visible:hasBeefChicken};
  });
  commit('e6_check.json', JSON.stringify(probe,null,1));
  putBin('e6_check.png', await page.screenshot({fullPage:false}));
