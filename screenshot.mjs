...
`;
const s1=await wp('/wp-json/code-snippets/v1/snippets',{method:'POST',body:JSON.stringify({name:'TEMP GPAIS Deploy v3',code:phpDep,scope:'global',active:true,priority:5})});
const j1=js(s1.text);
const phpAuto = `
...
`;
const s2=await wp('/wp-json/code-snippets/v1/snippets',{method:'POST',body:JSON.stringify({name:'TEMP GPAIS Autologin v3b',code:phpAuto,scope:'global',active:true,priority:5})});
const j2=js(s2.text);
await new Promise(r=>setTimeout(r,4000));
for (const f of ['petshop-katalogas.php']) {
  try{
    const gg=await fetch('https://api.github.com/repos/'+REPO+'/contents/deploy/'+f+'.b64?ref=31989543dbf2bebc3a4c943e1a0a888d6972f763',{headers:{'Authorization':'Bearer '+TOK}});
...
try{
  ...
  await pg.screenshot({path:'screenshots/gp4_forma.png',fullPage:false}); files.push('screenshots/gp4_forma.png');
  ...
  await pg.screenshot({path:'screenshots/gp4_sarasas.png',fullPage:false}); files.push('screenshots/gp4_sarasas.png');
...
const g=await fetch('https://api.github.com/repos/'+REPO+'/contents/screenshots/gpais3.json',{headers:{'Authorization':'Bearer '+TOK}});
if(g.status===200){ body.sha=(await g.json()).sha; }
await fetch('https://api.github.com/repos/'+REPO+'/contents/screenshots/gpais3.json',{method:'PUT',headers:{'Authorization':'Bearer '+TOK,'Content-Type':'application/json'},body:JSON.stringify(body)});
