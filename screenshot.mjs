...
const TS="1782201883";
const R=JSON.parse(fs.readFileSync('restore.json','utf8'));
const out={};
for(const pid of Object.keys(R)){
  try{
    fs.writeFileSync('/tmp/upd.json',JSON.stringify({description:R[pid].content}));
    ...
    out[pid]={restored_len:(u.description||'').length, target_len:R[pid].content.length};
  }catch(e){ out[pid]={err:e.message.slice(0,80)}; }
  execSync('sleep 1');
}
putResult('rst_'+TS+'.json', JSON.stringify(out));
