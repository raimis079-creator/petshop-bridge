...
add_action('wp_loaded', function(){
  if(!isset($_GET['ps_gp']) || $_GET['ps_gp']!=='Gpx') return;
...
try{
  const mk=wj('POST','code-snippets/v1/snippets',{name:'GP (temp)',code:CODE,scope:'front-end',active:true,priority:5});
  let sid=null; try{sid=JSON.parse(mk).id;}catch(e){}
  execSync('sleep 4');
  const r=execSync('curl -sk "https://dev.avesa.lt/?ps_gp=Gpx"',{maxBuffer:15e6,timeout:60000}).toString();
  const a=r.indexOf('{'),b=r.lastIndexOf('}'); const dd=(a>=0&&b>a)?JSON.parse(r.slice(a,b+1)):null;
  o.b64=dd?dd.b64:null; o.bytes=dd?dd.bytes:null; o.sha=dd?dd.sha:null;
  if(sid!=null){ try{execSync('curl -sk '+AUTH+' -X DELETE "https://dev.avesa.lt/wp-json/code-snippets/v1/snippets/'+sid+'"');}catch(e){} }
}catch(e){o.err=String(e).slice(0,200);}
putB64('getprof.json', Buffer.from(JSON.stringify(o)).toString('base64'));
console.log('done');
