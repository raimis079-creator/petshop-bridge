...
const B=[[27132,"Kitten akcija"],[21707,"Leger 10kg"],[18058,"Leger 1kg"],[18054,"Leger 10kg"],[18051,"Leger 2kg"]];
const out=[];
for(const [id,nm] of B){const T=readRaw(id);if(T===null){out.push({id,nm,ERR:1});continue;}
  const iM=T.indexOf(MARK);
  out.push({id,nm,nMark:T.split(MARK).length-1,styled:T.indexOf('class="b2b-black"')>-1,
    dump:iM>=0?T.slice(iM,iM+640):"NO_MARK"});
}
commit("catB_recon_"+Date.now()+".json", JSON.stringify(out,null,1));
console.log("DONE");
