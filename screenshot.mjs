...
(async()=>{try{
  // 1. Trinam #34600 galutinai (force=true = permanent, ne trash)
  const del=sh('curl -s -k -u "'+U+':'+P+'" -X DELETE "'+BASE+'/wp-json/wc/v3/orders/34600?force=true"');
  try{const o=JSON.parse(del);L('DELETE #34600 -> status po trynimo: '+(o.status||'?')+' id='+(o.id||'?'));}catch(e){L('delete resp: '+del.slice(0,300));}
  ...
  // 3. Ar #34600 tikrai nebeegzistuoja
  const g=sh('curl -s -k -u "'+U+':'+P+'" -w "|HTTP:%{http_code}" "'+BASE+'/wp-json/wc/v3/orders/34600"');
  L('GET #34600 po trynimo: '+(g.match(/HTTP:(\d+)/)||[])[1]+' (404 = puiku, istrintas)');
  L('DONE');
