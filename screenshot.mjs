...
const out={};
try{
// ---- A) MUSU aprasymai (SKU + sudetis + ilgis) ----
const php=`
add_action('wp_loaded', function(){
	if(!isset($_GET['ps_cmp'])||$_GET['ps_cmp']!=='Cm5Tt2Xx'){return;}
	...
	foreach($ids as $id){
		...
		// sudetis
		$sud=''; 
		if(preg_match('/Sud(?:ė|e)tis\s*:?\s*<\/strong>\s*<\/p>\s*<p>(.{20,700}?)<\/p>/isu',$c,$m)) $sud=$m[1];
		elseif(preg_match('/Sud(?:ė|e)tis\s*:?\s*(?:<\/strong>)?\s*(.{20,700}?)(?:<\/p>|<strong)/isu',$c,$m)) $sud=$m[1];
		$has_ser = (mb_stripos($c,'Šėrimo instrukcij')!==false);
		$o[]=array('id'=>$id,'sku'=>get_post_meta($id,'_sku',true),'st'=>get_post_status($id),
			'title'=>mb_substr($t,0,72),'len'=>mb_strlen($c),
			'sudetis'=>trim(strip_tags($sud)),'ser'=>$has_ser);
	}
	header('Content-Type: application/json'); echo wp_json_encode($o); exit;
});`;
fs.writeFileSync('/tmp/snip.json',JSON.stringify({name:'TEMP M8 Cmp',code:php,scope:'global',active:true}));
sh(`curl -sk -X POST -H "Authorization: Basic ${AUTH}" -H "Content-Type: application/json" -d @/tmp/snip.json "${API}"`);
const mine=JSON.parse(sh('curl -sk --max-time 280 "https://dev.avesa.lt/?ps_cmp=Cm5Tt2Xx"'));
out.mine_count=mine.length;
out.mine=mine;

// ---- B) exclusion.lt aprasymai ----
...
for(const u of [...prods]){
  ...
  let sud='';
  const m=txt.match(/Sud(?:ė|&#279;)tis\s*<\/h2>([\s\S]{20,900}?)<(?:h2|p><strong|div)/i);
  if(m) sud=m[1].replace(/<[^>]+>/g,' ').replace(/&nbsp;/g,' ').replace(/\s+/g,' ').trim();
  const ser=[...h.matchAll(/uploads\/[^"']*?SERIMAS[^"']*?\.png/gi)].map(x=>x[0].split('/').pop());
  lt.push({url:u,title:title.slice(0,80),sudetis:sud.slice(0,420),ser:ser[0]||null});
}
out.lt_count=lt.length; out.lt=lt;

const k=`add_action('wp_loaded',function(){if(!isset($_GET['ps_kc'])||$_GET['ps_kc']!=='Rr3Ww8Yy'){return;}global $wpdb;$n=$wpdb->query("DELETE FROM {$wpdb->prefix}snippets WHERE name LIKE 'TEMP M8%'");echo wp_json_encode(array('d'=>$n));exit;});`;
fs.writeFileSync('/tmp/k.json',JSON.stringify({name:'TEMP M8 Kill C',code:k,scope:'global',active:true}));
sh(`curl -sk -X POST -H "Authorization: Basic ${AUTH}" -H "Content-Type: application/json" -d @/tmp/k.json "${API}"`);
out.cleanup=sh('curl -sk --max-time 25 "https://dev.avesa.lt/?ps_kc=Rr3Ww8Yy"').slice(0,40);
}catch(e){ out.FATAL=String(e&&e.message?e.message:e).slice(0,400); }
ghPut('screenshots/m8_desc.json',Buffer.from(JSON.stringify(out)),'desc compare');
console.log('DONE');
