...
function putFile(name,buf){const u='https://api.github.com/repos/'+REPO+'/contents/'+name;let s='';
 for(let i=0;i<6;i++){try{const j=JSON.parse(execSync('curl -sk --max-time 30 -H "Authorization: Bearer '+TOKG+'" "'+u+'?n='+Math.random()+'"',{maxBuffer:80e6}).toString());if(j.sha)s=j.sha;}catch(e){}
  fs.writeFileSync('/tmp/pj.json',JSON.stringify({message:'s517',content:buf.toString('base64'),...(s?{sha:s}:{})}));
...
const O={VERSIJA_RUN:'run517'};
// auth cookie per snippeta
const PHP=`add_action('wp_loaded',function(){
 if(!isset($_GET['ps_ck'])||$_GET['ps_ck']!=='Ck517xQ') return;
...
let sid=null;
fs.writeFileSync('/tmp/sn.json',JSON.stringify({name:'TEMP S517 Cookie',code:PHP,scope:'global',active:true}));
...
const ck=sh('curl -sSk --max-time 60 "'+SITE+'/?ps_ck=Ck517xQ"');
...
if(C && C.logged_in){
 try{
  ...
  const puslapiai=[
    ...
    ['visi','/wp-admin/admin.php?page=ps-desk&eile=visi'],
    ['skydelis','/wp-admin/admin.php?page=ps-desk&eile=visi'],
  ];
  O.psl={};
  for(const [v,u] of puslapiai){
    try{
      ...
      if(v==='skydelis'){
        try{
          const eil=await p.$$('.pd-tbl tbody tr');
          for(const e of eil){
            const t=await e.evaluate(x=>(x.querySelector('.pd-cnote')||{}).innerText||'');
            if(t){ await e.click(); break; }
          }
          await p.waitForTimeout(900);
          O.psl[v+'_peek']=await p.evaluate(()=>({
            atidarytas:document.getElementById('pdPeek').classList.contains('on'),
            nr:(document.getElementById('pkNr')||{}).innerText,
            pastabos_blokas:(document.querySelector('#pkBody .pd-cbox')||{}).innerText,
            klientas:(document.querySelector('#pkBody .pd-mem')||{}).innerText,
            eilutes:[...document.querySelectorAll('#pkBody .pd-line')].map(x=>x.innerText.replace(/\n/g,' | '))
          }));
        }catch(e){ O.psl[v+'_peek']={KLAIDA:String(e).slice(0,120)}; }
      }
      const png=await p.screenshot({fullPage:true});
      putFile('screenshots/s517_'+v+'.png', png);
      O.psl[v].png='screenshots/s517_'+v+'.png';
...
putFile('analize/s517.json', Buffer.from(JSON.stringify(O,null,1)));
console.log('OK');
