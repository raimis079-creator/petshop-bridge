...
let sid=null;
try{
  const c=await fx(SNIP,{method:'POST',headers:A,body:JSON.stringify({name:'TEMP H250 v3 (3 failai)',code:Buffer.from(B64,'base64').toString('utf8'),scope:'global',active:true,priority:5})},'snip');
  ...
  if(j&&j.id){ sid=j.id; await miegok(9000);
    ...
    if(cookies.length && visi){
      ...
      out.laiskai={http:r2.status(), fatal:(await pg.content()).includes('Fatal error'),
        ...
        put:await put('screenshots/h250_laiskai.png',await pg.screenshot({fullPage:true}),'H250C')}; }
      ...
      out.dropship={http:r3.status(), fatal:(await pg.content()).includes('Fatal error'),
        ...
        put:await put('screenshots/h250_dropship.png',await pg.screenshot({fullPage:true}),'H250C')};
...
await put('screenshots/h250run.json', Buffer.from(JSON.stringify(out,null,1)), 'H250C');
